...
class EventEmitterTest extends \ECSPrefix20220305\PHPUnit\Framework\TestCase
{
    private $emitter;
    public function setUp()
    {
        $this->emitter = new \ECSPrefix20220305\Evenement\EventEmitter();
    }
    ...
    {
        $listener = new \ECSPrefix20220305\Evenement\Tests\Listener();
        $this->emitter->on('foo', [$listener, 'onFoo']);
    }
    public function testAddListenerWithStaticMethod()
    {
        $this->emitter->on('bar', ['ECSPrefix20220305\\Evenement\\Tests\\Listener', 'onBar']);
    }
    ...
    public function testCallablesStaticClass()
    {
        ...
        $this->emitter->emit('foo', ['bar']);
        self::assertSame(['bar'], \ECSPrefix20220305\Evenement\Tests\Listener::getStaticData());
    }
    public function testCallablesFunction()
    {
        $this->emitter->on('foo', 'ECSPrefix20220305\\Evenement\\Tests\\setGlobalTestData');
        $this->emitter->emit('foo', ['bar']);
