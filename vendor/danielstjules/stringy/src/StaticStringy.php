...
class StaticStringy
{
    ...
    public static function __callStatic($name, $arguments)
    {
        if (!static::$methodArgs) {
            $stringyClass = new \ReflectionClass('ECSPrefix20211203\\Stringy\\Stringy');
        ...
        $stringy = \ECSPrefix20211203\Stringy\Stringy::create($str, $encoding);
        $result = \call_user_func_array([$stringy, $name], $args);
        $cast = function ($val) {
            if (\is_object($val) && $val instanceof \ECSPrefix20211203\Stringy\Stringy) {
                return (string) $val;
            } else {
                return $val;
            }
