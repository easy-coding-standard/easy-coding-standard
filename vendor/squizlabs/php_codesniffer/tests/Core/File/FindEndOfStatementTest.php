...
final class FindEndOfStatementTest extends AbstractMethodUnitTest
{
    ...
    public function testEndIsNeverLessThanCurrentToken()
    {
        ...
        for ($i = 0; $i < self::$phpcsFile->numTokens; $i++) {
            ...
            if ($end < $i) {
                $errors[] = \sprintf('End of statement for token %1$d (%2$s: %3$s) on line %4$d is %5$d (%6$s), which is less than %1$d', $i, $tokens[$i]['type'], $tokens[$i]['content'], $tokens[$i]['line'], $end, $tokens[$end]['type']);
            }
        }
