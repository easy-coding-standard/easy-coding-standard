...
final class PopulateTokenListenersTest extends AbstractRulesetTestCase
{
    ...
    public function testSniffWhereRegisterDoesNotReturnAnArrayThrowsException()
    {
        ...
        $config = new ConfigDouble(["--standard={$standard}"]);
        $sniffClass = 'ECSPrefix202508\\Fixtures\\TestStandard\\Sniffs\\InvalidSniffs\\RegisterNoArraySniff';
    ...
    {
        $target = 'ECSPrefix202508\\Fixtures\\TestStandard\\Sniffs\\ValidSniffs\\RegisterEmptyArraySniff';
    ...
    {
        $exclude = 'ECSPrefix202508\\Fixtures\\TestStandard\\Sniffs\\SupportedTokenizers\\ListensForPHPAndCSSAndJSSniff';
        $expected = ['PHP' => 'PHP'];
        foreach (self::$ruleset->tokenListeners as $token => $listeners) {
            $this->assertTrue(\is_array($listeners), 'No listeners registered for token' . Tokens::tokenName($token));
            foreach ($listeners as $className => $details) {
    ...
    {
        $sniffClass = 'ECSPrefix202508\\Fixtures\\TestStandard\\Sniffs\\SupportedTokenizers\\ListensForPHPAndCSSAndJSSniff';
