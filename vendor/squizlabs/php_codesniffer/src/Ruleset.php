...
class Ruleset
{
                    ...
                    if (isset($prop['type']) === \true && (string) $prop['type'] === 'array') {
                        ...
                        if (isset($prop->element) === \true) {
                            ...
                            foreach ($prop->element as $element) {
                                ...
                                if (isset($element['key']) === \true && \trim($element['key']) !== '') {
    ...
    public function registerSniffs($files, $restrictions, $exclusions)
    {
        ...
        foreach ($files as $file) {
            ...
            if ($reflection->implementsInterface('PHP_CodeSniffer\\Sniffs\\Sniff') === \false) {
                ...
                if (\method_exists($className, 'register') === \false || \method_exists($className, 'process') === \false) {
    ...
    public function populateTokenListeners()
    {
        ...
        foreach ($this->sniffs as $sniffClass => $sniffObject) {
            ...
            if (\substr($sniffCode, 0, 1) === '.' || \substr($sniffCode, -1) === '.' || \strpos($sniffCode, '..') !== \false || \preg_match('`(^|\\.)Sniffs\\.`', $sniffCode) === 1 || \preg_match('`[^\\s\\.-]+\\\\Sniffs\\\\[^\\s\\.-]+\\\\[^\\s\\.-]+Sniff`', $sniffClass) !== 1) {
            ...
            if (empty($vars['supportedTokenizers']) === \false && $isDeprecated === \false && \in_array('PHP', $vars['supportedTokenizers'], \true) === \false) {
                if (\in_array('CSS', $vars['supportedTokenizers'], \true) === \true || \in_array('JS', $vars['supportedTokenizers'], \true) === \true) {
