...
final class Factory
{
    ...
    {
        $loop = self::construct();
        \ECSPrefix20210917\React\EventLoop\Loop::set($loop);
        return $loop;
    }
    /**
     * @internal
    ...
    private static function construct()
    {
        ...
        if (\function_exists('uv_loop_new')) {
            ...
            return new \ECSPrefix20210917\React\EventLoop\ExtUvLoop();
        }
        if (\class_exists('ECSPrefix20210917\\libev\\EventLoop', \false)) {
            return new \ECSPrefix20210917\React\EventLoop\ExtLibevLoop();
        }
        if (\class_exists('EvLoop', \false)) {
            return new \ECSPrefix20210917\React\EventLoop\ExtEvLoop();
        }
        if (\class_exists('EventBase', \false)) {
            return new \ECSPrefix20210917\React\EventLoop\ExtEventLoop();
        }
        if (\function_exists('event_base_new') && \PHP_MAJOR_VERSION === 5) {
            // only use ext-libevent on PHP 5 for now
            return new \ECSPrefix20210917\React\EventLoop\ExtLibeventLoop();
        }
        return new \ECSPrefix20210917\React\EventLoop\StreamSelectLoop();
