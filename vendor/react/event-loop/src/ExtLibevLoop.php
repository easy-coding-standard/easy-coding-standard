...
use ECSPrefix20210710\libev\EventLoop;
use ECSPrefix20210710\libev\IOEvent;
use ECSPrefix20210710\libev\SignalEvent;
use ECSPrefix20210710\libev\TimerEvent;
 ...
 */
final class ExtLibevLoop implements \ECSPrefix20210710\React\EventLoop\LoopInterface
{
    private $loop;
    ...
    {
        if (!\class_exists('ECSPrefix20210710\\libev\\EventLoop', \false)) {
            throw new \BadMethodCallException('Cannot create ExtLibevLoop, ext-libev extension missing');
        }
        $this->loop = new \ECSPrefix20210710\libev\EventLoop();
    ...
    public function addReadStream($stream, $listener)
    {
        ...
        $event = new \ECSPrefix20210710\libev\IOEvent($callback, $stream, \ECSPrefix20210710\libev\IOEvent::READ);
        $this->loop->add($event);
    ...
    public function addWriteStream($stream, $listener)
    {
        ...
        $event = new \ECSPrefix20210710\libev\IOEvent($callback, $stream, \ECSPrefix20210710\libev\IOEvent::WRITE);
        $this->loop->add($event);
    ...
    public function addTimer($interval, $callback)
    {
        ...
        $event = new \ECSPrefix20210710\libev\TimerEvent($callback, $timer->getInterval());
    ...
    public function addPeriodicTimer($interval, $callback)
    {
        ...
        $event = new \ECSPrefix20210710\libev\TimerEvent($callback, $interval, $interval);
        $this->timerEvents->attach($timer, $event);
        $this->loop->add($event);
        return $timer;
    }
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = new \ECSPrefix20210710\libev\SignalEvent(function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
    ...
    public function run()
    {
        ...
        while ($this->running) {
            ...
            $flags = \ECSPrefix20210710\libev\EventLoop::RUN_ONCE;
            if (!$this->running || !$this->futureTickQueue->isEmpty()) {
                $flags |= \ECSPrefix20210710\libev\EventLoop::RUN_NOWAIT;
