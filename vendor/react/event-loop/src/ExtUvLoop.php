 ...
 */
final class ExtUvLoop implements \ECSPrefix20211030\React\EventLoop\LoopInterface
{
    private $uv;
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \ECSPrefix20211030\uv_signal_init($this->uv);
            \ECSPrefix20211030\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \ECSPrefix20211030\uv_poll_init_socket($this->uv, $stream);
        }
