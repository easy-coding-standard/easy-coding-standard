...
use Event;
use EventBase;
use ECSPrefix20211107\React\EventLoop\Tick\FutureTickQueue;
use ECSPrefix20211107\React\EventLoop\Timer\Timer;
 ...
 */
final class ExtLibeventLoop implements \ECSPrefix20211107\React\EventLoop\LoopInterface
{
    /** @internal */
    ...
    public function __construct()
    {
        ...
        $this->futureTickQueue = new \ECSPrefix20211107\React\EventLoop\Tick\FutureTickQueue();
        $this->timerEvents = new \SplObjectStorage();
        $this->signals = new \ECSPrefix20211107\React\EventLoop\SignalsHandler();
    ...
    {
        $timer = new \ECSPrefix20211107\React\EventLoop\Timer\Timer($interval, $callback, \false);
        $this->scheduleTimer($timer);
        return $timer;
    }
    public function addPeriodicTimer($interval, $callback)
    {
        $timer = new \ECSPrefix20211107\React\EventLoop\Timer\Timer($interval, $callback, \true);
        $this->scheduleTimer($timer);
        return $timer;
    }
    /**
     ...
     */
    private function scheduleTimer(\ECSPrefix20211107\React\EventLoop\TimerInterface $timer)
    {
    ...
    private function createTimerCallback()
    {
        ...
        $this->timerCallback = function ($_, $__, $timer) use($timers, $that) {
            ...
            if ($timer->isPeriodic()) {
                \event_add($timers[$timer], $timer->getInterval() * \ECSPrefix20211107\React\EventLoop\ExtLibeventLoop::MICROSECONDS_PER_SECOND);
