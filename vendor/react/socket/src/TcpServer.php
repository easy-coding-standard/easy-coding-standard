 ...
 */
final class TcpServer extends \ECSPrefix20220220\Evenement\EventEmitter implements \ECSPrefix20220220\React\Socket\ServerInterface
{
    private $master;
    private $loop;
     ...
     */
    public function __construct($uri, \ECSPrefix20220220\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        ...
        if (\false === $this->master) {
            if ($errno === 0) {
                ...
                $errno = \ECSPrefix20220220\React\Socket\SocketServer::errno($errstr);
            }
            throw new \RuntimeException('Failed to listen on "' . $uri . '": ' . $errstr . \ECSPrefix20220220\React\Socket\SocketServer::errconst($errno), $errno);
        }
    ...
    {
        $this->emit('connection', array(new \ECSPrefix20220220\React\Socket\Connection($socket, $this->loop)));
    }
}
