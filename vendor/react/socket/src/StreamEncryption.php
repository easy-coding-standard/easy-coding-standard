...
use ECSPrefix20220306\React\Promise\Deferred;
...
class StreamEncryption
{
    ...
    private $method;
    private $server;
    public function __construct(\ECSPrefix20220306\React\EventLoop\LoopInterface $loop, $server = \true)
    {
        $this->loop = $loop;
        $this->server = $server;
    ...
    public function enable(\ECSPrefix20220306\React\Socket\Connection $stream)
    {
        return $this->toggle($stream, \true);
    }
    public function toggle(\ECSPrefix20220306\React\Socket\Connection $stream, $toggle)
    {
        // pause actual stream instance to continue operation on raw stream socket
        $stream->pause();
        // TODO: add write() event to make sure we're not sending any excessive data
        // cancelling this leaves this stream in an inconsistent state…
        $deferred = new \ECSPrefix20220306\React\Promise\Deferred(function () {
    ...
    public function toggleCrypto($socket, \ECSPrefix20220306\React\Promise\Deferred $deferred, $toggle, $method)
    {
        $error = null;
