...
final class SocketServer extends \ECSPrefix20220202\Evenement\EventEmitter implements \ECSPrefix20220202\React\Socket\ServerInterface
{
    private $server;
    /**
     ...
     */
    public function __construct($uri, array $context = array(), \ECSPrefix20220202\React\EventLoop\LoopInterface $loop = null)
    {
        ...
        if ($scheme === 'unix') {
            $server = new \ECSPrefix20220202\React\Socket\UnixServer($uri, $loop, $context['unix']);
        } elseif ($scheme === 'php') {
            $server = new \ECSPrefix20220202\React\Socket\FdServer($uri, $loop);
        } else {
            if (\preg_match('#^(?:\\w+://)?\\d+$#', $uri)) {
                throw new \InvalidArgumentException('Invalid URI given (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : 22);
            }
            $server = new \ECSPrefix20220202\React\Socket\TcpServer(\str_replace('tls://', '', $uri), $loop, $context['tcp']);
            if ($scheme === 'tls') {
                $server = new \ECSPrefix20220202\React\Socket\SecureServer($server, $loop, $context['tls']);
            }
        }
        $this->server = $server;
        $that = $this;
        $server->on('connection', function (\ECSPrefix20220202\React\Socket\ConnectionInterface $conn) use($that) {
