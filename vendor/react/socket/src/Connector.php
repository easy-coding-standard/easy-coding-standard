...
use ECSPrefix20211104\React\Dns\Config\Config as DnsConfig;
use ECSPrefix20211104\React\Dns\Resolver\Factory as DnsFactory;
 ...
 */
final class Connector implements \ECSPrefix20211104\React\Socket\ConnectorInterface
{
    ...
    public function __construct($context = array(), $loop = null)
    {
        ...
        if (($context instanceof \ECSPrefix20211104\React\EventLoop\LoopInterface || $context === null) && (\func_num_args() <= 1 || \is_array($loop))) {
            $swap = $loop === null ? array() : $loop;
            $loop = $context;
            $context = $swap;
        }
        if (!\is_array($context) || $loop !== null && !$loop instanceof \ECSPrefix20211104\React\EventLoop\LoopInterface) {
        ...
        if ($context['tcp'] instanceof \ECSPrefix20211104\React\Socket\ConnectorInterface) {
            $tcp = $context['tcp'];
        } else {
            $tcp = new \ECSPrefix20211104\React\Socket\TcpConnector($loop, \is_array($context['tcp']) ? $context['tcp'] : array());
        }
        if ($context['dns'] !== \false) {
            if ($context['dns'] instanceof \ECSPrefix20211104\React\Dns\Resolver\ResolverInterface) {
                ...
                if ($context['dns'] !== \true) {
                    ...
                    $config = \ECSPrefix20211104\React\Dns\Config\Config::loadSystemConfigBlocking();
                    if (!$config->nameservers) {
                        $config->nameservers[] = '8.8.8.8';
                        // @codeCoverageIgnore
                    }
                }
                $factory = new \ECSPrefix20211104\React\Dns\Resolver\Factory();
                $resolver = $factory->createCached($config, $loop);
            }
            if ($context['happy_eyeballs'] === \true) {
                $tcp = new \ECSPrefix20211104\React\Socket\HappyEyeBallsConnector($loop, $tcp, $resolver);
            } else {
                $tcp = new \ECSPrefix20211104\React\Socket\DnsConnector($tcp, $resolver);
            }
        }
        if ($context['tcp'] !== \false) {
            $context['tcp'] = $tcp;
            if ($context['timeout'] !== \false) {
                $context['tcp'] = new \ECSPrefix20211104\React\Socket\TimeoutConnector($context['tcp'], $context['timeout'], $loop);
            }
            $this->connectors['tcp'] = $context['tcp'];
        }
        if ($context['tls'] !== \false) {
            if (!$context['tls'] instanceof \ECSPrefix20211104\React\Socket\ConnectorInterface) {
                $context['tls'] = new \ECSPrefix20211104\React\Socket\SecureConnector($tcp, $loop, \is_array($context['tls']) ? $context['tls'] : array());
            }
            if ($context['timeout'] !== \false) {
                $context['tls'] = new \ECSPrefix20211104\React\Socket\TimeoutConnector($context['tls'], $context['timeout'], $loop);
            }
            $this->connectors['tls'] = $context['tls'];
        }
        if ($context['unix'] !== \false) {
            if (!$context['unix'] instanceof \ECSPrefix20211104\React\Socket\ConnectorInterface) {
                $context['unix'] = new \ECSPrefix20211104\React\Socket\UnixConnector($loop);
            }
    ...
    public function connect($uri)
    {
        ...
        if (!isset($this->connectors[$scheme])) {
            return \ECSPrefix20211104\React\Promise\reject(new \RuntimeException('No connector available for URI scheme "' . $scheme . '"'));
        }
