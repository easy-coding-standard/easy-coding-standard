...
final class TcpConnector implements \ECSPrefix20211111\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    public function __construct(\ECSPrefix20211111\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        $this->loop = $loop ?: \ECSPrefix20211111\React\EventLoop\Loop::get();
    ...
    public function connect($uri)
    {
        ...
        if (\false === \filter_var($ip, \FILTER_VALIDATE_IP)) {
            return \ECSPrefix20211111\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $ip . '" does not contain a valid host IP'));
        }
        ...
        if (\false === $stream) {
            return \ECSPrefix20211111\React\Promise\reject(new \RuntimeException(\sprintf("Connection to %s failed: %s", $uri, $errstr), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new \ECSPrefix20211111\React\Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    $resolve(new \ECSPrefix20211111\React\Socket\Connection($stream, $loop));
                }
            });
