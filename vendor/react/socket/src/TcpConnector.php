...
use ECSPrefix20220525\React\EventLoop\Loop;
use ECSPrefix20220525\React\EventLoop\LoopInterface;
use ECSPrefix20220525\React\Promise;
use InvalidArgumentException;
use RuntimeException;
final class TcpConnector implements \ECSPrefix20220525\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    public function __construct(\ECSPrefix20220525\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        $this->loop = $loop ?: \ECSPrefix20220525\React\EventLoop\Loop::get();
    ...
    public function connect($uri)
    {
        ...
        if (!$parts || !isset($parts['scheme'], $parts['host'], $parts['port']) || $parts['scheme'] !== 'tcp') {
            return \ECSPrefix20220525\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $uri . '" is invalid (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : 22));
        }
        $ip = \trim($parts['host'], '[]');
        if (@\inet_pton($ip) === \false) {
            return \ECSPrefix20220525\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $uri . '" does not contain a valid host IP (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : 22));
        }
        ...
        if (\false === $stream) {
            return \ECSPrefix20220525\React\Promise\reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \ECSPrefix20220525\React\Socket\SocketServer::errconst($errno), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new \ECSPrefix20220525\React\Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    // @codeCoverageIgnoreEnd
                    \fclose($stream);
                    $reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \ECSPrefix20220525\React\Socket\SocketServer::errconst($errno), $errno));
                } else {
                    $resolve(new \ECSPrefix20220525\React\Socket\Connection($stream, $loop));
                }
            });
