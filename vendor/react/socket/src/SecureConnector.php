...
final class SecureConnector implements \ECSPrefix20220607\React\Socket\ConnectorInterface
{
    private $connector;
    private $streamEncryption;
    private $context;
    public function __construct(\ECSPrefix20220607\React\Socket\ConnectorInterface $connector, \ECSPrefix20220607\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        $this->connector = $connector;
        $this->streamEncryption = new \ECSPrefix20220607\React\Socket\StreamEncryption($loop ?: \ECSPrefix20220607\React\EventLoop\Loop::get(), \false);
        $this->context = $context;
    }
    public function connect($uri)
    {
        if (!\function_exists('stream_socket_enable_crypto')) {
            return \ECSPrefix20220607\React\Promise\reject(new \BadMethodCallException('Encryption not supported on your platform (HHVM < 3.8?)'));
            // @codeCoverageIgnore
        }
        if (\strpos($uri, '://') === \false) {
            $uri = 'tls://' . $uri;
        ...
        $connected = \false;
        $promise = $this->connector->connect(\str_replace('tls://', '', $uri))->then(function (\ECSPrefix20220607\React\Socket\ConnectionInterface $connection) use($context, $encryption, $uri, &$promise, &$connected) {
            // (unencrypted) TCP/IP connection succeeded
            $connected = \true;
            if (!$connection instanceof \ECSPrefix20220607\React\Socket\Connection) {
                $connection->close();
