...
final class DnsConnector implements \ECSPrefix20211230\React\Socket\ConnectorInterface
{
    private $connector;
    private $resolver;
    public function __construct(\ECSPrefix20211230\React\Socket\ConnectorInterface $connector, \ECSPrefix20211230\React\Dns\Resolver\ResolverInterface $resolver)
    {
    ...
    public function connect($uri)
    {
        ...
        $resolved = null;
        return new \ECSPrefix20211230\React\Promise\Promise(function ($resolve, $reject) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
            // resolve/reject with result of DNS lookup
            $promise->then(function ($ip) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
                $resolved = $ip;
                return $promise = $connector->connect(\ECSPrefix20211230\React\Socket\Connector::uri($parts, $host, $ip))->then(null, function (\Exception $e) use($uri) {
            ...
            if ($promise instanceof \ECSPrefix20211230\React\Promise\CancellablePromiseInterface) {
