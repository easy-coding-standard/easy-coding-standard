...
final class DnsConnector implements \ECSPrefix20220607\React\Socket\ConnectorInterface
{
    private $connector;
    private $resolver;
    public function __construct(\ECSPrefix20220607\React\Socket\ConnectorInterface $connector, \ECSPrefix20220607\React\Dns\Resolver\ResolverInterface $resolver)
    {
    ...
    public function connect($uri)
    {
        ...
        $resolved = null;
        return new \ECSPrefix20220607\React\Promise\Promise(function ($resolve, $reject) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
            // resolve/reject with result of DNS lookup
            $promise->then(function ($ip) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
                $resolved = $ip;
                return $promise = $connector->connect(\ECSPrefix20220607\React\Socket\Connector::uri($parts, $host, $ip))->then(null, function (\Exception $e) use($uri) {
            ...
            if ($promise instanceof \ECSPrefix20220607\React\Promise\CancellablePromiseInterface) {
