 ...
 */
final class UnixServer extends \ECSPrefix20220605\Evenement\EventEmitter implements \ECSPrefix20220605\React\Socket\ServerInterface
{
    private $master;
    private $loop;
     ...
     */
    public function __construct($path, \ECSPrefix20220605\React\EventLoop\LoopInterface $loop = null, array $context = array())
    {
        ...
        if (\false === $this->master) {
            ...
            throw new \RuntimeException('Failed to listen on Unix domain socket "' . $path . '": ' . $errstr . \ECSPrefix20220605\React\Socket\SocketServer::errconst($errno), $errno);
        }
    ...
    public function resume()
    {
        ...
        $this->loop->addReadStream($this->master, function ($master) use($that) {
            try {
                $newSocket = \ECSPrefix20220605\React\Socket\SocketServer::accept($master);
    ...
    {
        $connection = new \ECSPrefix20220605\React\Socket\Connection($socket, $this->loop);
        $connection->unix = \true;
