...
use ECSPrefix20220527\React\Dns\Model\Message;
use ECSPrefix20220527\React\Dns\Resolver\ResolverInterface;
use ECSPrefix20220527\React\EventLoop\LoopInterface;
use ECSPrefix20220527\React\EventLoop\TimerInterface;
use ECSPrefix20220527\React\Promise;
use ECSPrefix20220527\React\Promise\CancellablePromiseInterface;
/**
 * @internal
 */
final class HappyEyeBallsConnectionBuilder
{
    ...
    public $uri;
    public $host;
    public $resolved = array(\ECSPrefix20220527\React\Dns\Model\Message::TYPE_A => \false, \ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA => \false);
    ...
    public $lastError6;
    public $lastError4;
    public function __construct(\ECSPrefix20220527\React\EventLoop\LoopInterface $loop, \ECSPrefix20220527\React\Socket\ConnectorInterface $connector, \ECSPrefix20220527\React\Dns\Resolver\ResolverInterface $resolver, $uri, $host, $parts)
    {
        $this->loop = $loop;
    ...
    public function connect()
    {
        $timer = null;
        $that = $this;
        return new \ECSPrefix20220527\React\Promise\Promise(function ($resolve, $reject) use($that, &$timer) {
            ...
            $that->resolverPromises[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA] = $that->resolve(\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA, $reject)->then($lookupResolve(\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA));
            $that->resolverPromises[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_A] = $that->resolve(\ECSPrefix20220527\React\Dns\Model\Message::TYPE_A, $reject)->then(function (array $ips) use($that, &$timer) {
                // happy path: IPv6 has resolved already (or could not resolve), continue with IPv4 addresses
                if ($that->resolved[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA] === \true || !$ips) {
                    return $ips;
                }
                // Otherwise delay processing IPv4 lookup until short timer passes or IPv6 resolves in the meantime
                $deferred = new \ECSPrefix20220527\React\Promise\Deferred();
                $timer = $that->loop->addTimer($that::RESOLUTION_DELAY, function () use($deferred, $ips) {
                    $deferred->resolve($ips);
                });
                $that->resolverPromises[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA]->then(function () use($that, $timer, $deferred, $ips) {
                    $that->loop->cancelTimer($timer);
                    $deferred->resolve($ips);
                });
                return $deferred->promise();
            })->then($lookupResolve(\ECSPrefix20220527\React\Dns\Model\Message::TYPE_A));
            ...
            $_ = $reject = null;
            $that->cleanUp();
            if ($timer instanceof \ECSPrefix20220527\React\EventLoop\TimerInterface) {
                $that->loop->cancelTimer($timer);
            }
        });
    }
    ...
    public function resolve($type, $reject)
    {
        ...
        return $that->resolver->resolveAll($that->host, $type)->then(null, function (\Exception $e) use($type, $reject, $that) {
            unset($that->resolverPromises[$type]);
            $that->resolved[$type] = \true;
            if ($type === \ECSPrefix20220527\React\Dns\Model\Message::TYPE_A) {
    ...
    public function check($resolve, $reject)
    {
        ...
        if ($this->nextAttemptTimer === null && (\count($this->connectQueue) > 0 || $this->resolved[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_A] === \false || $this->resolved[\ECSPrefix20220527\React\Dns\Model\Message::TYPE_AAAA] === \false)) {
    ...
    {
        $uri = \ECSPrefix20220527\React\Socket\Connector::uri($this->parts, $this->host, $ip);
    ...
    public function cleanUp()
    {
        ...
        foreach ($this->connectionPromises as $connectionPromise) {
            if ($connectionPromise instanceof \ECSPrefix20220527\React\Promise\CancellablePromiseInterface) {
                $connectionPromise->cancel();
            }
        }
        foreach ($this->resolverPromises as $resolverPromise) {
            if ($resolverPromise instanceof \ECSPrefix20220527\React\Promise\CancellablePromiseInterface) {
                $resolverPromise->cancel();
            }
        }
        if ($this->nextAttemptTimer instanceof \ECSPrefix20220527\React\EventLoop\TimerInterface) {
