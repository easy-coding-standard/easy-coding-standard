...
use ECSPrefix20211119\React\Dns\RecordNotFoundException;
/**
 * @see ResolverInterface for the base interface
 */
final class Resolver implements \ECSPrefix20211119\React\Dns\Resolver\ResolverInterface
{
    private $executor;
    public function __construct(\ECSPrefix20211119\React\Dns\Query\ExecutorInterface $executor)
    {
        $this->executor = $executor;
    }
    public function resolve($domain)
    {
        return $this->resolveAll($domain, \ECSPrefix20211119\React\Dns\Model\Message::TYPE_A)->then(function (array $ips) {
            return $ips[\array_rand($ips)];
        });
    }
    public function resolveAll($domain, $type)
    {
        $query = new \ECSPrefix20211119\React\Dns\Query\Query($domain, $type, \ECSPrefix20211119\React\Dns\Model\Message::CLASS_IN);
        $that = $this;
        return $this->executor->query($query)->then(function (\ECSPrefix20211119\React\Dns\Model\Message $response) use($query, $that) {
    ...
    public function extractValues($query, $response)
    {
        ...
        $code = $response->rcode;
        if ($code !== \ECSPrefix20211119\React\Dns\Model\Message::RCODE_OK) {
            switch ($code) {
                case \ECSPrefix20211119\React\Dns\Model\Message::RCODE_FORMAT_ERROR:
                    $message = 'Format Error';
                    break;
                case \ECSPrefix20211119\React\Dns\Model\Message::RCODE_SERVER_FAILURE:
                    $message = 'Server Failure';
                    break;
                case \ECSPrefix20211119\React\Dns\Model\Message::RCODE_NAME_ERROR:
                    $message = 'Non-Existent Domain / NXDOMAIN';
                    break;
                case \ECSPrefix20211119\React\Dns\Model\Message::RCODE_NOT_IMPLEMENTED:
                    $message = 'Not Implemented';
                    break;
                case \ECSPrefix20211119\React\Dns\Model\Message::RCODE_REFUSED:
                    $message = 'Refused';
                    break;
                default:
                    $message = 'Unknown error response code ' . $code;
            }
            throw new \ECSPrefix20211119\React\Dns\RecordNotFoundException('DNS query for ' . $query->describe() . ' returned an error response (' . $message . ')', $code);
        }
        ...
        if (0 === \count($addresses)) {
            throw new \ECSPrefix20211119\React\Dns\RecordNotFoundException('DNS query for ' . $query->describe() . ' did not return a valid answer (NOERROR / NODATA)');
        }
    ...
    private function valuesByNameAndType(array $answers, $name, $type)
    {
        ...
        $cnameRecords = $this->filterByType($named, \ECSPrefix20211119\React\Dns\Model\Message::TYPE_CNAME);
        if ($cnameRecords) {
