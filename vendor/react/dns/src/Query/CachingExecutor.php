...
use ECSPrefix20211207\React\Cache\CacheInterface;
use ECSPrefix20211207\React\Dns\Model\Message;
use ECSPrefix20211207\React\Promise\Promise;
final class CachingExecutor implements \ECSPrefix20211207\React\Dns\Query\ExecutorInterface
{
    /**
    ...
    private $executor;
    private $cache;
    public function __construct(\ECSPrefix20211207\React\Dns\Query\ExecutorInterface $executor, \ECSPrefix20211207\React\Cache\CacheInterface $cache)
    {
    ...
    public function query($query)
    {
        ...
        return new \ECSPrefix20211207\React\Promise\Promise(function ($resolve, $reject) use($query, $id, $cache, $executor, &$pending, $that) {
            $pending->then(function ($message) use($query, $id, $cache, $executor, &$pending, $that) {
                ...
                return $pending = $executor->query($query)->then(function (\ECSPrefix20211207\React\Dns\Model\Message $message) use($cache, $id, $that) {
