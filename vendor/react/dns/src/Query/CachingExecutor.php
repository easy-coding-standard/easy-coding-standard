...
use ECSPrefix20220316\React\Cache\CacheInterface;
use ECSPrefix20220316\React\Dns\Model\Message;
use ECSPrefix20220316\React\Promise\Promise;
final class CachingExecutor implements \ECSPrefix20220316\React\Dns\Query\ExecutorInterface
{
    /**
    ...
    private $executor;
    private $cache;
    public function __construct(\ECSPrefix20220316\React\Dns\Query\ExecutorInterface $executor, \ECSPrefix20220316\React\Cache\CacheInterface $cache)
    {
    ...
    public function query(\ECSPrefix20220316\React\Dns\Query\Query $query)
    {
        ...
        return new \ECSPrefix20220316\React\Promise\Promise(function ($resolve, $reject) use($query, $id, $cache, $executor, &$pending, $that) {
            $pending->then(function ($message) use($query, $id, $cache, $executor, &$pending, $that) {
                ...
                return $pending = $executor->query($query)->then(function (\ECSPrefix20220316\React\Dns\Model\Message $message) use($cache, $id, $that) {
     ...
     * @internal
     */
    public function ttl(\ECSPrefix20220316\React\Dns\Model\Message $message)
    {
