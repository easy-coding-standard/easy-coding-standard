...
use ECSPrefix20220403\React\Dns\Config\HostsFile;
 ...
 */
final class HostsFileExecutor implements \ECSPrefix20220403\React\Dns\Query\ExecutorInterface
{
    private $hosts;
    private $fallback;
    public function __construct(\ECSPrefix20220403\React\Dns\Config\HostsFile $hosts, \ECSPrefix20220403\React\Dns\Query\ExecutorInterface $fallback)
    {
        $this->hosts = $hosts;
    ...
    {
        if ($query->class === \ECSPrefix20220403\React\Dns\Model\Message::CLASS_IN && ($query->type === \ECSPrefix20220403\React\Dns\Model\Message::TYPE_A || $query->type === \ECSPrefix20220403\React\Dns\Model\Message::TYPE_AAAA)) {
            // forward lookup for type A or AAAA
            $records = array();
            $expectsColon = $query->type === \ECSPrefix20220403\React\Dns\Model\Message::TYPE_AAAA;
            foreach ($this->hosts->getIpsForHost($query->name) as $ip) {
                ...
                if ((\strpos($ip, ':') !== \false) === $expectsColon) {
                    $records[] = new \ECSPrefix20220403\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $ip);
                }
            }
            if ($records) {
                return \ECSPrefix20220403\React\Promise\resolve(\ECSPrefix20220403\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
            }
        } elseif ($query->class === \ECSPrefix20220403\React\Dns\Model\Message::CLASS_IN && $query->type === \ECSPrefix20220403\React\Dns\Model\Message::TYPE_PTR) {
            ...
            if ($ip !== null) {
                ...
                foreach ($this->hosts->getHostsForIp($ip) as $host) {
                    $records[] = new \ECSPrefix20220403\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $host);
                }
                if ($records) {
                    return \ECSPrefix20220403\React\Promise\resolve(\ECSPrefix20220403\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
                }
