 ...
 */
class TcpTransportExecutor implements \ECSPrefix20220207\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\ECSPrefix20220207\React\Dns\Query\Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \ECSPrefix20220207\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for TCP transport'));
        }
        ...
        $names =& $this->names;
        $that = $this;
        $deferred = new \ECSPrefix20220207\React\Promise\Deferred(function () use($that, &$names, $request) {
            // remove from list of pending names, but remember pending query
            $name = $names[$request->id];
            unset($names[$request->id]);
            $that->checkIdle();
            throw new \ECSPrefix20220207\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
