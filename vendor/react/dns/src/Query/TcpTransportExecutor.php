...
use ECSPrefix20220607\React\Dns\Protocol\BinaryDumper;
use ECSPrefix20220607\React\Dns\Protocol\Parser;
 ...
 */
class TcpTransportExecutor implements \ECSPrefix20220607\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\ECSPrefix20220607\React\Dns\Query\Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \ECSPrefix20220607\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for TCP transport'));
        }
        ...
        if ($this->socket === null) {
            ...
            if ($socket === \false) {
                return \ECSPrefix20220607\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
            }
        ...
        $names =& $this->names;
        $that = $this;
        $deferred = new \ECSPrefix20220607\React\Promise\Deferred(function () use($that, &$names, $request) {
            // remove from list of pending names, but remember pending query
            $name = $names[$request->id];
            unset($names[$request->id]);
            $that->checkIdle();
            throw new \ECSPrefix20220607\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
