...
final class RetryExecutor implements \ECSPrefix20220601\React\Dns\Query\ExecutorInterface
{
    private $executor;
    private $retries;
    public function __construct(\ECSPrefix20220601\React\Dns\Query\ExecutorInterface $executor, $retries = 2)
    {
    ...
    public function tryQuery(\ECSPrefix20220601\React\Dns\Query\Query $query, $retries)
    {
        $deferred = new \ECSPrefix20220601\React\Promise\Deferred(function () use(&$promise) {
        ...
        $errorback = function ($e) use($deferred, &$promise, $query, $success, &$errorback, &$retries, $executor) {
            if (!$e instanceof \ECSPrefix20220601\React\Dns\Query\TimeoutException) {
                $errorback = null;
                $deferred->reject($e);
            } elseif ($retries <= 0) {
                $errorback = null;
