...
final class RetryExecutor implements \ECSPrefix20211210\React\Dns\Query\ExecutorInterface
{
    private $executor;
    private $retries;
    public function __construct(\ECSPrefix20211210\React\Dns\Query\ExecutorInterface $executor, $retries = 2)
    {
    ...
    public function tryQuery($query, $retries)
    {
        ...
        $errorback = function ($e) use($deferred, &$promise, $query, $success, &$errorback, &$retries, $executor) {
            if (!$e instanceof \ECSPrefix20211210\React\Dns\Query\TimeoutException) {
                $errorback = null;
                $deferred->reject($e);
            } elseif ($retries <= 0) {
                $errorback = null;
