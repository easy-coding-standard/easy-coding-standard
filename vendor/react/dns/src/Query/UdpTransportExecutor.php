...
use ECSPrefix20220126\React\Dns\Protocol\BinaryDumper;
use ECSPrefix20220126\React\Dns\Protocol\Parser;
 ...
 */
final class UdpTransportExecutor implements \ECSPrefix20220126\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
     ...
     */
    public function __construct($nameserver, \ECSPrefix20220126\React\EventLoop\LoopInterface $loop = null)
    {
        ...
        $this->parser = new \ECSPrefix20220126\React\Dns\Protocol\Parser();
        $this->dumper = new \ECSPrefix20220126\React\Dns\Protocol\BinaryDumper();
    }
    public function query(\ECSPrefix20220126\React\Dns\Query\Query $query)
    {
        $request = \ECSPrefix20220126\React\Dns\Model\Message::createRequestForQuery($query);
        $queryData = $this->dumper->toBinary($request);
        if (isset($queryData[$this->maxPacketSize])) {
            return \ECSPrefix20220126\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \ECSPrefix20220126\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            ...
            return \ECSPrefix20220126\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . (isset($m[2]) ? $m[2] : $error['message']) . ')', isset($m[1]) ? (int) $m[1] : 0));
        }
        $loop = $this->loop;
        $deferred = new \ECSPrefix20220126\React\Promise\Deferred(function () use($loop, $socket, $query) {
            // cancellation should remove socket from loop and close socket
            $loop->removeReadStream($socket);
            \fclose($socket);
            throw new \ECSPrefix20220126\React\Dns\Query\CancellationException('DNS query for ' . $query->describe() . ' has been cancelled');
        });
