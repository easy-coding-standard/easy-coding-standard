...
use ECSPrefix202308\React\Dns\Protocol\BinaryDumper;
use ECSPrefix202308\React\Dns\Protocol\Parser;
...
final class UdpTransportExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if (isset($queryData[$this->maxPacketSize])) {
            return \ECSPrefix202308\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \ECSPrefix202308\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            return \ECSPrefix202308\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        $loop = $this->loop;
