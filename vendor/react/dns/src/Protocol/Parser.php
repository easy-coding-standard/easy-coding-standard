...
namespace ECSPrefix20220215\React\Dns\Protocol;

use ECSPrefix20220215\React\Dns\Model\Message;
use ECSPrefix20220215\React\Dns\Model\Record;
use ECSPrefix20220215\React\Dns\Query\Query;
...
final class Parser
{
    ...
    private function parse($data, $consumed)
    {
        ...
        $message = new \ECSPrefix20220215\React\Dns\Model\Message();
        $message->id = $id;
    ...
    private function parseQuestion($data, $consumed)
    {
        ...
        $consumed += 4;
        return array(new \ECSPrefix20220215\React\Dns\Query\Query(\implode('.', $labels), $type, $class), $consumed);
    }
    /**
     * @param string $data
     * @param int $consumed
    ...
    private function parseRecord($data, $consumed)
    {
        ...
        if (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_A === $type) {
            if ($rdLength === 4) {
                $rdata = \inet_ntop(\substr($data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_AAAA === $type) {
            if ($rdLength === 16) {
                $rdata = \inet_ntop(\substr($data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_CNAME === $type || \ECSPrefix20220215\React\Dns\Model\Message::TYPE_PTR === $type || \ECSPrefix20220215\React\Dns\Model\Message::TYPE_NS === $type) {
            list($rdata, $consumed) = $this->readDomain($data, $consumed);
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_TXT === $type || \ECSPrefix20220215\React\Dns\Model\Message::TYPE_SPF === $type) {
            $rdata = array();
        ...
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_MX === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_SRV === $type) {
            if ($rdLength > 6) {
        ...
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_SSHFP === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_SOA === $type) {
        ...
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_OPT === $type) {
            $rdata = array();
            while (isset($data[$consumed + 4 - 1])) {
                ...
                if ($code === \ECSPrefix20220215\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value === '') {
                    $value = null;
                } elseif ($code === \ECSPrefix20220215\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $length === 2) {
                    list($value) = \array_values(\unpack('n', $value));
                    $value = \round($value * 0.1, 1);
                } elseif ($code === \ECSPrefix20220215\React\Dns\Model\Message::OPT_TCP_KEEPALIVE) {
                    break;
                }
                $rdata[$code] = $value;
                $consumed += 4 + $length;
            }
        } elseif (\ECSPrefix20220215\React\Dns\Model\Message::TYPE_CAA === $type) {
            if ($rdLength > 3) {
        ...
        return array(new \ECSPrefix20220215\React\Dns\Model\Record($name, $type, $class, $ttl, $rdata), $consumed);
    }
