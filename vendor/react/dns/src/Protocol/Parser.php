...
final class Parser
{
    ...
    public function parseMessage($data)
    {
        ...
        $message = new \ECSPrefix20211216\React\Dns\Model\Message();
        $message->data = $data;
    ...
    private function parse($data, \ECSPrefix20211216\React\Dns\Model\Message $message)
    {
     ...
     * @return ?Query
     */
    private function parseQuestion(\ECSPrefix20211216\React\Dns\Model\Message $message)
    {
        ...
        return new \ECSPrefix20211216\React\Dns\Query\Query(\implode('.', $labels), $type, $class);
    }
    /**
     * @param Message $message
     * @return ?Record returns parsed Record on success or null if data is invalid/incomplete
     */
    private function parseRecord(\ECSPrefix20211216\React\Dns\Model\Message $message)
    {
        $consumed = $message->consumed;
        ...
        if (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_A === $type) {
            if ($rdLength === 4) {
                $rdata = \inet_ntop(\substr($message->data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_AAAA === $type) {
            if ($rdLength === 16) {
                $rdata = \inet_ntop(\substr($message->data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_CNAME === $type || \ECSPrefix20211216\React\Dns\Model\Message::TYPE_PTR === $type || \ECSPrefix20211216\React\Dns\Model\Message::TYPE_NS === $type) {
            list($rdata, $consumed) = $this->readDomain($message->data, $consumed);
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_TXT === $type || \ECSPrefix20211216\React\Dns\Model\Message::TYPE_SPF === $type) {
            $rdata = array();
        ...
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_MX === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_SRV === $type) {
            if ($rdLength > 6) {
        ...
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_SSHFP === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_SOA === $type) {
        ...
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_OPT === $type) {
            $rdata = array();
            while (isset($message->data[$consumed + 4 - 1])) {
                ...
                if ($code === \ECSPrefix20211216\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value === '') {
                    $value = null;
                } elseif ($code === \ECSPrefix20211216\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $length === 2) {
                    list($value) = \array_values(\unpack('n', $value));
                    $value = \round($value * 0.1, 1);
                } elseif ($code === \ECSPrefix20211216\React\Dns\Model\Message::OPT_TCP_KEEPALIVE) {
                    break;
                }
                $rdata[$code] = $value;
                $consumed += 4 + $length;
            }
        } elseif (\ECSPrefix20211216\React\Dns\Model\Message::TYPE_CAA === $type) {
            if ($rdLength > 3) {
        ...
        $message->consumed = $consumed;
        return new \ECSPrefix20211216\React\Dns\Model\Record($name, $type, $class, $ttl, $rdata);
    }
