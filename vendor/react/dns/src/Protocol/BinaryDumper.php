...
final class BinaryDumper
{
     ...
     * @return string
     */
    public function toBinary(\ECSPrefix20220313\React\Dns\Model\Message $message)
    {
        $data = '';
     ...
     * @return string
     */
    private function headerToBinary(\ECSPrefix20220313\React\Dns\Model\Message $message)
    {
        $data = '';
    ...
    private function recordsToBinary(array $records)
    {
        ...
        foreach ($records as $record) {
            /* @var $record Record */
            switch ($record->type) {
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_A:
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_AAAA:
                    $binary = \inet_pton($record->data);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_CNAME:
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_NS:
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_PTR:
                    $binary = $this->domainNameToBinary($record->data);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_TXT:
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_SPF:
                    $binary = $this->textsToBinary($record->data);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_MX:
                    $binary = \pack('n', $record->data['priority']);
                    $binary .= $this->domainNameToBinary($record->data['target']);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_SRV:
                    $binary = \pack('n*', $record->data['priority'], $record->data['weight'], $record->data['port']);
                    $binary .= $this->domainNameToBinary($record->data['target']);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_SOA:
                    ...
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_CAA:
                    $binary = \pack('C*', $record->data['flag'], \strlen($record->data['tag']));
                    $binary .= $record->data['tag'];
                    $binary .= $record->data['value'];
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_SSHFP:
                    $binary = \pack('CCH*', $record->data['algorithm'], $record->data['type'], $record->data['fingerprint']);
                    break;
                case \ECSPrefix20220313\React\Dns\Model\Message::TYPE_OPT:
                    $binary = '';
                    foreach ($record->data as $opt => $value) {
                        if ($opt === \ECSPrefix20220313\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value !== null) {
