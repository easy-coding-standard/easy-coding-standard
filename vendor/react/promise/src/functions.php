...
{
    if ($promiseOrValue instanceof \ECSPrefix20220403\React\Promise\ExtendedPromiseInterface) {
    ...
    if (\is_object($promiseOrValue) && \method_exists($promiseOrValue, 'then')) {
        ...
        return new \ECSPrefix20220403\React\Promise\Promise(function ($resolve, $reject, $notify) use($promiseOrValue) {
            $promiseOrValue->then($resolve, $reject, $notify);
        }, $canceller);
    }
    return new \ECSPrefix20220403\React\Promise\FulfilledPromise($promiseOrValue);
}
/**
...
{
    if ($promiseOrValue instanceof \ECSPrefix20220403\React\Promise\PromiseInterface) {
        return resolve($promiseOrValue)->then(function ($value) {
            return new \ECSPrefix20220403\React\Promise\RejectedPromise($value);
        });
    }
    return new \ECSPrefix20220403\React\Promise\RejectedPromise($promiseOrValue);
}
/**
...
{
    $cancellationQueue = new \ECSPrefix20220403\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \ECSPrefix20220403\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $cancellationQueue) {
...
function some($promisesOrValues, $howMany)
{
    $cancellationQueue = new \ECSPrefix20220403\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \ECSPrefix20220403\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $howMany, $cancellationQueue) {
        resolve($promisesOrValues)->done(function ($array) use($howMany, $cancellationQueue, $resolve, $reject, $notify) {
            ...
            $len = \count($array);
            if ($len < $howMany) {
                throw new \ECSPrefix20220403\React\Promise\Exception\LengthException(\sprintf('Input array must contain at least %d item%s but contains only %s item%s.', $howMany, 1 === $howMany ? '' : 's', $len, 1 === $len ? '' : 's'));
            }
            $toResolve = $howMany;
...
function map($promisesOrValues, callable $mapFunc)
{
    $cancellationQueue = new \ECSPrefix20220403\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \ECSPrefix20220403\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $mapFunc, $cancellationQueue) {
...
function reduce($promisesOrValues, callable $reduceFunc, $initialValue = null)
{
    $cancellationQueue = new \ECSPrefix20220403\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \ECSPrefix20220403\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $reduceFunc, $initialValue, $cancellationQueue) {
...
function _checkTypehint(callable $callback, $object)
{
    ...
    switch (\true) {
        ...
        case $type instanceof \ReflectionNamedType:
            $types = [$type];
            break;
        case $type instanceof \ECSPrefix20220403\ReflectionIntersectionType:
            $isTypeUnion = \false;
