 ...
 */
function timeout(\ECSPrefix20220514\React\Promise\PromiseInterface $promise, $time, \ECSPrefix20220514\React\EventLoop\LoopInterface $loop = null)
{
    // cancelling this promise will only try to cancel the input promise,
    // thus leaving responsibility to the input promise.
    $canceller = null;
    if ($promise instanceof \ECSPrefix20220514\React\Promise\CancellablePromiseInterface || !\interface_exists('ECSPrefix20220514\\React\\Promise\\CancellablePromiseInterface') && \method_exists($promise, 'cancel')) {
    ...
    if ($loop === null) {
        $loop = \ECSPrefix20220514\React\EventLoop\Loop::get();
    }
    return new \ECSPrefix20220514\React\Promise\Promise(function ($resolve, $reject) use($loop, $time, $promise) {
        $timer = null;
        ...
        $timer = $loop->addTimer($time, function () use($time, &$promise, $reject) {
            $reject(new \ECSPrefix20220514\React\Promise\Timer\TimeoutException($time, 'Timed out after ' . $time . ' seconds'));
 ...
 */
function sleep($time, \ECSPrefix20220514\React\EventLoop\LoopInterface $loop = null)
{
    if ($loop === null) {
        $loop = \ECSPrefix20220514\React\EventLoop\Loop::get();
    }
    $timer = null;
    return new \ECSPrefix20220514\React\Promise\Promise(function ($resolve) use($loop, $time, &$timer) {
 ...
 * @see sleep()
 */
function resolve($time, \ECSPrefix20220514\React\EventLoop\LoopInterface $loop = null)
{
 ...
 * @see sleep()
 */
function reject($time, \ECSPrefix20220514\React\EventLoop\LoopInterface $loop = null)
{
    return \sleep($time, $loop)->then(function () use($time) {
        throw new \ECSPrefix20220514\React\Promise\Timer\TimeoutException($time, 'Timer expired after ' . $time . ' seconds');
    });
}
