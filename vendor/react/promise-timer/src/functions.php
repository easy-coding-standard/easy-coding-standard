...
function timeout(\ECSPrefix20211023\React\Promise\PromiseInterface $promise, $time, \ECSPrefix20211023\React\EventLoop\LoopInterface $loop = null)
{
    ...
    if ($loop === null) {
        $loop = \ECSPrefix20211023\React\EventLoop\Loop::get();
    }
    return new \ECSPrefix20211023\React\Promise\Promise(function ($resolve, $reject) use($loop, $time, $promise) {
        $timer = null;
        ...
        $timer = $loop->addTimer($time, function () use($time, &$promise, $reject) {
            $reject(new \ECSPrefix20211023\React\Promise\Timer\TimeoutException($time, 'Timed out after ' . $time . ' seconds'));
...
function resolve($time, \ECSPrefix20211023\React\EventLoop\LoopInterface $loop = null)
{
    if ($loop === null) {
        $loop = \ECSPrefix20211023\React\EventLoop\Loop::get();
    }
    return new \ECSPrefix20211023\React\Promise\Promise(function ($resolve) use($loop, $time, &$timer) {
...
function reject($time, \ECSPrefix20211023\React\EventLoop\LoopInterface $loop = null)
{
    return resolve($time, $loop)->then(function ($time) {
        throw new \ECSPrefix20211023\React\Promise\Timer\TimeoutException($time, 'Timer expired after ' . $time . ' seconds');
    });
}
