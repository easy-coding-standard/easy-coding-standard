...
namespace ECSPrefix20220503\React\ChildProcess;

use ECSPrefix20220503\Evenement\EventEmitter;
use ECSPrefix20220503\React\EventLoop\Loop;
use ECSPrefix20220503\React\EventLoop\LoopInterface;
use ECSPrefix20220503\React\Stream\ReadableResourceStream;
use ECSPrefix20220503\React\Stream\ReadableStreamInterface;
use ECSPrefix20220503\React\Stream\WritableResourceStream;
use ECSPrefix20220503\React\Stream\WritableStreamInterface;
use ECSPrefix20220503\React\Stream\DuplexResourceStream;
use ECSPrefix20220503\React\Stream\DuplexStreamInterface;
/**
 * Process component.
 *
 ...
 */
class Process extends \ECSPrefix20220503\Evenement\EventEmitter
{
    /**
     ...
     */
    public function start(\ECSPrefix20220503\React\EventLoop\LoopInterface $loop = null, $interval = 0.1)
    {
        if ($this->isRunning()) {
            throw new \RuntimeException('Process is already running');
        }
        $loop = $loop ?: \ECSPrefix20220503\React\EventLoop\Loop::get();
        $cmd = $this->cmd;
        $fdSpec = $this->fds;
        $sigchild = null;
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \ECSPrefix20220503\React\Stream\DuplexResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            } elseif ($mode === 'w') {
                $stream = new \ECSPrefix20220503\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \ECSPrefix20220503\React\Stream\ReadableResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            }
            $this->pipes[$n] = $stream;
