...
namespace ECSPrefix20220417\React\ChildProcess;
...
use ECSPrefix20220417\React\Stream\ReadableResourceStream;
use ECSPrefix20220417\React\Stream\ReadableStreamInterface;
use ECSPrefix20220417\React\Stream\WritableResourceStream;
use ECSPrefix20220417\React\Stream\WritableStreamInterface;
use ECSPrefix20220417\React\Stream\DuplexResourceStream;
use ECSPrefix20220417\React\Stream\DuplexStreamInterface;
/**
 * Process component.
 *
 ...
 */
class Process extends \ECSPrefix20220417\Evenement\EventEmitter
{
    /**
     ...
     */
    public function start(\ECSPrefix20220417\React\EventLoop\LoopInterface $loop = null, $interval = 0.1)
    {
        if ($this->isRunning()) {
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \ECSPrefix20220417\React\Stream\DuplexResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            } elseif ($mode === 'w') {
                $stream = new \ECSPrefix20220417\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \ECSPrefix20220417\React\Stream\ReadableResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            }
            $this->pipes[$n] = $stream;
