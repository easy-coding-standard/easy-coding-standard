 ...
 */
final class Xdebug3Driver extends \ECSPrefix20210803\SebastianBergmann\CodeCoverage\Driver\Driver
{
    /**
    ...
    public function __construct(\ECSPrefix20210803\SebastianBergmann\CodeCoverage\Filter $filter)
    {
        ...
        if (\version_compare(\phpversion('xdebug'), '3', '<')) {
            throw new \ECSPrefix20210803\SebastianBergmann\CodeCoverage\Driver\WrongXdebugVersionException(\sprintf('This driver requires Xdebug 3 but version %s is loaded', \phpversion('xdebug')));
        }
        ...
        if ($mode === \false || !\in_array('coverage', \explode(',', $mode), \true)) {
            throw new \ECSPrefix20210803\SebastianBergmann\CodeCoverage\Driver\Xdebug3NotEnabledException();
        }
        if (!$filter->isEmpty()) {
