 ...
 */
final class IgnoredLinesFindingVisitor extends \ECSPrefix20210803\PhpParser\NodeVisitorAbstract
{
    /**
     * @psalm-var list<int>
     */
    ...
    public function enterNode(\ECSPrefix20210803\PhpParser\Node $node) : ?int
    {
        if (!$node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Class_ && !$node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Trait_ && !$node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Interface_ && !$node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\ClassMethod && !$node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Function_) {
            return null;
        }
        if ($node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Class_ && $node->isAnonymous()) {
            return null;
        }
        // Workaround for https://bugs.xdebug.org/view.php?id=1798
        if ($node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Class_ || $node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Trait_ || $node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Interface_) {
            $this->ignoredLines[] = $node->getStartLine();
        }
        if (!$this->useAnnotationsForIgnoringCode) {
            return null;
        }
        if ($node instanceof \ECSPrefix20210803\PhpParser\Node\Stmt\Interface_) {
            return null;
        }
