...
final class ComplexityCalculatingVisitor extends \ECSPrefix20210804\PhpParser\NodeVisitorAbstract
{
    /**
    ...
    public function enterNode($node)
    {
        ...
        $this->result[] = new \ECSPrefix20210804\SebastianBergmann\Complexity\Complexity($name, $this->cyclomaticComplexity($statements));
        if ($this->shortCircuitTraversal) {
            return \ECSPrefix20210804\PhpParser\NodeTraverser::DONT_TRAVERSE_CHILDREN;
        }
        return null;
    }
    public function result() : \ECSPrefix20210804\SebastianBergmann\Complexity\ComplexityCollection
    {
        return \ECSPrefix20210804\SebastianBergmann\Complexity\ComplexityCollection::fromList(...$this->result);
    }
    /**
    ...
    private function classMethodName(\ECSPrefix20210804\PhpParser\Node\Stmt\ClassMethod $node) : string
    {
        $parent = $node->getAttribute('parent');
        \assert($parent instanceof \ECSPrefix20210804\PhpParser\Node\Stmt\Class_ || $parent instanceof \ECSPrefix20210804\PhpParser\Node\Stmt\Trait_);
        \assert(isset($parent->namespacedName));
        \assert($parent->namespacedName instanceof \ECSPrefix20210804\PhpParser\Node\Name);
        return $parent->namespacedName->toString() . '::' . $node->name->toString();
    }
    private function functionName(\ECSPrefix20210804\PhpParser\Node\Stmt\Function_ $node) : string
    {
