 ...
 */
namespace ECSPrefix20211120\SebastianBergmann\Diff\Output;
...
use function substr;
use ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException;
use ECSPrefix20211120\SebastianBergmann\Diff\Differ;
/**
 * Strict Unified diff output builder.
 *
 * Generates (strict) Unified diff's (unidiffs) with hunks.
 */
final class StrictUnifiedDiffOutputBuilder implements \ECSPrefix20211120\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!\is_bool($options['collapseRanges'])) {
            throw new \ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException('collapseRanges', 'a bool', $options['collapseRanges']);
        }
        if (!\is_int($options['contextLines']) || $options['contextLines'] < 0) {
            throw new \ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException('contextLines', 'an int >= 0', $options['contextLines']);
        }
        if (!\is_int($options['commonLineThreshold']) || $options['commonLineThreshold'] <= 0) {
            throw new \ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException('commonLineThreshold', 'an int > 0', $options['commonLineThreshold']);
        }
    ...
    private function writeDiffHunks($output, array $diff) : void
    {
        ...
        if (0 === $diff[$upperLimit - 1][1]) {
            ...
            if ("\n" !== $lc) {
                \array_splice($diff, $upperLimit, 0, [["\n\\ No newline at end of file\n", \ECSPrefix20211120\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING]]);
            }
        } else {
        ...
        foreach ($diff as $i => $entry) {
            ...
            $sameCount = 0;
            if ($entry[1] === \ECSPrefix20211120\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING) {
                continue;
            }
            $this->changed = \true;
            if (\false === $hunkCapture) {
            ...
            if (\ECSPrefix20211120\SebastianBergmann\Diff\Differ::ADDED === $entry[1]) {
                // added
                ++$toRange;
            }
            if (\ECSPrefix20211120\SebastianBergmann\Diff\Differ::REMOVED === $entry[1]) {
                // removed
                ++$fromRange;
            }
        }
    ...
    private function writeHunk(array $diff, int $diffStartIndex, int $diffEndIndex, int $fromStart, int $fromRange, int $toStart, int $toRange, $output) : void
    {
        ...
        for ($i = $diffStartIndex; $i < $diffEndIndex; ++$i) {
            if ($diff[$i][1] === \ECSPrefix20211120\SebastianBergmann\Diff\Differ::ADDED) {
                $this->changed = \true;
                \fwrite($output, '+' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ECSPrefix20211120\SebastianBergmann\Diff\Differ::REMOVED) {
                $this->changed = \true;
                \fwrite($output, '-' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ECSPrefix20211120\SebastianBergmann\Diff\Differ::OLD) {
                \fwrite($output, ' ' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ECSPrefix20211120\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING) {
                $this->changed = \true;
                \fwrite($output, $diff[$i][0]);
            }
    ...
    private function assertString(array $options, string $option) : void
    {
        if (!\is_string($options[$option])) {
            throw new \ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException($option, 'a string', $options[$option]);
        }
    }
    private function assertStringOrNull(array $options, string $option) : void
    {
        if (null !== $options[$option] && !\is_string($options[$option])) {
            throw new \ECSPrefix20211120\SebastianBergmann\Diff\ConfigurationException($option, 'a string or <null>', $options[$option]);
        }
    }
}
