...
use function substr;
use ECSPrefix20210517\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface;
...
final class Differ
{
    ...
    {
        if ($outputBuilder instanceof \ECSPrefix20210517\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface) {
            $this->outputBuilder = $outputBuilder;
        } elseif (null === $outputBuilder) {
            $this->outputBuilder = new \ECSPrefix20210517\SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder();
            ...
            // @deprecated
            $this->outputBuilder = new \ECSPrefix20210517\SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder($outputBuilder);
        } else {
            throw new \ECSPrefix20210517\SebastianBergmann\Diff\InvalidArgumentException(\sprintf('Expected builder to be an instance of DiffOutputBuilderInterface, <null> or a string, got %s.', \is_object($outputBuilder) ? 'instance of "' . \get_class($outputBuilder) . '"' : \gettype($outputBuilder) . ' "' . $outputBuilder . '"'));
        }
    }
    /**
    ...
    public function diffToArray($from, $to, \ECSPrefix20210517\SebastianBergmann\Diff\LongestCommonSubsequenceCalculator $lcs = null) : array
    {
        if (\is_string($from)) {
            ...
            throw new \ECSPrefix20210517\SebastianBergmann\Diff\InvalidArgumentException('"from" must be an array or string.');
        }
        if (\is_string($to)) {
            $to = $this->splitStringByLines($to);
        } elseif (!\is_array($to)) {
            throw new \ECSPrefix20210517\SebastianBergmann\Diff\InvalidArgumentException('"to" must be an array or string.');
        }
    ...
    private function selectLcsImplementation(array $from, array $to) : \ECSPrefix20210517\SebastianBergmann\Diff\LongestCommonSubsequenceCalculator
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \ECSPrefix20210517\SebastianBergmann\Diff\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \ECSPrefix20210517\SebastianBergmann\Diff\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
