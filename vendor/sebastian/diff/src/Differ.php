...
use function substr;
use ECSPrefix20210514\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface;
...
final class Differ
{
    ...
    {
        if ($outputBuilder instanceof \ECSPrefix20210514\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface) {
            $this->outputBuilder = $outputBuilder;
        } elseif (null === $outputBuilder) {
            $this->outputBuilder = new \ECSPrefix20210514\SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder();
            ...
            // @deprecated
            $this->outputBuilder = new \ECSPrefix20210514\SebastianBergmann\Diff\Output\UnifiedDiffOutputBuilder($outputBuilder);
        } else {
            throw new \ECSPrefix20210514\SebastianBergmann\Diff\InvalidArgumentException(\sprintf('Expected builder to be an instance of DiffOutputBuilderInterface, <null> or a string, got %s.', \is_object($outputBuilder) ? 'instance of "' . \get_class($outputBuilder) . '"' : \gettype($outputBuilder) . ' "' . $outputBuilder . '"'));
        }
    }
    /**
     ...
     * @return string
     */
    public function diff($from, $to, \ECSPrefix20210514\SebastianBergmann\Diff\LongestCommonSubsequenceCalculator $lcs = null)
    {
     ...
     * @return mixed[]
     */
    public function diffToArray($from, $to, \ECSPrefix20210514\SebastianBergmann\Diff\LongestCommonSubsequenceCalculator $lcs = null)
    {
        if (\is_string($from)) {
            $from = $this->splitStringByLines($from);
        } elseif (!\is_array($from)) {
            throw new \ECSPrefix20210514\SebastianBergmann\Diff\InvalidArgumentException('"from" must be an array or string.');
        }
        if (\is_string($to)) {
            $to = $this->splitStringByLines($to);
        } elseif (!\is_array($to)) {
            throw new \ECSPrefix20210514\SebastianBergmann\Diff\InvalidArgumentException('"to" must be an array or string.');
        }
    ...
    private function selectLcsImplementation(array $from, array $to)
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \ECSPrefix20210514\SebastianBergmann\Diff\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \ECSPrefix20210514\SebastianBergmann\Diff\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
