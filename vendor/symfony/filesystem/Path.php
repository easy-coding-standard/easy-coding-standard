...
use ECSPrefix20220102\Symfony\Component\Filesystem\Exception\RuntimeException;
/**
...
final class Path
{
    ...
    public static function getHomeDirectory() : string
    {
        ...
        throw new \ECSPrefix20220102\Symfony\Component\Filesystem\Exception\RuntimeException("Cannot find the home directory path: Your environment or operation system isn't supported.");
    }
    /**
    ...
    {
        if ('' === $basePath) {
            throw new \ECSPrefix20220102\Symfony\Component\Filesystem\Exception\InvalidArgumentException(\sprintf('The base path must be a non-empty string. Got: "%s".', $basePath));
        }
        if (!self::isAbsolute($basePath)) {
            throw new \ECSPrefix20220102\Symfony\Component\Filesystem\Exception\InvalidArgumentException(\sprintf('The base path "%s" is not an absolute path.', $basePath));
        }
    ...
    public static function makeRelative(string $path, string $basePath) : string
    {
        ...
        if ('' !== $root && '' === $baseRoot) {
            throw new \ECSPrefix20220102\Symfony\Component\Filesystem\Exception\InvalidArgumentException(\sprintf('The absolute path "%s" cannot be made relative to the relative path "%s". You should provide an absolute base path instead.', $path, $basePath));
        }
        // Fail if the roots of the two paths are different
        if ($baseRoot && $root !== $baseRoot) {
            throw new \ECSPrefix20220102\Symfony\Component\Filesystem\Exception\InvalidArgumentException(\sprintf('The path "%s" cannot be made relative to "%s", because they have different roots ("%s" and "%s").', $path, $basePath, $root, $baseRoot));
        }
        if ('' === $relativeBasePath) {
            return $relativePath;
        }
