...
trait PriorityTaggedServiceTrait
{
    ...
    private function findAndSortTaggedServices($tagName, $container)
    {
        ...
        if ($tagName instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {
        ...
        foreach ($services as list(, , $index, $serviceId, $class)) {
            if (!$class) {
                $reference = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference($serviceId);
            } elseif ($index === $serviceId) {
                $reference = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($serviceId, $class);
            } else {
                $reference = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($serviceId, $class, \ECSPrefix20210507\Symfony\Component\DependencyInjection\ContainerBuilder::EXCEPTION_ON_INVALID_REFERENCE, $index);
            }
            if (null === $index) {
                $refs[] = $reference;
            } else {
...
class PriorityTaggedServiceUtil
{
    ...
    public static function getDefaultIndex($container, $serviceId, $class, $defaultIndexMethod, $tagName, $indexAttribute)
    {
        ...
        if (!($rm = $r->getMethod($defaultIndexMethod))->isStatic()) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode('be static', $message));
        }
        if (!$rm->isPublic()) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode('be public', $message));
        }
        $defaultIndex = $rm->invoke(null);
        if (!\is_string($defaultIndex)) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\implode(\sprintf('return a string (got "%s")', \get_debug_type($defaultIndex)), $message));
        }
        return $defaultIndex;
    }
    /**
    ...
    public static function getDefaultPriority($container, $serviceId, $class, $defaultPriorityMethod, $tagName)
    {
        ...
        if (!($rm = $r->getMethod($defaultPriorityMethod))->isStatic()) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Either method "%s::%s()" should be static or tag "%s" on service "%s" is missing attribute "priority".', $class, $defaultPriorityMethod, $tagName, $serviceId));
        }
        if (!$rm->isPublic()) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Either method "%s::%s()" should be public or tag "%s" on service "%s" is missing attribute "priority".', $class, $defaultPriorityMethod, $tagName, $serviceId));
        }
        $defaultPriority = $rm->invoke(null);
        if (!\is_int($defaultPriority)) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Method "%s::%s()" should return an integer (got "%s") or tag "%s" on service "%s" is missing attribute "priority".', $class, $defaultPriorityMethod, \get_debug_type($defaultPriority), $tagName, $serviceId));
        }
