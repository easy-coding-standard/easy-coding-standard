 ...
 */
class CheckExceptionOnInvalidReferenceBehaviorPass extends \ECSPrefix20220517\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * @var mixed[]
     */
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (\ECSPrefix20220517\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $value->getInvalidBehavior() || $this->container->has($id = (string) $value)) {
            return $value;
        }
        ...
        if (isset($this->serviceLocatorContextIds[$currentId])) {
            ...
            foreach ($locator->getArgument(0) as $k => $v) {
                if ($v->getValues()[0] === $value) {
                    ...
                    throw new \ECSPrefix20220517\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($id, $currentId, null, $this->getAlternatives($id));
                }
            }
        }
        if ('.' === $currentId[0] && $graph->hasNode($currentId)) {
            foreach ($graph->getNode($currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \ECSPrefix20220517\Symfony\Component\DependencyInjection\Reference || \ECSPrefix20220517\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $edge->getValue()->getInvalidBehavior()) {
                    continue;
                }
