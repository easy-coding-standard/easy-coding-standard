...
class ResolveChildDefinitionsPass extends \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\ChildDefinition) {
            $this->currentPath = [];
    ...
    private function resolveDefinition($definition)
    {
        try {
            ...
            throw $e;
        } catch (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\ExceptionInterface $e) {
    ...
    private function doResolveDefinition($definition)
    {
        if (!$this->container->has($parent = $definition->getParent())) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Parent definition "%s" does not exist.', $parent));
        }
        $searchKey = \array_search($parent, $this->currentPath);
        $this->currentPath[] = $parent;
        if (\false !== $searchKey) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($parent, \array_slice($this->currentPath, $searchKey));
        }
        $parentDef = $this->container->findDefinition($parent);
        if ($parentDef instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\ChildDefinition) {
            $id = $this->currentId;
        ...
        $def = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition();
        ...
        if (isset($changes['decorated_service'])) {
            ...
            if (null === $decoratedService) {
                ...
                $def->setDecoratedService($decoratedService[0], $decoratedService[1], $decoratedService[2], isset($decoratedService[3]) ? $decoratedService[3] : \ECSPrefix20210507\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE);
            }
        }
        // merge arguments
