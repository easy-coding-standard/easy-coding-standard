...
final class CheckTypeDeclarationsPass extends \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (!$value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition || $value->hasErrors() || $value->isDeprecated()) {
            return parent::processValue($value, $isRoot);
        }
        if (!$this->autoload && !\class_exists($class = $value->getClass(), \false) && !\interface_exists($class, \false)) {
            return parent::processValue($value, $isRoot);
        }
        if (\ECSPrefix20210507\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
        ...
        foreach ($value->getMethodCalls() as $methodCall) {
            ...
            } catch (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\RuntimeException $e) {
                if ($value->getFactory()) {
                    continue;
                }
                throw $e;
     ...
     */
    private function checkTypeDeclarations(\ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, \ReflectionType $reflectionType = null) : void
    {
        ...
        if ($reflectionType instanceof \ReflectionUnionType) {
            foreach ($reflectionType->getTypes() as $t) {
                try {
                    ...
                    return;
                } catch (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                }
            }
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
        }
        if (!$reflectionType instanceof \ReflectionNamedType) {
            return;
        }
        $type = $reflectionType->getName();
        if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference) {
            if (!$this->container->has($value = (string) $value)) {
                return;
            }
            if ('service_container' === $value && \is_a($type, \ECSPrefix20210507\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        $class = null;
        if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition) {
        ...
        if (null === $class) {
            if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
            } elseif ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                $class = \Closure::class;
            } elseif ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                $class = \ECSPrefix20210507\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
    }
    private function getExpressionLanguage() : \ECSPrefix20210507\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
