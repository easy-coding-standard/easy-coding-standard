...
class AutowirePass extends \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        } catch (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException $e) {
    ...
    private function doProcessValue($value, $isRoot = \false)
    {
        if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference) {
            ...
            if (\ECSPrefix20210507\Symfony\Component\DependencyInjection\ContainerBuilder::RUNTIME_EXCEPTION_ON_INVALID_REFERENCE === $value->getInvalidBehavior()) {
                ...
                return new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($id, $value->getType(), $value->getInvalidBehavior(), $value->getName());
            }
        }
        ...
        try {
            ...
            throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $e->getMessage(), 0, $e);
        }
        if ($constructor) {
    ...
    private function autowireCalls($reflectionClass, $isRoot)
    {
        ...
        foreach ($this->methodCalls as $i => $call) {
            ...
            if ($method instanceof \ReflectionFunctionAbstract) {
                ...
                $definition = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition($reflectionClass->name);
                try {
    ...
    private function autowireMethod($reflectionMethod, array $arguments)
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            $type = \ECSPrefix20210507\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
            if (!$type) {
                ...
                if (!$parameter->isDefaultValueAvailable()) {
                    ...
                    $type = \ECSPrefix20210507\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \false);
                    $type = $type ? \sprintf('is type-hinted "%s"', \ltrim($type, '\\')) : 'has no type-hint';
                    throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, \sprintf('Cannot autowire service "%s": argument "$%s" of method "%s()" %s, you should configure its value explicitly.', $this->currentId, $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method, $type));
                }
            ...
            $getValue = function () use($type, $parameter, $class, $method) {
                if (!($value = $this->getAutowiredReference($ref = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($type, $type, \ECSPrefix20210507\Symfony\Component\DependencyInjection\ContainerBuilder::EXCEPTION_ON_INVALID_REFERENCE, $parameter->name)))) {
                    $failureMessage = $this->createTypeNotFoundMessageCallback($ref, \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method));
                    if ($parameter->isDefaultValueAvailable()) {
                        $value = $parameter->getDefaultValue();
                    } elseif (!$parameter->allowsNull()) {
                        throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $failureMessage);
                    }
                }
                return $value;
            };
            if ($this->decoratedClass && ($isDecorated = \is_a($this->decoratedClass, $type, \true))) {
                if ($this->getPreviousValue) {
                    ...
                    $arguments[$index] = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($this->decoratedId, $this->decoratedClass);
    ...
    private function getAutowiredReference($reference)
    {
        ...
        if (null !== ($name = $reference->getName())) {
            if ($this->container->has($alias = $type . ' $' . $name) && !$this->container->findDefinition($alias)->isAbstract()) {
                return new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($alias, $type, $reference->getInvalidBehavior());
            }
            if ($this->container->has($name) && !$this->container->findDefinition($name)->isAbstract()) {
                foreach ($this->container->getAliases() as $id => $alias) {
                    if ($name === (string) $alias && 0 === \strpos($id, $type . ' $')) {
                        return new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($name, $type, $reference->getInvalidBehavior());
                    }
                }
            }
        }
        if ($this->container->has($type) && !$this->container->findDefinition($type)->isAbstract()) {
            return new \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference($type, $type, $reference->getInvalidBehavior());
        }
        return null;
    }
    /**
    ...
    private function createTypeNotFoundMessageCallback($reference, $label)
    {
        if (null === $this->typesClone->container) {
            $this->typesClone->container = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\ContainerBuilder($this->container->getParameterBag());
    ...
    private function createTypeNotFoundMessage($reference, $label, $currentId)
    {
        if (!($r = $this->container->getReflectionClass($type = $reference->getType(), \false))) {
            ...
            try {
                $resource = new \ECSPrefix20210507\Symfony\Component\Config\Resource\ClassExistenceResource($type, \false);
