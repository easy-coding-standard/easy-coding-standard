...
use ECSPrefix20211208\Symfony\Component\Config\Resource\ClassExistenceResource;
...
use ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\TaggedIterator;
use ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\TaggedLocator;
use ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\Target;
...
use ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException;
use ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\RuntimeException;
use ECSPrefix20211208\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper;
 ...
 */
class AutowirePass extends \ECSPrefix20211208\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * @var mixed[]
     */
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        } catch (\ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException $e) {
    ...
    private function doProcessValue($value, bool $isRoot = \false)
    {
        if ($value instanceof \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference) {
            ...
            if (\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder::RUNTIME_EXCEPTION_ON_INVALID_REFERENCE === $value->getInvalidBehavior()) {
                ...
                return new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($id, $value->getType(), $value->getInvalidBehavior(), $value->getName());
            }
        }
        ...
        } catch (\ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\RuntimeException $e) {
            throw new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $e->getMessage(), 0, $e);
        }
        if ($constructor) {
    ...
    private function autowireCalls(\ReflectionClass $reflectionClass, bool $isRoot, bool $checkAttributes) : array
    {
        ...
        foreach ($this->methodCalls as $i => $call) {
            ...
            if ($method instanceof \ReflectionFunctionAbstract) {
                ...
                $definition = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Definition($reflectionClass->name);
                try {
    ...
    private function autowireMethod(\ReflectionFunctionAbstract $reflectionMethod, array $arguments, bool $checkAttributes, int $methodIndex) : array
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            $type = \ECSPrefix20211208\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
            if ($checkAttributes) {
                foreach ([] as $attribute) {
                    if (\ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\TaggedIterator::class === $attribute->getName()) {
                        $attribute = $attribute->newInstance();
                        $arguments[$index] = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument($attribute->tag, $attribute->indexAttribute, $attribute->defaultIndexMethod, \false, $attribute->defaultPriorityMethod);
                        break;
                    }
                    if (\ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\TaggedLocator::class === $attribute->getName()) {
                        $attribute = $attribute->newInstance();
                        $arguments[$index] = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument(new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument($attribute->tag, $attribute->indexAttribute, $attribute->defaultIndexMethod, \true, $attribute->defaultPriorityMethod));
                        break;
                    }
                }
                if ('' !== ($arguments[$index] ?? '')) {
            ...
            if (!$type) {
                ...
                if (!$parameter->isDefaultValueAvailable()) {
                    ...
                    $type = \ECSPrefix20211208\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \false);
                    $type = $type ? \sprintf('is type-hinted "%s"', \ltrim($type, '\\')) : 'has no type-hint';
                    throw new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, \sprintf('Cannot autowire service "%s": argument "$%s" of method "%s()" %s, you should configure its value explicitly.', $this->currentId, $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method, $type));
                }
            ...
            $getValue = function () use($type, $parameter, $class, $method) {
                if (!($value = $this->getAutowiredReference($ref = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($type, $type, \ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder::EXCEPTION_ON_INVALID_REFERENCE, \ECSPrefix20211208\Symfony\Component\DependencyInjection\Attribute\Target::parseName($parameter)), \true))) {
                    $failureMessage = $this->createTypeNotFoundMessageCallback($ref, \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method));
                    if ($parameter->isDefaultValueAvailable()) {
                        $value = clone $this->defaultArgument;
                        $value->value = $parameter->getDefaultValue();
                    } elseif (!$parameter->allowsNull()) {
                        throw new \ECSPrefix20211208\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $failureMessage);
                    }
                }
                return $value;
            };
            if ($this->decoratedClass && ($isDecorated = \is_a($this->decoratedClass, $type, \true))) {
                if ($this->getPreviousValue) {
                    ...
                    $arguments[$index] = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($this->decoratedId, $this->decoratedClass);
     ...
     */
    private function getAutowiredReference(\ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference $reference, bool $filterType) : ?\ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference
    {
        ...
        if (null !== ($name = $reference->getName())) {
            if ($this->container->has($alias = $type . ' $' . $name) && !$this->container->findDefinition($alias)->isAbstract()) {
                return new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($alias, $type, $reference->getInvalidBehavior());
            }
            ...
            if ($this->container->has($name) && !$this->container->findDefinition($name)->isAbstract()) {
                foreach ($this->container->getAliases() + $this->combinedAliases as $id => $alias) {
                    if ($name === (string) $alias && \strncmp($id, $type . ' $', \strlen($type . ' $')) === 0) {
                        return new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($name, $type, $reference->getInvalidBehavior());
                    }
                }
            }
        }
        if ($this->container->has($type) && !$this->container->findDefinition($type)->isAbstract()) {
            return new \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference($type, $type, $reference->getInvalidBehavior());
        }
     ...
     */
    private function populateAvailableTypes(\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        $this->types = [];
     ...
     */
    private function populateAvailableType(\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $id, \ECSPrefix20211208\Symfony\Component\DependencyInjection\Definition $definition)
    {
    ...
    private function createTypeNotFoundMessageCallback(\ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference $reference, string $label) : \Closure
    {
        if (null === $this->typesClone->container) {
            $this->typesClone->container = new \ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder($this->container->getParameterBag());
    ...
    private function createTypeNotFoundMessage(\ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference $reference, string $label, string $currentId) : string
    {
        if (!($r = $this->container->getReflectionClass($type = $reference->getType(), \false))) {
            // either $type does not exist or a parent class does not exist
            try {
                $resource = new \ECSPrefix20211208\Symfony\Component\Config\Resource\ClassExistenceResource($type, \false);
    ...
    private function createTypeAlternatives(\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder $container, \ECSPrefix20211208\Symfony\Component\DependencyInjection\TypedReference $reference) : string
    {
    ...
    private function getAliasesSuggestionForType(\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $type) : ?string
    {
        $aliases = [];
    ...
    private function populateCombinedAliases(\ECSPrefix20211208\Symfony\Component\DependencyInjection\ContainerBuilder $container) : void
    {
        $this->combinedAliases = [];
        $reverseAliases = [];
        foreach ($container->getAliases() as $id => $alias) {
