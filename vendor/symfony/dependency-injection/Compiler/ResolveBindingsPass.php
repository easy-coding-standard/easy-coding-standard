...
class ResolveBindingsPass extends \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process($container)
    {
        ...
        try {
            ...
            foreach ($this->unusedBindings as list($key, $serviceId, $bindingType, $file)) {
                ...
                if (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\BoundArgument::DEFAULTS_BINDING === $bindingType) {
                    $message .= 'under "_defaults"';
                } elseif (\ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\BoundArgument::INSTANCEOF_BINDING === $bindingType) {
                ...
                throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException($message);
            }
        } finally {
            $this->usedBindings = [];
            $this->unusedBindings = [];
    ...
    {
        if ($value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\TypedReference && $value->getType() === (string) $value) {
            // Already checked
        ...
        if (!$value instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition || !($bindings = $value->getBindings())) {
            return parent::processValue($value, $isRoot);
        }
        $bindingNames = [];
        foreach ($bindings as $key => $binding) {
            ...
            if (null !== $bindingValue && !$bindingValue instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference && !$bindingValue instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition && !$bindingValue instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument && !$bindingValue instanceof \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value for binding key "%s" for service "%s": expected null, "%s", "%s", "%s" or ServiceLocatorArgument, "%s" given.', $key, $this->currentId, \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference::class, \ECSPrefix20210507\Symfony\Component\DependencyInjection\Definition::class, \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument::class, \get_debug_type($bindingValue)));
            }
        }
        if ($value->isAbstract()) {
            return parent::processValue($value, $isRoot);
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                $typeHint = \ECSPrefix20210507\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter);
