 ...
 */
class DefinitionErrorExceptionPass extends \ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * {@inheritdoc}
     * @param bool $isRoot
    ...
    {
        $isRoot = (bool) $isRoot;
        if (!$value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition || !$value->hasErrors()) {
        ...
        if ($isRoot && !$value->isPublic()) {
            ...
            foreach ($graph->getNode($this->currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference || \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::RUNTIME_EXCEPTION_ON_INVALID_REFERENCE !== $edge->getValue()->getInvalidBehavior()) {
                    $runtimeException = \false;
                    break;
                }
