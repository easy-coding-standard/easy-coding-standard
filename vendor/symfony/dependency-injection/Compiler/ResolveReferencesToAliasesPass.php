    ...
    private function getDefinitionId(string $id, ContainerBuilder $container) : string
    {
        ...
        if ($alias->isDeprecated()) {
            ...
            if (!$referencingDefinition->isDeprecated()) {
                ...
                \ECSPrefix202301\trigger_deprecation($deprecation['package'], $deprecation['version'], \rtrim($deprecation['message'], '. ') . '. It is being referenced by the "%s" ' . ($container->hasDefinition($this->currentId) ? 'service.' : 'alias.'), $this->currentId);
            }
        }
        $seen = [];
        do {
