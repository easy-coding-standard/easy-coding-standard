 ...
 */
class ResolveInstanceofConditionalsPass implements \ECSPrefix20210517\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
     * {@inheritdoc}
     */
    public function process(\ECSPrefix20210517\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        foreach ($container->getAutoconfiguredInstanceof() as $interface => $definition) {
            if ($definition->getArguments()) {
                throw new \ECSPrefix20210517\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Autoconfigured instanceof for type "%s" defines arguments but these are not supported and should be removed.', $interface));
            }
        }
        $tagsToKeep = [];
    ...
    private function processDefinition(\ECSPrefix20210517\Symfony\Component\DependencyInjection\ContainerBuilder $container, $id, \ECSPrefix20210517\Symfony\Component\DependencyInjection\Definition $definition, array $tagsToKeep)
    {
        ...
        if ($parent) {
            ...
            if (\ECSPrefix20210517\Symfony\Component\DependencyInjection\Definition::class === \get_class($abstract)) {
    ...
    private function mergeConditionals(array $autoconfiguredInstanceof, array $instanceofConditionals, \ECSPrefix20210517\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($instanceofConditionals as $interface => $instanceofDef) {
            ...
            if (!$container->getReflectionClass($interface)) {
                throw new \ECSPrefix20210517\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('"%s" is set as an "instanceof" conditional, but it does not exist.', $interface));
            }
