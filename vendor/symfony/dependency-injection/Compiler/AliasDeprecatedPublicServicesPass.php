...
final class AliasDeprecatedPublicServicesPass extends \ECSPrefix20210514\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    private $tagName;
    private $aliases = [];
    /**
    ...
    {
        $isRoot = (bool) $isRoot;
        if ($value instanceof \ECSPrefix20210514\Symfony\Component\DependencyInjection\Reference && isset($this->aliases[$id = (string) $value])) {
            return new \ECSPrefix20210514\Symfony\Component\DependencyInjection\Reference($this->aliases[$id], $value->getInvalidBehavior());
        }
    ...
    public function process(\ECSPrefix20210514\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        foreach ($container->findTaggedServiceIds($this->tagName) as $id => $tags) {
            if (null === ($package = isset($tags[0]['package']) ? $tags[0]['package'] : null)) {
                throw new \ECSPrefix20210514\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The "package" attribute is mandatory for the "%s" tag on the "%s" service.', $this->tagName, $id));
            }
            if (null === ($version = isset($tags[0]['version']) ? $tags[0]['version'] : null)) {
                throw new \ECSPrefix20210514\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The "version" attribute is mandatory for the "%s" tag on the "%s" service.', $this->tagName, $id));
            }
