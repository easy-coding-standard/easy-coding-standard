 ...
 */
final class AttributeAutoconfigurationPass extends \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * @return void
     */
    ...
    {
        if (!$value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition || !$value->isAutoconfigured() || $value->isAbstract() || $value->hasTag('container.ignore_attributes') || !($reflector = $this->container->getReflectionClass($value->getClass(), \false))) {
            return parent::processValue($value, $isRoot);
        }
        $autoconfiguredAttributes = $this->container->getAutoconfiguredAttributes();
        $instanceof = $value->getInstanceofConditionals();
        $conditionals = $instanceof[$reflector->getName()] ?? new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ChildDefinition('');
        foreach ($reflector->getAttributes() as $attribute) {
            if ($configurator = $autoconfiguredAttributes[$attribute->getName()] ?? null) {
                $configurator($conditionals, $attribute->newInstance(), $reflector);
            }
        }
        if (!isset($instanceof[$reflector->getName()]) && new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ChildDefinition('') != $conditionals) {
