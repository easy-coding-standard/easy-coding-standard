 ...
 */
final class AttributeAutoconfigurationPass extends \ECSPrefix20220503\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process(\ECSPrefix20220503\Symfony\Component\DependencyInjection\ContainerBuilder $container) : void
    {
        ...
        foreach ($container->getAutoconfiguredAttributes() as $attributeName => $callable) {
            ...
            if ($parameterType instanceof \ReflectionUnionType) {
                ...
                throw new \ECSPrefix20220503\Symfony\Component\DependencyInjection\Exception\LogicException(\sprintf('Argument "$%s" of attribute autoconfigurator should have a type, use one or more of "\\ReflectionClass|\\ReflectionMethod|\\ReflectionProperty|\\ReflectionParameter|\\Reflector" in "%s" on line "%d".', $reflectorParameter->getName(), $callableReflector->getFileName(), $callableReflector->getStartLine()));
            }
            try {
            ...
            foreach (['class', 'method', 'property', 'parameter'] as $symbol) {
                if (['Reflector'] !== $types) {
                    ...
                    if (!($targets & \constant('Attribute::TARGET_' . \strtoupper($symbol)))) {
                        throw new \ECSPrefix20220503\Symfony\Component\DependencyInjection\Exception\LogicException(\sprintf('Invalid type "Reflection%s" on argument "$%s": attribute "%s" cannot target a ' . $symbol . ' in "%s" on line "%d".', \ucfirst($symbol), $reflectorParameter->getName(), $attributeName, $callableReflector->getFileName(), $callableReflector->getStartLine()));
                    }
                }
    ...
    {
        if (!$value instanceof \ECSPrefix20220503\Symfony\Component\DependencyInjection\Definition || !$value->isAutoconfigured() || $value->isAbstract() || $value->hasTag('container.ignore_attributes') || !($classReflector = $this->container->getReflectionClass($value->getClass(), \false))) {
            return parent::processValue($value, $isRoot);
        }
        $instanceof = $value->getInstanceofConditionals();
        $conditionals = $instanceof[$classReflector->getName()] ?? new \ECSPrefix20220503\Symfony\Component\DependencyInjection\ChildDefinition('');
        ...
        if (!isset($instanceof[$classReflector->getName()]) && new \ECSPrefix20220503\Symfony\Component\DependencyInjection\ChildDefinition('') != $conditionals) {
