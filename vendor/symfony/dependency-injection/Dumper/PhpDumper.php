...
use ConfigTransformer20210601\Symfony\Component\Debug\DebugClassLoader as LegacyDebugClassLoader;
...
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceLocator;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode;
...
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\EnvParameterException;
...
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\ExpressionLanguage;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface as ProxyDumper;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Loader\FileLoader;
...
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\ServiceLocator as BaseServiceLocator;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\TypedReference;
use ConfigTransformer20210601\Symfony\Component\DependencyInjection\Variable;
use ConfigTransformer20210601\Symfony\Component\ErrorHandler\DebugClassLoader;
 ...
 */
class PhpDumper extends \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
     ...
     * {@inheritdoc}
     */
    public function __construct(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        if (!$container->isCompiled()) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\LogicException('Cannot dump an uncompiled container.');
        }
     ...
     */
    public function setProxyDumper(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface $proxyDumper)
    {
    ...
    public function dump(array $options = [])
    {
        ...
        if (0 !== \strpos($baseClass = $options['base_class'], '\\') && 'Container' !== $baseClass) {
            ...
            $this->baseClass = \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Container::class;
        } else {
            $this->baseClass = $baseClass;
        }
        $this->initializeMethodNamesMap('Container' === $baseClass ? \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Container::class : $baseClass);
        if ($this->getProxyDumper() instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper) {
            (new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\true, \false))->process($this->container);
            try {
                (new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass())->process($this->container);
            } catch (\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException $e) {
                $path = $e->getPath();
                \end($path);
                $path[\key($path)] .= '". Try running "composer require symfony/proxy-manager-bridge';
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($e->getServiceId(), $path);
            }
        }
...
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\Argument\\RewindableGenerator;
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
...
EOF;
            ...
            $hash = \ucfirst(\strtr(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerBuilder::hash($files), '._', 'xx'));
            $code = [];
...
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\Dumper\\Preloader;
...
EOF;
        ...
        if ($unusedEnvs) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\EnvParameterException($unusedEnvs, null, 'Environment variables "%s" are never used. Please, check your container\'s configuration.');
        }
        return $code;
    }
    /**
     ...
     */
    private function getProxyDumper() : \ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface
    {
        if (!$this->proxyDumper) {
            $this->proxyDumper = new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper();
        }
        return $this->proxyDumper;
    }
    private function analyzeReferences()
    {
        (new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\false, !$this->getProxyDumper() instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper))->process($this->container);
        $checkedNodes = [];
        $this->circularReferences = [];
        $this->singleUsePrivateIds = [];
        foreach ($this->container->getCompiler()->getServiceReferenceGraph()->getNodes() as $id => $node) {
            if (!$node->getValue() instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition) {
                continue;
            }
    ...
    private function collectCircularReferences(string $sourceId, array $edges, array &$checkedNodes, array &$loops = [], array $path = [], bool $byConstructor = \true)
    {
        ...
        foreach ($edges as $edge) {
            ...
            $id = $node->getId();
            if ($sourceId === $id || !$node->getValue() instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition || $edge->isLazy() || $edge->isWeak()) {
                continue;
            }
            if (isset($path[$id])) {
                $loop = null;
    ...
    private function generateProxyClasses() : array
    {
        ...
        $strip = '' === $this->docStar && \method_exists(\ConfigTransformer20210601\Symfony\Component\HttpKernel\Kernel::class, 'stripComments');
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = \substr(\ConfigTransformer20210601\Symfony\Component\HttpKernel\Kernel::stripComments($proxyCode), 5);
            }
    ...
    private function addServiceInclude(string $cId, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition) : string
    {
        $code = '';
        if ($this->inlineRequires && (!$this->isHotPath($definition) || $this->getProxyDumper()->isProxyCandidate($definition))) {
            ...
            foreach ($this->serviceCalls as $id => list($callCount, $behavior)) {
                if ('service_container' !== $id && $id !== $cId && \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $behavior && $this->container->has($id) && $this->isTrivialInstance($def = $this->container->findDefinition($id))) {
                    foreach ($this->getClasses($def, $cId) as $class) {
                        $this->collectLineage($class, $lineage);
                    }
                }
     ...
     */
    private function addServiceInstance(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, bool $isSimpleInstance) : string
    {
        ...
        if (0 === \strpos($class, "'") && \false === \strpos($class, '$') && !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid class name for the "%s" service.', $class, $id));
        }
    ...
    private function isTrivialInstance(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        ...
        foreach ($definition->getArguments() as $arg) {
            if (!$arg || $arg instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Parameter) {
                continue;
            }
            if (\is_array($arg) && 3 >= \count($arg)) {
                foreach ($arg as $k => $v) {
                    ...
                    if (!$v || $v instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Parameter) {
                        continue;
                    }
                    if ($v instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $v) && $this->container->findDefinition($id)->isSynthetic()) {
                        continue;
                    }
            ...
            } elseif ($arg instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $arg) && $this->container->findDefinition($id)->isSynthetic()) {
                continue;
     ...
     */
    private function addServiceMethodCalls(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $variableName, $sharedNonLazyId) : string
    {
        $lastWitherIndex = null;
        foreach ($definition->getMethodCalls() as $k => $call) {
            if ($call[2] ?? \false) {
    ...
    private function addServiceProperties(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        $code = '';
    ...
    private function addServiceConfigurator(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        if (!($callable = $definition->getConfigurator())) {
            return '';
        }
        if (\is_array($callable)) {
            if ($callable[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
    ...
    private function addService(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition) : array
    {
        $this->definitionVariables = new \SplObjectStorage();
        $this->referenceVariables = [];
        $this->variableCount = 0;
        $this->referenceVariables[$id] = new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Variable('instance');
        $return = [];
        if ($class = $definition->getClass()) {
            $class = $class instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
            $return[] = \sprintf(0 === \strpos($class, '%') ? '@return object A %1$s instance' : '@return \\%s', \ltrim($class, '\\'));
        } elseif ($definition->getFactory()) {
            $factory = $definition->getFactory();
            if (\is_string($factory)) {
                $return[] = \sprintf('@return object An instance returned by %s()', $factory);
            } elseif (\is_array($factory) && (\is_string($factory[0]) || $factory[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition || $factory[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference)) {
                $class = $factory[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition ? $factory[0]->getClass() : (string) $factory[0];
                $class = $class instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
...
EOF;
    ...
    private function addInlineVariables(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, array $arguments, bool $forConstructor) : string
    {
        $code = '';
        foreach ($arguments as $argument) {
            if (\is_array($argument)) {
                $code .= $this->addInlineVariables($id, $definition, $argument, $forConstructor);
            } elseif ($argument instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference) {
    ...
    private function addInlineReference(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $targetId, bool $forConstructor) : string
    {
        ...
        $this->referenceVariables[$targetId] = new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Variable($name);
        $reference = \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $behavior ? new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference($targetId, $behavior) : null;
...
, $this->container->getDefinition($id)->isPublic() ? 'services' : 'privates', $this->doExport($id));
    ...
    private function addInlineService(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $inlineDef = null, bool $forConstructor = \true) : string
    {
        $code = '';
        ...
        if (isset($this->definitionVariables[$inlineDef])) {
            ...
            $this->definitionVariables[$inlineDef] = new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Variable($name);
            $code .= '' !== $code ? "\n" : '';
            if ('instance' === $name) {
    ...
    private function addNewInstance(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $return = '', string $id = null) : string
    {
        $tail = $return ? ";\n" : '';
        if (\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ServiceLocator::class === $definition->getClass() && $definition->hasTag($this->serviceLocatorTag)) {
            $arguments = [];
            foreach ($definition->getArgument(0) as $k => $argument) {
                $arguments[$k] = $argument->getValues()[0];
            }
            return $return . $this->dumpValue(new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments)) . $tail;
        }
        $arguments = [];
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_array($callable)) {
                if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $callable[1])) {
                    throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $callable[1] ?: 'n/a'));
                }
                if ($callable[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
                ...
                if (0 === \strpos($class, "'") && \false === \strpos($class, '$')) {
                    if ("''" === $class) {
                        throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition: "%s" service is defined to be created by a factory but is missing the service reference, did you forget to define the factory service id or class?', $id ? 'The "' . $id . '"' : 'inline'));
                    }
        ...
        if (null === ($class = $definition->getClass())) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have no class nor factory.');
        }
...
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\Exception\\LogicException;
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;
use ConfigTransformer20210601\\Symfony\\Component\\DependencyInjection\\ParameterBag\\ParameterBagInterface;
...
EOF;
        ...
        if (\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Container::class !== $this->baseClass) {
            $r = $this->container->getReflectionClass($this->baseClass, \false);
            if (null !== $r && null !== ($constructor = $r->getConstructor()) && 0 === $constructor->getNumberOfRequiredParameters() && \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Container::class !== $constructor->getDeclaringClass()->name) {
...
EOF;
    ...
    private function addRemovedIds() : string
    {
        ...
        if ($this->asFiles) {
            ...
            \sort($ids);
            foreach ($ids as $id) {
                if (\preg_match(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Loader\FileLoader::ANONYMOUS_ID_REGEXP, $id)) {
                    continue;
                }
                $code .= '            ' . $this->doExport($id) . " => true,\n";
            }
    ...
    private function addDefaultParametersMethod() : string
    {
        ...
        foreach ($this->container->getParameterBag()->all() as $key => $value) {
            if ($key !== ($resolvedKey = $this->container->resolveEnvPlaceholders($key))) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Parameter name cannot use env parameters: "%s".', $resolvedKey));
            }
...
EOF;
    ...
    private function exportParameters(array $parameters, string $path = '', int $indent = 12) : string
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ArgumentInterface) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain special arguments. "%s" found in "%s".', \get_debug_type($value), $path . '/' . $key));
            } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Variable) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".', $value, $path . '/' . $key));
            } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".', $value->getClass(), $path . '/' . $key));
            } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").', $value, $path . '/' . $key));
            } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\ExpressionLanguage\Expression) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain expressions. Expression "%s" found in "%s".', $value, $path . '/' . $key));
            } else {
...
EOF;
    ...
    {
        $conditions = [];
        foreach (\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerBuilder::getInitializedConditionals($value) as $service) {
        ...
        foreach (\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerBuilder::getServiceConditionals($value) as $service) {
    ...
    private function dumpValue($value, bool $interpolate = \true) : string
    {
        if (\is_array($value)) {
            ...
            try {
                if ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                    $value = $value->getValues()[0];
                    $code = $this->dumpValue($value, $interpolate);
                    $returnedType = '';
                    if ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\TypedReference) {
                        $returnedType = \sprintf(': %s\\%s', \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $value->getInvalidBehavior() ? '' : '?', $value->getType());
                    }
                    $code = \sprintf('return %s;', $code);
                    return \sprintf("function ()%s {\n            %s\n        }", $returnedType, $code);
                }
                if ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                    $operands = [0];
                    $code = [];
                ...
                if ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                    $serviceMap = '';
                    $serviceTypes = '';
                    foreach ($value->getValues() as $k => $v) {
                        ...
                        $serviceMap .= \sprintf("\n            %s => [%s, %s, %s, %s],", $this->export($k), $this->export($definition->isShared() ? $definition->isPublic() ? 'services' : 'privates' : \false), $this->doExport($id), $this->export(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $v->getInvalidBehavior() && !\is_string($load) ? $this->generateMethodName($id) : null), $this->export($load));
                        $serviceTypes .= \sprintf("\n            %s => %s,", $this->export($k), $this->export($v instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\TypedReference ? $v->getType() : '?'));
                        $this->locatedIds[$id] = \true;
                    }
                    $this->addGetService = \true;
                    return \sprintf('new \\%s($this->getService, [%s%s], [%s%s])', \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\ServiceLocator::class, $serviceMap, $serviceMap ? "\n        " : '', $serviceTypes, $serviceTypes ? "\n        " : '');
                }
            } finally {
            ...
            if ($value->getMethodCalls()) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have method calls.');
            }
            if ($value->getProperties()) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have properties.');
            }
            if (null !== $value->getConfigurator()) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have a configurator.');
            }
        ...
        } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Parameter) {
        ...
        } elseif ($value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Argument\AbstractArgument) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException($value->getTextWithContext());
        } elseif (\is_object($value) || \is_resource($value)) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException('Unable to dump a service container if a parameter is an object or a resource.');
        }
    ...
    private function dumpLiteralClass(string $class) : string
    {
        ...
        if (0 !== \strpos($class, "'") || !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid class name (%s).', $class ?: 'n/a'));
        }
    ...
    private function getServiceCall(string $id, \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Reference $reference = null) : string
    {
        while ($this->container->hasAlias($id)) {
            $id = (string) $this->container->getAlias($id);
        }
        ...
        if ($this->container->hasDefinition($id) && ($definition = $this->container->getDefinition($id))) {
            ...
            } elseif (null !== $reference && \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
                if (!$definition->isShared()) {
                    return $code;
                }
        ...
        if (null !== $reference && \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
            return 'null';
        }
        if (null !== $reference && \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $reference->getInvalidBehavior()) {
            $code = \sprintf('$this->get(%s, /* ContainerInterface::NULL_ON_INVALID_REFERENCE */ %d)', $this->doExport($id), \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE);
        } else {
    ...
    private function generateMethodName(string $id) : string
    {
        ...
        $i = \strrpos($id, '\\');
        $name = \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Container::camelize(\false !== $i && isset($id[1 + $i]) ? \substr($id, 1 + $i) : $id);
    ...
    private function getExpressionLanguage() : \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            if (!\class_exists(\ConfigTransformer20210601\Symfony\Component\ExpressionLanguage\ExpressionLanguage::class)) {
                throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\LogicException('Unable to use expressions as the Symfony ExpressionLanguage component is not installed.');
            }
            $providers = $this->container->getExpressionLanguageProviders();
            $this->expressionLanguage = new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $providers, function ($arg) {
    ...
    private function isHotPath(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        return $this->hotPathTag && $definition->hasTag($this->hotPathTag) && !$definition->isDeprecated();
    }
    private function isSingleUsePrivateNode(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode $node) : bool
    {
        ...
        foreach ($node->getInEdges() as $edge) {
            ...
            if ($edge->isLazy() || !$value instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition || !$value->isShared()) {
                return \false;
            }
    ...
    private function getAutoloadFile()
    {
        ...
        foreach (\spl_autoload_functions() as $autoloader) {
            ...
            if ($autoloader[0] instanceof \ConfigTransformer20210601\Symfony\Component\ErrorHandler\DebugClassLoader || $autoloader[0] instanceof \ConfigTransformer20210601\Symfony\Component\Debug\DebugClassLoader) {
                $autoloader = $autoloader[0]->getClassLoader();
            }
            if (!\is_array($autoloader) || !$autoloader[0] instanceof \ConfigTransformer20210601\Composer\Autoload\ClassLoader || !$autoloader[0]->findFile(__CLASS__)) {
                continue;
            }
    ...
    private function getClasses(\ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition $definition, string $id) : array
    {
        $classes = [];
        while ($definition instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Definition) {
            foreach ($definition->getTag($this->preloadTags[0]) as $tag) {
                if (!isset($tag['class'])) {
                    throw new \ConfigTransformer20210601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Missing attribute "class" on tag "%s" for service "%s".', $this->preloadTags[0], $id));
                }
