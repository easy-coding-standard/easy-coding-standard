...
use ECSPrefix20210509\Symfony\Component\Debug\DebugClassLoader as LegacyDebugClassLoader;
...
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass;
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass;
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode;
...
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\EnvParameterException;
...
use ECSPrefix20210509\Symfony\Component\DependencyInjection\ExpressionLanguage;
use ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface as ProxyDumper;
use ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper;
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Loader\FileLoader;
...
use ECSPrefix20210509\Symfony\Component\DependencyInjection\Variable;
 ...
 */
class PhpDumper extends \ECSPrefix20210509\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
    ...
    public function __construct(\ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        if (!$container->isCompiled()) {
            throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\LogicException('Cannot dump an uncompiled container.');
        }
     ...
     */
    public function setProxyDumper(\ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface $proxyDumper)
    {
    ...
    public function dump(array $options = [])
    {
        ...
        if (0 !== \strpos($baseClass = $options['base_class'], '\\') && 'Container' !== $baseClass) {
            ...
            $this->baseClass = \ECSPrefix20210509\Symfony\Component\DependencyInjection\Container::class;
        } else {
            $this->baseClass = $baseClass;
        }
        $this->initializeMethodNamesMap('Container' === $baseClass ? \ECSPrefix20210509\Symfony\Component\DependencyInjection\Container::class : $baseClass);
        if ($this->getProxyDumper() instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper) {
            (new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\true, \false))->process($this->container);
            try {
                (new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass())->process($this->container);
                ...
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($e->getServiceId(), $path);
            }
        }
...
EOF;
            ...
            $hash = \ucfirst(\strtr(\ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerBuilder::hash($files), '._', 'xx'));
            $code = [];
...
use ECSPrefix20210509\\Symfony\\Component\\DependencyInjection\\Dumper\\Preloader;
...
EOF;
        ...
        if ($unusedEnvs) {
            throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\EnvParameterException($unusedEnvs, null, 'Environment variables "%s" are never used. Please, check your container\'s configuration.');
        }
        return $code;
    }
    /**
    ...
    {
        if (!$this->proxyDumper) {
            $this->proxyDumper = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper();
        }
        return $this->proxyDumper;
    }
    private function analyzeReferences()
    {
        (new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\false, !$this->getProxyDumper() instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper))->process($this->container);
        $checkedNodes = [];
        $this->circularReferences = [];
        $this->singleUsePrivateIds = [];
        foreach ($this->container->getCompiler()->getServiceReferenceGraph()->getNodes() as $id => $node) {
            if (!$node->getValue() instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition) {
                continue;
            }
    ...
    private function collectCircularReferences($sourceId, array $edges, array &$checkedNodes, array &$loops = [], array $path = [], $byConstructor = \true)
    {
        ...
        foreach ($edges as $edge) {
            ...
            $id = $node->getId();
            if ($sourceId === $id || !$node->getValue() instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition || $edge->isLazy() || $edge->isWeak()) {
                continue;
            }
            if (isset($path[$id])) {
                $loop = null;
    ...
    private function generateProxyClasses()
    {
        ...
        $strip = '' === $this->docStar && \method_exists(\ECSPrefix20210509\Symfony\Component\HttpKernel\Kernel::class, 'stripComments');
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = \substr(\ECSPrefix20210509\Symfony\Component\HttpKernel\Kernel::stripComments($proxyCode), 5);
            }
     ...
     * @return string
     */
    private function addServiceInclude($cId, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition)
    {
        $cId = (string) $cId;
        $code = '';
        if ($this->inlineRequires && (!$this->isHotPath($definition) || $this->getProxyDumper()->isProxyCandidate($definition))) {
            ...
            foreach ($this->serviceCalls as $id => list($callCount, $behavior)) {
                if ('service_container' !== $id && $id !== $cId && \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $behavior && $this->container->has($id) && $this->isTrivialInstance($def = $this->container->findDefinition($id))) {
                    foreach ($this->getClasses($def, $cId) as $class) {
                        $this->collectLineage($class, $lineage);
                    }
                }
     ...
     * @return string
     */
    private function addServiceInstance($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $isSimpleInstance)
    {
        $id = (string) $id;
        ...
        if (0 === \strpos($class, "'") && \false === \strpos($class, '$') && !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid class name for the "%s" service.', $class, $id));
        }
     ...
     * @return bool
     */
    private function isTrivialInstance(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition)
    {
        ...
        foreach ($definition->getArguments() as $arg) {
            if (!$arg || $arg instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Parameter) {
                continue;
            }
            if (\is_array($arg) && 3 >= \count($arg)) {
                foreach ($arg as $k => $v) {
                    ...
                    if (!$v || $v instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Parameter) {
                        continue;
                    }
                    if ($v instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $v) && $this->container->findDefinition($id)->isSynthetic()) {
                        continue;
                    }
            ...
            } elseif ($arg instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $arg) && $this->container->findDefinition($id)->isSynthetic()) {
                continue;
     ...
     * @return string
     */
    private function addServiceMethodCalls(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $variableName, $sharedNonLazyId)
    {
     ...
     * @return string
     */
    private function addServiceProperties(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $variableName = 'instance')
    {
        $variableName = (string) $variableName;
        $code = '';
     ...
     * @return string
     */
    private function addServiceConfigurator(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $variableName = 'instance')
    {
        $variableName = (string) $variableName;
        ...
        if (\is_array($callable)) {
            if ($callable[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
    ...
    private function addService($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition)
    {
        ...
        $this->referenceVariables[$id] = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Variable('instance');
        $return = [];
        if ($class = $definition->getClass()) {
            $class = $class instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
            $return[] = \sprintf(0 === \strpos($class, '%') ? '@return object A %1$s instance' : '@return \\%s', \ltrim($class, '\\'));
        } elseif ($definition->getFactory()) {
            $factory = $definition->getFactory();
            if (\is_string($factory)) {
                $return[] = \sprintf('@return object An instance returned by %s()', $factory);
            } elseif (\is_array($factory) && (\is_string($factory[0]) || $factory[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition || $factory[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference)) {
                $class = $factory[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition ? $factory[0]->getClass() : (string) $factory[0];
                $class = $class instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
...
EOF;
     ...
     * @return string
     */
    private function addInlineVariables($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, array $arguments, $forConstructor)
    {
        $id = (string) $id;
        ...
        foreach ($arguments as $argument) {
            ...
            } elseif ($argument instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference) {
     ...
     * @return string
     */
    private function addInlineReference($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $targetId, $forConstructor)
    {
        $id = (string) $id;
        ...
        $this->referenceVariables[$targetId] = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Variable($name);
        $reference = \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $behavior ? new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference($targetId, $behavior) : null;
...
, $this->container->getDefinition($id)->isPublic() ? 'services' : 'privates', $this->doExport($id));
     ...
     * @return string
     */
    private function addInlineService($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $inlineDef = null, $forConstructor = \true)
    {
        $id = (string) $id;
        $forConstructor = (bool) $forConstructor;
        $code = '';
        ...
        if (isset($this->definitionVariables[$inlineDef])) {
            ...
            $this->definitionVariables[$inlineDef] = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Variable($name);
            $code .= '' !== $code ? "\n" : '';
            if ('instance' === $name) {
     ...
     * @return string
     */
    private function addNewInstance(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $return = '', $id = null)
    {
        $return = (string) $return;
        $tail = $return ? ";\n" : '';
        if (\ECSPrefix20210509\Symfony\Component\DependencyInjection\ServiceLocator::class === $definition->getClass() && $definition->hasTag($this->serviceLocatorTag)) {
            $arguments = [];
            foreach ($definition->getArgument(0) as $k => $argument) {
                $arguments[$k] = $argument->getValues()[0];
            }
            return $return . $this->dumpValue(new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments)) . $tail;
        }
        $arguments = [];
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_array($callable)) {
                if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $callable[1])) {
                    throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $callable[1] ?: 'n/a'));
                }
                if ($callable[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
                ...
                if (0 === \strpos($class, "'") && \false === \strpos($class, '$')) {
                    if ("''" === $class) {
                        throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition: "%s" service is defined to be created by a factory but is missing the service reference, did you forget to define the factory service id or class?', $id ? 'The "' . $id . '"' : 'inline'));
                    }
        ...
        if (null === ($class = $definition->getClass())) {
            throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have no class nor factory.');
        }
...
use ECSPrefix20210509\\Symfony\\Component\\DependencyInjection\\Exception\\LogicException;
use ECSPrefix20210509\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
use ECSPrefix20210509\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;
...
EOF;
        ...
        if (\ECSPrefix20210509\Symfony\Component\DependencyInjection\Container::class !== $this->baseClass) {
            $r = $this->container->getReflectionClass($this->baseClass, \false);
            if (null !== $r && null !== ($constructor = $r->getConstructor()) && 0 === $constructor->getNumberOfRequiredParameters() && \ECSPrefix20210509\Symfony\Component\DependencyInjection\Container::class !== $constructor->getDeclaringClass()->name) {
...
EOF;
    ...
    private function addRemovedIds()
    {
        ...
        if ($this->asFiles) {
            ...
            \sort($ids);
            foreach ($ids as $id) {
                if (\preg_match(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Loader\FileLoader::ANONYMOUS_ID_REGEXP, $id)) {
                    continue;
                }
                $code .= '            ' . $this->doExport($id) . " => true,\n";
            }
    ...
    private function addDefaultParametersMethod()
    {
        ...
        foreach ($this->container->getParameterBag()->all() as $key => $value) {
            if ($key !== ($resolvedKey = $this->container->resolveEnvPlaceholders($key))) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Parameter name cannot use env parameters: "%s".', $resolvedKey));
            }
    ...
    private function exportParameters(array $parameters, $path = '', $indent = 12)
    {
        ...
        foreach ($parameters as $key => $value) {
            if (\is_array($value)) {
                ...
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain special arguments. "%s" found in "%s".', \get_debug_type($value), $path . '/' . $key));
            } elseif ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Variable) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".', $value, $path . '/' . $key));
            } elseif ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".', $value->getClass(), $path . '/' . $key));
            } elseif ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").', $value, $path . '/' . $key));
            } elseif ($value instanceof \ECSPrefix20210509\Symfony\Component\ExpressionLanguage\Expression) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain expressions. Expression "%s" found in "%s".', $value, $path . '/' . $key));
            } else {
...
EOF;
    ...
    {
        $conditions = [];
        foreach (\ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerBuilder::getInitializedConditionals($value) as $service) {
        ...
        foreach (\ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerBuilder::getServiceConditionals($value) as $service) {
        ...
        if (\is_array($value)) {
            ...
            try {
                if ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                    ...
                    $returnedType = '';
                    if ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\TypedReference) {
                        $returnedType = \sprintf(': %s\\%s', \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $value->getInvalidBehavior() ? '' : '?', $value->getType());
                    }
                ...
                if ($value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                    $serviceMap = '';
                    $serviceTypes = '';
                    foreach ($value->getValues() as $k => $v) {
                        ...
                        $serviceMap .= \sprintf("\n            %s => [%s, %s, %s, %s],", $this->export($k), $this->export($definition->isShared() ? $definition->isPublic() ? 'services' : 'privates' : \false), $this->doExport($id), $this->export(\ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $v->getInvalidBehavior() && !\is_string($load) ? $this->generateMethodName($id) : null), $this->export($load));
                        $serviceTypes .= \sprintf("\n            %s => %s,", $this->export($k), $this->export($v instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\TypedReference ? $v->getType() : '?'));
                        $this->locatedIds[$id] = \true;
                    }
                    $this->addGetService = \true;
                    return \sprintf('new \\%s($this->getService, [%s%s], [%s%s])', \ECSPrefix20210509\Symfony\Component\DependencyInjection\Argument\ServiceLocator::class, $serviceMap, $serviceMap ? "\n        " : '', $serviceTypes, $serviceTypes ? "\n        " : '');
                }
            } finally {
            ...
            if ($value->getMethodCalls()) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have method calls.');
            }
            if ($value->getProperties()) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have properties.');
            }
            if (null !== $value->getConfigurator()) {
                throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have a configurator.');
            }
    ...
    private function dumpLiteralClass($class)
    {
        ...
        if (0 !== \strpos($class, "'") || !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid class name (%s).', $class ?: 'n/a'));
        }
    ...
    private function getServiceCall($id, \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference $reference = null)
    {
        ...
        if ($this->container->hasDefinition($id) && ($definition = $this->container->getDefinition($id))) {
            ...
            } elseif (null !== $reference && \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
                if (!$definition->isShared()) {
                    return $code;
                }
        ...
        if (null !== $reference && \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
            return 'null';
        }
        if (null !== $reference && \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $reference->getInvalidBehavior()) {
            $code = \sprintf('$this->get(%s, /* ContainerInterface::NULL_ON_INVALID_REFERENCE */ %d)', $this->doExport($id), \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE);
        } else {
    ...
    private function generateMethodName($id)
    {
        ...
        $i = \strrpos($id, '\\');
        $name = \ECSPrefix20210509\Symfony\Component\DependencyInjection\Container::camelize(\false !== $i && isset($id[1 + $i]) ? \substr($id, 1 + $i) : $id);
    ...
    private function getExpressionLanguage()
    {
        if (null === $this->expressionLanguage) {
            ...
            $this->expressionLanguage = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $providers, function ($arg) {
     ...
     * @return bool
     */
    private function isHotPath(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition)
    {
     ...
     * @return bool
     */
    private function isSingleUsePrivateNode(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode $node)
    {
        ...
        foreach ($node->getInEdges() as $edge) {
            ...
            if ($edge->isLazy() || !$value instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition || !$value->isShared()) {
                return \false;
            }
    ...
    private function getAutoloadFile()
    {
        ...
        foreach (\spl_autoload_functions() as $autoloader) {
            ...
            if ($autoloader[0] instanceof \ECSPrefix20210509\Symfony\Component\ErrorHandler\DebugClassLoader || $autoloader[0] instanceof \ECSPrefix20210509\Symfony\Component\Debug\DebugClassLoader) {
                $autoloader = $autoloader[0]->getClassLoader();
            }
            if (!\is_array($autoloader) || !$autoloader[0] instanceof \ECSPrefix20210509\Composer\Autoload\ClassLoader || !$autoloader[0]->findFile(__CLASS__)) {
                continue;
            }
     ...
     * @return mixed[]
     */
    private function getClasses(\ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition $definition, $id)
    {
        $id = (string) $id;
        $classes = [];
        while ($definition instanceof \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition) {
            foreach ($definition->getTag($this->preloadTags[0]) as $tag) {
                if (!isset($tag['class'])) {
                    throw new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Missing attribute "class" on tag "%s" for service "%s".', $this->preloadTags[0], $id));
                }
