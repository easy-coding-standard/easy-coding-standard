...
use ECSPrefix20210508\Symfony\Component\Config\Loader\FileLoader as BaseFileLoader;
 ...
 */
abstract class FileLoader extends \ECSPrefix20210508\Symfony\Component\Config\Loader\FileLoader
{
    ...
    public function __construct(\ECSPrefix20210508\Symfony\Component\DependencyInjection\ContainerBuilder $container, \ECSPrefix20210508\Symfony\Component\Config\FileLocatorInterface $locator)
    {
    ...
    public function import($resource, $type = null, $ignoreErrors = \false, $sourceResource = null, $exclude = null)
    {
        ...
        try {
            parent::import(...$args);
        } catch (\ECSPrefix20210508\Symfony\Component\Config\Exception\LoaderLoadException $e) {
            if (!$ignoreNotFound || !($prev = $e->getPrevious()) instanceof \ECSPrefix20210508\Symfony\Component\Config\Exception\FileLocatorFileNotFoundException) {
                throw $e;
            }
            foreach ($prev->getTrace() as $frame) {
                if ('import' === (isset($frame['function']) ? $frame['function'] : null) && \is_a(isset($frame['class']) ? $frame['class'] : '', \ECSPrefix20210508\Symfony\Component\Config\Loader\Loader::class, \true)) {
                    break;
                }
            }
    ...
    public function registerClasses(\ECSPrefix20210508\Symfony\Component\DependencyInjection\Definition $prototype, $namespace, $resource, $exclude = null)
    {
        ...
        if (!\preg_match('/^(?:[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*+\\\\)++$/', $namespace)) {
            throw new \ECSPrefix20210508\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Namespace is not a valid PSR-4 prefix: "%s".', $namespace));
        }
    ...
    protected function setDefinition($id, \ECSPrefix20210508\Symfony\Component\DependencyInjection\Definition $definition)
    {
        ...
        if ($this->isLoadingInstanceof) {
            if (!$definition instanceof \ECSPrefix20210508\Symfony\Component\DependencyInjection\ChildDefinition) {
                throw new \ECSPrefix20210508\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid type definition "%s": ChildDefinition expected, "%s" given.', $id, \get_debug_type($definition)));
            }
    ...
    private function findClasses($namespace, $pattern, array $excludePatterns)
    {
        ...
        foreach ($this->glob($pattern, \true, $resource, \false, \false, $excludePaths) as $path => $info) {
            if (null === $prefixLen) {
                ...
                if ($excludePrefix && 0 !== \strpos($excludePrefix, $resource->getPrefix())) {
                    throw new \ECSPrefix20210508\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid "exclude" pattern when importing classes for "%s": make sure your "exclude" pattern (%s) is a subset of the "resource" pattern (%s).', $namespace, $excludePattern, $pattern));
                }
            }
            ...
            if (!$r) {
                throw new \ECSPrefix20210508\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Expected to find class "%s" in file "%s" while importing services from resource "%s", but it was not found! Check the namespace prefix used with the resource.', $class, $path, $pattern));
            }
        ...
        if ($resource instanceof \ECSPrefix20210508\Symfony\Component\Config\Resource\GlobResource) {
