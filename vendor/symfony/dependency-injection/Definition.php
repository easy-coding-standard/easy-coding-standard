 ...
 */
namespace ECSPrefix20220601\Symfony\Component\DependencyInjection;
...
use ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException;
/**
...
class Definition
{
    ...
    public function setFactory($factory)
    {
        ...
        } elseif ($factory instanceof \ECSPrefix20220601\Symfony\Component\DependencyInjection\Reference) {
     ...
     */
    public function setDecoratedService(?string $id, string $renamedId = null, int $priority = 0, int $invalidBehavior = \ECSPrefix20220601\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE)
    {
        if ($renamedId && $id === $renamedId) {
            throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The decorated service inner name for "%s" must be different than the service name itself.', $id));
        }
        ...
        if (null === $id) {
            ...
            if (\ECSPrefix20220601\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $invalidBehavior) {
    ...
    public function replaceArgument($index, $argument)
    {
        if (0 === \count($this->arguments)) {
            throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('Cannot replace arguments for class "%s" if none have been configured yet.', $this->class));
        }
        if (\is_int($index) && ($index < 0 || $index > \count($this->arguments) - 1)) {
            throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The index "%d" is not in the range [0, %d] of the arguments of class "%s".', $index, \count($this->arguments) - 1, $this->class));
        }
        if (!\array_key_exists($index, $this->arguments)) {
            throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The argument "%s" doesn\'t exist in class "%s".', $index, $this->class));
        }
    ...
    {
        if (empty($method)) {
            throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Method name cannot be empty.');
        }
    ...
    public function setDeprecated(string $package, string $version, string $message)
    {
        if ('' !== $message) {
            if (\preg_match('#[\\r\\n]|\\*/#', $message)) {
                throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Invalid characters found in deprecation template.');
            }
            if (\strpos($message, '%service_id%') === \false) {
                throw new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%service_id%" placeholder.');
            }
        }
    ...
    public function setConfigurator($configurator)
    {
        ...
        } elseif ($configurator instanceof \ECSPrefix20220601\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setBindings(array $bindings)
    {
        foreach ($bindings as $key => $binding) {
            ...
            if (!$binding instanceof \ECSPrefix20220601\Symfony\Component\DependencyInjection\Argument\BoundArgument) {
                $bindings[$key] = new \ECSPrefix20220601\Symfony\Component\DependencyInjection\Argument\BoundArgument($binding);
            }
        }
