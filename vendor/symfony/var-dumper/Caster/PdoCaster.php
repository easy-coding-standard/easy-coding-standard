    ...
    public static function castPdo(\PDO $c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        foreach (self::PDO_ATTRIBUTES as $k => $v) {
            ...
            try {
                ...
                if ($v && isset($v[$attr[$k]])) {
                    $attr[$k] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ConstStub($v[$attr[$k]], $attr[$k]);
                }
            } catch (\Exception $e) {
            }
        }
        if (isset($attr[$k = 'STATEMENT_CLASS'][1])) {
            if ($attr[$k][1]) {
                $attr[$k][1] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ArgsStub($attr[$k][1], '__construct', $attr[$k][0]);
            }
            $attr[$k][0] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ClassStub($attr[$k][0]);
        }
        $prefix = \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL;
        $a += [$prefix . 'inTransaction' => \method_exists($c, 'inTransaction'), $prefix . 'errorInfo' => $c->errorInfo(), $prefix . 'attributes' => new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\EnumStub($attr)];
