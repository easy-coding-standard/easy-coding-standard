...
use ECSPrefix202305\Symfony\Component\Uid\Ulid;
use ECSPrefix202305\Symfony\Component\Uid\Uuid;
use ECSPrefix202305\Symfony\Component\VarDumper\Cloner\Stub;
use ECSPrefix202305\Symfony\Component\VarExporter\Internal\LazyObjectState;
/**
 * @final
 */
class SymfonyCaster
{
    ...
    public static function castUuid(Uuid $uuid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($uuid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $uuid->getDateTime()->format('ECSPrefix202305\\Y-m-d H:i:s.u \\U\\T\\C');
        }
        return $a;
    }
    public static function castUlid(Ulid $ulid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($ulid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $ulid->getDateTime()->format('ECSPrefix202305\\Y-m-d H:i:s.v \\U\\T\\C');
        }
        return $a;
    }
}
