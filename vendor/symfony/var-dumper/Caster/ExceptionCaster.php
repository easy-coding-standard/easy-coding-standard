 ...
 */
namespace ECSPrefix202303\Symfony\Component\VarDumper\Caster;

use ECSPrefix202303\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext;
use ECSPrefix202303\Symfony\Component\VarDumper\Cloner\Stub;
use ECSPrefix202303\Symfony\Component\VarDumper\Exception\ThrowingCasterException;
/**
...
class ExceptionCaster
{
        ...
        for ($j += $trace->numberingOffset - $i++; isset($frames[$i]); ++$i, --$j) {
            ...
            if (isset($f[$prefix . 'src'])) {
                foreach ($f[$prefix . 'src']->value as $label => $frame) {
                    if (\strncmp($label, "\x00~collapse=0", \strlen("\x00~collapse=0")) === 0) {
                        if ($collapse) {
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (\is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && (\is_subclass_of($f['class'], 'ECSPrefix202303\\Twig\\Template') || \is_subclass_of($f['class'], 'ECSPrefix202303\\Twig_Template')) && \method_exists($f['class'], 'getDebugInfo')) {
                        $template = null;
                        if (isset($f['object'])) {
                            $template = $f['object'];
    ...
    private static function filterExceptionArray(string $xClass, array $a, string $xPrefix, int $filter) : array
    {
        ...
        if (isset($a[Caster::PREFIX_PROTECTED . 'message']) && \strpos($a[Caster::PREFIX_PROTECTED . 'message'], "@anonymous\x00") !== \false) {
