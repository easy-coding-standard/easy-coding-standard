        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (\is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && (\is_subclass_of($f['class'], 'ECSPrefix202306\\Twig\\Template') || \is_subclass_of($f['class'], 'ECSPrefix202306\\Twig_Template')) && \method_exists($f['class'], 'getDebugInfo')) {
                        $template = null;
                        if (isset($f['object'])) {
                            $template = $f['object'];
