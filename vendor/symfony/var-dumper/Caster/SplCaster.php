...
class SplCaster
{
    ...
    {
        $a += [\ECSPrefix20210508\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'heap' => \iterator_to_array(clone $c)];
        return $a;
    }
    /**
     * @param bool $isNested
    ...
    public static function castDoublyLinkedList(\SplDoublyLinkedList $c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        $a += [$prefix . 'mode' => new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ConstStub(($mode & \SplDoublyLinkedList::IT_MODE_LIFO ? 'IT_MODE_LIFO' : 'IT_MODE_FIFO') . ' | ' . ($mode & \SplDoublyLinkedList::IT_MODE_DELETE ? 'IT_MODE_DELETE' : 'IT_MODE_KEEP'), $mode), $prefix . 'dllist' => \iterator_to_array($c)];
    ...
    public static function castFileInfo(\SplFileInfo $c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        if (isset($a[$prefix . 'realPath'])) {
            $a[$prefix . 'realPath'] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\LinkStub($a[$prefix . 'realPath']);
        }
        if (isset($a[$prefix . 'perms'])) {
            $a[$prefix . 'perms'] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ConstStub(\sprintf('0%o', $a[$prefix . 'perms']), $a[$prefix . 'perms']);
        }
        static $mapDate = ['aTime', 'mTime', 'cTime'];
        foreach ($mapDate as $key) {
            if (isset($a[$prefix . $key])) {
                $a[$prefix . $key] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ConstStub(\date('Y-m-d H:i:s', $a[$prefix . $key]), $a[$prefix . $key]);
            }
        }
        return $a;
    }
    ...
    public static function castFileObject(\SplFileObject $c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        if (isset($a[$prefix . 'flags'])) {
            ...
            $a[$prefix . 'flags'] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ConstStub(\implode('|', $flagsArray), $a[$prefix . 'flags']);
        }
        if (isset($a[$prefix . 'fstat'])) {
            $a[$prefix . 'fstat'] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\CutArrayStub($a[$prefix . 'fstat'], ['dev', 'ino', 'nlink', 'rdev', 'blksize', 'blocks']);
        }
        return $a;
    }
    /**
    ...
    public static function castObjectStorage(\SplObjectStorage $c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        $a += [\ECSPrefix20210508\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'storage' => $storage];
        return $a;
    }
    /**
     * @param bool $isNested
    ...
    private static function castSplArray($c, array $a, \ECSPrefix20210508\Symfony\Component\VarDumper\Cloner\Stub $stub, $isNested)
    {
        ...
        if (!($flags & \ArrayObject::STD_PROP_LIST)) {
            ...
            $a = \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\Caster::castObject($c, \get_class($c), \method_exists($c, '__debugInfo'), $stub->class);
            $c->setFlags($flags);
        }
        ...
        if ($c instanceof \ArrayObject) {
            $a[$prefix . 'iteratorClass'] = new \ECSPrefix20210508\Symfony\Component\VarDumper\Caster\ClassStub($c->getIteratorClass());
        }
        return $a;
    }
}
