...
class PgSqlCaster
{
    ...
    public static function castLargeObject($lo, array $a, \ECSPrefix20210906\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a['seek position'] = \pg_lo_tell($lo);
        return $a;
    }
    public static function castLink($link, array $a, \ECSPrefix20210906\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a['status'] = \pg_connection_status($link);
        $a['status'] = new \ECSPrefix20210906\Symfony\Component\VarDumper\Caster\ConstStub(\PGSQL_CONNECTION_OK === $a['status'] ? 'PGSQL_CONNECTION_OK' : 'PGSQL_CONNECTION_BAD', $a['status']);
        $a['busy'] = \pg_connection_busy($link);
        $a['transaction'] = \pg_transaction_status($link);
        if (isset(self::TRANSACTION_STATUS[$a['transaction']])) {
            $a['transaction'] = new \ECSPrefix20210906\Symfony\Component\VarDumper\Caster\ConstStub(self::TRANSACTION_STATUS[$a['transaction']], $a['transaction']);
        }
        ...
        $a['param'] = new \ECSPrefix20210906\Symfony\Component\VarDumper\Caster\EnumStub($a['param']);
        return $a;
    }
    public static function castResult($result, array $a, \ECSPrefix20210906\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a['num rows'] = \pg_num_rows($result);
        $a['status'] = \pg_result_status($result);
        if (isset(self::RESULT_STATUS[$a['status']])) {
            $a['status'] = new \ECSPrefix20210906\Symfony\Component\VarDumper\Caster\ConstStub(self::RESULT_STATUS[$a['status']], $a['status']);
        }
        ...
        for ($i = 0; $i < $fields; ++$i) {
            ...
            $a['fields'][] = new \ECSPrefix20210906\Symfony\Component\VarDumper\Caster\EnumStub($field);
        }
        return $a;
    }
}
