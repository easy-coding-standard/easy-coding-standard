 ...
 */
class VarCloner extends \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\AbstractCloner
{
    private static $gid;
    ...
    protected function doClone($var)
    {
        ...
        $arrayStub = new \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub();
        $arrayStub->type = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_ARRAY;
        $fromObjCast = \false;
        for ($i = 0; $i < $len; ++$i) {
            ...
            foreach ($vals as $k => $v) {
                ...
                if ($zvalRef) {
                    ...
                    if (\PHP_VERSION_ID >= 70400 ? null !== ($vals[$k] = $hardRefs[$zvalRef] ?? null) : $v instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub && isset($hardRefs[\spl_object_id($v)])) {
                        if (\PHP_VERSION_ID >= 70400) {
                            $v = $vals[$k];
                        } else {
                            $refs[$k] = $vals[$k] = $v;
                        }
                        if ($v->value instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub && (\ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_OBJECT === $v->value->type || \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_RESOURCE === $v->value->type)) {
                            ++$v->value->refCount;
                        }
                        ++$v->refCount;
                        continue;
                    }
                    $vals[$k] = new \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub();
                    $vals[$k]->value = $v;
                ...
                switch (\true) {
                    ...
                    case \is_string($v):
                        ...
                        if (!\preg_match('//u', $v)) {
                            ...
                            $stub = new \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub();
                            $stub->type = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_STRING;
                            $stub->class = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::STRING_UTF8;
                            $stub->cut = $cut;
                    ...
                    case \is_array($v):
                        ...
                        $stub = $arrayStub;
                        $stub->class = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED;
                        $j = -1;
                        foreach ($v as $gk => $gv) {
                            if ($gk !== ++$j) {
                                $stub->class = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC;
                                break;
                            }
                        }
                        $a = $v;
                        if (\ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $stub->class) {
                            ...
                            if (\PHP_VERSION_ID < 80100 && ($a[$gid] = \true) && isset($v[$gid])) {
                                ...
                                foreach ($v as $gk => &$gv) {
                                    if ($v === $gv && (\PHP_VERSION_ID < 70400 || !isset($hardRefs[\ReflectionReference::fromArrayElement($v, $gk)->getId()]))) {
                                        unset($v);
                                        $v = new \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub();
                                        $v->value = [$v->cut = \count($gv), \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_ARRAY => 0];
                                        $v->handle = -1;
                                        if (\PHP_VERSION_ID >= 70400) {
                                            $gv =& $a[$gk];
                    ...
                    case \is_object($v):
                        if (empty($objRefs[$h = \spl_object_id($v)])) {
                            ...
                            if ($v !== $stub->value) {
                                if (\ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_OBJECT !== $stub->type || null === $stub->value) {
                                    break;
                                }
                    ...
                    default:
                        ...
                        if (empty($resRefs[$h = (int) $v])) {
                            $stub = new \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub();
                            $stub->type = \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Stub::TYPE_RESOURCE;
