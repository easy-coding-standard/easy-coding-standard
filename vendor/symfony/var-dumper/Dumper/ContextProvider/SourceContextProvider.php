...
use ECSPrefix20211002\Twig\Template;
/**
 ...
 */
final class SourceContextProvider implements \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\ContextProvider\ContextProviderInterface
{
    private $limit;
    private $charset;
    private $projectDir;
    private $fileLinkFormatter;
    public function __construct(string $charset = null, string $projectDir = null, \ECSPrefix20211002\Symfony\Component\HttpKernel\Debug\FileLinkFormatter $fileLinkFormatter = null, int $limit = 9)
    {
    ...
    public function getContext() : ?array
    {
        ...
        for ($i = 2; $i < $this->limit; ++$i) {
            if (isset($trace[$i]['class'], $trace[$i]['function']) && 'dump' === $trace[$i]['function'] && \ECSPrefix20211002\Symfony\Component\VarDumper\VarDumper::class === $trace[$i]['class']) {
                ...
                while (++$i < $this->limit) {
                    if (isset($trace[$i]['function'], $trace[$i]['file']) && empty($trace[$i]['class']) && \strncmp($trace[$i]['function'], 'call_user_func', \strlen('call_user_func')) !== 0) {
                        ...
                        $line = $trace[$i]['line'];
                        break;
                    } elseif (isset($trace[$i]['object']) && $trace[$i]['object'] instanceof \ECSPrefix20211002\Twig\Template) {
    ...
    {
        $html = '';
        $dumper = new \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\HtmlDumper(function ($line) use(&$html) {
            $html .= $line;
        }, $this->charset);
