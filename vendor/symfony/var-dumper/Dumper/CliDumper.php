...
use ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \ECSPrefix20210516\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
     ...
     * @param string $type
     */
    public function dumpScalar(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $value)
    {
        $type = (string) $type;
        $this->dumpKey($cursor);
        $style = 'const';
     ...
     * @param int $cut
     */
    public function dumpString(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $str, $bin, $cut)
    {
        $str = (string) $str;
        $bin = (bool) $bin;
        $cut = (int) $cut;
     ...
     * @param bool $hasChild
     */
    public function enterHash(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $class, $hasChild)
    {
        $type = (int) $type;
        ...
        if (\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
            $prefix = $this->style('note', $class . ' resource', $attr) . ($hasChild ? ' {' : ' ');
        } else {
            $prefix = $class && !(self::DUMP_LIGHT_ARRAY & $this->flags) ? $this->style('note', 'array:' . $class) . ' [' : '[';
        }
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
     ...
     * @param int $cut
     */
    public function leaveHash(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $class, $hasChild, $cut)
    {
        $type = (int) $type;
        $hasChild = (bool) $hasChild;
        $cut = (int) $cut;
        if (empty($cursor->attr['cut_hash'])) {
            ...
            $this->line .= \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
     ...
     */
    protected function dumpEllipsis(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $hasChild, $cut)
    {
        if ($cut) {
            $this->line .= ' …';
            if (0 < $cut) {
     ...
     */
    protected function dumpKey(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\0~\0" . $key;
                // no break
                case \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue(\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (-1 === $cursor->hashType) {
            return;
        }
        if (\ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \ECSPrefix20210516\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {
