 ...
 */
namespace ECSPrefix20211012\Symfony\Component\VarDumper\Dumper;

use ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \ECSPrefix20211012\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
    ...
    public function enterHash($cursor, $type, $class, $hasChild)
    {
        ...
        if (\ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
            $prefix = $this->style('note', $class . ' resource', $attr) . ($hasChild ? ' {' : ' ');
        } else {
            $prefix = $class && !(self::DUMP_LIGHT_ARRAY & $this->flags) ? $this->style('note', 'array:' . $class) . ' [' : '[';
        }
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
    ...
    public function leaveHash($cursor, $type, $class, $hasChild, $cut)
    {
        if (empty($cursor->attr['cut_hash'])) {
            ...
            $this->line .= \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
    ...
    protected function dumpKey($cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\0~\0" . $key;
                // no break
                case \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue($cursor)
    {
        ...
        if (\ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \ECSPrefix20211012\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {
