...
use ECSPrefix20210508\Symfony\Component\HttpKernel\Event\KernelEvent;
 ...
 * @final
 */
class DebugHandlersListener implements \ECSPrefix20210508\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    {
        if ($event instanceof \ECSPrefix20210508\Symfony\Component\Console\Event\ConsoleEvent && !\in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true)) {
            return;
        }
        if (!$event instanceof \ECSPrefix20210508\Symfony\Component\HttpKernel\Event\KernelEvent ? !$this->firstCall : !$event->isMasterRequest()) {
            return;
        }
        ...
        \restore_exception_handler();
        if (!$handler instanceof \ECSPrefix20210508\Symfony\Component\ErrorHandler\ErrorHandler) {
        ...
        if (!$this->exceptionHandler) {
            if ($event instanceof \ECSPrefix20210508\Symfony\Component\HttpKernel\Event\KernelEvent) {
            ...
            } elseif ($event instanceof \ECSPrefix20210508\Symfony\Component\Console\Event\ConsoleEvent && ($app = $event->getCommand()->getApplication())) {
     ...
     * @return void
     */
    private function setDefaultLoggers(\ECSPrefix20210508\Symfony\Component\ErrorHandler\ErrorHandler $handler)
    {
        if (\is_array($this->levels)) {
    ...
    {
        $events = [\ECSPrefix20210508\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['configure', 2048]];
        if (\defined('Symfony\\Component\\Console\\ConsoleEvents::COMMAND')) {
            $events[\ECSPrefix20210508\Symfony\Component\Console\ConsoleEvents::COMMAND] = ['configure', 2048];
        }
        return $events;
    }
}
