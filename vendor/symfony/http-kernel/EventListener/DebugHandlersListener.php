 ...
 */
class DebugHandlersListener implements \ECSPrefix20211002\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function __construct(callable $exceptionHandler = null, \ECSPrefix20211002\Psr\Log\LoggerInterface $logger = null, $levels = \E_ALL, ?int $throwAt = \E_ALL, bool $scream = \true, $fileLinkFormat = null, bool $scope = \true, \ECSPrefix20211002\Psr\Log\LoggerInterface $deprecationLogger = null)
    {
    ...
    {
        if ($event instanceof \ECSPrefix20211002\Symfony\Component\Console\Event\ConsoleEvent && !\in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true)) {
            return;
        }
        if (!$event instanceof \ECSPrefix20211002\Symfony\Component\HttpKernel\Event\KernelEvent ? !$this->firstCall : !$event->isMainRequest()) {
            return;
        }
        ...
        \restore_exception_handler();
        if (!$handler instanceof \ECSPrefix20211002\Symfony\Component\ErrorHandler\ErrorHandler) {
        ...
        if (!$this->exceptionHandler) {
            if ($event instanceof \ECSPrefix20211002\Symfony\Component\HttpKernel\Event\KernelEvent) {
            ...
            } elseif ($event instanceof \ECSPrefix20211002\Symfony\Component\Console\Event\ConsoleEvent && ($app = $event->getCommand()->getApplication())) {
    ...
    private function setDefaultLoggers(\ECSPrefix20211002\Symfony\Component\ErrorHandler\ErrorHandler $handler) : void
    {
        if (\is_array($this->levels)) {
    ...
    {
        $events = [\ECSPrefix20211002\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['configure', 2048]];
        if (\defined('Symfony\\Component\\Console\\ConsoleEvents::COMMAND')) {
            $events[\ECSPrefix20211002\Symfony\Component\Console\ConsoleEvents::COMMAND] = ['configure', 2048];
        }
        return $events;
    }
}
