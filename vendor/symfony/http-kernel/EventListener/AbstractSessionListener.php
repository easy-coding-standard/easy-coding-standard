...
use ECSPrefix20210708\Psr\Container\ContainerInterface;
use ECSPrefix20210708\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use ECSPrefix20210708\Symfony\Component\HttpFoundation\Session\Session;
use ECSPrefix20210708\Symfony\Component\HttpFoundation\Session\SessionInterface;
use ECSPrefix20210708\Symfony\Component\HttpKernel\Event\FinishRequestEvent;
...
use ECSPrefix20210708\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException;
 ...
 * @internal
 */
abstract class AbstractSessionListener implements \ECSPrefix20210708\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    private $debug;
    public function __construct(\ECSPrefix20210708\Psr\Container\ContainerInterface $container = null, bool $debug = \false)
    {
    ...
    public function onKernelRequest(\ECSPrefix20210708\Symfony\Component\HttpKernel\Event\RequestEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \ECSPrefix20210708\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\ECSPrefix20210708\Symfony\Component\HttpKernel\Event\ResponseEvent $event)
    {
        ...
        if ($session instanceof \ECSPrefix20210708\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() === \end($this->sessionUsageStack) : !$session->isStarted()) {
            return;
        }
        if ($autoCacheControl) {
        ...
        if ($this->debug) {
            throw new \ECSPrefix20210708\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException('Session was used while the request was declared stateless.');
        }
        if ($this->container->has('logger')) {
            $this->container->get('logger')->warning('Session was used while the request was declared stateless.');
        }
    }
    public function onFinishRequest(\ECSPrefix20210708\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMainRequest()) {
    ...
    {
        return [
            \ECSPrefix20210708\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \ECSPrefix20210708\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \ECSPrefix20210708\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
