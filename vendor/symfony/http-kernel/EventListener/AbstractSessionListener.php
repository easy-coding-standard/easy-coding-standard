...
use ECSPrefix20210710\Psr\Container\ContainerInterface;
use ECSPrefix20210710\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use ECSPrefix20210710\Symfony\Component\HttpFoundation\Session\Session;
use ECSPrefix20210710\Symfony\Component\HttpFoundation\Session\SessionInterface;
use ECSPrefix20210710\Symfony\Component\HttpKernel\Event\FinishRequestEvent;
...
use ECSPrefix20210710\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException;
 ...
 * @internal
 */
abstract class AbstractSessionListener implements \ECSPrefix20210710\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    private $debug;
    public function __construct(\ECSPrefix20210710\Psr\Container\ContainerInterface $container = null, bool $debug = \false)
    {
    ...
    public function onKernelRequest($event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \ECSPrefix20210710\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    /**
    ...
    public function onKernelResponse($event)
    {
        ...
        if ($session instanceof \ECSPrefix20210710\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() === \end($this->sessionUsageStack) : !$session->isStarted()) {
            return;
        }
        if ($autoCacheControl) {
        ...
        if ($this->debug) {
            throw new \ECSPrefix20210710\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException('Session was used while the request was declared stateless.');
        }
    ...
    {
        return [
            \ECSPrefix20210710\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \ECSPrefix20210710\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \ECSPrefix20210710\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
