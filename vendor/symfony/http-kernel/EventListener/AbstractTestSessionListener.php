 ...
 * @internal
 */
abstract class AbstractTestSessionListener implements \ECSPrefix20210509\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    private $sessionId;
    ...
    public function onKernelResponse(\ECSPrefix20210509\Symfony\Component\HttpKernel\Event\ResponseEvent $event)
    {
        ...
        if ($session instanceof \ECSPrefix20210509\Symfony\Component\HttpFoundation\Session\Session ? !$session->isEmpty() || null !== $this->sessionId && $session->getId() !== $this->sessionId : $wasStarted) {
            ...
            $event->getResponse()->headers->setCookie(new \ECSPrefix20210509\Symfony\Component\HttpFoundation\Cookie($session->getName(), $session->getId(), 0 === $params['lifetime'] ? 0 : \time() + $params['lifetime'], $params['path'], $params['domain'], $params['secure'], $params['httponly'], \false, $params['samesite'] ?: null));
    ...
    {
        return [\ECSPrefix20210509\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 192], \ECSPrefix20210509\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -128]];
    }
    /**
