 ...
 */
class FragmentRendererPass implements \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process($container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->rendererTag, \true) as $id => $tags) {
            ...
            if (!$r->isSubclassOf(\ECSPrefix20210507\Symfony\Component\HttpKernel\Fragment\FragmentRendererInterface::class)) {
                throw new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Service "%s" must implement interface "%s".', $id, \ECSPrefix20210507\Symfony\Component\HttpKernel\Fragment\FragmentRendererInterface::class));
            }
            foreach ($tags as $tag) {
                $renderers[$tag['alias']] = new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference($id);
            }
        }
        $definition->replaceArgument(0, \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\ServiceLocatorTagPass::register($container, $renderers));
    }
}
