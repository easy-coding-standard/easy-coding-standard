 ...
 */
class ControllerArgumentValueResolverPass implements \ECSPrefix20210507\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process($container)
    {
        ...
        if ($container->getParameter('kernel.debug') && \class_exists(\ECSPrefix20210507\Symfony\Component\Stopwatch\Stopwatch::class) && $container->has($this->traceableResolverStopwatch)) {
            foreach ($resolvers as $resolverReference) {
                $id = (string) $resolverReference;
                $container->register("debug.{$id}", \ECSPrefix20210507\Symfony\Component\HttpKernel\Controller\ArgumentResolver\TraceableValueResolver::class)->setDecoratedService($id)->setArguments([new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference("debug.{$id}.inner"), new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Reference($this->traceableResolverStopwatch)]);
            }
        }
        $container->getDefinition($this->argumentResolverService)->replaceArgument(1, new \ECSPrefix20210507\Symfony\Component\DependencyInjection\Argument\IteratorArgument($resolvers));
    }
}
