 ...
 * @final
 */
class LoggerDataCollector extends \ECSPrefix20210917\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \ECSPrefix20210917\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
     ...
     * @param object $logger
     */
    public function __construct($logger = null, string $containerPathPrefix = null, \ECSPrefix20210917\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \ECSPrefix20210917\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \ECSPrefix20210917\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \ECSPrefix20210917\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \ECSPrefix20210917\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
