...
use ECSPrefix20210912\Symfony\Component\VarDumper\Caster\ClassStub;
/**
 * @author Fabien Potencier <fabien@symfony.com>
 *
 * @final
 */
class ConfigDataCollector extends \ECSPrefix20210912\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \ECSPrefix20210912\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    /**
     * @var KernelInterface
     */
    ...
    {
        $eom = \DateTime::createFromFormat('d/m/Y', '01/' . \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::END_OF_MAINTENANCE);
        $eol = \DateTime::createFromFormat('d/m/Y', '01/' . \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::END_OF_LIFE);
        $this->data = ['token' => $response->headers->get('X-Debug-Token'), 'symfony_version' => \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::VERSION, 'symfony_minor_version' => \sprintf('%s.%s', \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::MAJOR_VERSION, \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::MINOR_VERSION), 'symfony_lts' => 4 === \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::MINOR_VERSION, 'symfony_state' => $this->determineSymfonyState(), 'symfony_eom' => $eom->format('F Y'), 'symfony_eol' => $eol->format('F Y'), 'env' => isset($this->kernel) ? $this->kernel->getEnvironment() : 'n/a', 'debug' => isset($this->kernel) ? $this->kernel->isDebug() : 'n/a', 'php_version' => \PHP_VERSION, 'php_architecture' => \PHP_INT_SIZE * 8, 'php_intl_locale' => \class_exists(\Locale::class, \false) && \Locale::getDefault() ? \Locale::getDefault() : 'n/a', 'php_timezone' => \date_default_timezone_get(), 'xdebug_enabled' => \extension_loaded('xdebug'), 'apcu_enabled' => \extension_loaded('apcu') && \filter_var(\ini_get('apc.enabled'), \FILTER_VALIDATE_BOOLEAN), 'zend_opcache_enabled' => \extension_loaded('Zend OPcache') && \filter_var(\ini_get('opcache.enable'), \FILTER_VALIDATE_BOOLEAN), 'bundles' => [], 'sapi_name' => \PHP_SAPI];
        if (isset($this->kernel)) {
            foreach ($this->kernel->getBundles() as $name => $bundle) {
                $this->data['bundles'][$name] = new \ECSPrefix20210912\Symfony\Component\VarDumper\Caster\ClassStub(\get_class($bundle));
            }
        }
    ...
    {
        $now = new \DateTime();
        $eom = \DateTime::createFromFormat('d/m/Y', '01/' . \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::END_OF_MAINTENANCE)->modify('last day of this month');
        $eol = \DateTime::createFromFormat('d/m/Y', '01/' . \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::END_OF_LIFE)->modify('last day of this month');
        if ($now > $eol) {
            $versionState = 'eol';
        } elseif ($now > $eom) {
            $versionState = 'eom';
        } elseif ('' !== \ECSPrefix20210912\Symfony\Component\HttpKernel\Kernel::EXTRA_VERSION) {
            $versionState = 'dev';
        } else {
            $versionState = 'stable';
        }
