 ...
 * @final
 */
class DumpDataCollector extends \ECSPrefix20210507\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
    ...
    public function __construct($stopwatch = null, $fileLinkFormat = null, $charset = null, $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \ECSPrefix20210507\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    ...
    public function dump($data)
    {
        ...
        if ($this->dumper instanceof \ECSPrefix20210507\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect($request, $response, $exception = null)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function __wakeup()
    {
        ...
        self::__construct($this->stopwatch, \is_string($fileLinkFormat) || $fileLinkFormat instanceof \ECSPrefix20210507\Symfony\Component\HttpKernel\Debug\FileLinkFormatter ? $fileLinkFormat : null, \is_string($charset) ? $charset : null);
    }
    /**
     * @return int
     */
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1)
    {
        ...
        if ('html' === $format) {
            $dumper = new \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    {
        if ($dumper instanceof \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \ECSPrefix20210507\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
