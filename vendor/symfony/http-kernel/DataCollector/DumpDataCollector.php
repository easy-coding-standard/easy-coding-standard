 ...
 * @final
 */
class DumpDataCollector extends \ECSPrefix20211002\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\ECSPrefix20211002\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \ECSPrefix20211002\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    ...
    public function dump($data)
    {
        ...
        if ($this->dumper instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect($request, $response, $exception = null)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \strpos($response->headers->get('Content-Type'), 'html') === \false || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \strpos($response->headers->get('Content-Type'), 'html') !== \false) {
                $dumper = new \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function __wakeup()
    {
        ...
        self::__construct($this->stopwatch, \is_string($fileLinkFormat) || $fileLinkFormat instanceof \ECSPrefix20211002\Symfony\Component\HttpKernel\Debug\FileLinkFormatter ? $fileLinkFormat : null, \is_string($charset) ? $charset : null);
    }
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1) : array
    {
        ...
        if ('html' === $format) {
            $dumper = new \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, \ECSPrefix20211002\Symfony\Component\VarDumper\Cloner\Data $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \ECSPrefix20211002\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
