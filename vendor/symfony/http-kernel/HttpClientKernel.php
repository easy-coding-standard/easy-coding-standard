...
final class HttpClientKernel implements \ECSPrefix20210507\Symfony\Component\HttpKernel\HttpKernelInterface
{
    ...
    {
        if (null === $client && !\class_exists(\ECSPrefix20210507\Symfony\Component\HttpClient\HttpClient::class)) {
    ...
    public function handle($request, $type = \ECSPrefix20210507\Symfony\Component\HttpKernel\HttpKernelInterface::MASTER_REQUEST, $catch = \true)
    {
        ...
        $response = new \ECSPrefix20210507\Symfony\Component\HttpFoundation\Response($response->getContent(!$catch), $response->getStatusCode(), $response->getHeaders(!$catch));
    ...
    private function getBody($request)
    {
        ...
        if (!\class_exists(\ECSPrefix20210507\Symfony\Component\Mime\Part\AbstractPart::class)) {
            throw new \LogicException('You cannot pass non-empty bodies as the Mime component is not installed. Try running "composer require symfony/mime".');
        }
        if ($content = $request->getContent()) {
            return new \ECSPrefix20210507\Symfony\Component\Mime\Part\TextPart($content, 'utf-8', 'plain', '8bit');
        }
        $fields = $request->request->all();
        foreach ($request->files->all() as $name => $file) {
            $fields[$name] = \ECSPrefix20210507\Symfony\Component\Mime\Part\DataPart::fromPath($file->getPathname(), $file->getClientOriginalName(), $file->getClientMimeType());
        }
