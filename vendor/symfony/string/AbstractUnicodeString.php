 ...
 */
namespace ECSPrefix20220607\Symfony\Component\String;

use ECSPrefix20220607\Symfony\Component\String\Exception\ExceptionInterface;
use ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException;
use ECSPrefix20220607\Symfony\Component\String\Exception\RuntimeException;
/**
 ...
 */
abstract class AbstractUnicodeString extends \ECSPrefix20220607\Symfony\Component\String\AbstractString
{
                ...
                if ('nfd' === ($rule = \strtolower($rule))) {
                    ...
                    if (null === ($transliterator = self::$transliterators[$rule] ?? (self::$transliterators[$rule] = \Transliterator::create($rule)))) {
                        ...
                        if (null === $transliterator) {
                            throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Unknown transliteration rule "%s".', $rule));
                        }
    ...
    public function join(array $strings, string $lastGlue = null)
    {
        ...
        if (!\preg_match('//u', $str->string)) {
            throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
        return $str;
    }
    /**
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0) : array
    {
        ...
        \set_error_handler(static function ($t, $m) {
            throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException($m);
        });
        try {
            if (\false === $match($regexp . 'u', $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \substr($k, -6)) {
                        throw new \ECSPrefix20220607\Symfony\Component\String\Exception\RuntimeException('Matching failed with ' . $k . '.');
                    }
                }
                throw new \ECSPrefix20220607\Symfony\Component\String\Exception\RuntimeException('Matching failed with unknown error code.');
            }
        } finally {
    ...
    public function normalize(int $form = self::NFC)
    {
        if (!\in_array($form, [self::NFC, self::NFD, self::NFKC, self::NFKD])) {
            throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException('Unsupported normalization form.');
        }
        $str = clone $this;
        ...
        if (\is_array($to) || $to instanceof \Closure) {
            ...
            $to = static function (array $m) use($to) : string {
                ...
                if ('' !== $to && (!\is_string($to) || !\preg_match('//u', $to))) {
                    throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException('Replace callback must return a valid UTF-8 string.');
                }
                return $to;
            };
    ...
    public function snake()
    {
        ...
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'ECSPrefix20220607\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
    /**
     * @return $this
    ...
    public function trim(string $chars = " \t\n\r\0\v\f ﻿")
    {
        if (" \t\n\r\0\v\f ﻿" !== $chars && !\preg_match('//u', $chars)) {
            throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 chars.');
        }
    ...
    private function pad(int $len, self $pad, int $type)
    {
        ...
        switch ($type) {
            ...
            default:
                throw new \ECSPrefix20220607\Symfony\Component\String\Exception\InvalidArgumentException('Invalid padding type.');
        }
    }
    /**
