 ...
 */
class ByteString extends \ECSPrefix20210514\Symfony\Component\String\AbstractString
{
    ...
    public static function fromRandom($length = 16, $alphabet = null)
    {
        $length = (int) $length;
        if ($length <= 0) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('A strictly positive length is expected, "%d" given.', $length));
        }
        $alphabet = isset($alphabet) ? $alphabet : self::ALPHABET_ALPHANUMERIC;
        $alphabetSize = \strlen($alphabet);
        $bits = (int) \ceil(\log($alphabetSize, 2.0));
        if ($bits <= 0 || $bits > 56) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException('The length of the alphabet must in the [2^1, 2^56] range.');
        }
        $ret = '';
        while ($length > 0) {
    ...
    public function chunk($length = 1)
    {
        $length = (int) $length;
        if (1 > $length) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException('The chunk length must be greater than zero.');
        }
        if ('' === $this->string) {
            return [];
        }
    ...
    public function snake()
    {
        ...
        $str->string = \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z\\d])([A-Z])/'], 'ECSPrefix20210514\\1_\\2', $str->string));
        return $str;
    }
    /**
    ...
    public function split($delimiter, $limit = null, $flags = null)
    {
        ...
        if (1 > ($limit = isset($limit) ? $limit : \PHP_INT_MAX)) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException('Split limit must be a positive integer.');
        }
        if ('' === $delimiter) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException('Split delimiter is empty.');
        }
        if (null !== $flags) {
    ...
    {
        return new \ECSPrefix20210514\Symfony\Component\String\UnicodeString($this->toCodePointString($fromEncoding)->string);
    }
    /**
     * @param string|null $fromEncoding
    ...
    {
        $u = new \ECSPrefix20210514\Symfony\Component\String\CodePointString();
        ...
        try {
            ...
            } catch (\ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException $e) {
        ...
        if (!$validEncoding) {
            throw new \ECSPrefix20210514\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Invalid "%s" string.', isset($fromEncoding) ? $fromEncoding : 'Windows-1252'));
        }
    ...
    public function width($ignoreAnsiDecoration = \true)
    {
        ...
        return (new \ECSPrefix20210514\Symfony\Component\String\CodePointString($string))->width($ignoreAnsiDecoration);
    }
}
