 ...
 */
class ByteString extends \ECSPrefix20210825\Symfony\Component\String\AbstractString
{
    ...
    {
        if ($length <= 0) {
            throw new \ECSPrefix20210825\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('A strictly positive length is expected, "%d" given.', $length));
        }
        $alphabet = $alphabet ?? self::ALPHABET_ALPHANUMERIC;
        $alphabetSize = \strlen($alphabet);
        $bits = (int) \ceil(\log($alphabetSize, 2.0));
        if ($bits <= 0 || $bits > 56) {
            throw new \ECSPrefix20210825\Symfony\Component\String\Exception\InvalidArgumentException('The length of the alphabet must in the [2^1, 2^56] range.');
        }
        $ret = '';
        while ($length > 0) {
    ...
    public function snake()
    {
        ...
        $str->string = \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z\\d])([A-Z])/'], 'ECSPrefix20210825\\1_\\2', $str->string));
        return $str;
    }
    /**
     ...
     */
    public function toUnicodeString($fromEncoding = null) : \ECSPrefix20210825\Symfony\Component\String\UnicodeString
    {
        return new \ECSPrefix20210825\Symfony\Component\String\UnicodeString($this->toCodePointString($fromEncoding)->string);
    }
    /**
     * @param string|null $fromEncoding
     */
    public function toCodePointString($fromEncoding = null) : \ECSPrefix20210825\Symfony\Component\String\CodePointString
    {
        $u = new \ECSPrefix20210825\Symfony\Component\String\CodePointString();
        ...
        try {
            ...
            } catch (\ECSPrefix20210825\Symfony\Component\String\Exception\InvalidArgumentException $e) {
        ...
        if (!$validEncoding) {
            throw new \ECSPrefix20210825\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Invalid "%s" string.', $fromEncoding ?? 'Windows-1252'));
        }
    ...
    public function width($ignoreAnsiDecoration = \true) : int
    {
        ...
        return (new \ECSPrefix20210825\Symfony\Component\String\CodePointString($string))->width($ignoreAnsiDecoration);
    }
}
