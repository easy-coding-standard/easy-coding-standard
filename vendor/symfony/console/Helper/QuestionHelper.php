...
class QuestionHelper extends \ECSPrefix20210507\Symfony\Component\Console\Helper\Helper
{
    ...
    public function ask($input, $output, $question)
    {
        ...
        if ($input instanceof \ECSPrefix20210507\Symfony\Component\Console\Input\StreamableInputInterface && ($stream = $input->getStream())) {
        ...
        } catch (\ECSPrefix20210507\Symfony\Component\Console\Exception\MissingInputException $exception) {
    ...
    private function doAsk($output, $question)
    {
        ...
        if (null === $autocomplete || !self::$stty || !\ECSPrefix20210507\Symfony\Component\Console\Terminal::hasSttyAvailable()) {
            $ret = \false;
            if ($question->isHidden()) {
                try {
                    $hiddenResponse = $this->getHiddenResponse($output, $inputStream, $question->isTrimmable());
                    $ret = $question->isTrimmable() ? \trim($hiddenResponse) : $hiddenResponse;
                } catch (\ECSPrefix20210507\Symfony\Component\Console\Exception\RuntimeException $e) {
            ...
            if (\false === $ret) {
                ...
                if (\false === $ret) {
                    throw new \ECSPrefix20210507\Symfony\Component\Console\Exception\MissingInputException('Aborted.');
                }
        ...
        if ($output instanceof \ECSPrefix20210507\Symfony\Component\Console\Output\ConsoleSectionOutput) {
    ...
    private function getDefaultAnswer($question)
    {
        ...
        } elseif ($question instanceof \ECSPrefix20210507\Symfony\Component\Console\Question\ChoiceQuestion) {
    ...
    protected function writePrompt($output, $question)
    {
        ...
        if ($question instanceof \ECSPrefix20210507\Symfony\Component\Console\Question\ChoiceQuestion) {
    ...
    {
        $cursor = new \ECSPrefix20210507\Symfony\Component\Console\Cursor($output, $inputStream);
        $fullChoice = '';
        $ret = '';
        $i = 0;
        $ofs = -1;
        ...
        $output->getFormatter()->setStyle('hl', new \ECSPrefix20210507\Symfony\Component\Console\Formatter\OutputFormatterStyle('black', 'white'));
        // Read a keypress
        while (!\feof($inputStream)) {
            ...
            if (\false === $c || '' === $ret && '' === $c && null === $question->getDefault()) {
                ...
                if (0 === $numMatches && 0 !== $i) {
                    --$i;
                    $cursor->moveLeft(\ECSPrefix20210507\Symfony\Component\String\s($fullChoice)->slice(-1)->width(\false));
                ...
                ++$i;
                $tempRet = $ret;
                if ($question instanceof \ECSPrefix20210507\Symfony\Component\Console\Question\ChoiceQuestion && $question->isMultiselect()) {
            ...
            if ($numMatches > 0 && -1 !== $ofs) {
                ...
                $output->write('<hl>' . \ECSPrefix20210507\Symfony\Component\Console\Formatter\OutputFormatter::escapeTrailingBackslash(\substr($matches[$ofs], $charactersEntered)) . '</hl>');
    ...
    private function getHiddenResponse($output, $inputStream, $trimmable = \true)
    {
        ...
        if (self::$stty && \ECSPrefix20210507\Symfony\Component\Console\Terminal::hasSttyAvailable()) {
            $sttyMode = \shell_exec('stty -g');
            \shell_exec('stty -echo');
        } elseif ($this->isInteractiveInput($inputStream)) {
            throw new \ECSPrefix20210507\Symfony\Component\Console\Exception\RuntimeException('Unable to hide the response.');
        }
