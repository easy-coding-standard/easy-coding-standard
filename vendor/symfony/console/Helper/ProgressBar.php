...
final class ProgressBar
{
     ...
     */
    public function __construct(\ECSPrefix20210710\Symfony\Component\Console\Output\OutputInterface $output, int $max = 0, float $minSecondsBetweenRedraws = 1 / 25)
    {
        ...
        $this->startTime = \time();
        $this->cursor = new \ECSPrefix20210710\Symfony\Component\Console\Cursor($output);
    }
    /**
    ...
    public function setMaxSteps(int $max)
    {
        $this->format = null;
        $this->max = \max(0, $max);
        $this->stepWidth = $this->max ? \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::width((string) $this->max) : 4;
    }
    /**
            ...
            if (null !== $this->previousMessage) {
                if ($this->output instanceof \ECSPrefix20210710\Symfony\Component\Console\Output\ConsoleSectionOutput) {
                    ...
                    foreach ($messageLines as $messageLine) {
                        $messageLineLength = \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::width(\ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), $messageLine));
    ...
    {
        return ['bar' => function (self $bar, \ECSPrefix20210710\Symfony\Component\Console\Output\OutputInterface $output) {
            $completeBars = $bar->getBarOffset();
            $display = \str_repeat($bar->getBarCharacter(), $completeBars);
            if ($completeBars < $bar->getBarWidth()) {
                $emptyBars = $bar->getBarWidth() - $completeBars - \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::length(\ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::removeDecoration($output->getFormatter(), $bar->getProgressCharacter()));
                $display .= $bar->getProgressCharacter() . \str_repeat($bar->getEmptyBarCharacter(), $emptyBars);
            }
            return $display;
        }, 'elapsed' => function (self $bar) {
            return \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::formatTime(\time() - $bar->getStartTime());
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \ECSPrefix20210710\Symfony\Component\Console\Exception\LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            return \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::formatTime($bar->getRemaining());
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \ECSPrefix20210710\Symfony\Component\Console\Exception\LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            return \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::formatTime($bar->getEstimated());
    ...
    private function buildLine() : string
    {
        ...
        $linesLength = \array_map(function ($subLine) {
            return \ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::width(\ECSPrefix20210710\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), \rtrim($subLine, "\r")));
        }, \explode("\n", $line));
