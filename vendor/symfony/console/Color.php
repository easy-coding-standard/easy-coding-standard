...
final class Color
{
    ...
    public function __construct($foreground = '', $background = '', array $options = [])
    {
        ...
        foreach ($options as $option) {
            if (!isset(self::AVAILABLE_OPTIONS[$option])) {
                throw new \ECSPrefix20210516\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid option specified: "%s". Expected one of (%s).', $option, \implode(', ', \array_keys(self::AVAILABLE_OPTIONS))));
            }
    ...
    private function parseColor($color)
    {
        ...
        if ('#' === $color[0]) {
            ...
            if (6 !== \strlen($color)) {
                throw new \ECSPrefix20210516\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid "%s" color.', $color));
            }
            return $this->convertHexColorToAnsi(\hexdec($color));
        }
        if (!isset(self::COLORS[$color])) {
            throw new \ECSPrefix20210516\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid "%s" color; expected one of (%s).', $color, \implode(', ', \array_keys(self::COLORS))));
        }
