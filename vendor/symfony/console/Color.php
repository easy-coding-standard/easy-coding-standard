...
final class Color
{
    ...
    public function __construct(string $foreground = '', string $background = '', array $options = [])
    {
        ...
        foreach ($options as $option) {
            if (!isset(self::AVAILABLE_OPTIONS[$option])) {
                throw new \ECSPrefix20210927\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid option specified: "%s". Expected one of (%s).', $option, \implode(', ', \array_keys(self::AVAILABLE_OPTIONS))));
            }
    ...
    private function parseColor(string $color, bool $background = \false) : string
    {
        ...
        if ('#' === $color[0]) {
            ...
            if (6 !== \strlen($color)) {
                throw new \ECSPrefix20210927\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid "%s" color.', $color));
            }
        ...
        throw new \ECSPrefix20210927\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Invalid "%s" color; expected one of (%s).', $color, \implode(', ', \array_merge(\array_keys(self::COLORS), \array_keys(self::BRIGHT_COLORS)))));
    }
