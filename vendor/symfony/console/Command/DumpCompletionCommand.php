 ...
 */
final class DumpCompletionCommand extends \ECSPrefix20211202\Symfony\Component\Console\Command\Command
{
...
EOH
)->addArgument('shell', \ECSPrefix20211202\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'The shell type (e.g. "bash"), the value of the "$SHELL" env var will be used if this is not given')->addOption('debug', null, \ECSPrefix20211202\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Tail the completion debug log');
    }
    /**
    ...
    protected function execute($input, $output) : int
    {
        ...
        if (!\file_exists($completionFile)) {
            ...
            ($output instanceof \ECSPrefix20211202\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : $output)->writeln(\sprintf('<error>Detected shell "%s", which is not supported by Symfony shell completion (supported shells: "%s").</>', $shell, \implode('", "', $supportedShells)));
            return self::INVALID;
        }
    ...
    private function tailDebugLog(string $commandName, \ECSPrefix20211202\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
        $debugFile = \sys_get_temp_dir() . '/sf_' . $commandName . '.log';
        if (!\file_exists($debugFile)) {
            \touch($debugFile);
        }
        $process = new \ECSPrefix20211202\Symfony\Component\Process\Process(['tail', '-f', $debugFile], null, null, null, 0);
