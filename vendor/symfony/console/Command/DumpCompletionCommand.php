 ...
 */
final class DumpCompletionCommand extends \ECSPrefix20211210\Symfony\Component\Console\Command\Command
{
...
EOH
    ...
    protected function execute(\ECSPrefix20211210\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20211210\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        if (!\file_exists($completionFile)) {
            ...
            ($output instanceof \ECSPrefix20211210\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : $output)->writeln(\sprintf('<error>Detected shell "%s", which is not supported by Symfony shell completion (supported shells: "%s").</>', $shell, \implode('", "', $supportedShells)));
            return self::INVALID;
        }
    ...
    private function tailDebugLog(string $commandName, \ECSPrefix20211210\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
        $debugFile = \sys_get_temp_dir() . '/sf_' . $commandName . '.log';
        if (!\file_exists($debugFile)) {
            \touch($debugFile);
        }
        $process = new \ECSPrefix20211210\Symfony\Component\Process\Process(['tail', '-f', $debugFile], null, null, null, 0);
