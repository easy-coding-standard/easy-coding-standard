 ...
 */
final class CompleteCommand extends \ECSPrefix20220607\Symfony\Component\Console\Command\Command
{
    ...
    public function __construct(array $completionOutputs = [])
    {
        ...
        $this->completionOutputs = $completionOutputs + ['bash' => \ECSPrefix20220607\Symfony\Component\Console\Completion\Output\BashCompletionOutput::class];
        parent::__construct();
    }
    protected function configure() : void
    {
        $this->addOption('shell', 's', \ECSPrefix20220607\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The shell type ("' . \implode('", "', \array_keys($this->completionOutputs)) . '")')->addOption('input', 'i', \ECSPrefix20220607\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED | \ECSPrefix20220607\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY, 'An array of input tokens (e.g. COMP_WORDS or argv)')->addOption('current', 'c', \ECSPrefix20220607\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The index of the "input" array that the cursor is in (e.g. COMP_CWORD)')->addOption('symfony', 'S', \ECSPrefix20220607\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The version of the completion script');
    }
    ...
    protected function execute(\ECSPrefix20220607\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20220607\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        try {
            ...
            $suggestions = new \ECSPrefix20220607\Symfony\Component\Console\Completion\CompletionSuggestions();
            ...
            if (null === $command) {
                ...
                if (\ECSPrefix20220607\Symfony\Component\Console\Completion\CompletionInput::TYPE_OPTION_NAME === $completionInput->getCompletionType()) {
                    $this->log('  Completing option names for the <comment>' . \get_class($command instanceof \ECSPrefix20220607\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> command.');
                    $suggestions->suggestOptions($command->getDefinition()->getOptions());
                } else {
                    $this->log(['  Completing using the <comment>' . \get_class($command instanceof \ECSPrefix20220607\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> class.', '  Completing <comment>' . $completionInput->getCompletionType() . '</> for <comment>' . $completionInput->getCompletionName() . '</>']);
    ...
    private function createCompletionInput(\ECSPrefix20220607\Symfony\Component\Console\Input\InputInterface $input) : \ECSPrefix20220607\Symfony\Component\Console\Completion\CompletionInput
    {
        $currentIndex = $input->getOption('current');
        if (!$currentIndex || !\ctype_digit($currentIndex)) {
            throw new \RuntimeException('The "--current" option must be set and it must be an integer.');
        }
        $completionInput = \ECSPrefix20220607\Symfony\Component\Console\Completion\CompletionInput::fromTokens($input->getOption('input'), (int) $currentIndex);
        try {
    ...
    private function findCommand(\ECSPrefix20220607\Symfony\Component\Console\Completion\CompletionInput $completionInput, \ECSPrefix20220607\Symfony\Component\Console\Output\OutputInterface $output) : ?\ECSPrefix20220607\Symfony\Component\Console\Command\Command
    {
        try {
        ...
        } catch (\ECSPrefix20220607\Symfony\Component\Console\Exception\CommandNotFoundException $e) {
        }
        return null;
    }
