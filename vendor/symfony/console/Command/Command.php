...
class Command
{
    ...
    {
        throw new \ECSPrefix20210507\Symfony\Component\Console\Exception\LogicException('You must override the execute() method in the concrete command class.');
    }
    /**
        ...
        if (null !== $this->processTitle) {
            if (\function_exists('cli_set_process_title')) {
                if (!@\cli_set_process_title($this->processTitle)) {
                    if ('Darwin' === \PHP_OS) {
                        $output->writeln('<comment>Running "cli_set_process_title" as an unprivileged user is not supported on MacOS.</comment>', \ECSPrefix20210507\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE);
                    } else {
            ...
            } elseif (\ECSPrefix20210507\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE === $output->getVerbosity()) {
    ...
    public function mergeApplicationDefinition($mergeArgs = \true)
    {
        ...
        $this->fullDefinition = new \ECSPrefix20210507\Symfony\Component\Console\Input\InputDefinition();
    ...
    {
        if ($definition instanceof \ECSPrefix20210507\Symfony\Component\Console\Input\InputDefinition) {
    ...
    public function getNativeDefinition()
    {
        if (null === $this->definition) {
            throw new \ECSPrefix20210507\Symfony\Component\Console\Exception\LogicException(\sprintf('Command class "%s" is not correctly initialized. You probably forgot to call the parent constructor.', static::class));
        }
    ...
    {
        $this->definition->addArgument(new \ECSPrefix20210507\Symfony\Component\Console\Input\InputArgument($name, $mode, $description, $default));
        if (null !== $this->fullDefinition) {
            $this->fullDefinition->addArgument(new \ECSPrefix20210507\Symfony\Component\Console\Input\InputArgument($name, $mode, $description, $default));
        }
        return $this;
    }
    /**
    ...
    {
        $this->definition->addOption(new \ECSPrefix20210507\Symfony\Component\Console\Input\InputOption($name, $shortcut, $mode, $description, $default));
        if (null !== $this->fullDefinition) {
            $this->fullDefinition->addOption(new \ECSPrefix20210507\Symfony\Component\Console\Input\InputOption($name, $shortcut, $mode, $description, $default));
        }
        return $this;
    }
    /**
    ...
    {
        if (!\preg_match('/^[^\\:]++(\\:[^\\:]++)*$/', $name)) {
            throw new \ECSPrefix20210507\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Command name "%s" is invalid.', $name));
        }
    }
}
