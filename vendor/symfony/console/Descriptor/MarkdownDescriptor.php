 ...
 * @internal
 */
class MarkdownDescriptor extends \ECSPrefix20210511\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function describeCommand(\ECSPrefix20210511\Symfony\Component\Console\Command\Command $command, array $options = [])
    {
        ...
        $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \ECSPrefix20210511\Symfony\Component\Console\Helper\Helper::strlen($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce(\array_merge([$command->getSynopsis()], $command->getAliases(), $command->getUsages()), function ($carry, $usage) {
            return $carry . '* `' . $usage . '`' . "\n";
        }));
    ...
    protected function describeApplication(\ECSPrefix20210511\Symfony\Component\Console\Application $application, array $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \ECSPrefix20210511\Symfony\Component\Console\Helper\Helper::strlen($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\ECSPrefix20210511\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
     ...
     * @return string
     */
    private function getApplicationTitle(\ECSPrefix20210511\Symfony\Component\Console\Application $application)
    {
