 ...
 * @internal
 */
class MarkdownDescriptor extends \ECSPrefix20210823\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     * @param object $object
    ...
    protected function describeCommand($command, $options = [])
    {
        if ($options['short'] ?? \false) {
            $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \ECSPrefix20210823\Symfony\Component\Console\Helper\Helper::width($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce($command->getAliases(), function ($carry, $usage) {
                return $carry . '* `' . $usage . '`' . "\n";
            }));
            return;
        }
    ...
    protected function describeApplication($application, $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \ECSPrefix20210823\Symfony\Component\Console\Helper\Helper::width($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\ECSPrefix20210823\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
    ...
    private function getApplicationTitle(\ECSPrefix20210823\Symfony\Component\Console\Application $application) : string
    {
