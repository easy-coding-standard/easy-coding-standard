 ...
 * @internal
 */
class MarkdownDescriptor extends \ConfigTransformer20210601\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function describeCommand(\ConfigTransformer20210601\Symfony\Component\Console\Command\Command $command, array $options = [])
    {
        if ($options['short'] ?? \false) {
            $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \ConfigTransformer20210601\Symfony\Component\Console\Helper\Helper::width($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce($command->getAliases(), function ($carry, $usage) {
                return $carry . '* `' . $usage . '`' . "\n";
            }));
            return;
        }
    ...
    protected function describeApplication(\ConfigTransformer20210601\Symfony\Component\Console\Application $application, array $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \ConfigTransformer20210601\Symfony\Component\Console\Helper\Helper::width($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\ConfigTransformer20210601\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
    ...
    private function getApplicationTitle(\ConfigTransformer20210601\Symfony\Component\Console\Application $application) : string
    {
