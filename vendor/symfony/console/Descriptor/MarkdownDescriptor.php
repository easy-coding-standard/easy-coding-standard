 ...
 * @internal
 */
class MarkdownDescriptor extends \ECSPrefix20220418\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     * @param object $object
    ...
    protected function describeCommand(\ECSPrefix20220418\Symfony\Component\Console\Command\Command $command, array $options = [])
    {
        if ($options['short'] ?? \false) {
            $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \ECSPrefix20220418\Symfony\Component\Console\Helper\Helper::width($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce($command->getAliases(), function ($carry, $usage) {
                return $carry . '* `' . $usage . '`' . "\n";
            }));
            return;
        }
    ...
    protected function describeApplication(\ECSPrefix20220418\Symfony\Component\Console\Application $application, array $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \ECSPrefix20220418\Symfony\Component\Console\Helper\Helper::width($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\ECSPrefix20220418\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
    ...
    private function getApplicationTitle(\ECSPrefix20220418\Symfony\Component\Console\Application $application) : string
    {
