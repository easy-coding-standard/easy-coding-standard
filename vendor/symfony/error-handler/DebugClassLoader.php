...
use ECSPrefix20210823\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use ECSPrefix20210823\Doctrine\Persistence\Proxy;
use ECSPrefix20210823\Mockery\MockInterface;
use ECSPrefix20210823\Phake\IMock;
use ECSPrefix20210823\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use ECSPrefix20210823\PHPUnit\Framework\MockObject\MockObject;
use ECSPrefix20210823\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\ECSPrefix20210823\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\ECSPrefix20210823\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \ECSPrefix20210823\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\Mockery\MockInterface::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210823\Phake\IMock::class)) {
    ...
    public function checkAnnotations($refl, $class) : array
    {
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && 0 === \strpos($className, 'Symfony\\') && (!\class_exists(\ECSPrefix20210823\Composer\InstalledVersions::class) || 'symfony/symfony' !== \ECSPrefix20210823\Composer\InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \ECSPrefix20210823\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
