...
use ECSPrefix202305\Composer\InstalledVersions;
use ECSPrefix202305\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use ECSPrefix202305\Doctrine\Persistence\Proxy;
use ECSPrefix202305\Mockery\MockInterface;
use ECSPrefix202305\Phake\IMock;
use ECSPrefix202305\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use ECSPrefix202305\PHPUnit\Framework\MockObject\MockObject;
use ECSPrefix202305\Prophecy\Prophecy\ProphecySubjectInterface;
use ECSPrefix202305\ProxyManager\Proxy\ProxyInterface;
use ECSPrefix202305\Symfony\Component\ErrorHandler\Internal\TentativeTypes;
use ECSPrefix202305\Symfony\Component\HttpClient\HttplugClient;
use ECSPrefix202305\Symfony\Component\VarExporter\LazyObjectInterface;
/**
...
class DebugClassLoader
{
     ...
     * @var mixed[]
     */
    private static $finalProperties = [];
    /**
     * @var mixed[]
     */
    ...
    public function __construct(callable $classLoader)
    {
        ...
        if (!isset(self::$caseCheck)) {
            ...
            if (\false === $test || \false === $i) {
                ...
                self::$caseCheck = 0;
            } elseif (\substr_compare($test, $file, -\strlen($file)) === 0) {
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\ECSPrefix202305\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\ECSPrefix202305\Psr\Log\LogLevel::class);
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        $deprecations = [];
        $className = \strpos($class, "@anonymous\x00") !== \false ? ((\get_parent_class($class) ?: \key(\class_implements($class))) ?: 'class') . '@anonymous' : $class;
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$deprecated[$use]) && \strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && !isset(self::$deprecated[$class]) && !(HttplugClient::class === $class && \in_array($use, [\ECSPrefix202305\Http\Message\RequestFactory::class, \ECSPrefix202305\Http\Message\StreamFactory::class, \ECSPrefix202305\Http\Message\UriFactory::class], \true))) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && \strncmp($className, 'Symfony\\', \strlen('Symfony\\')) === 0 && (!\class_exists(InstalledVersions::class) || 'symfony/symfony' !== InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($parentAndOwnInterfaces as $use) {
            ...
            if (null !== (TentativeTypes::RETURN_TYPES[$use] ?? null)) {
                foreach (TentativeTypes::RETURN_TYPES[$use] as $method => $returnType) {
                    ...
                    self::$returnTypes[$class] += [$method => [$returnType, \strncmp($returnType, '?', \strlen('?')) === 0 ? \substr($returnType, 1) . '|null' : $returnType, $use, '']];
                }
            }
        }
        foreach ($refl->getMethods() as $method) {
            ...
            if ($canAddReturnType = null !== $forcePatchTypes && \strpos($method->getFileName(), \DIRECTORY_SEPARATOR . 'vendor' . \DIRECTORY_SEPARATOR) === \false) {
        ...
        foreach ($finals as $type => $reflectors) {
            foreach ($reflectors as $r) {
                ...
                if (isset($doc['final']) || 'finalProperties' === $type && \strncmp($class, 'Symfony\\', \strlen('Symfony\\')) === 0 && !$r->hasType()) {
    ...
    private function darwinRealpath(string $real) : string
    {
        ...
        if (!isset($dirFiles[$file]) && \substr_compare($file, ') : eval()\'d code', -\strlen(') : eval()\'d code')) === 0) {
    ...
    private function setReturnType(string $types, string $class, string $method, string $filename, ?string $parent, \ReflectionType $returnType = null) : void
    {
        ...
        if ($nullable = \strncmp($types, 'null|', \strlen('null|')) === 0) {
            $types = \substr($types, 5);
        } elseif ($nullable = \substr_compare($types, '|null', -\strlen('|null')) === 0) {
            $types = \substr($types, 0, -5);
        }
        $arrayType = ['array' => 'array'];
        $typesMap = [];
        $glue = \strpos($types, '&') !== \false ? '&' : '|';
        ...
        foreach ($typesMap as $n => $t) {
            if ('null' !== $n) {
                $iterable = $iterable && (\in_array($n, ['array', 'iterable']) || \strpos($n, 'Iterator') !== \false);
    ...
    {
        if (\count(\method_exists($method, 'getAttributes') ? $method->getAttributes(\ReturnTypeWillChange::class) : [])) {
            return;
        }
    ...
    private function patchMethod(\ReflectionMethod $method, string $returnType, string $declaringFile, string $normalizedType)
    {
        ...
        if ($nullable = \substr_compare($returnType, '|null', -\strlen('|null')) === 0) {
            $returnType = \substr($returnType, 0, -5);
        }
        $glue = \strpos($returnType, '&') !== \false ? '&' : '|';
        ...
        if ('docblock' === $this->patchTypes['force'] || 'object' === $normalizedType && '7.1' === $this->patchTypes['php']) {
            ...
            if (\strpos($code[$startLine], '#[') !== \false) {
                --$startLine;
            }
...
EOTXT;
    ...
    private static function getUseStatements(string $file) : array
    {
        ...
        for ($i = 0; $i < \count($file); ++$i) {
            ...
            if (\strncmp($file[$i], 'namespace ', \strlen('namespace ')) === 0) {
                $namespace = \substr($file[$i], \strlen('namespace '), -2) . '\\';
                $useOffset = $i + 2;
            }
            if (\strncmp($file[$i], 'use ', \strlen('use ')) === 0) {
                $useOffset = $i;
                for (; \strncmp($file[$i], 'use ', \strlen('use ')) === 0; ++$i) {
    ...
    private function fixReturnStatements(\ReflectionMethod $method, string $returnType)
    {
        if ('docblock' !== $this->patchTypes['force']) {
            ...
            if ('8.0' > $this->patchTypes['php'] && (\strpos($returnType, '|') !== \false || \in_array($returnType, ['mixed', 'static'], \true))) {
                return;
            }
            if ('8.1' > $this->patchTypes['php'] && \strpos($returnType, '&') !== \false) {
                return;
            }
        }
        if (!\is_file($file = $method->getFileName())) {
