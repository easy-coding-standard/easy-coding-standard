...
use ECSPrefix20210509\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use ECSPrefix20210509\Doctrine\Persistence\Proxy;
use ECSPrefix20210509\Mockery\MockInterface;
use ECSPrefix20210509\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use ECSPrefix20210509\PHPUnit\Framework\MockObject\MockObject;
use ECSPrefix20210509\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable()
    {
        ...
        \class_exists(\ECSPrefix20210509\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\ECSPrefix20210509\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses()
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \ECSPrefix20210509\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210509\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210509\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210509\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210509\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ECSPrefix20210509\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, $class)
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \ECSPrefix20210509\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
