...
class SerializerErrorRenderer implements \ECSPrefix20210507\Symfony\Component\ErrorHandler\ErrorRenderer\ErrorRendererInterface
{
    ...
    public function render($exception)
    {
        ...
        $flattenException = \ECSPrefix20210507\Symfony\Component\ErrorHandler\Exception\FlattenException::createFromThrowable($exception, null, $headers);
        try {
            $format = \is_string($this->format) ? $this->format : ($this->format)($flattenException);
            $headers = ['Content-Type' => isset(\ECSPrefix20210507\Symfony\Component\HttpFoundation\Request::getMimeTypes($format)[0]) ? \ECSPrefix20210507\Symfony\Component\HttpFoundation\Request::getMimeTypes($format)[0] : $format, 'Vary' => 'Accept'];
        ...
        } catch (\ECSPrefix20210507\Symfony\Component\Serializer\Exception\NotEncodableValueException $e) {
    ...
    public static function getPreferredFormat($requestStack)
    {
        return static function () use($requestStack) {
            if (!($request = $requestStack->getCurrentRequest())) {
                throw new \ECSPrefix20210507\Symfony\Component\Serializer\Exception\NotEncodableValueException();
            }
