...
class HtmlErrorRenderer implements ErrorRendererInterface
{
    ...
    private $charset;
    /**
     * @var string|mixed[]|\Symfony\Component\HttpKernel\Debug\FileLinkFormatter|false
     */
    ...
    public function __construct($debug = \false, string $charset = null, $fileLinkFormat = null, string $projectDir = null, $outputBuffer = '', LoggerInterface $logger = null)
    {
        ...
        $fileLinkFormat = $fileLinkFormat ?? $_ENV['SYMFONY_IDE'] ?? $_SERVER['SYMFONY_IDE'] ?? null;
        $this->fileLinkFormat = \is_string($fileLinkFormat) ? ErrorRendererInterface::IDE_LINK_FORMATS[$fileLinkFormat] ?? $fileLinkFormat ?: \false : ((($fileLinkFormat ?: \ini_get('xdebug.file_link_format')) ?: \get_cfg_var('xdebug.file_link_format')) ?: \false);
        $this->projectDir = $projectDir;
        $this->outputBuffer = \is_string($outputBuffer) ? $outputBuffer : \Closure::fromCallable($outputBuffer);
        $this->logger = $logger;
    }
    ...
    private function getFileRelative(string $file) : ?string
    {
        ...
        if (null !== $this->projectDir && \strncmp($file, $this->projectDir, \strlen($this->projectDir)) === 0) {
