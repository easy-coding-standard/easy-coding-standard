<div class="trace trace-as-html" id="trace-box-<?= $index; ?>">
    <div class="trace-details">
            ...
            <div class="sf-toggle" data-toggle-selector="#trace-html-<?= $index; ?>" data-toggle-initial="<?= $expand ? 'display' : ''; ?>">
                ...
                <?php } else { ?>
                    <h3 class="trace-class">
                        <?php if ('' !== $namespace) { ?>
                            <span class="trace-namespace"><?php <?= $namespace; ?></span>
                        <?php } ?>
                        <?php <?= $class; ?>
                    </h3>
                <?php } ?>
                <?php if ($exception['message'] && $index > 1) { ?>
                    <p class="break-long-words trace-message"><?php <?= $this->escape($exception['message']); ?></p>
                <?php } ?>
            </div>
        </div>
        ...
        foreach ($exception['trace'] as $i => $trace) {
            ...
            <div class="trace-line <?php <?= $isVendorTrace ? 'trace-from-vendor' : ''; ?>">
                <?php <?= $this->include('views/trace.html.php', [
                    'prefix' => $index,
                    'i' => $i,
                    'trace' => $trace,
