...
class PdoAdapter extends \ECSPrefix20210507\Symfony\Component\Cache\Adapter\AbstractAdapter implements \ECSPrefix20210507\Symfony\Component\Cache\PruneableInterface
{
    ...
    public function __construct($connOrDsn, $namespace = '', $defaultLifetime = 0, array $options = [], $marshaller = null)
    {
        if (isset($namespace[0]) && \preg_match('#[^-+.A-Za-z0-9]#', $namespace, $match)) {
            throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Namespace contains "%s" but only characters in [-+.A-Za-z0-9] are allowed.', $match[0]));
        }
        if ($connOrDsn instanceof \PDO) {
            if (\PDO::ERRMODE_EXCEPTION !== $connOrDsn->getAttribute(\PDO::ATTR_ERRMODE)) {
                throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO error mode attribute be set to throw Exceptions (i.e. $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)).', __CLASS__));
            }
            $this->conn = $connOrDsn;
        } elseif ($connOrDsn instanceof \ECSPrefix20210507\Doctrine\DBAL\Connection) {
            $this->conn = $connOrDsn;
        } elseif (\is_string($connOrDsn)) {
            $this->dsn = $connOrDsn;
        } else {
            throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO or Doctrine\\DBAL\\Connection instance or DSN string as first argument, "%s" given.', __CLASS__, \get_debug_type($connOrDsn)));
        }
    ...
    public function createTable()
    {
        ...
        if ($conn instanceof \ECSPrefix20210507\Doctrine\DBAL\Connection) {
            $schema = new \ECSPrefix20210507\Doctrine\DBAL\Schema\Schema();
    ...
    public function prune()
    {
        ...
        } catch (\ECSPrefix20210507\Doctrine\DBAL\Exception\TableNotFoundException $e) {
            return \true;
    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        foreach ($values as $id => $data) {
            ...
            if (null === $driver && !(\is_object($result) ? $result->rowCount() : $stmt->rowCount())) {
                try {
                    $insertStmt->execute();
                } catch (\ECSPrefix20210507\Doctrine\DBAL\DBALException $e) {
                } catch (\ECSPrefix20210507\Doctrine\DBAL\Exception $e) {
                } catch (\PDOException $e) {
    ...
    private function getConnection()
    {
        if (null === $this->conn) {
            if (\strpos($this->dsn, '://')) {
                if (!\class_exists(\ECSPrefix20210507\Doctrine\DBAL\DriverManager::class)) {
                    throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Failed to parse the DSN "%s". Try running "composer require doctrine/dbal".', $this->dsn));
                }
                $this->conn = \ECSPrefix20210507\Doctrine\DBAL\DriverManager::getConnection(['url' => $this->dsn]);
            } else {
    ...
    private function getServerVersion()
    {
        if (null === $this->serverVersion) {
            ...
            } elseif ($conn instanceof \ECSPrefix20210507\Doctrine\DBAL\Driver\ServerInfoAwareConnection) {
