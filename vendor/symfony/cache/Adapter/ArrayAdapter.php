 ...
 */
class ArrayAdapter implements \ConfigTransformer20210601\Symfony\Component\Cache\Adapter\AdapterInterface, \ConfigTransformer20210601\Symfony\Contracts\Cache\CacheInterface, \ConfigTransformer20210601\Psr\Log\LoggerAwareInterface, \ConfigTransformer20210601\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        if (0 > $maxLifetime) {
            throw new \ConfigTransformer20210601\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxLifetime must be positive, %F passed.', $maxLifetime));
        }
        if (0 > $maxItems) {
            throw new \ConfigTransformer20210601\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxItems must be a positive integer, %d passed.', $maxItems));
        }
    ...
    private function unfreeze(string $key, bool &$isHit)
    {
        ...
        if (\is_string($value) && isset($value[2]) && ':' === $value[1]) {
            try {
                ...
                \ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to unserialize key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
                $value = \false;
            }
            if (\false === $value) {
                $value = null;
    ...
    private function validateKeys(array $keys) : bool
    {
        foreach ($keys as $key) {
            if (!\is_string($key) || !isset($this->expiries[$key])) {
                \ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::validateKey($key);
            }
        }
        return \true;
    }
