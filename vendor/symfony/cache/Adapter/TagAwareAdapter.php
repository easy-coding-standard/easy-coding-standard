 ...
 */
class TagAwareAdapter implements \ConfigTransformer20210601\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \ConfigTransformer20210601\Symfony\Contracts\Cache\TagAwareCacheInterface, \ConfigTransformer20210601\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer20210601\Symfony\Component\Cache\ResettableInterface, \ConfigTransformer20210601\Psr\Log\LoggerAwareInterface
{
    ...
    public function __construct(\ConfigTransformer20210601\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \ConfigTransformer20210601\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?: $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        self::$createCacheItem ?? (self::$createCacheItem = \Closure::bind(static function ($key, $value, \ConfigTransformer20210601\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        self::$setCacheItemTags ?? (self::$setCacheItemTags = \Closure::bind(static function (\ConfigTransformer20210601\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        self::$getTagsByKey ?? (self::$getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::class));
        self::$invalidateTags ?? (self::$invalidateTags = \Closure::bind(static function (\ConfigTransformer20210601\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags)
    {
        ...
        $invalidatedTags = [];
        foreach ($tags as $tag) {
            \assert('' !== \ConfigTransformer20210601\Symfony\Component\Cache\CacheItem::validateKey($tag));
    ...
    public function getItems(array $keys = [])
    {
        ...
        } catch (\ConfigTransformer20210601\Psr\Cache\InvalidArgumentException $e) {
