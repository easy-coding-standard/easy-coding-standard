...
class RedisTagAwareAdapter extends \ECSPrefix20210507\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    ...
    {
        if ($redisClient instanceof \ECSPrefix20210507\Predis\ClientInterface && $redisClient->getConnection() instanceof \ECSPrefix20210507\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \ECSPrefix20210507\Predis\Connection\Aggregate\PredisCluster) {
            throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \ECSPrefix20210507\Predis\Connection\Aggregate\PredisCluster::class, \get_debug_type($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \ECSPrefix20210507\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \ECSPrefix20210507\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
    ...
    protected function doSave(array $values, $lifetime, $addTagData = [], $delTagData = [])
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            throw new \ECSPrefix20210507\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
...
EOLUA;
    ...
    {
        if (!$this->redis instanceof \ECSPrefix20210507\Predis\ClientInterface || !$this->redis->getConnection() instanceof \ECSPrefix20210507\Predis\Connection\Aggregate\PredisCluster) {
