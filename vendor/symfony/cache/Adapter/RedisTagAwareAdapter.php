...
use ConfigTransformer20210601\Predis\Connection\Aggregate\PredisCluster;
...
use ConfigTransformer20210601\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ConfigTransformer20210601\Symfony\Component\Cache\Traits\RedisTrait;
/**
 ...
 */
class RedisTagAwareAdapter extends \ConfigTransformer20210601\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
     ...
     */
    public function __construct($redisClient, string $namespace = '', int $defaultLifetime = 0, \ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if ($redisClient instanceof \ConfigTransformer20210601\Predis\ClientInterface && $redisClient->getConnection() instanceof \ConfigTransformer20210601\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \ConfigTransformer20210601\Predis\Connection\Aggregate\PredisCluster) {
            throw new \ConfigTransformer20210601\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \ConfigTransformer20210601\Predis\Connection\Aggregate\PredisCluster::class, \get_debug_type($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \ConfigTransformer20210601\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \ConfigTransformer20210601\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
    ...
    protected function doSave(array $values, int $lifetime, $addTagData = [], $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            throw new \ConfigTransformer20210601\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
...
EOLUA;
    ...
    {
        if (!$this->redis instanceof \ConfigTransformer20210601\Predis\ClientInterface || !$this->redis->getConnection() instanceof \ConfigTransformer20210601\Predis\Connection\Aggregate\PredisCluster) {
