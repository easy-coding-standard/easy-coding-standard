 ...
 */
class TagAwareMarshaller implements \ECSPrefix20210508\Symfony\Component\Cache\Marshaller\MarshallerInterface
{
    private $marshaller;
    public function __construct(\ECSPrefix20210508\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        ...
        foreach ($values as $id => $value) {
            if (\is_array($value) && \is_array(isset($value['tags']) ? $value['tags'] : null) && \array_key_exists('value', $value) && \count($value) === 2 + (\is_string(isset($value['meta']) ? $value['meta'] : null) && 8 === \strlen($value['meta']))) {
                ...
                if ($f) {
                    ...
                    $serialized[$id] = "" . (isset($value['meta']) ? $value['meta'] : "\0\0\0\0\0\0\0\0") . \pack('N', \strlen($v['tags'])) . $v['tags'] . $v['value'];
                    $serialized[$id][9] = "_";
                }
            } else {
