    ...
    private function init($namespace, $directory)
    {
        ...
        if (isset($namespace[0])) {
            if (\preg_match('#[^-+_.A-Za-z0-9]#', $namespace, $match)) {
                throw new \ECSPrefix20210509\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Namespace contains "%s" but only characters in [-+_.A-Za-z0-9] are allowed.', $match[0]));
            }
        ...
        if ('\\' === \DIRECTORY_SEPARATOR && \strlen($directory) > 234) {
            throw new \ECSPrefix20210509\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache directory too long (%s).', $directory));
        }
