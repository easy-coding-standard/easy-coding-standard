...
use ECSPrefix20210508\Predis\Command\Redis\UNLINK;
use ECSPrefix20210508\Predis\Connection\Aggregate\ClusterInterface;
use ECSPrefix20210508\Predis\Connection\Aggregate\RedisCluster;
...
trait RedisTrait
{
    ...
    private function init($redisClient, $namespace, $defaultLifetime, $marshaller)
    {
        ...
        if (!$redisClient instanceof \Redis && !$redisClient instanceof \RedisArray && !$redisClient instanceof \RedisCluster && !$redisClient instanceof \ECSPrefix20210508\Predis\ClientInterface && !$redisClient instanceof \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisProxy && !$redisClient instanceof \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisClusterProxy) {
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s()" expects parameter 1 to be Redis, RedisArray, RedisCluster or Predis\\ClientInterface, "%s" given.', __METHOD__, \get_debug_type($redisClient)));
        }
        if ($redisClient instanceof \ECSPrefix20210508\Predis\ClientInterface && $redisClient->getOptions()->exceptions) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (0 === \strpos($dsn, 'redis:')) {
            ...
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Redis DSN: "%s" does not start with "redis:" or "rediss".', $dsn));
        }
        if (!\extension_loaded('redis') && !\class_exists(\ECSPrefix20210508\Predis\Client::class)) {
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\CacheException(\sprintf('Cannot find the "redis" extension nor the "predis/predis" package: "%s".', $dsn));
        }
        ...
        if (\false === ($params = \parse_url($params))) {
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Redis DSN: "%s".', $dsn));
        }
        $query = $hosts = [];
        $tls = 'rediss' === $scheme;
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\ECSPrefix20210508\Predis\Client::class)) {
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\CacheException(\sprintf('Redis Sentinel support requires the "predis/predis" package: "%s".', $dsn));
        }
        if (null === $params['class'] && !isset($params['redis_sentinel']) && \extension_loaded('redis')) {
            $class = $params['redis_cluster'] ? \RedisCluster::class : (1 < \count($hosts) ? \RedisArray::class : \Redis::class);
        } else {
            $class = null === $params['class'] ? \ECSPrefix20210508\Predis\Client::class : $params['class'];
        }
        if (\is_a($class, \Redis::class, \true)) {
            ...
            $initializer = static function ($redis) use($connect, $params, $dsn, $auth, $hosts, $tls) {
                ...
                try {
                    ...
                    if (!$isConnected) {
                        ...
                        throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Redis connection "%s" failed: ', $dsn) . $error . '.');
                    }
                    if (null !== $auth && !$redis->auth($auth) || $params['dbindex'] && !$redis->select($params['dbindex'])) {
                        $e = \preg_replace('/^ERR /', '', $redis->getLastError());
                        throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Redis connection "%s" failed: ', $dsn) . $e . '.');
                    }
                    if (0 < $params['tcp_keepalive'] && \defined('Redis::OPT_TCP_KEEPALIVE')) {
                        $redis->setOption(\Redis::OPT_TCP_KEEPALIVE, $params['tcp_keepalive']);
                    }
                } catch (\RedisException $e) {
                    throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Redis connection "%s" failed: ', $dsn) . $e->getMessage());
                }
                return \true;
            };
            if ($params['lazy']) {
                $redis = new \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisProxy($redis, $initializer);
            } else {
                $initializer($redis);
            }
            ...
            $redis = $params['lazy'] ? new \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisClusterProxy($initializer) : $initializer();
        } elseif (\is_a($class, \ECSPrefix20210508\Predis\ClientInterface::class, \true)) {
            if ($params['redis_cluster']) {
                $params['cluster'] = 'redis';
                if (isset($params['redis_sentinel'])) {
                    throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cannot use both "redis_cluster" and "redis_sentinel" at the same time: "%s".', $dsn));
                }
            ...
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" is not a subclass of "Redis", "RedisArray", "RedisCluster" nor "Predis\\ClientInterface".', $class));
        } else {
            throw new \ECSPrefix20210508\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Class "%s" does not exist.', $class));
        }
        return $redis;
    }
    /**
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \ECSPrefix20210508\Predis\ClientInterface && $this->redis->getConnection() instanceof \ECSPrefix20210508\Predis\Connection\Aggregate\ClusterInterface) {
    ...
    protected function doClear($namespace)
    {
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \ECSPrefix20210508\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $namespace . '*', 'COUNT', 1000) : $host->scan($cursor, $namespace . '*', 1000);
    ...
    protected function doDelete(array $ids)
    {
        ...
        if ($this->redis instanceof \ECSPrefix20210508\Predis\ClientInterface && $this->redis->getConnection() instanceof \ECSPrefix20210508\Predis\Connection\Aggregate\ClusterInterface) {
            static $del;
            $del = isset($del) ? $del : (\class_exists(\ECSPrefix20210508\Predis\Command\Redis\UNLINK::class) ? 'unlink' : 'del');
    ...
    private function pipeline(\Closure $generator, $redis = null)
    {
        ...
        if ($redis instanceof \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \ECSPrefix20210508\Predis\ClientInterface && $redis->getConnection() instanceof \ECSPrefix20210508\Predis\Connection\Aggregate\RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \ECSPrefix20210508\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \ECSPrefix20210508\Predis\ClientInterface) {
    ...
    private function getHosts()
    {
        ...
        if ($this->redis instanceof \ECSPrefix20210508\Predis\ClientInterface) {
            ...
            if ($connection instanceof \ECSPrefix20210508\Predis\Connection\Aggregate\ClusterInterface && $connection instanceof \Traversable) {
                $hosts = [];
                foreach ($connection as $c) {
                    $hosts[] = new \ECSPrefix20210508\Predis\Client($c);
                }
            }
        ...
        } elseif ($this->redis instanceof \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisClusterProxy || $this->redis instanceof \RedisCluster) {
            $hosts = [];
            foreach ($this->redis->_masters() as $host) {
                $hosts[] = new \ECSPrefix20210508\Symfony\Component\Cache\Traits\RedisClusterNodeProxy($host, $this->redis);
            }
        }
        return $hosts;
    }
