...
use ECSPrefix20210509\Symfony\Component\Cache\Adapter\TraceableAdapter;
use ECSPrefix20210509\Symfony\Component\Cache\Adapter\TraceableTagAwareAdapter;
 ...
 */
class CacheCollectorPass implements \ECSPrefix20210509\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
     ...
     * @param string $name
     */
    private function addToCollector($id, $name, \ECSPrefix20210509\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        $id = (string) $id;
        $name = (string) $name;
        ...
        $recorder = new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Definition(\is_subclass_of($definition->getClass(), \ECSPrefix20210509\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface::class) ? \ECSPrefix20210509\Symfony\Component\Cache\Adapter\TraceableTagAwareAdapter::class : \ECSPrefix20210509\Symfony\Component\Cache\Adapter\TraceableAdapter::class);
        $recorder->setTags($definition->getTags());
        if (!$definition->isPublic() || !$definition->isPrivate()) {
            $recorder->setPublic($definition->isPublic());
        }
        $recorder->setArguments([new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference($innerId = $id . $this->cachePoolRecorderInnerSuffix)]);
        $definition->setTags([]);
        $definition->setPublic(\false);
        $container->setDefinition($innerId, $definition);
        $container->setDefinition($id, $recorder);
        // Tell the collector to add the new instance
        $collectorDefinition->addMethodCall('addInstance', [$name, new \ECSPrefix20210509\Symfony\Component\DependencyInjection\Reference($id)]);
