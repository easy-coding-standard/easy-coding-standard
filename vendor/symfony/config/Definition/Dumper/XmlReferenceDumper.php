...
class XmlReferenceDumper
{
    ...
    private function writeNode(\ECSPrefix20210909\Symfony\Component\Config\Definition\NodeInterface $node, int $depth = 0, bool $root = \false, string $namespace = null)
    {
        ...
        if ($node instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\ArrayNode) {
            ...
            if ($node instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\PrototypedArrayNode) {
                ...
                if ($prototype instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\PrototypedArrayNode) {
                    $prototype->setName($key ?? '');
                    $children = [$key => $prototype];
                } elseif ($prototype instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\ArrayNode) {
            ...
            foreach ($children as $child) {
                if ($child instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\ArrayNode) {
                    // get elements
                    $rootChildren[] = $child;
                    continue;
                }
                ...
                // comments
                $comments = [];
                if ($child instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\BaseNode && ($info = $child->getInfo())) {
                    $comments[] = $info;
                }
                if ($child instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\BaseNode && ($example = $child->getExample())) {
                    $comments[] = 'Example: ' . $example;
                }
                if ($child->isRequired()) {
                    $comments[] = 'Required';
                }
                if ($child instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\BaseNode && $child->isDeprecated()) {
                    $deprecation = $child->getDeprecation($child->getName(), $node->getPath());
                    $comments[] = \sprintf('Deprecated (%s)', ($deprecation['package'] || $deprecation['version'] ? "Since {$deprecation['package']} {$deprecation['version']}: " : '') . $deprecation['message']);
                }
                if ($child instanceof \ECSPrefix20210909\Symfony\Component\Config\Definition\EnumNode) {
