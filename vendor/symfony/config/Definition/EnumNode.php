...
class EnumNode extends \ECSPrefix20210507\Symfony\Component\Config\Definition\ScalarNode
{
    ...
    protected function finalizeValue($value)
    {
        ...
        if (!\in_array($value, $this->values, \true)) {
            $ex = new \ECSPrefix20210507\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The value %s is not allowed for path "%s". Permissible values: %s', \json_encode($value), $this->getPath(), \implode(', ', \array_map('json_encode', $this->values))));
