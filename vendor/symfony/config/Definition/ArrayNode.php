...
use ECSPrefix20220306\Symfony\Component\Config\Definition\Exception\InvalidTypeException;
 ...
 */
class ArrayNode extends \ECSPrefix20220306\Symfony\Component\Config\Definition\BaseNode implements \ECSPrefix20220306\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
        ...
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    ...
                    $ex = new \ECSPrefix20220306\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($message);
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \ECSPrefix20220306\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected "array", but got "%s"', $this->getPath(), \get_debug_type($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \ECSPrefix20220306\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml(array $value) : array
    {
        foreach ($this->xmlRemappings as [$singular, $plural]) {
            ...
            $value[$plural] = \ECSPrefix20220306\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);
        ...
        foreach ($rightSide as $k => $v) {
            ...
            if (!\array_key_exists($k, $leftSide)) {
                if (!$this->allowNewKeys) {
                    $ex = new \ECSPrefix20220306\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('You are not allowed to define new elements for path "%s". Please define all elements for this path in one config file. If you are trying to overwrite an element, make sure you redefine it with the same name.', $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
                $leftSide[$k] = $v;
