...
', ['NAMESPACE' => $this->namespace, 'REQUIRE' => $require, 'USE' => $use, 'CLASS' => $this->getName(), 'IMPLEMENTS' => $implements, 'BODY' => $body]);
    ...
    {
        $this->methods[] = new \ECSPrefix20210810\Symfony\Component\Config\Builder\Method(\strtr($body, ['NAME' => $this->camelCase($name)] + $params));
    }
    /**
     * @param string $name
     * @param string|null $classType
     */
    public function addProperty($name, $classType = null) : \ECSPrefix20210810\Symfony\Component\Config\Builder\Property
    {
        $property = new \ECSPrefix20210810\Symfony\Component\Config\Builder\Property($name, $this->camelCase($name));
        if (null !== $classType) {
