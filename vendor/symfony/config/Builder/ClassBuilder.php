...
', ['NAMESPACE' => $this->namespace, 'REQUIRE' => $require, 'USE' => $use, 'CLASS' => $this->getName(), 'IMPLEMENTS' => $implements, 'BODY' => $body]);
    ...
    {
        $this->methods[] = new \ECSPrefix20220215\Symfony\Component\Config\Builder\Method(\strtr($body, ['NAME' => $this->camelCase($name)] + $params));
    }
    public function addProperty(string $name, string $classType = null) : \ECSPrefix20220215\Symfony\Component\Config\Builder\Property
    {
        $property = new \ECSPrefix20220215\Symfony\Component\Config\Builder\Property($name, '_' !== $name[0] ? $this->camelCase($name) : $name);
        if (null !== $classType) {
