...
use ECSPrefix20210509\Symfony\Component\Config\Exception\FileLoaderImportCircularReferenceException;
 ...
 */
abstract class FileLoader extends \ECSPrefix20210509\Symfony\Component\Config\Loader\Loader
{
    protected static $loading = [];
    protected $locator;
    private $currentDir;
    public function __construct(\ECSPrefix20210509\Symfony\Component\Config\FileLocatorInterface $locator)
    {
    ...
    protected function glob($pattern, $recursive, &$resource = null, $ignoreErrors = \false, $forExclusion = \false, array $excluded = [])
    {
        ...
        } catch (\ECSPrefix20210509\Symfony\Component\Config\Exception\FileLocatorFileNotFoundException $e) {
            if (!$ignoreErrors) {
                throw $e;
            }
            $resource = [];
            foreach ($e->getPaths() as $path) {
                $resource[] = new \ECSPrefix20210509\Symfony\Component\Config\Resource\FileExistenceResource($path);
            }
            return;
        }
        $resource = new \ECSPrefix20210509\Symfony\Component\Config\Resource\GlobResource($prefix, $pattern, $recursive, $forExclusion, $excluded);
        yield from $resource;
    }
    /**
     * @param string $type
    ...
    private function doImport($resource, $type = null, $ignoreErrors = \false, $sourceResource = null)
    {
        ...
        try {
            ...
            for ($i = 0; $i < ($resourcesCount = \count($resources)); ++$i) {
                if (isset(self::$loading[$resources[$i]])) {
                    if ($i == $resourcesCount - 1) {
                        throw new \ECSPrefix20210509\Symfony\Component\Config\Exception\FileLoaderImportCircularReferenceException(\array_keys(self::$loading));
                    }
                } else {
            ...
            return $ret;
        } catch (\ECSPrefix20210509\Symfony\Component\Config\Exception\FileLoaderImportCircularReferenceException $e) {
            throw $e;
        } catch (\Exception $e) {
            if (!$ignoreErrors) {
                // prevent embedded imports from nesting multiple exceptions
                if ($e instanceof \ECSPrefix20210509\Symfony\Component\Config\Exception\LoaderLoadException) {
                    throw $e;
                }
                throw new \ECSPrefix20210509\Symfony\Component\Config\Exception\LoaderLoadException($resource, $sourceResource, 0, $e, $type);
            }
        }
        return null;
    }
