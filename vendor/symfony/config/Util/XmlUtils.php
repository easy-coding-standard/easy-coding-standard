 ...
 */
namespace ECSPrefix20220514\Symfony\Component\Config\Util;

use ECSPrefix20220514\Symfony\Component\Config\Util\Exception\InvalidXmlException;
use ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException;
/**
...
class XmlUtils
{
    ...
    public static function parse(string $content, $schemaOrCallable = null) : \DOMDocument
    {
        ...
        if (!$dom->loadXML($content, \LIBXML_NONET | \LIBXML_COMPACT)) {
            throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException(\implode("\n", static::getXmlErrors($internalErrors)));
        }
        $dom->normalizeDocument();
        \libxml_use_internal_errors($internalErrors);
        foreach ($dom->childNodes as $child) {
            if (\XML_DOCUMENT_TYPE_NODE === $child->nodeType) {
                throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException('Document types are not allowed.');
            }
        }
        if (null !== $schemaOrCallable) {
            ...
            if (\is_callable($schemaOrCallable)) {
                ...
                \libxml_use_internal_errors($internalErrors);
                throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException(\sprintf('Invalid XSD file: "%s".', $schemaOrCallable));
            }
            if (!$valid) {
                $messages = static::getXmlErrors($internalErrors);
                if (empty($messages)) {
                    throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\InvalidXmlException('The XML is not valid.', 0, $e);
                }
                throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException(\implode("\n", $messages), 0, $e);
            }
        }
    ...
    public static function loadFile(string $file, $schemaOrCallable = null) : \DOMDocument
    {
        ...
        } catch (\ECSPrefix20220514\Symfony\Component\Config\Util\Exception\InvalidXmlException $e) {
            throw new \ECSPrefix20220514\Symfony\Component\Config\Util\Exception\XmlParsingException(\sprintf('The XML file "%s" is not valid.', $file), 0, $e->getPrevious());
        }
    }
    /**
