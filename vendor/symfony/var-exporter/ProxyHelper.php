...
use ECSPrefix202212\Symfony\Component\VarExporter\Exception\LogicException;
use ECSPrefix202212\Symfony\Component\VarExporter\Internal\Hydrator;
use ECSPrefix202212\Symfony\Component\VarExporter\Internal\LazyObjectRegistry;
/**
...
EOPHP;
    ...
    public static function exportSignature(\ReflectionFunctionAbstract $function, bool $withParameterTypes = \true) : string
    {
        ...
        foreach ($function->getParameters() as $param) {
            $parameters[] = ((\method_exists($param, 'getAttributes') ? $param->getAttributes(\ECSPrefix202212\SensitiveParameter::class) : []) ? '#[\\SensitiveParameter] ' : '') . ($withParameterTypes && $param->hasType() ? self::exportType($param) . ' ' : '') . ($param->isPassedByReference() ? '&' : '') . ($param->isVariadic() ? '...' : '') . '$' . $param->name . ($param->isOptional() && !$param->isVariadic() ? ' = ' . self::exportDefault($param) : '');
        }
