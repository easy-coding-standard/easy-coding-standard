...
class Registry
{
    ...
    public static function getClassReflector($class, $instantiableWithoutConstructor = \false, $cloneable = null)
    {
        if (!($isClass = \class_exists($class)) && !\interface_exists($class, \false) && !\trait_exists($class, \false)) {
            throw new \ECSPrefix20210507\Symfony\Component\VarExporter\Exception\ClassNotFoundException($class);
        }
        $reflector = new \ReflectionClass($class);
        if ($instantiableWithoutConstructor) {
            $proto = $reflector->newInstanceWithoutConstructor();
        } elseif (!$isClass || $reflector->isAbstract()) {
            throw new \ECSPrefix20210507\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException($class);
            ...
            try {
                ...
                if ('C:' === $proto && !$reflector->getMethod('unserialize')->isInternal()) {
                    ...
                    try {
                        ...
                        throw new \ECSPrefix20210507\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException($class, $e);
                    }
                    if (\false === $proto) {
                        throw new \ECSPrefix20210507\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException($class);
                    }
