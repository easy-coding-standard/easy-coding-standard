    ...
    public function __construct($optionsResolver)
    {
        $this->get = \Closure::bind(function ($property, $option, $message) {
            ...
            if (!$this->isDefined($option)) {
                throw new \ECSPrefix20210507\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist.', $option));
            }
            if (!\array_key_exists($option, $this->{$property})) {
                throw new \ECSPrefix20210507\Symfony\Component\OptionsResolver\Exception\NoConfigurationException($message);
            }
