 ...
 */
namespace ECSPrefix20210912\Symfony\Component\OptionsResolver;

use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidArgumentException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\MissingOptionsException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\NoSuchOptionException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\OptionDefinitionException;
use ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException;
/**
 ...
 */
class OptionsResolver implements \ECSPrefix20210912\Symfony\Component\OptionsResolver\Options
{
    ...
    public function setDefault($option, $value)
    {
        ...
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Default values cannot be set from a lazy option or normalizer.');
        }
        ...
        if ($value instanceof \Closure) {
            ...
            if (isset($params[0]) && \ECSPrefix20210912\Symfony\Component\OptionsResolver\Options::class === $this->getParameterClassName($params[0])) {
            ...
            if (isset($params[0]) && null !== ($type = $params[0]->getType()) && self::class === $type->getName() && (!isset($params[1]) || ($type = $params[1]->getType()) instanceof \ReflectionNamedType && \ECSPrefix20210912\Symfony\Component\OptionsResolver\Options::class === $type->getName())) {
                // Store closure for later evaluation
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be made required from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be defined from a lazy option or normalizer.');
        }
        foreach ((array) $optionNames as $option) {
            $this->defined[$option] = \true;
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be deprecated from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist, defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
        $args = \func_get_args();
        if (\func_num_args() < 3) {
        ...
        if (!\is_string($message) && !$message instanceof \Closure) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidArgumentException(\sprintf('Invalid type for deprecation message argument, expected string or \\Closure, but got "%s".', \get_debug_type($message)));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Normalizers cannot be set from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Allowed values cannot be set from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Allowed values cannot be added from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Allowed types cannot be set from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Allowed types cannot be added from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
     ...
     * @param string $option
     */
    public function define($option) : \ECSPrefix20210912\Symfony\Component\OptionsResolver\OptionConfigurator
    {
        if (isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\OptionDefinitionException(\sprintf('The option "%s" is already defined.', $option));
        }
        return new \ECSPrefix20210912\Symfony\Component\OptionsResolver\OptionConfigurator($option, $this);
    }
    /**
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('The Info message cannot be set from a lazy option or normalizer.');
        }
        if (!isset($this->defined[$option])) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('The prototype property cannot be set from a lazy option or normalizer.');
        }
        if (null === $this->prototype && $prototype) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('The prototype property cannot be set from a root definition.');
        }
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be removed from a lazy option or normalizer.');
        }
        foreach ((array) $optionNames as $option) {
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be cleared from a lazy option or normalizer.');
        }
        $this->defined = [];
        $this->defaults = [];
        $this->nested = [];
    ...
    {
        if ($this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Options cannot be resolved from a lazy option or normalizer.');
        }
        ...
        if (\count($diff) > 0) {
            \ksort($clone->defined);
            \ksort($diff);
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException(\sprintf((\count($diff) > 1 ? 'The options "%s" do not exist.' : 'The option "%s" does not exist.') . ' Defined options are: "%s".', $this->formatOptions(\array_keys($diff)), \implode('", "', \array_keys($clone->defined))));
        }
        ...
        if (\count($diff) > 0) {
            \ksort($diff);
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\MissingOptionsException(\sprintf(\count($diff) > 1 ? 'The required options "%s" are missing.' : 'The required option "%s" is missing.', $this->formatOptions(\array_keys($diff))));
        }
        // Lock the container
        $clone->locked = \true;
    ...
    {
        if (!$this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Array access is only supported within closures of lazy options and normalizers.');
        }
        ...
        if (!isset($this->defaults[$option]) && !\array_key_exists($option, $this->defaults)) {
            if (!isset($this->defined[$option])) {
                throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\NoSuchOptionException(\sprintf('The option "%s" does not exist. Defined options are: "%s".', $this->formatOptions([$option]), \implode('", "', \array_keys($this->defined))));
            }
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\NoSuchOptionException(\sprintf('The optional option "%s" has no value set. You should make sure it is set with "isset" before reading it.', $this->formatOptions([$option])));
        }
        ...
        if (isset($this->nested[$option])) {
            ...
            if (isset($this->calling[$option])) {
                throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\OptionDefinitionException(\sprintf('The options "%s" have a cyclic dependency.', $this->formatOptions(\array_keys($this->calling))));
            }
            if (!\is_array($value)) {
                throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('The nested option "%s" with value %s is expected to be of type array, but is of type "%s".', $this->formatOptions([$option]), $this->formatValue($value), \get_debug_type($value)));
            }
                ...
                if ($resolver->prototype) {
                    ...
                    foreach ($value as $index => $prototypeValue) {
                        if (!\is_array($prototypeValue)) {
                            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('The value of the option "%s" is expected to be of type array of array, but is of type array of "%s".', $this->formatOptions([$option]), \get_debug_type($prototypeValue)));
                        }
        ...
        if (isset($this->allowedTypes[$option])) {
            ...
            if (!$valid) {
                ...
                if (\is_array($value) && $allowedContainsArrayType) {
                    throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('The option "%s" with value %s is expected to be of type "%s", but one of the elements is of type "%s".', $this->formatOptions([$option]), $fmtActualValue, $fmtAllowedTypes, $fmtProvidedTypes));
                }
                throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('The option "%s" with value %s is expected to be of type "%s", but is of type "%s".', $this->formatOptions([$option]), $fmtActualValue, $fmtAllowedTypes, $fmtProvidedTypes));
            }
        }
        ...
        if (isset($this->allowedValues[$option])) {
            ...
            if (!$success) {
                ...
                throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException($message);
            }
        }
            ...
            if ($message instanceof \Closure) {
                ...
                try {
                    if (!\is_string($message = $message($this, $value))) {
                        throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Invalid type for deprecation message, expected string but got "%s", return an empty string to ignore.', \get_debug_type($message)));
                    }
                } finally {
                    unset($this->calling[$option]);
                }
    ...
    {
        throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Setting options via array access is not supported. Use setDefault() instead.');
    }
    /**
     * Not supported.
     *
    ...
    {
        throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Removing options via array access is not supported. Use remove() instead.');
    }
    /**
    ...
    {
        if (!$this->locked) {
            throw new \ECSPrefix20210912\Symfony\Component\OptionsResolver\Exception\AccessException('Counting is only supported within closures of lazy options and normalizers.');
        }
