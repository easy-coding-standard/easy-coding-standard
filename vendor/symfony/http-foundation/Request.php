...
use ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\ConflictingHeadersException;
use ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\JsonException;
use ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException;
use ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException;
use ECSPrefix20210727\Symfony\Component\HttpFoundation\Session\SessionInterface;
// Help opcache.preload discover always-needed symbols
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\FileBag::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderBag::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderUtils::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\ParameterBag::class);
\class_exists(\ECSPrefix20210727\Symfony\Component\HttpFoundation\ServerBag::class);
/**
...
class Request
{
    ...
    {
        $this->request = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($request);
        $this->query = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($query);
        $this->attributes = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        $this->cookies = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($cookies);
        $this->files = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\FileBag($files);
        $this->server = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\ServerBag($server);
        $this->headers = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderBag($this->server->getHeaders());
    ...
    public static function createFromGlobals()
    {
        ...
        if (\strncmp($request->headers->get('CONTENT_TYPE', ''), 'application/x-www-form-urlencoded', \strlen('application/x-www-form-urlencoded')) === 0 && \in_array(\strtoupper($request->server->get('REQUEST_METHOD', 'GET')), ['PUT', 'DELETE', 'PATCH'])) {
            ...
            $request->request = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($data);
        }
        return $request;
    }
    /**
    ...
    public function duplicate($query = null, $request = null, $attributes = null, $cookies = null, $files = null, $server = null)
    {
        $dup = clone $this;
        if (null !== $query) {
            $dup->query = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($query);
        }
        if (null !== $request) {
            $dup->request = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($request);
        }
        if (null !== $attributes) {
            $dup->attributes = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        }
        if (null !== $cookies) {
            $dup->cookies = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\InputBag($cookies);
        }
        if (null !== $files) {
            $dup->files = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\FileBag($files);
        }
        if (null !== $server) {
            $dup->server = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\ServerBag($server);
            $dup->headers = new \ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderBag($dup->server->getHeaders());
        }
        $dup->languages = null;
        $dup->charsets = null;
        $dup->encodings = null;
    ...
    public static function normalizeQueryString($qs)
    {
        ...
        $qs = \ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderUtils::parseQuery($qs);
        \ksort($qs);
    ...
    public function getSession()
    {
        ...
        if (!$session instanceof \ECSPrefix20210727\Symfony\Component\HttpFoundation\Session\SessionInterface && null !== $session) {
            $this->setSession($session = $session());
        }
        if (null === $session) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException('Session has not been set.');
        }
        return $session;
    }
    /**
    ...
    public function getHost()
    {
        ...
        if ($host && '' !== \preg_replace('/(?:^\\[)?[a-zA-Z0-9-:\\]_]+\\.?/', '', $host)) {
            ...
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid Host "%s".', $host));
        }
        if (\count(self::$trustedHostPatterns) > 0) {
            ...
            $this->isHostValid = \false;
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Untrusted Host "%s".', $host));
        }
        return $host;
    }
    /**
    ...
    public function getMethod()
    {
        ...
        if (!\preg_match('/^[A-Z]++$/D', $method)) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid method override "%s".', $method));
        }
        return $this->method = $method;
    }
    /**
    ...
    public function toArray()
    {
        if ('' === ($content = $this->getContent())) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\JsonException('Request body is empty.');
        }
        try {
            $content = \json_decode($content, \true, 512, \JSON_BIGINT_AS_STRING | (\PHP_VERSION_ID >= 70300 ? \JSON_THROW_ON_ERROR : 0));
        } catch (\JsonException $e) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\JsonException('Could not decode request body.', $e->getCode(), $e);
        }
        if (\PHP_VERSION_ID < 70300 && \JSON_ERROR_NONE !== \json_last_error()) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\JsonException('Could not decode request body: ' . \json_last_error_msg(), \json_last_error());
        }
        if (!\is_array($content)) {
            throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\JsonException(\sprintf('JSON content was expected to decode to an array, "%s" returned.', \get_debug_type($content)));
        }
        return $content;
    }
    /**
    ...
    public function getLanguages()
    {
        ...
        $languages = \ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Language'))->all();
        $this->languages = [];
    ...
    public function getCharsets()
    {
        ...
        return $this->charsets = \array_keys(\ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Charset'))->all());
    }
    /**
    ...
    public function getEncodings()
    {
        ...
        return $this->encodings = \array_keys(\ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Encoding'))->all());
    }
    /**
    ...
    public function getAcceptableContentTypes()
    {
        ...
        return $this->acceptableContentTypes = \array_keys(\ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept'))->all());
    }
    /**
    ...
    public function preferSafeContent() : bool
    {
        ...
        $this->isSafeContentPreferred = \ECSPrefix20210727\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Prefer'))->has('safe');
        return $this->isSafeContentPreferred;
    }
    /*
    ...
    {
        return self::$trustedProxies && \ECSPrefix20210727\Symfony\Component\HttpFoundation\IpUtils::checkIp($this->server->get('REMOTE_ADDR', ''), self::$trustedProxies);
    }
    private function getTrustedValues(int $type, string $ip = null) : array
    {
        ...
        if (self::$trustedHeaderSet & self::HEADER_FORWARDED && isset(self::FORWARDED_PARAMS[$type]) && $this->headers->has(self::TRUSTED_HEADERS[self::HEADER_FORWARDED])) {
            ...
            $parts = \ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderUtils::split($forwarded, ',;=');
            $forwardedValues = [];
            $param = self::FORWARDED_PARAMS[$type];
            foreach ($parts as $subParts) {
                if (null === ($v = \ECSPrefix20210727\Symfony\Component\HttpFoundation\HeaderUtils::combine($subParts)[$param] ?? null)) {
                    continue;
                }
        ...
        throw new \ECSPrefix20210727\Symfony\Component\HttpFoundation\Exception\ConflictingHeadersException(\sprintf('The request has both a trusted "%s" header and a trusted "%s" header, conflicting with each other. You should either configure your proxy to remove one of them, or configure your project to distrust the offending one.', self::TRUSTED_HEADERS[self::HEADER_FORWARDED], self::TRUSTED_HEADERS[$type]));
    }
    private function normalizeAndFilterClientIps(array $clientIps, string $ip) : array
    {
        ...
        foreach ($clientIps as $key => $clientIp) {
            ...
            if (\ECSPrefix20210727\Symfony\Component\HttpFoundation\IpUtils::checkIp($clientIp, self::$trustedProxies)) {
                unset($clientIps[$key]);
