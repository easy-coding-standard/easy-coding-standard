...
use ECSPrefix202306\Doctrine\DBAL\Schema\Schema;
use ECSPrefix202306\Doctrine\DBAL\Types\Types;
/**
...
class PdoSessionHandler extends AbstractSessionHandler
{
     ...
     *
     * @param \PDO|string $pdoOrDsn A \PDO instance or DSN string or URL string or null
     *
    ...
    {
        if ($schema->hasTable($this->table) || $isSameDatabase && !$isSameDatabase(\Closure::fromCallable([$this->getConnection(), 'exec']))) {
            return;
        }
        ...
        switch ($this->driver) {
            ...
            case 'oci':
                ...
                break;
            case 'sqlsrv':
                $table->addColumn($this->idCol, Types::TEXT)->setLength(128)->setNotnull(\true);
                $table->addColumn($this->dataCol, Types::BLOB)->setNotnull(\true);
                $table->addColumn($this->lifetimeCol, Types::INTEGER)->setUnsigned(\true)->setNotnull(\true);
                $table->addColumn($this->timeCol, Types::INTEGER)->setUnsigned(\true)->setNotnull(\true);
                break;
    ...
    public function createTable()
    {
        ...
        $this->getConnection();
        $sql = $this->driver === 'mysql' ? "CREATE TABLE {$this->table} ({$this->idCol} VARBINARY(128) NOT NULL PRIMARY KEY, {$this->dataCol} BLOB NOT NULL, {$this->lifetimeCol} INTEGER UNSIGNED NOT NULL, {$this->timeCol} INTEGER UNSIGNED NOT NULL) COLLATE utf8mb4_bin, ENGINE = InnoDB" : ($this->driver === 'sqlite' ? "CREATE TABLE {$this->table} ({$this->idCol} TEXT NOT NULL PRIMARY KEY, {$this->dataCol} BLOB NOT NULL, {$this->lifetimeCol} INTEGER NOT NULL, {$this->timeCol} INTEGER NOT NULL)" : ($this->driver === 'pgsql' ? "CREATE TABLE {$this->table} ({$this->idCol} VARCHAR(128) NOT NULL PRIMARY KEY, {$this->dataCol} BYTEA NOT NULL, {$this->lifetimeCol} INTEGER NOT NULL, {$this->timeCol} INTEGER NOT NULL)" : ($this->driver === 'oci' ? "CREATE TABLE {$this->table} ({$this->idCol} VARCHAR2(128) NOT NULL PRIMARY KEY, {$this->dataCol} BLOB NOT NULL, {$this->lifetimeCol} INTEGER NOT NULL, {$this->timeCol} INTEGER NOT NULL)" : ($this->driver === 'sqlsrv' ? "CREATE TABLE {$this->table} ({$this->idCol} VARCHAR(128) NOT NULL PRIMARY KEY, {$this->dataCol} VARBINARY(MAX) NOT NULL, {$this->lifetimeCol} INTEGER NOT NULL, {$this->timeCol} INTEGER NOT NULL)" : null))));
        try {
            $this->pdo->exec($sql);
