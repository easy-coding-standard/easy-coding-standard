 ...
 */
class RegisterListenersPass implements \ECSPrefix20210511\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\ECSPrefix20210511\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->listenerTag, \true) as $id => $events) {
            ...
            foreach ($events as $event) {
                ...
                $dispatcherDefinition->addMethodCall('addListener', [$event['event'], [new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument(new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Reference($id)), $event['method']], $priority]);
        ...
        $extractingDispatcher = new \ECSPrefix20210511\Symfony\Component\EventDispatcher\DependencyInjection\ExtractingEventDispatcher();
        foreach ($container->findTaggedServiceIds($this->subscriberTag, \true) as $id => $tags) {
            ...
            if (!$r->isSubclassOf(\ECSPrefix20210511\Symfony\Component\EventDispatcher\EventSubscriberInterface::class)) {
                throw new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Service "%s" must implement interface "%s".', $id, \ECSPrefix20210511\Symfony\Component\EventDispatcher\EventSubscriberInterface::class));
            }
            $class = $r->name;
            ...
            $noPreload = 0;
            \ECSPrefix20210511\Symfony\Component\EventDispatcher\DependencyInjection\ExtractingEventDispatcher::$aliases = $aliases;
            \ECSPrefix20210511\Symfony\Component\EventDispatcher\DependencyInjection\ExtractingEventDispatcher::$subscriber = $class;
            $extractingDispatcher->addSubscriber($extractingDispatcher);
            foreach ($extractingDispatcher->listeners as $args) {
                $args[1] = [new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument(new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Reference($id)), $args[1]];
            ...
            \ECSPrefix20210511\Symfony\Component\EventDispatcher\DependencyInjection\ExtractingEventDispatcher::$aliases = [];
        }
    }
    /**
     * @param string $id
     ...
     * @return string
     */
    private function getEventFromTypeDeclaration(\ECSPrefix20210511\Symfony\Component\DependencyInjection\ContainerBuilder $container, $id, $method)
    {
        $id = (string) $id;
        $method = (string) $method;
        if (null === ($class = $container->getDefinition($id)->getClass()) || !($r = $container->getReflectionClass($class, \false)) || !$r->hasMethod($method) || 1 > ($m = $r->getMethod($method))->getNumberOfParameters() || !($type = $m->getParameters()[0]->getType()) instanceof \ReflectionNamedType || $type->isBuiltin() || \ECSPrefix20210511\Symfony\Contracts\EventDispatcher\Event::class === ($name = $type->getName())) {
            throw new \ECSPrefix20210511\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Service "%s" must define the "event" attribute on "%s" tags.', $id, $this->listenerTag));
        }
        return $name;
    }
}
 ...
 * @internal
 */
class ExtractingEventDispatcher extends \ECSPrefix20210511\Symfony\Component\EventDispatcher\EventDispatcher implements \ECSPrefix20210511\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    public $listeners = [];
