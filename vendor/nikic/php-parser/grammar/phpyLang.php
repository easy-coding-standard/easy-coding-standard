...
{
    $code = \ECSPrefix20210804\resolveNodes($code);
    $code = \ECSPrefix20210804\resolveMacros($code);
    $code = \ECSPrefix20210804\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \ECSPrefix20210804\resolveNodes($matches['params']);
        $params = \ECSPrefix20210804\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \ECSPrefix20210804\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \ECSPrefix20210804\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \ECSPrefix20210804\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \ECSPrefix20210804\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \ECSPrefix20210804\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \ECSPrefix20210804\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\ECSPrefix20210804\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
