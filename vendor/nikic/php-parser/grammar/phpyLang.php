...
{
    $code = \ConfigTransformer20210601\resolveNodes($code);
    $code = \ConfigTransformer20210601\resolveMacros($code);
    $code = \ConfigTransformer20210601\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \ConfigTransformer20210601\resolveNodes($matches['params']);
        $params = \ConfigTransformer20210601\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \ConfigTransformer20210601\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \ConfigTransformer20210601\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \ConfigTransformer20210601\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \ConfigTransformer20210601\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \ConfigTransformer20210601\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \ConfigTransformer20210601\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\ConfigTransformer20210601\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
