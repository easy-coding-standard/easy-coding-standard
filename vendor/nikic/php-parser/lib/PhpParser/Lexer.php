...
use ConfigTransformer20210601\PhpParser\Parser\Tokens;
class Lexer
{
    protected $code;
    protected $tokens;
    ...
    private function handleInvalidCharacterRange($start, $end, $line, \ConfigTransformer20210601\PhpParser\ErrorHandler $errorHandler)
    {
        $tokens = [];
        for ($i = $start; $i < $end; $i++) {
            $chr = $this->code[$i];
            ...
            $errorHandler->handleError(new \ConfigTransformer20210601\PhpParser\Error($errorMsg, ['startLine' => $line, 'endLine' => $line, 'startFilePos' => $i, 'endFilePos' => $i]));
        }
        return $tokens;
    }
    /**
    ...
    protected function postprocessTokens(\ConfigTransformer20210601\PhpParser\ErrorHandler $errorHandler)
    {
        ...
        if ($filePos !== \strlen($this->code)) {
            if (\substr($this->code, $filePos, 2) === '/*') {
                ...
                $errorHandler->handleError(new \ConfigTransformer20210601\PhpParser\Error('Unterminated comment', ['startLine' => $line, 'endLine' => $line + \substr_count($comment, "\n"), 'startFilePos' => $filePos, 'endFilePos' => $filePos + \strlen($comment)]));
        ...
        if (\count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new \ConfigTransformer20210601\PhpParser\Error('Unterminated comment', ['startLine' => $line - \substr_count($lastToken[1], "\n"), 'endLine' => $line, 'startFilePos' => $filePos - \strlen($lastToken[1]), 'endFilePos' => $filePos]));
            }
        }
    }
    /**
            ...
            if (\is_string($token)) {
                ...
                if (\T_COMMENT === $token[0] || \T_DOC_COMMENT === $token[0]) {
                    if ($this->attributeCommentsUsed) {
                        $comment = \T_DOC_COMMENT === $token[0] ? new \ConfigTransformer20210601\PhpParser\Comment\Doc($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos) : new \ConfigTransformer20210601\PhpParser\Comment($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos);
    ...
    public function handleHaltCompiler() : string
    {
        ...
        if (!\preg_match('~^\\s*\\(\\s*\\)\\s*(?:;|\\?>\\r?\\n?)~', $textAfter, $matches)) {
            throw new \ConfigTransformer20210601\PhpParser\Error('__HALT_COMPILER must be followed by "();"');
        }
    ...
    protected function createTokenMap() : array
    {
        ...
        for ($i = 256; $i < 1000; ++$i) {
            if (\T_DOUBLE_COLON === $i) {
                ...
                $tokenMap[$i] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_PAAMAYIM_NEKUDOTAYIM;
            } elseif (\T_OPEN_TAG_WITH_ECHO === $i) {
                // T_OPEN_TAG_WITH_ECHO with dropped T_OPEN_TAG results in T_ECHO
                $tokenMap[$i] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_ECHO;
                ...
                if ('T_HASHBANG' === $name) {
                    ...
                    $tokenMap[$i] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_INLINE_HTML;
                } elseif (\defined($name = \ConfigTransformer20210601\PhpParser\Parser\Tokens::class . '::' . $name)) {
        ...
        if (\defined('T_ONUMBER')) {
            $tokenMap[\T_ONUMBER] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_DNUMBER;
        }
        // HHVM also has a separate token for the __COMPILER_HALT_OFFSET__ constant
        if (\defined('T_COMPILER_HALT_OFFSET')) {
            $tokenMap[\T_COMPILER_HALT_OFFSET] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_STRING;
        }
        // Assign tokens for which we define compatibility constants, as token_name() does not know them.
        $tokenMap[\T_FN] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_FN;
        $tokenMap[\T_COALESCE_EQUAL] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_COALESCE_EQUAL;
        $tokenMap[\T_NAME_QUALIFIED] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_NAME_QUALIFIED;
        $tokenMap[\T_NAME_FULLY_QUALIFIED] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_NAME_FULLY_QUALIFIED;
        $tokenMap[\T_NAME_RELATIVE] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_NAME_RELATIVE;
        $tokenMap[\T_MATCH] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_MATCH;
        $tokenMap[\T_NULLSAFE_OBJECT_OPERATOR] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_NULLSAFE_OBJECT_OPERATOR;
        $tokenMap[\T_ATTRIBUTE] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_ATTRIBUTE;
        $tokenMap[\T_ENUM] = \ConfigTransformer20210601\PhpParser\Parser\Tokens::T_ENUM;
        return $tokenMap;
    }
