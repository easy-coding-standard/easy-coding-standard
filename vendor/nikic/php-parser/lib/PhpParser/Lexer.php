...
use ECSPrefix20210804\PhpParser\Parser\Tokens;
class Lexer
{
    protected $code;
    protected $tokens;
    ...
    private function handleInvalidCharacterRange($start, $end, $line, \ECSPrefix20210804\PhpParser\ErrorHandler $errorHandler)
    {
        $tokens = [];
        for ($i = $start; $i < $end; $i++) {
            $chr = $this->code[$i];
            ...
            $errorHandler->handleError(new \ECSPrefix20210804\PhpParser\Error($errorMsg, ['startLine' => $line, 'endLine' => $line, 'startFilePos' => $i, 'endFilePos' => $i]));
        }
        return $tokens;
    }
    /**
    ...
    protected function postprocessTokens(\ECSPrefix20210804\PhpParser\ErrorHandler $errorHandler)
    {
        ...
        if ($filePos !== \strlen($this->code)) {
            if (\substr($this->code, $filePos, 2) === '/*') {
                ...
                $errorHandler->handleError(new \ECSPrefix20210804\PhpParser\Error('Unterminated comment', ['startLine' => $line, 'endLine' => $line + \substr_count($comment, "\n"), 'startFilePos' => $filePos, 'endFilePos' => $filePos + \strlen($comment)]));
        ...
        if (\count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new \ECSPrefix20210804\PhpParser\Error('Unterminated comment', ['startLine' => $line - \substr_count($lastToken[1], "\n"), 'endLine' => $line, 'startFilePos' => $filePos - \strlen($lastToken[1]), 'endFilePos' => $filePos]));
            }
        }
    }
    /**
            ...
            if (\is_string($token)) {
                ...
                if (\T_COMMENT === $token[0] || \T_DOC_COMMENT === $token[0]) {
                    if ($this->attributeCommentsUsed) {
                        $comment = \T_DOC_COMMENT === $token[0] ? new \ECSPrefix20210804\PhpParser\Comment\Doc($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos) : new \ECSPrefix20210804\PhpParser\Comment($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos);
    ...
    public function handleHaltCompiler() : string
    {
        ...
        if (!\preg_match('~^\\s*\\(\\s*\\)\\s*(?:;|\\?>\\r?\\n?)~', $textAfter, $matches)) {
            throw new \ECSPrefix20210804\PhpParser\Error('__HALT_COMPILER must be followed by "();"');
        }
    ...
    protected function createTokenMap() : array
    {
        ...
        for ($i = 256; $i < 1000; ++$i) {
            if (\T_DOUBLE_COLON === $i) {
                ...
                $tokenMap[$i] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_PAAMAYIM_NEKUDOTAYIM;
            } elseif (\T_OPEN_TAG_WITH_ECHO === $i) {
                // T_OPEN_TAG_WITH_ECHO with dropped T_OPEN_TAG results in T_ECHO
                $tokenMap[$i] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_ECHO;
                ...
                if ('T_HASHBANG' === $name) {
                    ...
                    $tokenMap[$i] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_INLINE_HTML;
                } elseif (\defined($name = \ECSPrefix20210804\PhpParser\Parser\Tokens::class . '::' . $name)) {
        ...
        if (\defined('T_ONUMBER')) {
            $tokenMap[\T_ONUMBER] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_DNUMBER;
        }
        // HHVM also has a separate token for the __COMPILER_HALT_OFFSET__ constant
        if (\defined('T_COMPILER_HALT_OFFSET')) {
            $tokenMap[\T_COMPILER_HALT_OFFSET] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_STRING;
        }
        // Assign tokens for which we define compatibility constants, as token_name() does not know them.
        $tokenMap[\T_FN] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_FN;
        $tokenMap[\T_COALESCE_EQUAL] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_COALESCE_EQUAL;
        $tokenMap[\T_NAME_QUALIFIED] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_NAME_QUALIFIED;
        $tokenMap[\T_NAME_FULLY_QUALIFIED] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_NAME_FULLY_QUALIFIED;
        $tokenMap[\T_NAME_RELATIVE] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_NAME_RELATIVE;
        $tokenMap[\T_MATCH] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_MATCH;
        $tokenMap[\T_NULLSAFE_OBJECT_OPERATOR] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_NULLSAFE_OBJECT_OPERATOR;
        $tokenMap[\T_ATTRIBUTE] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_ATTRIBUTE;
        $tokenMap[\T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG;
        $tokenMap[\T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG;
        $tokenMap[\T_ENUM] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_ENUM;
        $tokenMap[\T_READONLY] = \ECSPrefix20210804\PhpParser\Parser\Tokens::T_READONLY;
        return $tokenMap;
    }
