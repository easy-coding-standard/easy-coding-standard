...
class Trait_ extends \ECSPrefix20210804\PhpParser\Builder\Declaration
{
    protected $name;
    protected $uses = [];
    ...
    public function addStmt($stmt)
    {
        ...
        if ($stmt instanceof \ECSPrefix20210804\PhpParser\Node\Stmt\Property) {
            ...
            $this->methods[] = $stmt;
        } elseif ($stmt instanceof \ECSPrefix20210804\PhpParser\Node\Stmt\TraitUse) {
            $this->uses[] = $stmt;
        } else {
    ...
    {
        return new \ECSPrefix20210804\PhpParser\Node\Stmt\Trait_($this->name, ['stmts' => \array_merge($this->uses, $this->properties, $this->methods), 'attrGroups' => $this->attributeGroups], $this->attributes);
    }
}
