...
class XdebugHandler
{
    ...
    public function __construct($envPrefix)
    {
        ...
        $this->statusWriter = new \ECSPrefix20210906\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
    ...
    {
        $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::CHECK, $this->loaded . '|' . $this->mode);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (empty($envArgs[0]) && $this->requiresRestart(self::$xdebugActive)) {
            // Restart required
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::RESTARTED);
            \ECSPrefix20210906\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
            if (!$this->loaded) {
        ...
        $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::NORESTART);
    ...
    {
        $this->tryEnableSignals();
        $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::RESTARTING, \implode(' ', $command));
        if (\PHP_VERSION_ID >= 70400) {
            $cmd = $command;
        } else {
            $cmd = \ECSPrefix20210906\Composer\XdebugHandler\Process::escapeShellCommand($command);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
            @\unlink($this->tmpIni);
        }
        exit($exitCode);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function setEnvRestartSettings($envArgs)
    {
        ...
        \ECSPrefix20210906\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings)
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \ECSPrefix20210906\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\ECSPrefix20210906\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
