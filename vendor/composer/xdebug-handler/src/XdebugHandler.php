...
class XdebugHandler
{
    ...
    public function __construct(string $envPrefix)
    {
        ...
        $this->statusWriter = new \ECSPrefix20220416\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
     * Activates status message output to a PSR3 logger
     */
    public function setLogger(\ECSPrefix20220416\Psr\Log\LoggerInterface $logger) : self
    {
    ...
    {
        $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::CHECK, self::$xdebugVersion . '|' . self::$xdebugMode);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (!(bool) $envArgs[0] && $this->requiresRestart(self::$xdebugActive)) {
            // Restart required
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::RESTARTED);
            \ECSPrefix20220416\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
        ...
        $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::NORESTART);
    ...
    {
        $this->tryEnableSignals();
        $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::RESTARTING, \implode(' ', $command));
        if (\PHP_VERSION_ID >= 70400) {
            $cmd = $command;
        } else {
            $cmd = \ECSPrefix20220416\Composer\XdebugHandler\Process::escapeShellCommand($command);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
    ...
    private function prepareRestart() : bool
    {
        ...
        if ($error !== null) {
            $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return $error === null;
    }
    /**
    ...
    private function writeTmpIni(array $iniFiles, string $tmpDir, ?string &$error) : bool
    {
        ...
        foreach ($iniFiles as $file) {
            ...
            if (\ECSPrefix20220416\Composer\Pcre\Preg::isMatchWithOffsets($sectionRegex, $data, $matches, \PREG_OFFSET_CAPTURE)) {
                $data = \substr($data, 0, $matches[0][1]);
            }
            $content .= \ECSPrefix20220416\Composer\Pcre\Preg::replace($xdebugRegex, ';$1', $data) . \PHP_EOL;
        }
    ...
    private function setEnvRestartSettings(array $envArgs) : void
    {
        ...
        \ECSPrefix20220416\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings) : void
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \ECSPrefix20220416\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\ECSPrefix20220416\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
    ...
    private static function setXdebugDetails() : void
    {
        ...
        if (\ECSPrefix20220416\Composer\Pcre\Preg::isMatch('/^,+$/', \str_replace(' ', '', self::$xdebugMode))) {
            self::$xdebugMode = 'off';
        }
        self::$xdebugActive = self::$xdebugMode !== 'off';
    }
