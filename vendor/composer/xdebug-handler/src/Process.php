...
use ECSPrefix20211223\Composer\Pcre\Preg;
/**
...
class Process
{
    ...
    public static function escape($arg, $meta = \true, $module = \false)
    {
        ...
        $arg = \ECSPrefix20211223\Composer\Pcre\Preg::replace('/(\\\\*)"/', '$1$1\\"', $arg, -1, $dquotes);
        if ($meta) {
            $meta = $dquotes || \ECSPrefix20211223\Composer\Pcre\Preg::isMatch('/%[^%]+%/', $arg);
            if (!$meta) {
        ...
        if ($quote) {
            $arg = '"' . \ECSPrefix20211223\Composer\Pcre\Preg::replace('/(\\\\*)$/', '$1$1', $arg) . '"';
        }
        if ($meta) {
            $arg = \ECSPrefix20211223\Composer\Pcre\Preg::replace('/(["^&|<>()%])/', '^$1', $arg);
        }
        return $arg;
    }
    /**
