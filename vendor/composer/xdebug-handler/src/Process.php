...
namespace ECSPrefix20220305\Composer\XdebugHandler;

use ECSPrefix20220305\Composer\Pcre\Preg;
/**
...
class Process
{
    ...
    public static function escape(string $arg, bool $meta = \true, bool $module = \false) : string
    {
        ...
        $arg = \ECSPrefix20220305\Composer\Pcre\Preg::replace('/(\\\\*)"/', '$1$1\\"', $arg, -1, $dquotes);
        if ($meta) {
            $meta = $dquotes || \ECSPrefix20220305\Composer\Pcre\Preg::isMatch('/%[^%]+%/', $arg);
            if (!$meta) {
        ...
        if ($quote) {
            $arg = '"' . \ECSPrefix20220305\Composer\Pcre\Preg::replace('/(\\\\*)$/', '$1$1', $arg) . '"';
        }
        if ($meta) {
            $arg = \ECSPrefix20220305\Composer\Pcre\Preg::replace('/(["^&|<>()%])/', '^$1', $arg);
        }
        return $arg;
    }
    /**
