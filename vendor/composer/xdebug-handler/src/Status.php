...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \ECSPrefix20210507\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \ECSPrefix20210507\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \ECSPrefix20210507\Psr\Log\LogLevel::WARNING : null;
