...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \ECSPrefix20210601\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \ECSPrefix20210601\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \ECSPrefix20210601\Psr\Log\LogLevel::WARNING);
    }
    ...
    {
        $this->output($this->getLoadedMessage());
        \ECSPrefix20210601\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    private function reportRestarted()
    {
        $loaded = $this->getLoadedMessage();
        $text = \sprintf('Restarted (%d ms). %s', $this->time, $loaded);
        $level = $this->loaded ? \ECSPrefix20210601\Psr\Log\LogLevel::WARNING : null;
