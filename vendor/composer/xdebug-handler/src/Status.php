...
class Status
{
    ...
    public function __construct(string $envAllowXdebug, bool $debug)
    {
        ...
        \ECSPrefix20220325\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger !== null) {
            $this->logger->log($level !== null ? $level : \ECSPrefix20220325\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \ECSPrefix20220325\Psr\Log\LogLevel::WARNING);
    }
    /**
     * Info status message
     */
    ...
    {
        $this->output($this->getLoadedMessage());
        \ECSPrefix20220325\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    /**
    ...
    private function reportRestarted() : void
    {
        ...
        $level = $this->loaded !== null ? \ECSPrefix20220325\Psr\Log\LogLevel::WARNING : null;
