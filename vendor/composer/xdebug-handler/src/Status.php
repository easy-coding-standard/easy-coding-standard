...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \ECSPrefix20211212\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger !== null) {
            $this->logger->log($level !== null ? $level : \ECSPrefix20211212\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \ECSPrefix20211212\Psr\Log\LogLevel::WARNING);
    }
    /**
     * @param string $info
     *
    ...
    {
        $this->output($this->getLoadedMessage());
        \ECSPrefix20211212\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    /**
     * @return void
     */
    private function reportRestarted()
    {
        ...
        $level = $this->loaded !== null ? \ECSPrefix20211212\Psr\Log\LogLevel::WARNING : null;
