 ...
 */
namespace ECSPrefix20220111\Composer\Semver\Constraint;

/**
 * Defines a conjunctive or disjunctive set of constraints.
 */
class MultiConstraint implements \ECSPrefix20220111\Composer\Semver\Constraint\ConstraintInterface
{
    /**
     ...
     * @return bool
     */
    public function matches(\ECSPrefix20220111\Composer\Semver\Constraint\ConstraintInterface $provider)
    {
        ...
        if ($provider instanceof \ECSPrefix20220111\Composer\Semver\Constraint\MultiConstraint && $provider->isDisjunctive()) {
    ...
    public static function create(array $constraints, $conjunctive = \true)
    {
        if (0 === \count($constraints)) {
            return new \ECSPrefix20220111\Composer\Semver\Constraint\MatchAllConstraint();
        }
        ...
        if (!$conjunctive) {
            ...
            for ($i = 1, $l = \count($constraints); $i < $l; $i++) {
                ...
                if ($left instanceof self && $left->conjunctive && $right instanceof self && $right->conjunctive && \count($left->constraints) === 2 && \count($right->constraints) === 2 && ($left0 = (string) $left->constraints[0]) && $left0[0] === '>' && $left0[1] === '=' && ($left1 = (string) $left->constraints[1]) && $left1[0] === '<' && ($right0 = (string) $right->constraints[0]) && $right0[0] === '>' && $right0[1] === '=' && ($right1 = (string) $right->constraints[1]) && $right1[0] === '<' && \substr($left1, 2) === \substr($right0, 3)) {
                    $optimized = \true;
                    $left = new \ECSPrefix20220111\Composer\Semver\Constraint\MultiConstraint(array($left->constraints[0], $right->constraints[1]), \true);
                } else {
