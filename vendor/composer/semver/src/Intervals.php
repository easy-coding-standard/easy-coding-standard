...
use ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint;
...
class Intervals
{
    ...
    {
        if ($constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($candidate instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint || $constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::get(new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint(array($candidate, $constraint), \true));
    ...
    {
        if ($a instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchAllConstraint || $b instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($a instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint || $b instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::generateIntervals(new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint(array($a, $b), \true), \true);
    ...
    {
        if (!$constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint) {
            return $constraint;
        }
        $intervals = self::generateIntervals($constraint);
        $constraints = array();
        $hasNumericMatchAll = \false;
        if (\count($intervals['numeric']) === 1 && (string) $intervals['numeric'][0]->getStart() === (string) \ECSPrefix20211209\Composer\Semver\Interval::fromZero() && (string) $intervals['numeric'][0]->getEnd() === (string) \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity()) {
            ...
            for ($i = 0, $count = \count($intervals['numeric']); $i < $count; $i++) {
                ...
                if ($interval->getEnd()->getOperator() === '<' && $i + 1 < $count) {
                    ...
                    if ($interval->getEnd()->getVersion() === $nextInterval->getStart()->getVersion() && $nextInterval->getStart()->getOperator() === '>') {
                        ...
                        if (\count($unEqualConstraints) === 0 && (string) $interval->getStart() !== (string) \ECSPrefix20211209\Composer\Semver\Interval::fromZero()) {
                            $unEqualConstraints[] = $interval->getStart();
                        }
                        $unEqualConstraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('!=', $interval->getEnd()->getVersion());
                        continue;
                    }
                }
                if (\count($unEqualConstraints) > 0) {
                    // this is where the end of the following interval of a != constraint is added as explained above
                    if ((string) $interval->getEnd() !== (string) \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity()) {
                        $unEqualConstraints[] = $interval->getEnd();
                    }
                    // count is 1 if entire constraint is just one != expression
                    if (\count($unEqualConstraints) > 1) {
                        $constraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint($unEqualConstraints, \true);
                    } else {
                ...
                if ($interval->getStart()->getVersion() === $interval->getEnd()->getVersion() && $interval->getStart()->getOperator() === '>=' && $interval->getEnd()->getOperator() === '<=') {
                    $constraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('==', $interval->getStart()->getVersion());
                    continue;
                }
                if ((string) $interval->getStart() === (string) \ECSPrefix20211209\Composer\Semver\Interval::fromZero()) {
                    $constraints[] = $interval->getEnd();
                } elseif ((string) $interval->getEnd() === (string) \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity()) {
                    $constraints[] = $interval->getStart();
                } else {
                    $constraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint(array($interval->getStart(), $interval->getEnd()), \true);
                }
            }
        }
        ...
        if (0 === \count($intervals['branches']['names'])) {
            ...
            foreach ($intervals['branches']['names'] as $branchName) {
                if ($intervals['branches']['exclude']) {
                    $devConstraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('!=', $branchName);
                } else {
                    $devConstraints[] = new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('==', $branchName);
                }
            }
            ...
            if ($intervals['branches']['exclude']) {
                if (\count($constraints) > 1) {
                    return new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint(\array_merge(array(new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint($constraints, \false)), $devConstraints), \true);
                }
                if (\count($constraints) === 1 && (string) $constraints[0] === (string) \ECSPrefix20211209\Composer\Semver\Interval::fromZero()) {
                    if (\count($devConstraints) > 1) {
                        return new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint($devConstraints, \true);
                    }
                    return $devConstraints[0];
                }
                return new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint(\array_merge($constraints, $devConstraints), \true);
            }
        ...
        if (\count($constraints) > 1) {
            return new \ECSPrefix20211209\Composer\Semver\Constraint\MultiConstraint($constraints, \false);
        }
        if (\count($constraints) === 1) {
            return $constraints[0];
        }
        return new \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint();
    }
    /**
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}
     */
    private static function generateIntervals(\ECSPrefix20211209\Composer\Semver\Constraint\ConstraintInterface $constraint, $stopOnFirstValidInterval = \false)
    {
        if ($constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchAllConstraint) {
            return array('numeric' => array(new \ECSPrefix20211209\Composer\Semver\Interval(\ECSPrefix20211209\Composer\Semver\Interval::fromZero(), \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \ECSPrefix20211209\Composer\Semver\Interval::anyDev());
        }
        if ($constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\MatchNoneConstraint) {
            return array('numeric' => array(), 'branches' => array('names' => array(), 'exclude' => \false));
        }
        if ($constraint instanceof \ECSPrefix20211209\Composer\Semver\Constraint\Constraint) {
        ...
        if ($constraint->isDisjunctive()) {
            $branches = \ECSPrefix20211209\Composer\Semver\Interval::noDev();
            ...
            $branches = \ECSPrefix20211209\Composer\Semver\Interval::anyDev();
            foreach ($constraintBranches as $b) {
                if ($b['exclude']) {
                    if ($branches['exclude']) {
        ...
        foreach ($borders as $border) {
            ...
            if (!$start && $activeIntervals >= $activationThreshold) {
                $start = new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint($border['operator'], $border['version']);
                ...
                if (\version_compare($start->getVersion(), $border['version'], '=') && ($start->getOperator() === '>' && $border['operator'] === '<=' || $start->getOperator() === '>=' && $border['operator'] === '<')) {
                    ...
                    $intervals[$index] = new \ECSPrefix20211209\Composer\Semver\Interval($start, new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint($border['operator'], $border['version']));
                    $index++;
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}}
     */
    private static function generateSingleConstraintIntervals(\ECSPrefix20211209\Composer\Semver\Constraint\Constraint $constraint)
    {
        ...
        if (\strpos($constraint->getVersion(), 'dev-') === 0) {
            ...
            if ($op === '!=') {
                $intervals[] = new \ECSPrefix20211209\Composer\Semver\Interval(\ECSPrefix20211209\Composer\Semver\Interval::fromZero(), \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity());
        ...
        if ($op[0] === '>') {
            // > & >=
            return array('numeric' => array(new \ECSPrefix20211209\Composer\Semver\Interval($constraint, \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \ECSPrefix20211209\Composer\Semver\Interval::noDev());
        }
        if ($op[0] === '<') {
            // < & <=
            return array('numeric' => array(new \ECSPrefix20211209\Composer\Semver\Interval(\ECSPrefix20211209\Composer\Semver\Interval::fromZero(), $constraint)), 'branches' => \ECSPrefix20211209\Composer\Semver\Interval::noDev());
        }
        if ($op === '!=') {
            // convert !=x to intervals of 0 - <x && >x - +inf + dev*
            return array('numeric' => array(new \ECSPrefix20211209\Composer\Semver\Interval(\ECSPrefix20211209\Composer\Semver\Interval::fromZero(), new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('<', $constraint->getVersion())), new \ECSPrefix20211209\Composer\Semver\Interval(new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('>', $constraint->getVersion()), \ECSPrefix20211209\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \ECSPrefix20211209\Composer\Semver\Interval::anyDev());
        }
        // convert ==x to an interval of >=x - <=x
        return array('numeric' => array(new \ECSPrefix20211209\Composer\Semver\Interval(new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('>=', $constraint->getVersion()), new \ECSPrefix20211209\Composer\Semver\Constraint\Constraint('<=', $constraint->getVersion()))), 'branches' => \ECSPrefix20211209\Composer\Semver\Interval::noDev());
    }
}
