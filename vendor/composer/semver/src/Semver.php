...
class Semver
{
    ...
    public static function satisfies($version, $constraints)
    {
        if (null === self::$versionParser) {
            self::$versionParser = new \ECSPrefix20220517\Composer\Semver\VersionParser();
        }
        $versionParser = self::$versionParser;
        $provider = new \ECSPrefix20220517\Composer\Semver\Constraint\Constraint('==', $versionParser->normalize($version));
    ...
    public static function satisfiedBy(array $versions, $constraints)
    {
        $versions = \array_filter($versions, function ($version) use($constraints) {
            return \ECSPrefix20220517\Composer\Semver\Semver::satisfies($version, $constraints);
        });
    ...
    private static function usort(array $versions, $direction)
    {
        ...
        \usort($normalized, function (array $left, array $right) use($direction) {
            ...
            if (\ECSPrefix20220517\Composer\Semver\Comparator::lessThan($left[0], $right[0])) {
                return -$direction;
            }
            return $direction;
        });
