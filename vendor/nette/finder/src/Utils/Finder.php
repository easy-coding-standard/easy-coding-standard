...
class Finder implements \IteratorAggregate, \Countable
{
    ...
    {
        if ($this->paths) {
            throw new \ECSPrefix20210526\Nette\InvalidStateException('Directory to search has already been specified.');
        }
    ...
    {
        if (!$this->paths) {
            throw new \ECSPrefix20210526\Nette\InvalidStateException('Call in() or from() to specify directory to search.');
    ...
    public function size(string $operator, int $size = null)
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?((?:\\d*\\.)?\\d+)\\s*(K|M|G|)B?$#Di', $operator, $matches)) {
                throw new \ECSPrefix20210526\Nette\InvalidArgumentException('Invalid size predicate format.');
            }
    ...
    public function date(string $operator, $date = null)
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?(.+)$#Di', $operator, $matches)) {
                throw new \ECSPrefix20210526\Nette\InvalidArgumentException('Invalid date predicate format.');
            }
            list(, $operator, $date) = $matches;
            $operator = $operator ?: '=';
        }
        $date = \ECSPrefix20210526\Nette\Utils\DateTime::from($date)->format('U');
    ...
    public static function compare($l, string $operator, $r) : bool
    {
        switch ($operator) {
            ...
            case '<>':
                return $l != $r;
            default:
                throw new \ECSPrefix20210526\Nette\InvalidArgumentException("Unknown operator {$operator}.");
        }
    }
    /********************* extension methods ****************d*g**/
    public function __call(string $name, array $args)
    {
        return isset(self::$extMethods[$name]) ? self::$extMethods[$name]($this, ...$args) : \ECSPrefix20210526\Nette\Utils\ObjectHelpers::strictCall(\get_class($this), $name, \array_keys(self::$extMethods));
    }
    /**
     * @return void
     */
