...
class Finder implements \IteratorAggregate, \Countable
{
    ...
    {
        if ($this->paths) {
            throw new \ECSPrefix20210514\Nette\InvalidStateException('Directory to search has already been specified.');
        }
    ...
    {
        if (!$this->paths) {
            throw new \ECSPrefix20210514\Nette\InvalidStateException('Call in() or from() to specify directory to search.');
    ...
    public function size($operator, $size = null)
    {
        ...
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?((?:\\d*\\.)?\\d+)\\s*(K|M|G|)B?$#Di', $operator, $matches)) {
                throw new \ECSPrefix20210514\Nette\InvalidArgumentException('Invalid size predicate format.');
            }
    ...
    public function date($operator, $date = null)
    {
        ...
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?(.+)$#Di', $operator, $matches)) {
                throw new \ECSPrefix20210514\Nette\InvalidArgumentException('Invalid date predicate format.');
            }
            list(, $operator, $date) = $matches;
            $operator = $operator ?: '=';
        }
        $date = \ECSPrefix20210514\Nette\Utils\DateTime::from($date)->format('U');
    ...
    public static function compare($l, $operator, $r)
    {
        ...
        switch ($operator) {
            ...
            case '<>':
                return $l != $r;
            default:
                throw new \ECSPrefix20210514\Nette\InvalidArgumentException("Unknown operator {$operator}.");
        }
    }
    ...
    {
        $name = (string) $name;
        return isset(self::$extMethods[$name]) ? self::$extMethods[$name]($this, ...$args) : \ECSPrefix20210514\Nette\Utils\ObjectHelpers::strictCall(\get_class($this), $name, \array_keys(self::$extMethods));
    }
    /**
     * @return void
     * @param string $name
