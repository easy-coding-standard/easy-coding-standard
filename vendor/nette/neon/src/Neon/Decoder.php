...
final class Decoder
{
    ...
    public function decode(string $input)
    {
        ...
        if ($this->tokens === \false) {
            throw new \ECSPrefix20210706\Nette\Neon\Exception('Invalid UTF-8 sequence.');
        }
        ...
        for (; $n < $count; $n++) {
            ...
            if ($t === ',') {
                ...
                if ($hasValue) {
                    ...
                    $n++;
                    if ($value instanceof \ECSPrefix20210706\Nette\Neon\Entity && $value->value === \ECSPrefix20210706\Nette\Neon\Neon::CHAIN) {
                        \end($value->attributes)->attributes = $this->parse(\false, []);
                    } else {
                        $value = new \ECSPrefix20210706\Nette\Neon\Entity($value, $this->parse(\false, []));
                    }
                } else {
                    $n++;
                ...
                if ($hasValue) {
                    if ($value instanceof \ECSPrefix20210706\Nette\Neon\Entity) {
                        // Entity chaining
                        if ($value->value !== \ECSPrefix20210706\Nette\Neon\Neon::CHAIN) {
                            $value = new \ECSPrefix20210706\Nette\Neon\Entity(\ECSPrefix20210706\Nette\Neon\Neon::CHAIN, [$value]);
                        }
                        $value->attributes[] = new \ECSPrefix20210706\Nette\Neon\Entity($converted);
                    } else {
                        $this->error();
                    }
                } else {
    ...
    private function error(string $message = "Unexpected '%s'")
    {
        ...
        throw new \ECSPrefix20210706\Nette\Neon\Exception(\str_replace('%s', $token, $message) . " on line {$line}, column {$col}.");
    }
}
