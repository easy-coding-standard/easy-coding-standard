...
final class Traverser
{
     ...
     */
    public function traverse(\ECSPrefix20220325\Nette\Neon\Node $node, ?callable $enter = null, ?callable $leave = null) : \ECSPrefix20220325\Nette\Neon\Node
    {
        $this->enter = $enter;
        $this->leave = $leave;
        $this->stop = \false;
    ...
    private function traverseNode(\ECSPrefix20220325\Nette\Neon\Node $node) : \ECSPrefix20220325\Nette\Neon\Node
    {
        $children = \true;
        if ($this->enter) {
            $res = ($this->enter)($node);
            if ($res instanceof \ECSPrefix20220325\Nette\Neon\Node) {
                $node = $res;
