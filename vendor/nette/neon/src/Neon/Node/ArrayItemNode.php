...
/** @internal */
final class ArrayItemNode extends \ECSPrefix20211128\Nette\Neon\Node
{
    /** @var ?Node */
    public $key;
    /** @var Node */
    ...
    public static function itemsToBlockString($items) : string
    {
        ...
        foreach ($items as $item) {
            ...
            $res .= ($item->key ? $item->key->toString() . ':' : '-') . ($item->value instanceof \ECSPrefix20211128\Nette\Neon\Node\BlockArrayNode && $item->value->items ? "\n" . $v . (\substr($v, -2, 1) === "\n" ? '' : "\n") : ' ' . $v . "\n");
        }
        return $res;
    }
