...
namespace ECSPrefix20220303\Nette\Neon\Node;

use ECSPrefix20220303\Nette;
use ECSPrefix20220303\Nette\Neon\Node;
/** @internal */
final class StringNode extends \ECSPrefix20220303\Nette\Neon\Node
{
    ...
    public static function parse(string $s) : string
    {
        ...
        return \preg_replace_callback('#\\\\(?:ud[89ab][0-9a-f]{2}\\\\ud[c-f][0-9a-f]{2}|u[0-9a-f]{4}|x[0-9a-f]{2}|.)#i', function (array $m) : string {
            ...
            if (isset(self::ESCAPE_SEQUENCES[$sq[1]])) {
                ...
                throw new \ECSPrefix20220303\Nette\Neon\Exception("Invalid UTF-8 sequence {$sq}");
            } elseif ($sq[1] === 'x' && \strlen($sq) === 4) {
                \trigger_error("Neon: '{$sq}' is deprecated, use '\\uXXXX' instead.", \E_USER_DEPRECATED);
                return \chr(\hexdec(\substr($sq, 2)));
            } else {
                throw new \ECSPrefix20220303\Nette\Neon\Exception("Invalid escaping sequence {$sq}");
            }
        }, $res);
    }
    public function toString() : string
    {
        ...
        if ($res === \false) {
            throw new \ECSPrefix20220303\Nette\Neon\Exception('Invalid UTF-8 sequence: ' . $this->value);
        }
