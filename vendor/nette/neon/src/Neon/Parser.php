...
final class Parser
{
    ...
    /** @var TokenStream */
    private $tokens;
    public function parse(\ECSPrefix20211031\Nette\Neon\TokenStream $tokens) : \ECSPrefix20211031\Nette\Neon\Node
    {
        $this->tokens = $tokens;
        while ($this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE)) {
        }
    ...
    private function parseBlock(string $indent, bool $onlyBullets = \false) : \ECSPrefix20211031\Nette\Neon\Node
    {
        $res = new \ECSPrefix20211031\Nette\Neon\Node\ArrayNode($indent, $this->tokens->getPos());
        $keyCheck = [];
        loop:
        $item = new \ECSPrefix20211031\Nette\Neon\Node\ArrayItemNode($this->tokens->getPos());
        if ($this->tokens->consume('-')) {
            // continue
        } elseif (!$this->tokens->isNext() || $onlyBullets) {
            return $res->items ? $res : new \ECSPrefix20211031\Nette\Neon\Node\LiteralNode(null, $this->tokens->getPos());
        } else {
        ...
        $res->items[] = $item;
        $item->value = new \ECSPrefix20211031\Nette\Neon\Node\LiteralNode(null, $this->tokens->getPos());
        if ($this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE)) {
            while ($this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE)) {
            }
            ...
            if ($this->tokens->isNext() && !$this->tokens->isNext(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE)) {
                $this->tokens->error();
            }
        }
    ...
    private function parseValue() : \ECSPrefix20211031\Nette\Neon\Node
    {
        if ($token = $this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::STRING)) {
            $node = new \ECSPrefix20211031\Nette\Neon\Node\StringNode($this->decodeString($token->value), $this->tokens->getPos() - 1);
        } elseif ($token = $this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::LITERAL)) {
            $pos = $this->tokens->getPos() - 1;
            $node = new \ECSPrefix20211031\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value, $this->tokens->isNext(':', '=')), $pos);
        } elseif ($this->tokens->isNext('[', '(', '{')) {
    ...
    private function parseEntity(\ECSPrefix20211031\Nette\Neon\Node $node) : \ECSPrefix20211031\Nette\Neon\Node
    {
        if (!$this->tokens->isNext('(')) {
            return $node;
        }
        $attributes = $this->parseBraces();
        $entities[] = new \ECSPrefix20211031\Nette\Neon\Node\EntityNode($node, $attributes->items, $node->startPos, $attributes->endPos);
        while ($token = $this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::LITERAL)) {
            $valueNode = new \ECSPrefix20211031\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value), $this->tokens->getPos() - 1);
            if ($this->tokens->isNext('(')) {
                $attributes = $this->parseBraces();
                $entities[] = new \ECSPrefix20211031\Nette\Neon\Node\EntityNode($valueNode, $attributes->items, $valueNode->startPos, $attributes->endPos);
            } else {
                $entities[] = new \ECSPrefix20211031\Nette\Neon\Node\EntityNode($valueNode, [], $valueNode->startPos);
                break;
            }
        }
        return \count($entities) === 1 ? $entities[0] : new \ECSPrefix20211031\Nette\Neon\Node\EntityChainNode($entities, $node->startPos, \end($entities)->endPos);
    }
    private function parseBraces() : \ECSPrefix20211031\Nette\Neon\Node\ArrayNode
    {
        $token = $this->tokens->consume();
        $endBrace = ['[' => ']', '{' => '}', '(' => ')'][$token->value];
        $res = new \ECSPrefix20211031\Nette\Neon\Node\ArrayNode(null, $this->tokens->getPos() - 1);
        $keyCheck = [];
        loop:
        while ($this->tokens->consume(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE)) {
        }
        ...
        $res->items[] = $item = new \ECSPrefix20211031\Nette\Neon\Node\ArrayItemNode($this->tokens->getPos());
        $value = $this->parseValue();
        if ($this->tokens->consume(':', '=')) {
            $this->checkArrayKey($value, $keyCheck);
            $item->key = $value;
            $item->value = $this->tokens->isNext(\ECSPrefix20211031\Nette\Neon\Token::NEWLINE, ',', $endBrace) ? new \ECSPrefix20211031\Nette\Neon\Node\LiteralNode(null, $this->tokens->getPos()) : $this->parseValue();
        } else {
            $item->value = $value;
        }
    ...
    private function checkArrayKey(\ECSPrefix20211031\Nette\Neon\Node $key, array &$arr) : void
    {
        if (!$key instanceof \ECSPrefix20211031\Nette\Neon\Node\StringNode && !$key instanceof \ECSPrefix20211031\Nette\Neon\Node\LiteralNode || !\is_scalar($key->value)) {
