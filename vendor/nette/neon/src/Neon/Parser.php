...
final class Parser
{
    ...
    private function parseValue() : \ECSPrefix20211123\Nette\Neon\Node
    {
        if ($token = $this->tokens->consume(\ECSPrefix20211123\Nette\Neon\Token::STRING)) {
            $node = new \ECSPrefix20211123\Nette\Neon\Node\StringNode($this->decodeString($token->value), $this->tokens->getPos() - 1);
        } elseif ($token = $this->tokens->consume(\ECSPrefix20211123\Nette\Neon\Token::LITERAL)) {
            $pos = $this->tokens->getPos() - 1;
            $node = new \ECSPrefix20211123\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value, $this->tokens->isNext(':', '=')), $pos);
        } elseif ($this->tokens->isNext('[', '(', '{')) {
    ...
    private function parseEntity(\ECSPrefix20211123\Nette\Neon\Node $node) : \ECSPrefix20211123\Nette\Neon\Node
    {
        ...
        while ($token = $this->tokens->consume(\ECSPrefix20211123\Nette\Neon\Token::LITERAL)) {
            $valueNode = new \ECSPrefix20211123\Nette\Neon\Node\LiteralNode($this->literalToValue($token->value), $this->tokens->getPos() - 1);
            if ($this->tokens->isNext('(')) {
                $attributes = $this->parseBraces();
