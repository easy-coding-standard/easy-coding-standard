...
final class Encoder
{
    ...
    public function encode($var, int $flags = 0) : string
    {
        ...
        } elseif ($var instanceof \ECSPrefix20210627\Nette\Neon\Entity) {
            if ($var->value === \ECSPrefix20210627\Nette\Neon\Neon::CHAIN) {
        ...
        if (\is_array($var)) {
            ...
            if (!\preg_match('~[\\x00-\\x1F]|^[+-.]?\\d|^(true|false|yes|no|on|off|null)$~Di', $var) && \preg_match('~^' . \ECSPrefix20210627\Nette\Neon\Decoder::PATTERNS[1] . '$~Dx', $var)) {
                return $var;
            }
            $res = \json_encode($var, \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES);
            if ($res === \false) {
                throw new \ECSPrefix20210627\Nette\Neon\Exception('Invalid UTF-8 sequence: ' . $var);
            }
            if (\strpos($var, "\n") !== \false) {
