...
class OutputHelper
{
    ...
    /** @var string */
    private $key;
    public function __construct(\ECSPrefix20210517\Nette\Caching\Cache $cache, $key)
    {
        $this->cache = $cache;
        $this->key = $key;
        \ob_start();
    ...
    {
        if ($this->cache === null) {
            throw new \ECSPrefix20210517\Nette\InvalidStateException('Output cache has already been saved.');
        }
