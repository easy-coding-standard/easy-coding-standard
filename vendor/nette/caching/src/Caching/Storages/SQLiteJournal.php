 ...
 */
class SQLiteJournal implements \ECSPrefix20210516\Nette\Caching\Storages\Journal
{
    ...
    public function __construct($path)
    {
        ...
        if (!\extension_loaded('pdo_sqlite')) {
            throw new \ECSPrefix20210516\Nette\NotSupportedException('SQLiteJournal requires PHP extension pdo_sqlite which is not loaded.');
        }
        $this->path = $path;
    }
    /**
    ...
    public function write($key, array $dependencies)
    {
        ...
        if (!empty($dependencies[\ECSPrefix20210516\Nette\Caching\Cache::PRIORITY])) {
            $this->pdo->prepare('REPLACE INTO priorities (key, priority) VALUES (?, ?)')->execute([$key, (int) $dependencies[\ECSPrefix20210516\Nette\Caching\Cache::PRIORITY]]);
        }
    ...
    public function clean(array $conditions)
    {
        ...
        if (!empty($conditions[\ECSPrefix20210516\Nette\Caching\Cache::TAGS])) {
            $tags = (array) $conditions[\ECSPrefix20210516\Nette\Caching\Cache::TAGS];
            $unions[] = 'SELECT DISTINCT key FROM tags WHERE tag IN (?' . \str_repeat(', ?', \count($tags) - 1) . ')';
            $args = $tags;
        }
        if (!empty($conditions[\ECSPrefix20210516\Nette\Caching\Cache::PRIORITY])) {
            $unions[] = 'SELECT DISTINCT key FROM priorities WHERE priority <= ?';
            $args[] = (int) $conditions[\ECSPrefix20210516\Nette\Caching\Cache::PRIORITY];
        }
        if (empty($unions)) {
            return [];
        }
