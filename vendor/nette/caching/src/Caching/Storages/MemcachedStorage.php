 ...
 */
class MemcachedStorage implements \ECSPrefix20210606\Nette\Caching\Storage, \ECSPrefix20210606\Nette\Caching\BulkReader
{
    ...
    public function __construct(string $host = 'localhost', int $port = 11211, string $prefix = '', \ECSPrefix20210606\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!static::isAvailable()) {
            throw new \ECSPrefix20210606\Nette\NotSupportedException("PHP extension 'memcached' is not loaded.");
        }
        $this->prefix = $prefix;
    ...
    public function addServer(string $host = 'localhost', int $port = 11211)
    {
        if (@$this->memcached->addServer($host, $port, 1) === \false) {
            ...
            throw new \ECSPrefix20210606\Nette\InvalidStateException("Memcached::addServer(): {$error['message']}.");
        }
    }
    ...
    public function write(string $key, $data, array $dp)
    {
        if (isset($dp[\ECSPrefix20210606\Nette\Caching\Cache::ITEMS])) {
            throw new \ECSPrefix20210606\Nette\NotSupportedException('Dependent items are not supported by MemcachedStorage.');
        }
        $key = \urlencode($this->prefix . $key);
        $meta = [self::META_DATA => $data];
        $expire = 0;
        if (isset($dp[\ECSPrefix20210606\Nette\Caching\Cache::EXPIRATION])) {
            $expire = (int) $dp[\ECSPrefix20210606\Nette\Caching\Cache::EXPIRATION];
