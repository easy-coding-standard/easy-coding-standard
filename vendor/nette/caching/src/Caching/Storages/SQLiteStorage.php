 ...
 * SQLite storage.
 */
class SQLiteStorage implements \ECSPrefix20210604\Nette\Caching\Storage, \ECSPrefix20210604\Nette\Caching\BulkReader
{
    ...
    {
        $expire = isset($dependencies[\ECSPrefix20210604\Nette\Caching\Cache::EXPIRATION]) ? $dependencies[\ECSPrefix20210604\Nette\Caching\Cache::EXPIRATION] + \time() : null;
        $slide = isset($dependencies[\ECSPrefix20210604\Nette\Caching\Cache::SLIDING]) ? $dependencies[\ECSPrefix20210604\Nette\Caching\Cache::EXPIRATION] : null;
        $this->pdo->exec('BEGIN TRANSACTION');
        $this->pdo->prepare('REPLACE INTO cache (key, data, expire, slide) VALUES (?, ?, ?, ?)')->execute([$key, \serialize($data), $expire, $slide]);
        if (!empty($dependencies[\ECSPrefix20210604\Nette\Caching\Cache::TAGS])) {
            foreach ($dependencies[\ECSPrefix20210604\Nette\Caching\Cache::TAGS] as $tag) {
                $arr[] = $key;
                $arr[] = $tag;
            }
    ...
    public function clean(array $conditions)
    {
        if (!empty($conditions[\ECSPrefix20210604\Nette\Caching\Cache::ALL])) {
            ...
            $args = [\time()];
            if (!empty($conditions[\ECSPrefix20210604\Nette\Caching\Cache::TAGS])) {
                $tags = $conditions[\ECSPrefix20210604\Nette\Caching\Cache::TAGS];
