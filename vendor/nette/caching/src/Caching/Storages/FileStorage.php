 ...
 * Cache file storage.
 */
class FileStorage implements \ECSPrefix20210509\Nette\Caching\Storage
{
     ...
     * @param string $dir
     */
    public function __construct($dir, \ECSPrefix20210509\Nette\Caching\Storages\Journal $journal = null)
    {
        $dir = (string) $dir;
        if (!\is_dir($dir)) {
            throw new \ECSPrefix20210509\Nette\DirectoryNotFoundException("Directory '{$dir}' not found.");
        }
        $this->dir = $dir;
    ...
    public function write($key, $data, array $dp)
    {
        ...
        if (isset($dp[\ECSPrefix20210509\Nette\Caching\Cache::EXPIRATION])) {
            if (empty($dp[\ECSPrefix20210509\Nette\Caching\Cache::SLIDING])) {
                $meta[self::META_EXPIRE] = $dp[\ECSPrefix20210509\Nette\Caching\Cache::EXPIRATION] + \time();
                // absolute time
            } else {
                $meta[self::META_DELTA] = (int) $dp[\ECSPrefix20210509\Nette\Caching\Cache::EXPIRATION];
                // sliding time
            }
        }
        if (isset($dp[\ECSPrefix20210509\Nette\Caching\Cache::ITEMS])) {
            foreach ($dp[\ECSPrefix20210509\Nette\Caching\Cache::ITEMS] as $item) {
    ...
    {
        $all = !empty($conditions[\ECSPrefix20210509\Nette\Caching\Cache::ALL]);
        $collector = empty($conditions);
        $namespaces = isset($conditions[\ECSPrefix20210509\Nette\Caching\Cache::NAMESPACES]) ? $conditions[\ECSPrefix20210509\Nette\Caching\Cache::NAMESPACES] : null;
        // cleaning using file iterator
        if ($all || $collector) {
            $now = \time();
            foreach (\ECSPrefix20210509\Nette\Utils\Finder::find('_*')->from($this->dir)->childFirst() as $entry) {
                $path = (string) $entry;
                if ($entry->isDir()) {
            ...
            foreach ($namespaces as $namespace) {
                ...
                foreach (\ECSPrefix20210509\Nette\Utils\Finder::findFiles('_*')->in($dir) as $entry) {
