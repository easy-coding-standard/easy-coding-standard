...
class Cache
{
     ...
     */
    public function __construct(\ECSPrefix20210513\Nette\Caching\Storage $storage, $namespace = null)
    {
    ...
    public function bulkLoad(array $keys, callable $generator = null)
    {
        ...
        foreach ($keys as $key) {
            if (!\is_scalar($key)) {
                throw new \ECSPrefix20210513\Nette\InvalidArgumentException('Only scalar keys are allowed in bulkLoad()');
            }
        }
        $result = [];
        if (!$this->storage instanceof \ECSPrefix20210513\Nette\Caching\BulkReader) {
            foreach ($keys as $key) {
    ...
    private function completeDependencies($dp)
    {
        ...
        if (isset($dp[self::EXPIRATION])) {
            $dp[self::EXPIRATION] = \ECSPrefix20210513\Nette\Utils\DateTime::from($dp[self::EXPIRATION])->format('U') - \time();
        }
        // make list from TAGS
    ...
    public function capture($key)
    {
        $data = $this->load($key);
        if ($data === null) {
            return new \ECSPrefix20210513\Nette\Caching\OutputHelper($this, $key);
        }
        echo $data;
        return null;
    }
