...
class Cache
{
    ...
    /** @var string */
    private $namespace;
    public function __construct(\ECSPrefix20210520\Nette\Caching\Storage $storage, string $namespace = null)
    {
     ...
     */
    public final function getStorage() : \ECSPrefix20210520\Nette\Caching\Storage
    {
        return $this->storage;
    }
    /**
    ...
    public function bulkLoad(array $keys, callable $generator = null) : array
    {
        ...
        foreach ($keys as $key) {
            if (!\is_scalar($key)) {
                throw new \ECSPrefix20210520\Nette\InvalidArgumentException('Only scalar keys are allowed in bulkLoad()');
            }
        }
        $result = [];
        if (!$this->storage instanceof \ECSPrefix20210520\Nette\Caching\BulkReader) {
            foreach ($keys as $key) {
    ...
    private function completeDependencies($dp) : array
    {
        ...
        if (isset($dp[self::EXPIRATION])) {
            $dp[self::EXPIRATION] = \ECSPrefix20210520\Nette\Utils\DateTime::from($dp[self::EXPIRATION])->format('U') - \time();
        }
        // make list from TAGS
    ...
    public function capture($key)
    {
        $data = $this->load($key);
        if ($data === null) {
            return new \ECSPrefix20210520\Nette\Caching\OutputHelper($this, $key);
        }
        echo $data;
        return null;
    }
