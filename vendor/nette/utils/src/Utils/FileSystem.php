...
final class FileSystem
{
    ...
    public static function createDir($dir, $mode = 0777)
    {
        if (!\is_dir($dir) && !@\mkdir($dir, $mode, \true) && !\is_dir($dir)) {
            ...
            throw new \ECSPrefix20210804\Nette\IOException("Unable to create directory '{$dir}' with mode " . \decoct($mode) . '. ' . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
        }
    }
    /**
    ...
    public static function copy($origin, $target, $overwrite = \true)
    {
        if (\stream_is_local($origin) && !\file_exists($origin)) {
            ...
            if (($s = @\fopen($origin, 'rb')) && ($d = @\fopen($target, 'wb')) && @\stream_copy_to_stream($s, $d) === \false) {
                ...
                throw new \ECSPrefix20210804\Nette\IOException("Unable to copy file '{$origin}' to '{$target}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function delete($path)
    {
        if (\is_file($path) || \is_link($path)) {
            ...
            if (!@$func($path)) {
                // @ is escalated to exception
                throw new \ECSPrefix20210804\Nette\IOException("Unable to delete '{$path}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
            }
        } elseif (\is_dir($path)) {
            ...
            if (!@\rmdir($path)) {
                // @ is escalated to exception
                throw new \ECSPrefix20210804\Nette\IOException("Unable to delete directory '{$path}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function rename($origin, $target, $overwrite = \true)
    {
        if (!$overwrite && \file_exists($target)) {
            ...
            if (!@\rename($origin, $target)) {
                // @ is escalated to exception
                throw new \ECSPrefix20210804\Nette\IOException("Unable to rename file or directory '{$origin}' to '{$target}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function read($file) : string
    {
        ...
        // @ is escalated to exception
        if ($content === \false) {
            throw new \ECSPrefix20210804\Nette\IOException("Unable to read file '{$file}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
        }
        return $content;
    }
    /**
    ...
    public static function write($file, $content, $mode = 0666)
    {
        ...
        if (@\file_put_contents($file, $content) === \false) {
            // @ is escalated to exception
            throw new \ECSPrefix20210804\Nette\IOException("Unable to write file '{$file}'. " . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
        }
        if ($mode !== null && !@\chmod($file, $mode)) {
            // @ is escalated to exception
            throw new \ECSPrefix20210804\Nette\IOException("Unable to chmod file '{$file}' to mode " . \decoct($mode) . '. ' . \ECSPrefix20210804\Nette\Utils\Helpers::getLastError());
        }
    }
    /**
