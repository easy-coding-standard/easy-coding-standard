...
class Validators
{
    ...
    protected static $validators = [
        ...
        'list' => [\ECSPrefix20210516\Nette\Utils\Arrays::class, 'isList'],
    ...
    /** @var array<string,callable> */
    protected static $counters = ['string' => 'strlen', 'unicode' => [\ECSPrefix20210516\Nette\Utils\Strings::class, 'length'], 'array' => 'count', 'list' => 'count', 'alnum' => 'strlen', 'alpha' => 'strlen', 'digit' => 'strlen', 'lower' => 'strlen', 'space' => 'strlen', 'upper' => 'strlen', 'xdigit' => 'strlen'];
    /**
    ...
    public static function assert($value, $expected, $label = 'variable')
    {
        ...
        if (!static::is($value, $expected)) {
            ...
            throw new \ECSPrefix20210516\Nette\Utils\AssertionException("The {$label} expects to be {$expected}, {$type} given.");
        }
    }
    /**
    ...
    public static function assertField(array $array, $key, $expected = null, $label = "item '%' in array")
    {
        ...
        if (!\array_key_exists($key, $array)) {
            throw new \ECSPrefix20210516\Nette\Utils\AssertionException('Missing ' . \str_replace('%', $key, $label) . '.');
        } elseif ($expected) {
    ...
    public static function is($value, $expected)
    {
        ...
        foreach (\explode('|', $expected) as $item) {
            ...
            if (isset(static::$validators[$type])) {
                ...
                if (\ECSPrefix20210516\Nette\Utils\Strings::match($value, '|^' . (isset($item[1]) ? $item[1] : '') . '$|D')) {
                    return \true;
                }
                continue;
    ...
    {
        return \ECSPrefix20210516\Nette\Utils\Arrays::isList($value);
    }
    /**
