...
class Strings
{
    ...
    public static function chr($code) : string
    {
        if ($code < 0 || $code >= 0xd800 && $code <= 0xdfff || $code > 0x10ffff) {
            throw new \ECSPrefix20211107\Nette\InvalidArgumentException('Code point must be in range 0x0 to 0xD7FF or 0xE000 to 0x10FFFF.');
        } elseif (!\extension_loaded('iconv')) {
            throw new \ECSPrefix20211107\Nette\NotSupportedException(__METHOD__ . '() requires ICONV extension that is not loaded.');
        }
    ...
    public static function substring($s, $start, $length = null) : string
    {
        if (\function_exists('mb_substr')) {
            ...
            throw new \ECSPrefix20211107\Nette\NotSupportedException(__METHOD__ . '() requires extension ICONV or MBSTRING, neither is loaded.');
        } elseif ($length === null) {
    ...
    private static function pos(string $haystack, string $needle, int $nth = 1) : ?int
    {
        ...
        return \ECSPrefix20211107\Nette\Utils\Helpers::falseToNull($pos);
    }
    /**
    ...
    public static function replace($subject, $pattern, $replacement = '', $limit = -1) : string
    {
        if (\is_object($replacement) || \is_array($replacement)) {
            if (!\is_callable($replacement, \false, $textual)) {
                throw new \ECSPrefix20211107\Nette\InvalidStateException("Callback '{$textual}' is not callable.");
            }
    ...
    {
        $res = \ECSPrefix20211107\Nette\Utils\Callback::invokeSafe($func, $args, function (string $message) use($args) : void {
            // compile-time error, not detectable by preg_last_error
            throw new \ECSPrefix20211107\Nette\Utils\RegexpException($message . ' in pattern: ' . \implode(' or ', (array) $args[0]));
        });
        if (($code = \preg_last_error()) && ($res === null || !\in_array($func, ['preg_filter', 'preg_replace_callback', 'preg_replace'], \true))) {
            throw new \ECSPrefix20211107\Nette\Utils\RegexpException((\ECSPrefix20211107\Nette\Utils\RegexpException::MESSAGES[$code] ?? 'Unknown error') . ' (pattern: ' . \implode(' or ', (array) $args[0]) . ')', $code);
        }
        return $res;
    }
}
