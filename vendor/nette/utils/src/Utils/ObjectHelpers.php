...
use ECSPrefix20211210\Nette\MemberAccessException;
/**
...
final class ObjectHelpers
{
    ...
    public static function strictGet(string $class, string $name) : void
    {
        ...
        throw new \ECSPrefix20211210\Nette\MemberAccessException("Cannot read an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /**
     * @return never
     * @throws MemberAccessException
    ...
    public static function strictSet(string $class, string $name) : void
    {
        ...
        throw new \ECSPrefix20211210\Nette\MemberAccessException("Cannot write to an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /**
     * @return never
     * @throws MemberAccessException
    ...
    public static function strictCall(string $class, string $method, array $additionalMethods = []) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ECSPrefix20211210\Nette\MemberAccessException("Call to {$visibility}method {$class}::{$method}() from " . ($context ? "scope {$context}." : 'global scope.'));
        } else {
            ...
            throw new \ECSPrefix20211210\Nette\MemberAccessException("Call to undefined method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /**
     * @return never
    ...
    public static function strictStaticCall(string $class, string $method) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ECSPrefix20211210\Nette\MemberAccessException("Call to undefined static method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /**
