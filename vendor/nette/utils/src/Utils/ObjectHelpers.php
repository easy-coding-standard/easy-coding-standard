...
use ECSPrefix20210923\Nette\MemberAccessException;
/**
...
final class ObjectHelpers
{
    ...
    public static function strictGet($class, $name) : void
    {
        ...
        throw new \ECSPrefix20210923\Nette\MemberAccessException("Cannot read an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /** @throws MemberAccessException
     * @param string $class
     * @param string $name */
    public static function strictSet($class, $name) : void
    {
        ...
        throw new \ECSPrefix20210923\Nette\MemberAccessException("Cannot write to an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /** @throws MemberAccessException
     * @param string $class
     * @param string $method
    ...
    public static function strictCall($class, $method, $additionalMethods = []) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ECSPrefix20210923\Nette\MemberAccessException("Call to {$visibility}method {$class}::{$method}() from " . ($context ? "scope {$context}." : 'global scope.'));
        } else {
            ...
            throw new \ECSPrefix20210923\Nette\MemberAccessException("Call to undefined method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /** @throws MemberAccessException
     * @param string $class
    ...
    public static function strictStaticCall($class, $method) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ECSPrefix20210923\Nette\MemberAccessException("Call to undefined static method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /**
