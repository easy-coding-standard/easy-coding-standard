...
use ECSPrefix20211011\Nette\HtmlStringable;
 ...
 */
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, \ECSPrefix20211011\Nette\HtmlStringable
{
    ...
    public static function el($name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\ECSPrefix20211011\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
    ...
    {
        if (!$text instanceof \ECSPrefix20211011\Nette\HtmlStringable) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \ECSPrefix20211011\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;
