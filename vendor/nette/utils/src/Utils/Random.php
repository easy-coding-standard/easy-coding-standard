...
final class Random
{
    ...
    public static function generate($length = 10, $charlist = '0-9a-z')
    {
        ...
        if ($length < 1) {
            throw new \ECSPrefix20210509\Nette\InvalidArgumentException('Length must be greater than zero.');
        } elseif ($chLen < 2) {
            throw new \ECSPrefix20210509\Nette\InvalidArgumentException('Character list must contain at least two chars.');
        }
        $res = '';
        for ($i = 0; $i < $length; $i++) {
