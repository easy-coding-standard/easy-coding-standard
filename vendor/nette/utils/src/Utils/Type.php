...
final class Type
{
    ...
    public static function fromReflection($reflection) : ?self
    {
        ...
        if ($type === null) {
        ...
        } elseif ($type instanceof \ReflectionUnionType || $type instanceof \ECSPrefix20220516\ReflectionIntersectionType) {
            ...
            throw new \ECSPrefix20220516\Nette\InvalidStateException('Unexpected type of ' . \ECSPrefix20220516\Nette\Utils\Reflection::toString($reflection));
        }
    }
    /**
    ...
    public static function fromString(string $type) : self
    {
        if (!\preg_match('#(?:
            ...
            throw new \ECSPrefix20220516\Nette\InvalidArgumentException("Invalid type '{$type}'.");
        }
        [, $nType, $iType] = $m;
        if ($nType) {
    ...
    {
        return $this->single && \ECSPrefix20220516\Nette\Utils\Reflection::isBuiltinType($this->types[0]);
    }
    /**
    ...
    {
        return $this->single && \ECSPrefix20220516\Nette\Utils\Reflection::isClassKeyword($this->types[0]);
    }
    /**
    ...
    public function allows(string $type) : bool
    {
        ...
        if ($this->isIntersection()) {
            ...
            return \ECSPrefix20220516\Nette\Utils\Arrays::every($this->types, function ($currentType) use($type) {
                $builtin = \ECSPrefix20220516\Nette\Utils\Reflection::isBuiltinType($currentType);
                return \ECSPrefix20220516\Nette\Utils\Arrays::some($type->types, function ($testedType) use($currentType, $builtin) {
                    return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
                });
            });
        }
        $method = $type->isIntersection() ? 'some' : 'every';
        return \ECSPrefix20220516\Nette\Utils\Arrays::$method($type->types, function ($testedType) {
            $builtin = \ECSPrefix20220516\Nette\Utils\Reflection::isBuiltinType($testedType);
            return \ECSPrefix20220516\Nette\Utils\Arrays::some($this->types, function ($currentType) use($testedType, $builtin) {
                return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
            });
        });
    }
