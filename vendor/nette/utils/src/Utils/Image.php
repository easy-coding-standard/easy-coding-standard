...
class Image
{
    ...
    public static function fromFile($file, &$type = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \ECSPrefix20211124\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
        $type = self::detectTypeFromFile($file);
        if (!$type) {
            throw new \ECSPrefix20211124\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
        $method = 'imagecreatefrom' . self::FORMATS[$type];
        return new static(\ECSPrefix20211124\Nette\Utils\Callback::invokeSafe($method, [$file], function (string $message) : void {
            throw new \ECSPrefix20211124\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromString($s, &$type = null)
    {
        ...
        if (!$type) {
            throw new \ECSPrefix20211124\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
        return new static(\ECSPrefix20211124\Nette\Utils\Callback::invokeSafe('imagecreatefromstring', [$s], function (string $message) : void {
            throw new \ECSPrefix20211124\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromBlank($width, $height, $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \ECSPrefix20211124\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
    ...
    public static function typeToExtension($type) : string
    {
        if (!isset(self::FORMATS[$type])) {
            throw new \ECSPrefix20211124\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
    ...
    protected function setImageResource($image)
    {
        if (!$image instanceof \GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
            throw new \ECSPrefix20211124\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
    ...
    public static function calculateSize($srcWidth, $srcHeight, $newWidth, $newHeight, $flags = self::FIT) : array
    {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (!$newWidth || !$newHeight) {
                throw new \ECSPrefix20211124\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (!$newWidth && !$newHeight) {
                throw new \ECSPrefix20211124\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
    ...
    public function save($file, $quality = null, $type = null) : void
    {
        if ($type === null) {
            ...
            if (!isset($extensions[$ext])) {
                throw new \ECSPrefix20211124\Nette\InvalidArgumentException("Unsupported file extension '{$ext}'.");
            }
            $type = $extensions[$ext];
        }
    ...
    {
        return \ECSPrefix20211124\Nette\Utils\Helpers::capture(function () use($type, $quality) {
    ...
    private function output(int $type, ?int $quality, string $file = null) : void
    {
        ...
        if (!$success) {
            throw new \ECSPrefix20211124\Nette\Utils\ImageException(\ECSPrefix20211124\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \ECSPrefix20211124\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
    ...
    private static function isPercent(&$num) : bool
    {
        ...
        throw new \ECSPrefix20211124\Nette\InvalidArgumentException("Expected dimension in int|string, '{$num}' given.");
    }
    /**
    ...
    {
        throw new \ECSPrefix20211124\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
