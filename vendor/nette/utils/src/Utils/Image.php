...
class Image
{
    ...
    public static function fromFile(string $file, ?int &$type = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \ECSPrefix20220317\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
        $type = self::detectTypeFromFile($file);
        if (!$type) {
            throw new \ECSPrefix20220317\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
    ...
    public static function fromString(string $s, ?int &$type = null)
    {
        ...
        if (!$type) {
            throw new \ECSPrefix20220317\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
    ...
    {
        $errors = [];
        $res = \ECSPrefix20220317\Nette\Utils\Callback::invokeSafe($func, [$arg], function (string $message) use(&$errors) : void {
            $errors[] = $message;
        });
        if (!$res) {
            throw new \ECSPrefix20220317\Nette\Utils\ImageException($message . ' Errors: ' . \implode(', ', $errors));
        } elseif ($errors) {
    ...
    public static function fromBlank(int $width, int $height, ?array $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \ECSPrefix20220317\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
    ...
    public static function typeToExtension(int $type) : string
    {
        if (!isset(self::FORMATS[$type])) {
            throw new \ECSPrefix20220317\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
    ...
    protected function setImageResource($image)
    {
        if (!$image instanceof \GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
            throw new \ECSPrefix20220317\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
    ...
    public static function calculateSize(int $srcWidth, int $srcHeight, $newWidth, $newHeight, int $flags = self::FIT) : array
    {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (!$newWidth || !$newHeight) {
                throw new \ECSPrefix20220317\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (!$newWidth && !$newHeight) {
                throw new \ECSPrefix20220317\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
    ...
    public function save(string $file, ?int $quality = null, ?int $type = null) : void
    {
        if ($type === null) {
            ...
            if (!isset($extensions[$ext])) {
                throw new \ECSPrefix20220317\Nette\InvalidArgumentException("Unsupported file extension '{$ext}'.");
            }
            $type = $extensions[$ext];
        }
    ...
    {
        return \ECSPrefix20220317\Nette\Utils\Helpers::capture(function () use($type, $quality) {
    ...
    private function output(int $type, ?int $quality, ?string $file = null) : void
    {
        ...
        if (!$success) {
            throw new \ECSPrefix20220317\Nette\Utils\ImageException(\ECSPrefix20220317\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \ECSPrefix20220317\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
    ...
    private static function isPercent(&$num) : bool
    {
        ...
        throw new \ECSPrefix20220317\Nette\InvalidArgumentException("Expected dimension in int|string, '{$num}' given.");
    }
    /**
    ...
    {
        throw new \ECSPrefix20220317\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
