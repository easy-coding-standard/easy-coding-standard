...
class Image
{
    ...
    {
        $errors = [];
        $res = \ECSPrefix20220124\Nette\Utils\Callback::invokeSafe($func, [$arg], function (string $message) use(&$errors) : void {
            $errors[] = $message;
        });
        if (!$res) {
            throw new \ECSPrefix20220124\Nette\Utils\ImageException($message . ' Errors: ' . \implode(', ', $errors));
        } elseif ($errors) {
