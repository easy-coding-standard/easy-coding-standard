...
class Image
{
    ...
    public static function fromFile(string $file, int &$type = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \ECSPrefix20210618\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
        $type = self::detectTypeFromFile($file);
        if (!$type) {
            throw new \ECSPrefix20210618\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
        $method = 'imagecreatefrom' . self::FORMATS[$type];
        return new static(\ECSPrefix20210618\Nette\Utils\Callback::invokeSafe($method, [$file], function (string $message) {
            throw new \ECSPrefix20210618\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromString(string $s, int &$type = null)
    {
        ...
        if (!$type) {
            throw new \ECSPrefix20210618\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
        return new static(\ECSPrefix20210618\Nette\Utils\Callback::invokeSafe('imagecreatefromstring', [$s], function (string $message) {
            throw new \ECSPrefix20210618\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromBlank(int $width, int $height, array $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \ECSPrefix20210618\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
    ...
    public static function typeToExtension(int $type) : string
    {
        if (!isset(self::FORMATS[$type])) {
            throw new \ECSPrefix20210618\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
    ...
    protected function setImageResource($image)
    {
        if (!$image instanceof \GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
            throw new \ECSPrefix20210618\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
    ...
    public static function calculateSize(int $srcWidth, int $srcHeight, $newWidth, $newHeight, int $flags = self::FIT) : array
    {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (!$newWidth || !$newHeight) {
                throw new \ECSPrefix20210618\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (!$newWidth && !$newHeight) {
                throw new \ECSPrefix20210618\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
    ...
    public function save(string $file, int $quality = null, int $type = null)
    {
        if ($type === null) {
            ...
            if (!isset($extensions[$ext])) {
                throw new \ECSPrefix20210618\Nette\InvalidArgumentException("Unsupported file extension '{$ext}'.");
            }
            $type = $extensions[$ext];
        }
    ...
    {
        return \ECSPrefix20210618\Nette\Utils\Helpers::capture(function () use($type, $quality) {
    ...
    private function output(int $type, $quality, string $file = null)
    {
        ...
        if (!$success) {
            throw new \ECSPrefix20210618\Nette\Utils\ImageException(\ECSPrefix20210618\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \ECSPrefix20210618\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
    ...
    private static function isPercent(&$num) : bool
    {
        ...
        throw new \ECSPrefix20210618\Nette\InvalidArgumentException("Expected dimension in int|string, '{$num}' given.");
    }
    /**
    ...
    {
        throw new \ECSPrefix20210618\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
