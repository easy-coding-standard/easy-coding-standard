...
final class Reflection
{
    ...
    {
        $type = \ECSPrefix20220207\Nette\Utils\Type::fromReflection($func);
    ...
    {
        $type = \ECSPrefix20220207\Nette\Utils\Type::fromReflection($param);
        return $type ? $type->getNames() : [];
    }
    /**
    ...
    {
        $type = \ECSPrefix20220207\Nette\Utils\Type::fromReflection($prop);
        return $type ? $type->getNames() : [];
    }
    /**
    ...
    private static function getType($reflection, ?\ReflectionType $type) : ?string
    {
        if ($type === null) {
            ...
            return \ECSPrefix20220207\Nette\Utils\Type::resolve($type->getName(), $reflection);
        } elseif ($type instanceof \ReflectionUnionType || $type instanceof \ECSPrefix20220207\ReflectionIntersectionType) {
            throw new \ECSPrefix20220207\Nette\InvalidStateException('The ' . self::toString($reflection) . ' is not expected to have a union or intersection type.');
        } else {
            throw new \ECSPrefix20220207\Nette\InvalidStateException('Unexpected type of ' . self::toString($reflection));
        }
    }
    /**
    ...
    public static function getParameterDefaultValue(\ReflectionParameter $param)
    {
        if ($param->isDefaultValueConstant()) {
            ...
            $pair = \explode('::', $const);
            if (isset($pair[1])) {
                $pair[0] = \ECSPrefix20220207\Nette\Utils\Type::resolve($pair[0], $param);
                try {
    ...
    public static function toString(\Reflector $ref) : string
    {
        if ($ref instanceof \ReflectionClass) {
            ...
            throw new \ECSPrefix20220207\Nette\InvalidArgumentException();
        }
    }
    /**
    ...
    public static function expandClassName(string $name, \ReflectionClass $context) : string
    {
        ...
        if (empty($name)) {
            throw new \ECSPrefix20220207\Nette\InvalidArgumentException('Class name must not be empty.');
    ...
    public static function getUseStatements(\ReflectionClass $class) : array
    {
        if ($class->isAnonymous()) {
            throw new \ECSPrefix20220207\Nette\NotImplementedException('Anonymous classes are not supported.');
        }
        static $cache = [];
