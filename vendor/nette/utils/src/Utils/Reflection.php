...
final class Reflection
{
    ...
    private static function getType($reflection, $type, bool $asArray = \false)
    {
        if ($type === null) {
            ...
            throw new \ECSPrefix20210723\Nette\InvalidStateException('The ' . self::toString($reflection) . ' is not expected to have a union type.');
        } else {
            throw new \ECSPrefix20210723\Nette\InvalidStateException('Unexpected type of ' . self::toString($reflection));
        }
    }
    /**
     * @param  \ReflectionFunction|\ReflectionMethod|\ReflectionParameter|\ReflectionProperty  $reflection
    ...
    public static function toString($ref) : string
    {
        if ($ref instanceof \ReflectionClass) {
            ...
            throw new \ECSPrefix20210723\Nette\InvalidArgumentException();
        }
    }
    /**
    ...
    public static function expandClassName($name, $context) : string
    {
        ...
        if (empty($name)) {
            throw new \ECSPrefix20210723\Nette\InvalidArgumentException('Class name must not be empty.');
    ...
    public static function getUseStatements($class) : array
    {
        if ($class->isAnonymous()) {
            throw new \ECSPrefix20210723\Nette\NotImplementedException('Anonymous classes are not supported.');
        }
        static $cache = [];
