...
class DateTime extends \DateTime implements \JsonSerializable
{
    ...
    public static function fromParts(int $year, int $month, int $day, int $hour = 0, int $minute = 0, float $second = 0.0)
    {
        ...
        if (!\checkdate($month, $day, $year) || $hour < 0 || $hour > 23 || $minute < 0 || $minute > 59 || $second < 0 || $second >= 60) {
            throw new \ECSPrefix20220418\Nette\InvalidArgumentException("Invalid date '{$s}'");
        }
        return new static($s);
    }
    /**
    ...
    public static function createFromFormat($format, $time, $timezone = null)
    {
        if ($timezone === null) {
            ...
            throw new \ECSPrefix20220418\Nette\InvalidArgumentException('Invalid timezone given');
        }
