...
class Arrays
{
    ...
    public static function get($array, $key, $default = null)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($array) && \array_key_exists($k, $array)) {
                ...
                if (\func_num_args() < 3) {
                    throw new \ECSPrefix20210913\Nette\InvalidArgumentException("Missing item '{$k}'.");
                }
                return $default;
            }
        }
    ...
    public static function &getRef(&$array, $key)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($array) || $array === null) {
                ...
                throw new \ECSPrefix20210913\Nette\InvalidArgumentException('Traversed item is not an array.');
            }
        }
        return $array;
    }
    ...
    {
        return \ECSPrefix20210913\Nette\Utils\Helpers::falseToNull(\array_search(self::toKey($key), \array_keys($array), \true));
    }
    /**
    ...
    {
        return \ECSPrefix20210913\Nette\Utils\Strings::pcre('preg_grep', [$pattern, $array, $flags]);
    }
    /**
     ...
     * @param mixed[] $array
     */
    public static function associate($array, $path)
    {
        $parts = \is_array($path) ? $path : \preg_split('#(\\[\\]|->|=|\\|)#', $path, -1, \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_NO_EMPTY);
        if (!$parts || $parts === ['->'] || $parts[0] === '=' || $parts[0] === '|') {
            throw new \ECSPrefix20210913\Nette\InvalidArgumentException("Invalid path '{$path}'.");
        }
     ...
     * @param mixed[] $array
     */
    public static function normalize($array, $filling = null) : array
    {
        $res = [];
    ...
    public static function pick(&$array, $key, $default = null)
    {
        if (\array_key_exists($key, $array)) {
            ...
            throw new \ECSPrefix20210913\Nette\InvalidArgumentException("Missing item '{$key}'.");
        } else {
            return $default;
        }
    }
     ...
     */
    public static function some($array, $callback) : bool
    {
        foreach ($array as $k => $v) {
     ...
     * @param string $suffix
     */
    public static function wrap($array, $prefix = '', $suffix = '') : array
    {
        $res = [];
        foreach ($array as $k => $v) {
