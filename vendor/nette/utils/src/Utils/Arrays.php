...
class Arrays
{
    ...
    public static function get($array, $key, $default = null)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($array) && \array_key_exists($k, $array)) {
                ...
                if (\func_num_args() < 3) {
                    throw new \ECSPrefix20211210\Nette\InvalidArgumentException("Missing item '{$k}'.");
                }
                return $default;
            }
        }
    ...
    public static function &getRef(&$array, $key)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($array) || $array === null) {
                ...
                throw new \ECSPrefix20211210\Nette\InvalidArgumentException('Traversed item is not an array.');
            }
        }
        return $array;
    }
    ...
    {
        return \ECSPrefix20211210\Nette\Utils\Helpers::falseToNull(\array_search(self::toKey($key), \array_keys($array), \true));
    }
    /**
    ...
    {
        return \ECSPrefix20211210\Nette\Utils\Strings::pcre('preg_grep', [$pattern, $array, $flags]);
    }
    /**
    ...
    public static function associate($array, $path)
    {
        ...
        if (!$parts || $parts === ['->'] || $parts[0] === '=' || $parts[0] === '|') {
            throw new \ECSPrefix20211210\Nette\InvalidArgumentException("Invalid path '{$path}'.");
        }
    ...
    public static function pick(&$array, $key, $default = null)
    {
        if (\array_key_exists($key, $array)) {
            ...
            throw new \ECSPrefix20211210\Nette\InvalidArgumentException("Missing item '{$key}'.");
        } else {
            return $default;
        }
    }
