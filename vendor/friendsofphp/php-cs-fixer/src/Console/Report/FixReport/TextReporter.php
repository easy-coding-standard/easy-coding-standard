...
final class TextReporter implements \PhpCsFixer\Console\Report\FixReport\ReporterInterface
{
    ...
    public function generate(\PhpCsFixer\Console\Report\FixReport\ReportSummary $reportSummary) : string
    {
        ...
        foreach ($reportSummary->getChanged() as $file => $fixResult) {
            ...
            if ($reportSummary->shouldAddAppliedFixers()) {
                $output .= $this->getAppliedFixers($reportSummary->isDecoratedOutput(), $fixResult['appliedFixers']);
            }
            $output .= $this->getDiff($reportSummary->isDecoratedOutput(), $fixResult['diff']);
            $output .= \PHP_EOL;
        }
        return $output . $this->getFooter($reportSummary->getTime(), $identifiedFiles, $reportSummary->getFilesCount(), $reportSummary->getMemory(), $reportSummary->isDryRun());
    }
    /**
    ...
    {
        return \sprintf($isDecoratedOutput ? ' (<comment>%s</comment>)' : ' (%s)', \implode(', ', $appliedFixers));
    }
