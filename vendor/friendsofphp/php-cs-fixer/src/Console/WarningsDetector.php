    ...
    public function detectHigherPhpVersion() : void
    {
        try {
            ...
            if (\version_compare($currentPhpMajorMinor, $minPhpVersion, '>')) {
                $this->warnings[] = \sprintf('You are running PHP CS Fixer on PHP %1$s, but the minimum supported version in composer.json is PHP %2$s. This may introduce syntax or features not yet available in PHP %2$s, which could cause issues under that version. It is recommended to run PHP CS Fixer on PHP %2$s, to fit your project specifics.', $currentPhpVersion, $minPhpVersion);
            }
        } catch (\Throwable $e) {
            $this->warnings[] = \sprintf('Unable to determine minimum supported PHP version from composer.json: %s', $e->getMessage());
        }
    }
    /**
     * @return list<string>
