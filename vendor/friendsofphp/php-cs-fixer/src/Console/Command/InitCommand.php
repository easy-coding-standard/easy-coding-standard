...
final class InitCommand extends Command
{
    ...
    private function prepareConfigurationFileContent(SymfonyStyle $io): string
    {
        ...
        $sets = $io->choice('Do you want to use any of the other recommended rulesets? (multi-choice)', array_combine($extraSets, array_map(
            static function (string $item) use ($setsByName): string {
                return self::formatReference($setsByName[$item]->getDescription());
            },
            // @phpstan-ignore-line offsetAccess.notFound
            $extraSets
        )) + ['none' => 'none'], 'none', \true);
        ...
        $sets = array_filter($sets, static function ($item) {
            return 'none' !== $item;
        });
        foreach ($sets as $set) {
            $rules[$set] = \true;
        ...
        return str_replace(['/*{{ IS_RISKY_ALLOWED }}*/', '/*{{ RULES }}*/', '/*{{ CUSTOMIZABLE_FINDER }}*/'], [$isRiskyAllowed ? 'true' : 'false', "[\n" . implode(",\n", array_map(static function (string $name, $configuration): string {
            return \sprintf("        '%s' => %s", $name, Utils::toString($configuration));
        }, array_keys($rules), $rules)) . "\n    ]", $useDefaultFinder ? '' : "// 💡 additional files, e.g. bin entry file\n            // ->append([__DIR__.'/bin-entry-file'])\n            // 💡 folders to exclude, if any\n            // ->exclude([/* ... */])\n            // 💡 path patterns to exclude, if any\n            // ->notPath([/* ... */])\n            // 💡 extra configs\n            // ->ignoreDotFiles(false) // true by default in v3, false in v4 or future mode\n            // ->ignoreVCS(true) // true by default"], $readResult);
    }
    private function prepareGitIgnoreContent(SymfonyStyle $io, string $currentContent): ?string
    {
        ...
        foreach ($entries as &$entry) {
            $entry['exists'] = strpos($currentContent, $entry['name']) !== \false;
            if (\false === $entry['exists']) {
                $entriesToAdd[] = '/' . $entry['name'];
            }
        }
        $io->listing(array_map(static function (array $entry): string {
            return \sprintf('<fg=blue>`%s`</> - %s %s', $entry['name'], self::formatReference($entry['description']), \true === $entry['exists'] ? '(already present)' : '(will be added)');
        }, $entries));
        if ([] === $entriesToAdd) {
            return null;
        }
    ...
    {
        $text = Preg::replace('/``(.+?)``/', '<fg=blue>$1</>', $text);
        return Preg::replace('/`(.+?) <(.+?)>`_/', '<href=$2;fg=bright-blue;options=underscore>$1 ($2)</>', $text);
    }
    /**
    ...
    private function askForPhpUnitCallType(SymfonyStyle $io, array $resolvedSetNames): ?string
    {
        ...
        if (!\in_array((new AutoPHPUnitMigrationRiskySet())->getName(), $resolvedSetNames, \true)) {
            return null;
        }
        ...
        $callType = $io->choice('Which call type do you use for PHPUnit methods?', ['this' => '`$this->assertSame()`', 'self' => '`self::assertSame()`', 'static' => '`static::assertSame()`', 'none' => 'none - do not enforce any'], 'none');
