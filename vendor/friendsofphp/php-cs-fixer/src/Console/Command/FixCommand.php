...
use ECSPrefix20210922\Symfony\Component\EventDispatcher\EventDispatcher;
use ECSPrefix20210922\Symfony\Component\EventDispatcher\EventDispatcherInterface;
 ...
 * @internal
 */
final class FixCommand extends \ECSPrefix20210922\Symfony\Component\Console\Command\Command
{
    /**
     * @var string
     */
    ...
    public function __construct(\PhpCsFixer\ToolInfoInterface $toolInfo)
    {
        ...
        $this->eventDispatcher = new \ECSPrefix20210922\Symfony\Component\EventDispatcher\EventDispatcher();
        $this->stopwatch = new \ECSPrefix20210922\Symfony\Component\Stopwatch\Stopwatch();
...
EOF;
    ...
    {
        $this->setDefinition([new \ECSPrefix20210922\Symfony\Component\Console\Input\InputArgument('path', \ECSPrefix20210922\Symfony\Component\Console\Input\InputArgument::IS_ARRAY, 'The path.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('path-mode', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Specify path mode (can be override or intersection).', \PhpCsFixer\Console\ConfigurationResolver::PATH_MODE_OVERRIDE), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('allow-risky', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Are risky fixers allowed (can be yes or no).'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('config', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The path to a .php-cs-fixer.php file.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('dry-run', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Only shows which files would have been modified.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('rules', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The rules.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('using-cache', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Does cache should be used (can be yes or no).'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('cache-file', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The path to the cache file.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('diff', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Also produce diff for each file.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('format', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'To output results in other formats.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('stop-on-violation', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Stop execution on first violation.'), new \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption('show-progress', '', \ECSPrefix20210922\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Type of progress indicator (none, dots).')])->setDescription('Fixes a directory or a file.');
    }
    /**
     * {@inheritdoc}
    ...
    protected function execute($input, $output) : int
    {
        ...
        $stdErr = $output instanceof \ECSPrefix20210922\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : ('txt' === $reporter->getFormat() ? $output : null);
        if (null !== $stdErr) {
            if (\ECSPrefix20210922\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $verbosity) {
        ...
        if ('none' === $progressType || null === $stdErr) {
            ...
            $progressOutput = new \PhpCsFixer\Console\Output\ProcessOutput($stdErr, $this->eventDispatcher, (new \ECSPrefix20210922\Symfony\Component\Console\Terminal())->getWidth(), \count($finder));
        }
        ...
        $reportSummary = new \PhpCsFixer\Console\Report\FixReport\ReportSummary($changed, $fixEvent->getDuration(), $fixEvent->getMemory(), \ECSPrefix20210922\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $verbosity, $resolver->isDryRun(), $output->isDecorated());
        $output->isDecorated() ? $output->write($reporter->generate($reportSummary)) : $output->write($reporter->generate($reportSummary), \false, \ECSPrefix20210922\Symfony\Component\Console\Output\OutputInterface::OUTPUT_RAW);
