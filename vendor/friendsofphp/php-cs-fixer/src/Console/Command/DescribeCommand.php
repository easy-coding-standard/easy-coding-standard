 ...
 * @internal
 */
final class DescribeCommand extends \ECSPrefix20220205\Symfony\Component\Console\Command\Command
{
    /**
     * @var string
     */
    ...
    {
        $this->setDefinition([new \ECSPrefix20220205\Symfony\Component\Console\Input\InputArgument('name', \ECSPrefix20220205\Symfony\Component\Console\Input\InputArgument::REQUIRED, 'Name of rule / set.')])->setDescription('Describe rule / ruleset.');
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (\ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity() && $output instanceof \ECSPrefix20220205\Symfony\Component\Console\Output\ConsoleOutputInterface) {
    ...
    private function describeRule(\ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface $output, string $name) : void
    {
        ...
        if ($output->getVerbosity() >= \ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE) {
        ...
        if ($fixer instanceof \PhpCsFixer\Fixer\ConfigurableFixerInterface) {
            ...
            foreach ($options as $option) {
                $line = '* <info>' . \ECSPrefix20220205\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getName()) . '</info>';
                ...
                $description = \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \ECSPrefix20220205\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDescription()));
                ...
                if ($option instanceof \PhpCsFixer\FixerConfiguration\DeprecatedFixerOption) {
                    $line .= '. <error>DEPRECATED</error>: ' . \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \ECSPrefix20220205\Symfony\Component\Console\Formatter\OutputFormatter::escape(\lcfirst($option->getDeprecationMessage())));
                }
    ...
    private function describeSet(\ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface $output, string $name) : void
    {
     ...
     */
    private function describeList(\ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface $output, string $type) : void
    {
        if ($output->getVerbosity() >= \ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE) {
            $describe = ['sets' => $this->getSetNames(), 'rules' => $this->getFixers()];
        } elseif ($output->getVerbosity() >= \ECSPrefix20220205\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE) {
