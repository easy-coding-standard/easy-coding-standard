...
use ECSPrefix20210517\Symfony\Component\Filesystem\Filesystem;
use ECSPrefix20210517\Symfony\Component\Finder\Finder;
use ECSPrefix20210517\Symfony\Component\Finder\SplFileInfo;
/**
 * @internal
 */
final class DocumentationCommand extends \ECSPrefix20210517\Symfony\Component\Console\Command\Command
{
    ...
    private function generateFixersDocs(array $fixers)
    {
        ...
        foreach ((new \ECSPrefix20210517\Symfony\Component\Finder\Finder())->files()->in($this->generator->getFixersDocumentationDirectoryPath())->notPath($docForFixerRelativePaths) as $file) {
    ...
    private function generateRuleSetsDocs(array $fixers)
    {
        $filesystem = new \ECSPrefix20210517\Symfony\Component\Filesystem\Filesystem();
        /** @var SplFileInfo $file */
        foreach ((new \ECSPrefix20210517\Symfony\Component\Finder\Finder())->files()->in($this->generator->getRuleSetsDocumentationDirectoryPath()) as $file) {
            $filesystem->remove($file->getPathname());
        }
