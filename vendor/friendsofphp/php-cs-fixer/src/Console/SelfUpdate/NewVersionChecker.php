...
use ECSPrefix20220109\Composer\Semver\Comparator;
use ECSPrefix20220109\Composer\Semver\Semver;
use ECSPrefix20220109\Composer\Semver\VersionParser;
/**
 * @internal
 */
final class NewVersionChecker implements \PhpCsFixer\Console\SelfUpdate\NewVersionCheckerInterface
{
    ...
    public function __construct(\PhpCsFixer\Console\SelfUpdate\GithubClientInterface $githubClient)
    {
        ...
        $this->versionParser = new \ECSPrefix20220109\Composer\Semver\VersionParser();
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function getLatestVersionOfMajor(int $majorVersion) : ?string
    {
        ...
        foreach ($this->availableVersions as $availableVersion) {
            if (\ECSPrefix20220109\Composer\Semver\Semver::satisfies($availableVersion, $semverConstraint)) {
    ...
    public function compareVersions(string $versionA, string $versionB) : int
    {
        ...
        if (\ECSPrefix20220109\Composer\Semver\Comparator::lessThan($versionA, $versionB)) {
            return -1;
        }
        if (\ECSPrefix20220109\Composer\Semver\Comparator::greaterThan($versionA, $versionB)) {
            return 1;
        }
        return 0;
    }
    private function retrieveAvailableVersions() : void
    {
        ...
        foreach ($this->githubClient->getTags() as $tag) {
            ...
            try {
                ...
                if ('stable' === \ECSPrefix20220109\Composer\Semver\VersionParser::parseStability($version)) {
        ...
        $this->availableVersions = \ECSPrefix20220109\Composer\Semver\Semver::rsort($this->availableVersions);
    }
}
