...
use ECSPrefix20210510\Composer\Semver\Comparator;
use ECSPrefix20210510\Composer\Semver\Semver;
...
final class NewVersionChecker implements \PhpCsFixer\Console\SelfUpdate\NewVersionCheckerInterface
{
    ...
    public function __construct(\PhpCsFixer\Console\SelfUpdate\GithubClientInterface $githubClient)
    {
        ...
        $this->versionParser = new \ECSPrefix20210510\Composer\Semver\VersionParser();
    }
    /**
     * {@inheritdoc}
     * @return string
    ...
    public function getLatestVersionOfMajor($majorVersion)
    {
        ...
        foreach ($this->availableVersions as $availableVersion) {
            if (\ECSPrefix20210510\Composer\Semver\Semver::satisfies($availableVersion, $semverConstraint)) {
    ...
    public function compareVersions($versionA, $versionB)
    {
        ...
        if (\ECSPrefix20210510\Composer\Semver\Comparator::lessThan($versionA, $versionB)) {
            return -1;
        }
        if (\ECSPrefix20210510\Composer\Semver\Comparator::greaterThan($versionA, $versionB)) {
            return 1;
        }
        return 0;
    }
    ...
    private function retrieveAvailableVersions()
    {
        ...
        foreach ($this->githubClient->getTags() as $tag) {
            ...
            try {
                ...
                if ('stable' === \ECSPrefix20210510\Composer\Semver\VersionParser::parseStability($version)) {
        ...
        $this->availableVersions = \ECSPrefix20210510\Composer\Semver\Semver::rsort($this->availableVersions);
    }
}
