...
final class ErrorOutput
{
    ...
    public function listErrors($process, array $errors)
    {
        ...
        $showDetails = $this->output->getVerbosity() >= \ECSPrefix20210509\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE;
        $showTrace = $this->output->getVerbosity() >= \ECSPrefix20210509\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_DEBUG;
        foreach ($errors as $i => $error) {
            ...
            if ($showTrace && !$e instanceof \PhpCsFixer\Linter\LintingException) {
                ...
                foreach ($stackTrace as $trace) {
                    if (isset($trace['class'], $trace['function']) && \ECSPrefix20210509\Symfony\Component\Console\Command\Command::class === $trace['class'] && 'run' === $trace['function']) {
                        $this->output->writeln('      [ ... ]');
                        break;
                    }
    ...
    private function prepareOutput($string)
    {
        ...
        return $this->isDecorated ? \ECSPrefix20210509\Symfony\Component\Console\Formatter\OutputFormatter::escape($string) : $string;
    }
}
