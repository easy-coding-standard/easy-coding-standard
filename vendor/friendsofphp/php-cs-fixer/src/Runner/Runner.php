...
use ECSPrefix202410\React\Socket\TcpServer;
...
final class Runner
{
    ...
    private function fixParallel() : array
    {
        ...
        $server->on('connection', static function (ConnectionInterface $connection) use($processPool, $getFileChunk) : void {
            ...
            $decoder = new Decoder($connection, \true, 512, $jsonInvalidUtf8Ignore, self::PARALLEL_BUFFER_SIZE);
        ...
        for ($i = 0; $i < $processesToSpawn; ++$i) {
            ...
            $process = $processFactory->create($streamSelectLoop, new \PhpCsFixer\Runner\RunnerConfig($this->isDryRun, $this->stopOnViolation, $this->parallelConfig, $this->configFile), $identifier, $serverPort);
            ...
            $process->start(
                ...
                static function ($exitCode, string $output) use($processPool, $identifier) : void {
                    ...
                    if ($errorsReported > 0) {
                        throw WorkerException::fromRaw(\json_decode($matches[1][0], \true));
                    }
                }
            );
        }
        ...
        if ($oldHash !== $newHash) {
            ...
            if (!$this->isDryRun) {
                ...
                if (\false === @\file_put_contents($fileName, $new)) {
                    ...
                    throw new IOException(\sprintf('Failed to write file "%s", "%s".', $fileName, null !== $error ? $error['message'] : 'no reason available'), 0, null, $fileName);
                }
            }
        }
        $this->cacheManager->setFileHash($name, $newHash);
        $this->dispatchEvent(FixerFileProcessedEvent::NAME, new FixerFileProcessedEvent(null !== $fixInfo ? FixerFileProcessedEvent::STATUS_FIXED : FixerFileProcessedEvent::STATUS_NO_CHANGES, $name, $newHash));
        return $fixInfo;
    }
    /**
