...
final class CurlyBracesPositionFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
    ...
    {
        return new FixerDefinition('Curly braces must be placed as configured.', [new CodeSample('<?php
class Foo {
}
...
', new VersionSpecification(70000)), new CodeSample('<?php
if (foo()) {
    bar();
}
...
', ['classes_opening_brace' => self::SAME_LINE]), new VersionSpecificCodeSample('<?php
$foo = new class {
};
', new VersionSpecification(70000), ['anonymous_classes_opening_brace' => self::NEXT_LINE_UNLESS_NEWLINE_AT_SIGNATURE_END]), new VersionSpecificCodeSample('<?php
$foo = new class { };
$bar = new class { private $baz; };
', new VersionSpecification(70000), ['allow_single_line_empty_anonymous_classes' => \true]), new CodeSample('<?php
        ...
        foreach ($tokens as $index => $token) {
            ...
            if ($allowSingleLine || $allowSingleLineIfEmpty || $index < $allowSingleLineUntil) {
                ...
                for ($indexInsideBraces = $openBraceIndex + 1; $indexInsideBraces < $closeBraceIndex; ++$indexInsideBraces) {
                    ...
                    if ($allowSingleLineIfEmpty && !$tokenInsideBraces->isWhitespace() && !$tokenInsideBraces->isComment() || $tokenInsideBraces->isWhitespace() && 1 === Preg::match('/\\R/', $tokenInsideBraces->getContent())) {
                        $addNewlinesInsideBraces = \true;
                        break;
                    }
                }
            ...
            if (self::NEXT_LINE_UNLESS_NEWLINE_AT_SIGNATURE_END === $this->configuration[$positionOption]) {
                ...
                if ($tokens[$previousTokenIndex]->equals(')')) {
                    ...
                    if ($tokens[$previousTokenIndex]->isWhitespace() && 1 === Preg::match('/\\R/', $tokens[$previousTokenIndex]->getContent())) {
                        $whitespace = ' ';
                    }
                }
            }
