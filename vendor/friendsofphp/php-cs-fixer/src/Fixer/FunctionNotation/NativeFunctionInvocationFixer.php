        ...
        return new \PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('exclude', 'List of functions to ignore.'))->setAllowedTypes(['array'])->setAllowedValues([static function (array $value) {
            foreach ($value as $functionName) {
                if (!\is_string($functionName) || '' === \trim($functionName) || \trim($functionName) !== $functionName) {
                    throw new \ECSPrefix20210723\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Each element must be a non-empty, trimmed string, got "%s" instead.', \is_object($functionName) ? \get_class($functionName) : \gettype($functionName)));
                }
            }
            return \true;
            ...
            foreach ($value as $functionName) {
                ...
                if ('@' === $functionName[0] && !\in_array($functionName, $sets, \true)) {
                    throw new \ECSPrefix20210723\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Unknown set "%s", known sets are "%s".', $functionName, \implode('", "', $sets)));
                }
            }
            return \true;
