...
final class NoSpacesAfterFunctionNameFixer extends AbstractFixer
{
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            } elseif ($tokens[$lastTokenIndex]->equalsAny([')', ']', [CT::T_DYNAMIC_VAR_BRACE_CLOSE], [CT::T_ARRAY_INDEX_BRACE_CLOSE]])) {
                $block = Tokens::detectBlockType($tokens[$lastTokenIndex]);
                if (Tokens::BLOCK_TYPE_INDEX_BRACE === $block['type'] || Tokens::BLOCK_TYPE_DYNAMIC_VAR_BRACE === $block['type'] || Tokens::BLOCK_TYPE_INDEX_BRACKET === $block['type'] || Tokens::BLOCK_TYPE_PARENTHESIS === $block['type']) {
                    $this->fixFunctionCall($tokens, $index);
                }
            }
        }
