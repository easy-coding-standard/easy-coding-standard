    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        foreach ($tokens as $index => $token) {
            ...
            \usort($interfaces, function (array $first, array $second) : int {
                $score = self::ORDER_LENGTH === $this->configuration[self::OPTION_ORDER] ? \strlen($first['normalized']) - \strlen($second['normalized']) : ($this->configuration['case_sensitive'] ? \strcmp($first['normalized'], $second['normalized']) : \strcasecmp($first['normalized'], $second['normalized']));
