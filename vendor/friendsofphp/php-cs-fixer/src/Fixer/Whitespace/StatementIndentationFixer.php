...
', ['stick_comment_to_next_continuous_control_statement' => \true])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        $blockSignatureFirstTokens = [\T_USE, \T_IF, \T_ELSE, \T_ELSEIF, \T_FOR, \T_FOREACH, \T_WHILE, \T_DO, \T_SWITCH, \T_CASE, \T_DEFAULT, \T_TRY, \T_CLASS, \T_INTERFACE, \T_TRAIT, \T_EXTENDS, \T_IMPLEMENTS];
        $controlStructurePossibiblyWithoutBracesTokens = [\T_IF, \T_ELSE, \T_ELSEIF, \T_FOR, \T_FOREACH, \T_WHILE, \T_DO];
        if (\defined('T_MATCH')) {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if (isset($noBracesBlockStarts[$index])) {
                $scopes[] = ['type' => 'block', 'skip' => \false, 'end_index' => $this->findStatementEndIndex($tokens, $index, \count($tokens) - 1), 'end_index_inclusive' => \true, 'initial_indent' => $this->getLineIndentationWithBracesCompatibility($tokens, $index, $lastIndent), 'is_indented_block' => \true];
                ++$currentScope;
            }
            ...
            if ($token->isGivenKind($blockSignatureFirstTokens)) {
                ...
                $scopes[] = ['type' => 'block_signature', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \true, 'initial_indent' => $this->getLineIndentationWithBracesCompatibility($tokens, $index, $lastIndent), 'is_indented_block' => $token->isGivenKind([\T_EXTENDS, \T_IMPLEMENTS])];
                continue;
            }
            ...
            if ($token->isGivenKind(\T_FUNCTION)) {
                ...
                while (null !== $x && $tokens[$x]->isGivenKind($methodModifierTokens) && \array_key_exists($x, $methodModifierIndents)) {
            ...
            if ('statement' !== $scopes[$currentScope]['type'] && 'block_signature' !== $scopes[$currentScope]['type']) {
                ...
                $scopes[] = ['type' => 'statement', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \false, 'initial_indent' => $previousLineInitialIndent, 'new_indent' => $previousLineNewIndent];
            }
        }
    }
