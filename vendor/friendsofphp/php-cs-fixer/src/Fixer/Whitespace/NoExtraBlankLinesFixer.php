...
', ['tokens' => ['switch', 'case', 'default']])]);
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('tokens', 'List of tokens to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset(self::$availableTokens)])->setDefault(['extra'])->getOption()]);
    }
    /**
     * @uses fixAfterToken()
