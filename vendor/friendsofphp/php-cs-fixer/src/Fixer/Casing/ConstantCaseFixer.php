...
final class ConstantCaseFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if ($this->isNeighbourAccepted($tokens, $tokens->getPrevMeaningfulToken($index)) && $this->isNeighbourAccepted($tokens, $nextIndex) && !$tokens[$nextIndex]->equals('=') && !$this->isEnumCaseName($tokens, $index)) {
    ...
    private function isNeighbourAccepted(Tokens $tokens, int $index) : bool
    {
        ...
        if (null === $forbiddenTokens) {
            $forbiddenTokens = \array_merge([\T_AS, \T_CLASS, \T_EXTENDS, \T_IMPLEMENTS, \T_INSTANCEOF, \T_INSTEADOF, \T_INTERFACE, \T_NEW, \T_NS_SEPARATOR, \T_PAAMAYIM_NEKUDOTAYIM, \T_TRAIT, \T_USE, CT::T_USE_TRAIT, CT::T_USE_LAMBDA], Token::getObjectOperatorKinds());
        }
        $token = $tokens[$index];
        if ($token->equalsAny(['{', '}'])) {
            return \false;
