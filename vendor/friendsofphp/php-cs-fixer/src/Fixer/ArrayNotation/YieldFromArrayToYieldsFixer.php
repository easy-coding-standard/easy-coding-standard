...
final class YieldFromArrayToYieldsFixer extends AbstractFixer
{
    ...
    {
        return new FixerDefinition('Yield from array must be unpacked to series of yields.', [new CodeSample('<?php function generate() {
    yield from [
        1,
        2,
        3,
...
')], 'The conversion will make the array in `yield from` changed in arrays of 1 less dimension.');
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        foreach ($this->getYieldsFromToUnpack($tokens) as $index => [$startIndex, $endIndex]) {
            ...
            foreach ($this->findArrayItemCommaIndex($tokens, $tokens->getNextMeaningfulToken($startIndex), $tokens->getPrevMeaningfulToken($endIndex)) as $commaIndex) {
