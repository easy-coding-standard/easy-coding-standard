    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            ...
            for ($i = $endIndex - 1; $i > $startIndex; --$i) {
                ...
                } elseif ($this->configuration['ensure_single_space'] && ' ' !== $tokens[$i + 1]->getContent() && 1 === Preg::match('/^\\h+$/', $tokens[$i + 1]->getContent()) && (!$tokens[$i + 2]->isComment() || 1 === Preg::match('/^\\h+$/', $tokens[$i + 3]->getContent()))) {
