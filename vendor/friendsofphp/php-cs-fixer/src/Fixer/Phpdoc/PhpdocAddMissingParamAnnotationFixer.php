...
', ['only_untyped' => \false])]);
    ...
    private function prepareArgumentInformation(Tokens $tokens, int $start, int $end) : array
    {
        ...
        for ($index = $start; $index <= $end; ++$index) {
            $token = $tokens[$index];
            if ($token->isComment() || $token->isWhitespace() || $token->isGivenKind([CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PRIVATE, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PROTECTED, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PUBLIC]) || \defined('T_READONLY') && $token->isGivenKind(\T_READONLY)) {
                continue;
            }
