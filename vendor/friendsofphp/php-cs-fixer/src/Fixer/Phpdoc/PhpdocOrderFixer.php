    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('order', 'Sequence in which annotations in PHPDoc should be ordered.'))->setAllowedTypes(['string[]'])->setAllowedValues([static function (array $order): bool {
            ...
            if (\count($order) !== \count($unique)) {
                ...
                throw new InvalidOptionsException(\sprintf('The option "order" value is invalid. Tag%s %s %s duplicated.', \count($duplicates) > 1 ? 's' : '', Utils::naturalLanguageJoin(array_map(static function ($tag): string {
                    return (string) $tag;
                }, $duplicates)), \count($duplicates) > 1 ? 'are' : 'is'));
            }
            return \true;
