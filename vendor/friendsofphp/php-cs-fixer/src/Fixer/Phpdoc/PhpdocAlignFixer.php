...
EOF;
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        $tags->setAllowedTypes(['array'])->setDefault(self::DEFAULT_TAGS);
        ...
        $spacing->setAllowedTypes(['int', 'int[]'])->setAllowedValues([$allowPositiveIntegers])->setDefault(self::DEFAULT_SPACING);
    ...
    private function fixDocBlock(DocBlock $docBlock) : void
    {
        ...
        for ($i = 0, $l = \count($docBlock->getLines()); $i < $l; ++$i) {
            ...
            foreach ($items as $j => $item) {
                ...
                if ('' !== $item['var']) {
                    $line .= $this->getIndent((0 !== $hintMax ? $hintMax : -1) - \strlen($item['hint']) + $spacingForTag, $spacingForTag) . $item['var'] . ('' !== $item['desc'] ? $this->getIndent($varMax - \strlen($item['var']) + $spacingForTag, $spacingForTag) . $item['desc'] : '');
                } elseif ('' !== $item['desc']) {
