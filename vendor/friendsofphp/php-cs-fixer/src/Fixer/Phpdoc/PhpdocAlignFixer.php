        ...
        for ($i = 0, $l = \count($docBlock->getLines()); $i < $l; ++$i) {
            ...
            foreach ($items as $j => $item) {
                if (null === $item['tag']) {
                    ...
                    $line = $item['indent'] . ' *  ' . $this->getIndent($tagMax + $hintMax + $extraIndent, $this->getLeftAlignedDescriptionIndent($items, $j)) . $item['desc'];
                    $docBlock->getLine($current + $j)->setContent($line . $lineEnding);
                    continue;
                }
                $currTag = $item['tag'];
                ...
                if (!empty($item['var'])) {
                    $line .= $this->getIndent(($hintMax ?: -1) - \strlen($item['hint']) + 1) . $item['var'] . (!empty($item['desc']) ? $this->getIndent($varMax - \strlen($item['var']) + 1) . $item['desc'] : '');
