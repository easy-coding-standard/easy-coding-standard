...
final class DoctrineAnnotationIndentationFixer extends \PhpCsFixer\AbstractDoctrineAnnotationFixer
{
    ...
    protected function fixAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $doctrineAnnotationTokens) : void
    {
        ...
        for ($index = 0, $max = \count($doctrineAnnotationTokens); $index < $max; ++$index) {
            if (!$doctrineAnnotationTokens[$index]->isType(\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_AT)) {
                continue;
            }
        ...
        foreach ($doctrineAnnotationTokens as $index => $token) {
            if (!$token->isType(\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_NONE) || \strpos($token->getContent(), "\n") === \false) {
                continue;
            }
    ...
    private function getLineBracesCount(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $index) : array
    {
        ...
        while (isset($tokens[++$index])) {
            ...
            if ($token->isType([\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS, \ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES])) {
                ++$opening;
                continue;
            }
            if (!$token->isType([\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS, \ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES])) {
                continue;
            }
            if ($opening > 0) {
                --$opening;
    ...
    private function isClosingLineWithMeaningfulContent(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $index) : bool
    {
        while (isset($tokens[++$index])) {
            ...
            return !$token->isType([\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS, \ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES]);
        }
        return \false;
    }
    /**
    ...
    private function indentationCanBeFixed(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $newLineTokenIndex, array $annotationPositions) : bool
    {
        ...
        for ($index = $newLineTokenIndex + 1, $max = \count($tokens); $index < $max; ++$index) {
            ...
            return $tokens[$index]->isType(\ECSPrefix20220513\Doctrine\Common\Annotations\DocLexer::T_AT);
        }
        return \false;
    }
}
