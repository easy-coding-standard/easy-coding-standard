...
final class DoctrineAnnotationBracesFixer extends \PhpCsFixer\AbstractDoctrineAnnotationFixer
{
    ...
    private function addBracesToAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens)
    {
        foreach ($tokens as $index => $token) {
            ...
            if (null !== $braceIndex && $tokens[$braceIndex]->isType(\ECSPrefix20210726\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
                continue;
            }
            $tokens->insertAt($index + 2, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210726\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS, '('));
            $tokens->insertAt($index + 3, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210726\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS, ')'));
        }
    }
    /**
     * @return void
    ...
    private function removesBracesFromAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens)
    {
        for ($index = 0, $max = \count($tokens); $index < $max; ++$index) {
            ...
            if (!$tokens[$openBraceIndex]->isType(\ECSPrefix20210726\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
                continue;
            }
            ...
            if (!$tokens[$closeBraceIndex]->isType(\ECSPrefix20210726\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
                continue;
            }
