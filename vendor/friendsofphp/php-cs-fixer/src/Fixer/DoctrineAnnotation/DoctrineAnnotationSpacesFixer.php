...
final class DoctrineAnnotationSpacesFixer extends \PhpCsFixer\AbstractDoctrineAnnotationFixer
{
    ...
    private function fixSpacesAroundParentheses(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens) : void
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            } elseif ($tokens[$index]->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_AT)) {
            ...
            if (!$token->isType([\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS, \ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS])) {
                continue;
            }
            if ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
                $token = $tokens[$index - 1];
                if ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
                    $token->clear();
                }
                $token = $tokens[$index + 1];
            } else {
    ...
    private function fixSpacesAroundCommas(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens) : void
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if (!$token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_COMMA)) {
                continue;
            }
            $token = $tokens[$index - 1];
            if ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
            ...
            if ($index < \count($tokens) - 1 && !\PhpCsFixer\Preg::match('/^\\s/', $tokens[$index + 1]->getContent())) {
                $tokens->insertAt($index + 1, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_NONE, ' '));
            }
        }
    }
    private function fixAroundAssignments(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens) : void
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if ($tokens[$index]->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_AT)) {
                $scopes[] = \ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS;
                continue;
            }
            if ($tokens[$index]->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES)) {
                $scopes[] = \ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES;
                continue;
            }
            if (\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS === $endScopeType && $token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_EQUALS)) {
                $this->updateSpacesAfter($tokens, $index, $afterArguments);
                $this->updateSpacesBefore($tokens, $index, $beforeArguments);
                continue;
            }
            if (\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES === $endScopeType) {
                if ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_EQUALS)) {
                    $this->updateSpacesAfter($tokens, $index, $afterArraysEquals);
                    $this->updateSpacesBefore($tokens, $index, $beforeArraysEquals);
                    continue;
                }
                if ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_COLON)) {
    ...
    private function updateSpacesAt(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $index, int $insertIndex, ?bool $insert) : void
    {
        ...
        if ($insert) {
            ...
            $token->setContent(' ');
        } elseif ($token->isType(\ECSPrefix20220202\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
            $token->clear();
        }
    }
}
