...
final class SingleSpaceAroundConstructFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    private static $tokenMapContainASingleSpace = [
        ...
        for ($index = $tokens->count() - 2; $index >= 0; --$index) {
            ...
            if ($tokens[$whitespaceTokenIndex]->isWhitespace() && \strpos($tokens[$whitespaceTokenIndex]->getContent(), "\n") !== \false) {
                ...
                if (\defined('T_ATTRIBUTE')) {
                    // @TODO: drop condition and else when PHP 8.0+ is required
                    if ($nextNextToken->isGivenKind(\T_ATTRIBUTE)) {
                        continue;
                    }
                } else {
                    if ($nextNextToken->isComment() && \strncmp($nextNextToken->getContent(), '#[', \strlen('#[')) === 0) {
                        continue;
                    }
                }
