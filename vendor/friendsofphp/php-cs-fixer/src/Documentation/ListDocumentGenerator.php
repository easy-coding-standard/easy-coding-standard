        ...
        foreach ($fixers as $fixer) {
            ...
            if ($fixer instanceof DeprecatedFixerInterface) {
                ...
                if (0 !== \count($alternatives)) {
                    $documentation .= \PhpCsFixer\Documentation\RstUtils::toRst(\sprintf('   Use %s instead.', Utils::naturalLanguageJoinWithBackticks($alternatives)), 3);
                }
                $documentation .= "\n";
            }
            if ($fixer->isRisky()) {
            ...
            if ($fixer instanceof ConfigurableFixerInterface) {
                ...
                foreach ($configurationDefinition->getOptions() as $option) {
                    $documentation .= "\n   - | ``{$option->getName()}``";
                    $documentation .= "\n     | " . \PhpCsFixer\Documentation\RstUtils::ensureProperInlineCode($option->getDescription());
