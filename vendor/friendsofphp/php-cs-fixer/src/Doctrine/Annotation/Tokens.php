...
final class Tokens extends \SplFixedArray
{
    ...
    public static function createFromDocComment($input, $ignoredTags = []) : self
    {
        ...
        while (\false !== ($nextAtPosition = \strpos($content, '@', $currentPosition))) {
            ...
            $lexer = new \ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer();
            ...
            while (null !== ($token = $lexer->peek())) {
                if (0 === $index && \ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_AT !== $token['type']) {
                    break;
                }
                if (1 === $index) {
                    if (\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER !== $token['type'] || \in_array($token['value'], $ignoredTags, \true)) {
                        break;
                    }
                    $nbScannedTokensToUse = 2;
                }
                if ($index >= 2 && 0 === $nbScopes && !\in_array($token['type'], [\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE, \ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS], \true)) {
                    break;
                }
                $scannedTokens[] = $token;
                if (\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS === $token['type']) {
                    ++$nbScopes;
                } elseif (\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS === $token['type']) {
                    if (0 === --$nbScopes) {
            ...
            if (0 !== $nbScannedTokensToUse) {
                ...
                if (0 !== $ignoredTextLength) {
                    $tokens[] = new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE, \substr($content, $ignoredTextPosition, $ignoredTextLength));
                }
                $lastTokenEndIndex = 0;
                foreach (\array_slice($scannedTokens, 0, $nbScannedTokensToUse) as $token) {
                    if (\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_STRING === $token['type']) {
                        $token['value'] = '"' . \str_replace('"', '""', $token['value']) . '"';
                    }
                    $missingTextLength = $token['position'] - $lastTokenEndIndex;
                    if ($missingTextLength > 0) {
                        $tokens[] = new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE, \substr($content, $nextAtPosition + $lastTokenEndIndex, $missingTextLength));
                    }
        ...
        if ($ignoredTextPosition < \strlen($content)) {
            $tokens[] = new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE, \substr($content, $ignoredTextPosition));
        }
        return $tokens;
    }
    /**
    ...
    public function getAnnotationEnd($index) : ?int
    {
        ...
        if (isset($this[$index + 2])) {
            if ($this[$index + 2]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
                $currentIndex = $index + 2;
            } elseif (isset($this[$index + 3]) && $this[$index + 2]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE) && $this[$index + 3]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS) && \PhpCsFixer\Preg::match('/^(\\R\\s*\\*\\s*)*\\s*$/', $this[$index + 2]->getContent())) {
        ...
        if (null !== $currentIndex) {
            ...
            for ($max = \count($this); $currentIndex < $max; ++$currentIndex) {
                if ($this[$currentIndex]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
                    ++$level;
                } elseif ($this[$currentIndex]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
                    --$level;
                }
                if (0 === $level) {
                    return $currentIndex;
    ...
    public function getArrayEnd($index) : ?int
    {
        ...
        for (++$index, $max = \count($this); $index < $max; ++$index) {
            if ($this[$index]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES)) {
                ++$level;
            } elseif ($this[$index]->isType($index, \ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES)) {
                --$level;
            }
            if (0 === $level) {
                return $index;
    ...
    private function getMeaningfulTokenSibling(int $index, int $direction) : ?int
    {
        while (\true) {
            ...
            if (!$this[$index]->isType(\ECSPrefix20211002\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
                return $index;
            }
        }
        return null;
