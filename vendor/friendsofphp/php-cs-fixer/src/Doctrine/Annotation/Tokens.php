...
use ECSPrefix202306\Doctrine\Common\Annotations\DocLexer;
use ECSPrefix202306\Doctrine\Common\Lexer\Token as LexerToken;
use PhpCsFixer\Preg;
    ...
    public static function createFromDocComment(PhpToken $input, array $ignoredTags = []) : self
    {
        ...
        while (\false !== ($nextAtPosition = \strpos($content, '@', $currentPosition))) {
            ...
            while (null !== ($token = $lexer->peek())) {
                ...
                if (1 === $index) {
                    if (!$token->isA(DocLexer::T_IDENTIFIER) || \in_array($token->value, $ignoredTags, \true)) {
                        break;
                    }
            ...
            if (0 !== $nbScannedTokensToUse) {
                ...
                foreach (\array_slice($scannedTokens, 0, $nbScannedTokensToUse) as $token) {
                    if ($token->isA(DocLexer::T_STRING)) {
                        $token = new LexerToken('"' . \str_replace('"', '""', $token->value) . '"', $token->type, $token->position);
                    }
