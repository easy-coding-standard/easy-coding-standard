...
final class ProcessLinter implements \PhpCsFixer\Linter\LinterInterface
{
    ...
    {
        if (null === $executable) {
            $executableFinder = new \ECSPrefix20210605\Symfony\Component\Process\PhpExecutableFinder();
     ...
     */
    private function createProcessForFile(string $path) : \ECSPrefix20210605\Symfony\Component\Process\Process
    {
        // in case php://stdin
        if (!\is_file($path)) {
     ...
     */
    private function createProcessForSource(string $source) : \ECSPrefix20210605\Symfony\Component\Process\Process
    {
        if (null === $this->temporaryFile) {
        ...
        if (\false === @\file_put_contents($this->temporaryFile, $source)) {
            throw new \ECSPrefix20210605\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to write file "%s".', $this->temporaryFile), 0, null, $this->temporaryFile);
        }
