...
' . self::REGEX_TYPE . '
    ...
    {
        foreach (\array_reverse($this->innerTypeExpressions) as ['start_index' => $startIndex, 'expression' => $inner]) {
    ...
    private function parse() : void
    {
        $index = 0;
        while (\true) {
            Preg::match('{\\G' . self::REGEX_TYPE . '(?:\\h*(?<glue>[|&])\\h*|$)}x', $this->value, $matches, \PREG_OFFSET_CAPTURE, $index);
            if ([] === $matches) {
                // invalid phpdoc type
        ...
        if ('' !== ($matches['generic'][0] ?? '') && $matches['generic'][1] === $nullableLength) {
            ...
            $this->parseCommaSeparatedInnerTypes($index + \strlen($matches['callable_start'][0]), $matches['callable_arguments'][0] ?? '');
            ...
            $this->parseObjectLikeArrayInnerTypes($index + \strlen($matches['object_like_array_start'][0]), $matches['object_like_array_inners'][0] ?? '');
    ...
    private function parseCommaSeparatedInnerTypes(int $startIndex, string $value) : void
    {
        ...
        while (\strlen($value) !== $index) {
            Preg::match('{\\G' . self::REGEX_TYPES . '(?:\\h*,\\h*|$)}x', $value, $matches, 0, $index);
    ...
    private function parseObjectLikeArrayInnerTypes(int $startIndex, string $value) : void
    {
        $index = 0;
        while (\strlen($value) !== $index) {
            Preg::match('{\\G(?:(?=1)0' . self::REGEX_TYPES . '|(?<_object_like_array_inner>(?&object_like_array_inner))(?:\\h*,\\h*|$))}x', $value, $prematches, 0, $index);
            ...
            $addedPrefix = 'array{';
            Preg::match('{^' . self::REGEX_TYPES . '$}x', $addedPrefix . $consumedValue . '}', $matches, \PREG_OFFSET_CAPTURE);
            $this->innerTypeExpressions[] = ['start_index' => $startIndex + $index + $matches['object_like_array_inner_value'][1] - \strlen($addedPrefix), 'expression' => $this->inner($matches['object_like_array_inner_value'][0])];
    ...
    {
        return ['boolean' => 'bool', 'callback' => 'callable', 'double' => 'float', 'false' => 'bool', 'integer' => 'int', 'list' => 'array', 'real' => 'float', 'true' => 'bool'];
    }
}
