...
' . self::REGEX_TYPE . '
    ...
    public function sortTypes(\Closure $compareCallback) : void
    {
        $this->walkTypes(static function (self $type) use($compareCallback) : void {
            if ($type->isUnionType) {
                $type->innerTypeExpressions = Utils::stableSort($type->innerTypeExpressions, static function (array $type) : self {
    ...
    private function parse() : void
    {
        $index = 0;
        while (\true) {
            Preg::match('{\\G' . self::REGEX_TYPE . '(?:\\h*(?<glue>[|&])\\h*|$)}', $this->value, $matches, \PREG_OFFSET_CAPTURE, $index);
            if ([] === $matches) {
        ...
        if ('' !== ($matches['generic'][0] ?? '') && $matches['generic'][1] === $nullableLength) {
            ...
            $this->parseArrayShapeInnerTypes($index + \strlen($matches['array_shape_start'][0]), $matches['array_shape_inners'][0]);
    ...
    private function parseArrayShapeInnerTypes(int $startIndex, string $value) : void
    {
        $index = 0;
        while (\strlen($value) !== $index) {
            Preg::match('{\\G(?:(?=1)0' . self::REGEX_TYPES . '|(?<_array_shape_inner>(?&array_shape_inner))(?:\\h*,\\h*|$))}', $value, $prematches, 0, $index);
