        ...
        return \sprintf($template, \implode(\PHP_EOL, \array_map(static function (string $line) use($isDecorated, $lineTemplate) : string {
            if ($isDecorated) {
                ...
                $line = \PhpCsFixer\Preg::replaceCallback(['/^(\\+.*)/', '/^(\\-.*)/', '/^(@.*)/'], static function (array $matches) : string {
                    ...
                    return \sprintf('<fg=%s>%s</fg=%s>', $colour, \ECSPrefix20211128\Symfony\Component\Console\Formatter\OutputFormatter::escape($matches[0]), $colour);
                }, $line, 1, $count);
                if (0 === $count) {
                    $line = \ECSPrefix20211128\Symfony\Component\Console\Formatter\OutputFormatter::escape($line);
                }
            }
