...
final class JsonInliner
{
    ...
    public function inlineSections($jsonContent)
    {
        ...
        foreach ($inlineSections as $inlineSection) {
            ...
            $jsonContent = \ECSPrefix20210512\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \ECSPrefix20210512\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
