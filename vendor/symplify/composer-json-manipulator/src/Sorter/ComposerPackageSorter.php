...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority($requirementName)
    {
        ...
        if ($this->isPlatformPackage($requirementName)) {
            return \ECSPrefix20210512\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
                if ($name === 'php') {
                    return '0-' . $name;
                }
    ...
    {
        $name = (string) $name;
        return (bool) \ECSPrefix20210512\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
