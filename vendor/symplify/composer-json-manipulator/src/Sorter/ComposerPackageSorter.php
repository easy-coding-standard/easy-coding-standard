...
namespace ECSPrefix20220219\Symplify\ComposerJsonManipulator\Sorter;
...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return \ECSPrefix20220219\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
                if ($name === 'php') {
                    return '0-' . $name;
                }
    ...
    {
        return (bool) \ECSPrefix20220219\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
