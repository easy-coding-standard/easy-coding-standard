...
namespace ECSPrefix20210610\Symplify\Skipper\FileSystem;
...
final class PathNormalizer
{
    ...
    {
        // ends with *
        if (\ECSPrefix20210610\Nette\Utils\Strings::match($path, self::ONLY_ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\ECSPrefix20210610\Nette\Utils\Strings::match($path, self::ONLY_STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        if (\ECSPrefix20210610\Nette\Utils\Strings::contains($path, '..')) {
            $path = \realpath($path);
            if ($path === \false) {
                return '';
            }
