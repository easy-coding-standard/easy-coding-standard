...
namespace ECSPrefix20211120\Symplify\Skipper\FileSystem;
...
final class FnMatchPathNormalizer
{
    ...
    {
        // ends with *
        if (\ECSPrefix20211120\Nette\Utils\Strings::match($path, self::ONLY_ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\ECSPrefix20211120\Nette\Utils\Strings::match($path, self::ONLY_STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
