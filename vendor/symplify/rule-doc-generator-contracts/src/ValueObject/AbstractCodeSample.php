...
abstract class AbstractCodeSample implements \ECSPrefix20210728\Symplify\RuleDocGenerator\Contract\CodeSampleInterface
{
    /**
     * @var string
     */
    ...
    public function __construct(string $badCode, string $goodCode)
    {
        ...
        if ($badCode === '') {
            throw new \ECSPrefix20210728\Symplify\RuleDocGenerator\Exception\ShouldNotHappenException('Bad sample good code cannot be empty');
        }
        if ($goodCode === $badCode) {
            $errorMessage = \sprintf('Good and bad code cannot be identical: "%s"', $goodCode);
            throw new \ECSPrefix20210728\Symplify\RuleDocGenerator\Exception\ShouldNotHappenException($errorMessage);
        }
