...
use ECSPrefix20210811\Symplify\RuleDocGenerator\Exception\PoorDocumentationException;
...
final class RuleDefinition
{
    ...
    public function __construct(string $description, array $codeSamples)
    {
        ...
        if ($codeSamples === []) {
            throw new \ECSPrefix20210811\Symplify\RuleDocGenerator\Exception\PoorDocumentationException('Provide at least one code sample, so people can practically see what the rule does');
        }
    ...
    public function getRuleClass() : string
    {
        if ($this->ruleClass === null) {
            throw new \ECSPrefix20210811\Symplify\RuleDocGenerator\Exception\ShouldNotHappenException();
        }
    ...
    public function getRuleShortClass() : string
    {
        ...
        return (string) \ECSPrefix20210811\Nette\Utils\Strings::after($this->ruleClass, '\\', -1);
    }
    /**
    ...
    public function isConfigurable() : bool
    {
        foreach ($this->codeSamples as $codeSample) {
            if ($codeSample instanceof \ECSPrefix20210811\Symplify\RuleDocGenerator\ValueObject\CodeSample\ConfiguredCodeSample) {
                return \true;
            }
        }
        return \false;
