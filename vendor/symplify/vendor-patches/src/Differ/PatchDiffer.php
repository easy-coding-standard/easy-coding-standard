...
namespace ECSPrefix20220527\Symplify\VendorPatches\Differ;
...
final class PatchDiffer
{
     ...
     */
    private $differ;
    public function __construct(\ECSPrefix20220527\SebastianBergmann\Diff\Differ $differ)
    {
        $this->differ = $differ;
    }
    public function diff(\ECSPrefix20220527\Symplify\VendorPatches\ValueObject\OldAndNewFileInfo $oldAndNewFileInfo) : string
    {
        ...
        $clearedDiff = \ECSPrefix20220527\Nette\Utils\Strings::replace($diff, self::START_ORIGINAL_REGEX, '--- /dev/null');
        return \ECSPrefix20220527\Nette\Utils\Strings::replace($clearedDiff, self::START_NEW_REGEX, '+++ ' . $patchedFileRelativePath);
    }
    private function resolveFileInfoPathRelativeFilePath(\ECSPrefix20220527\Symplify\SmartFileSystem\SmartFileInfo $beforeFileInfo) : string
    {
        $match = \ECSPrefix20220527\Nette\Utils\Strings::match($beforeFileInfo->getRealPath(), self::LOCAL_PATH_REGEX);
