...
namespace ECSPrefix20220514\Symplify\VendorPatches\Finder;
...
use ECSPrefix20220514\Symplify\VendorPatches\Composer\PackageNameResolver;
...
final class OldToNewFilesFinder
{
    ...
    public function __construct(\ECSPrefix20220514\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \ECSPrefix20220514\Symplify\VendorPatches\Composer\PackageNameResolver $packageNameResolver)
    {
    ...
    public function find(string $directory) : array
    {
        ...
        foreach ($oldFileInfos as $oldFileInfo) {
            ...
            $newFileInfo = new \ECSPrefix20220514\Symplify\SmartFileSystem\SmartFileInfo($newFilePath);
            $packageName = $this->packageNameResolver->resolveFromFileInfo($newFileInfo);
            $oldAndNewFileInfos[] = new \ECSPrefix20220514\Symplify\VendorPatches\ValueObject\OldAndNewFileInfo($oldFileInfo, $newFileInfo, $packageName);
        }
    ...
    {
        $finder = \ECSPrefix20220514\Symfony\Component\Finder\Finder::create()->in($directory)->files()->exclude('composer/')->exclude('ocramius/')->name('*.old');
