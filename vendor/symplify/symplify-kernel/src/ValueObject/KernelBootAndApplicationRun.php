...
namespace ECSPrefix20220220\Symplify\SymplifyKernel\ValueObject;
...
use ECSPrefix20220220\Symfony\Component\HttpKernel\KernelInterface;
use ECSPrefix20220220\Symplify\PackageBuilder\Console\Input\StaticInputDetector;
...
use ECSPrefix20220220\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
/**
 * @api
 */
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel()
    {
        ...
        if (\is_a($kernelClass, \ECSPrefix20220220\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
            return new $kernelClass();
        }
        $environment = 'prod' . \random_int(1, 100000);
        return new $kernelClass($environment, \ECSPrefix20220220\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    }
    private function booKernelAndRunApplication() : void
    {
        $kernel = $this->createKernel();
        if ($kernel instanceof \ECSPrefix20220220\Symplify\SymplifyKernel\Contract\LightKernelInterface) {
        ...
        $application = $container->get(\ECSPrefix20220220\Symfony\Component\Console\Application::class);
    ...
    {
        if (\is_a($kernelClass, \ECSPrefix20220220\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            return;
        }
        if (\is_a($kernelClass, \ECSPrefix20220220\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
            return;
        ...
        $errorMessage = \sprintf('Class "%s" must by type of "%s" or "%s". "%s" given', $kernelClass, \ECSPrefix20220220\Symfony\Component\HttpKernel\KernelInterface::class, \ECSPrefix20220220\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, $currentValueType);
        throw new \ECSPrefix20220220\Symplify\SymplifyKernel\Exception\BootException($errorMessage);
    }
}
