...
use ECSPrefix20210517\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use ECSPrefix20210517\Symplify\SmartFileSystem\SmartFileInfo;
use ECSPrefix20210517\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel()
    {
        ...
        $kernel = new $kernelClass($environment, \ECSPrefix20210517\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    ...
    private function booKernelAndRunApplication()
    {
        ...
        if ($kernel instanceof \ECSPrefix20210517\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface && $this->extraConfigs !== []) {
            $kernel->setConfigs($this->extraConfigs);
        }
        $kernel->boot();
        $container = $kernel->getContainer();
        /** @var Application $application */
        $application = $container->get(\ECSPrefix20210517\Symfony\Component\Console\Application::class);
    ...
    private function setExtraConfigs(\ECSPrefix20210517\Symfony\Component\HttpKernel\KernelInterface $kernel, $kernelClass)
    {
        ...
        if (\is_a($kernel, \ECSPrefix20210517\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class, \true)) {
            /** @var ExtraConfigAwareKernelInterface $kernel */
            $kernel->setConfigs($this->extraConfigs);
        } else {
            $message = \sprintf('Extra configs are set, but the "%s" kernel class is missing "%s" interface', $kernelClass, \ECSPrefix20210517\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class);
