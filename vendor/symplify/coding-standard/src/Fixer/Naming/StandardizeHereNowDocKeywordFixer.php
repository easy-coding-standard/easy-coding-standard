 ...
 */
final class StandardizeHereNowDocKeywordFixer extends \Symplify\CodingStandard\Fixer\AbstractSymplifyFixer implements \ECSPrefix20210805\Symplify\RuleDocGenerator\Contract\ConfigurableRuleInterface, \ECSPrefix20210805\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface, \PhpCsFixer\Fixer\ConfigurableFixerInterface
{
    /**
     * @api
     * @var string
...
, [self::KEYWORD => 'CODE_SNIPPET'])]);
    ...
    {
        $match = \ECSPrefix20210805\Nette\Utils\Strings::match($token->getContent(), self::START_HEREDOC_NOWDOC_NAME_REGEX);
        if (!isset($match['name'])) {
            return;
        }
        $newContent = \ECSPrefix20210805\Nette\Utils\Strings::replace($token->getContent(), self::START_HEREDOC_NOWDOC_NAME_REGEX, '$1' . $this->keyword . '$4');
