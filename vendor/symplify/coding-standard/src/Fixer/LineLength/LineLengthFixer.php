...
CODE_SAMPLE
    ...
    public function configure($configuration = null)
    {
        ...
        $this->breakLongLines = isset($configuration[self::BREAK_LONG_LINES]) ? $configuration[self::BREAK_LONG_LINES] : true;
        $this->inlineShortLines = isset($configuration[self::INLINE_SHORT_LINES]) ? $configuration[self::INLINE_SHORT_LINES] : true;
    }
     ...
     * @param int $position
     */
    private function processMethodCall(Tokens $tokens, $position)
    {
     ...
     * @param int $position
     */
    private function processFunctionOrArray(Tokens $tokens, $position)
    {
        $position = (int) $position;
     ...
     * @return bool
     */
    private function shouldSkip(Tokens $tokens, BlockInfo $blockInfo)
    {
     ...
     * @return bool
     */
    private function isHerenowDoc(Tokens $tokens, BlockInfo $blockInfo)
    {
        // heredoc/nowdoc => skip
