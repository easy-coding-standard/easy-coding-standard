 ...
 */
final class RemoveCommentedCodeFixer extends \Symplify\CodingStandard\Fixer\AbstractSymplifyFixer implements \ECSPrefix20210607\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    /**
     * @var string
     */
    ...
    public function fix(\SplFileInfo $file, \PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($i = 0; $i < $tokens->count(); ++$i) {
            ...
            if (!\ECSPrefix20210607\Nette\Utils\Strings::startsWith($token->getContent(), '//')) {
                continue;
            }
...
CODE_SAMPLE
    ...
    private function resolveRealStart(\Symplify\CodingStandard\ValueObject\StartAndEnd $startAndEnd, \PhpCsFixer\Tokenizer\Tokens $tokens) : int
    {
        ...
        if (\ECSPrefix20210607\Nette\Utils\Strings::endsWith($preStartToken->getContent(), '    ')) {
            return $realStart - 1;
        }
        return $realStart;
    }
