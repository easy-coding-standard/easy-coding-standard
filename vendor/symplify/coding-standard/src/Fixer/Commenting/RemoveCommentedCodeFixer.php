...
use ECSPrefix20210524\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample;
 ...
 */
final class RemoveCommentedCodeFixer extends \Symplify\CodingStandard\Fixer\AbstractSymplifyFixer implements \ECSPrefix20210524\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    /**
     * @var string
     */
    ...
    public function fix(\SplFileInfo $file, \PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($i = 0; $i < $tokens->count(); ++$i) {
            ...
            if (!\ECSPrefix20210524\Nette\Utils\Strings::startsWith($token->getContent(), '//')) {
                continue;
            }
    ...
    {
        return new \ECSPrefix20210524\Symplify\RuleDocGenerator\ValueObject\RuleDefinition(self::ERROR_MESSAGE, [new \ECSPrefix20210524\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
// $one = 1;
// $two = 2;
// $three = 3;
CODE_SAMPLE
...
CODE_SAMPLE
    ...
    private function resolveRealStart(\Symplify\CodingStandard\ValueObject\StartAndEnd $startAndEnd, \PhpCsFixer\Tokenizer\Tokens $tokens) : int
    {
        ...
        if (\ECSPrefix20210524\Nette\Utils\Strings::endsWith($preStartToken->getContent(), '    ')) {
            return $realStart - 1;
        }
        return $realStart;
    }
