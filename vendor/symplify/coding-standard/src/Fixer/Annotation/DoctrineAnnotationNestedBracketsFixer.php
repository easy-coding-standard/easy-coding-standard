...
use ECSPrefix20220316\Webmozart\Assert\Assert;
final class DoctrineAnnotationNestedBracketsFixer extends \Symplify\CodingStandard\Fixer\AbstractSymplifyFixer implements \ECSPrefix20220316\Symplify\RuleDocGenerator\Contract\ConfigurableRuleInterface, \ECSPrefix20220316\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    /**
     * @var string
     */
    ...
    {
        return new \ECSPrefix20220316\Symplify\RuleDocGenerator\ValueObject\RuleDefinition(self::ERROR_MESSAGE, [new \ECSPrefix20220316\Symplify\RuleDocGenerator\ValueObject\CodeSample\ConfiguredCodeSample(<<<'CODE_SAMPLE'
/**
* @MainAnnotation(
*     @NestedAnnotation(),
*     @NestedAnnotation(),
...
, [self::ANNOTATION_CLASSES => ['MainAnnotation']])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \ECSPrefix20220316\Webmozart\Assert\Assert::isArray($annotationsClasses);
        \ECSPrefix20220316\Webmozart\Assert\Assert::allString($annotationsClasses);
    ...
    private function fixAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $doctrineAnnotationTokens, array $useDeclarations) : void
    {
        foreach ($doctrineAnnotationTokens as $index => $token) {
            if (!$token->isType(\ECSPrefix20220316\Doctrine\Common\Annotations\DocLexer::T_AT)) {
                continue;
            }
            ...
            // add closing brace
            $doctrineAnnotationTokens->insertAt($closingBraceIndex, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20220316\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES, '}'));
            // add opening brace
            $doctrineAnnotationTokens->insertAt($braceIndex + 1, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20220316\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES, '{'));
        }
    }
}
