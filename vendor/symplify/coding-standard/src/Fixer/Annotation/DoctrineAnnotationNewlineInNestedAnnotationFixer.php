 ...
 */
final class DoctrineAnnotationNewlineInNestedAnnotationFixer extends \PhpCsFixer\AbstractDoctrineAnnotationFixer implements \ECSPrefix20210612\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    /**
     * @var string
     */
...
CODE_SAMPLE
    ...
    protected function fixAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens)
    {
        ...
        for ($index = 0; $index < $tokenCount; ++$index) {
            ...
            if (!$currentToken->isType(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_AT)) {
                continue;
            }
            ...
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE, ' * '));
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE, "\n"));
            $tNone = $previousToken->isType(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE);
    ...
    private function processEndBracket(int $index, \PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $previousTokenPosition)
    {
        ...
        if ($previousToken->isType(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
            return;
        }
        // reset
        ...
        if ($this->currentBlockInfo !== null) {
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE, ' * '));
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210612\Doctrine\Common\Annotations\DocLexer::T_NONE, "\n"));
        }
    }
    /**
