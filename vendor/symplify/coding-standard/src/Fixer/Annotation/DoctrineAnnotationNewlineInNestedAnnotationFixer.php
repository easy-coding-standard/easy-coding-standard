 ...
 */
final class DoctrineAnnotationNewlineInNestedAnnotationFixer extends \PhpCsFixer\AbstractDoctrineAnnotationFixer implements \ECSPrefix20210519\Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    /**
     * @var string
     */
...
CODE_SAMPLE
    ...
    protected function fixAnnotations(\PhpCsFixer\Doctrine\Annotation\Tokens $tokens)
    {
        ...
        for ($index = 0; $index < $tokenCount; ++$index) {
            ...
            if (!$currentToken->isType(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_AT)) {
                continue;
            }
            ...
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE, ' * '));
            $tokens->insertAt($index, new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE, "\n"));
            $tNone = $previousToken->isType(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE);
    ...
    private function isDocOpener(\PhpCsFixer\Doctrine\Annotation\Token $token) : bool
    {
        if ($token->isType(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
            return \ECSPrefix20210519\Nette\Utils\Strings::contains($token->getContent(), '*');
        }
        return \false;
    }
    /**
    ...
    private function processEndBracket(int $index, \PhpCsFixer\Doctrine\Annotation\Tokens $tokens, int $previousTokenPosition)
    {
        ...
        if ($previousToken->isType(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE)) {
            return;
        }
        // reset
        ...
        if ($this->currentBlockInfo !== null) {
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE, ' * '));
            $tokens->insertAt($this->currentBlockInfo->getEnd(), new \PhpCsFixer\Doctrine\Annotation\Token(\ECSPrefix20210519\Doctrine\Common\Annotations\DocLexer::T_NONE, "\n"));
        }
    }
    /**
    ...
    private function shouldSkip(int $index, \PhpCsFixer\Doctrine\Annotation\Tokens $tokens, \PhpCsFixer\Doctrine\Annotation\Token $previousToken) : bool
    {
        ...
        if (!\ECSPrefix20210519\Nette\Utils\Strings::startsWith($nextToken->getContent(), 'ORM')) {
            return \true;
        }
