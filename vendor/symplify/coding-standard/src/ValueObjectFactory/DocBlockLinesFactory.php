...
final class DocBlockLinesFactory
{
    ...
    {
        // Remove the prefix '/**'
        $docBlock = \ECSPrefix20210710\Nette\Utils\Strings::replace($docBlock, self::BEGINNING_OF_DOC_BLOCK_REGEX);
        // Remove the suffix '*/'
        $docBlock = \ECSPrefix20210710\Nette\Utils\Strings::replace($docBlock, self::END_OF_DOC_BLOCK_REGEX);
        ...
        $docBlockLines = \array_map(function (string $line) : string {
            $noWhitespace = \ECSPrefix20210710\Nette\Utils\Strings::trim($line, \ECSPrefix20210710\Nette\Utils\Strings::TRIM_CHARACTERS);
            // Remove asterisks on the left side, plus additional whitespace
            return \ltrim($noWhitespace, \ECSPrefix20210710\Nette\Utils\Strings::TRIM_CHARACTERS . '*');
        }, $docBlockLines);
