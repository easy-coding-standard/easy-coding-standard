...
final class DocBlockLinesFactory
{
    ...
    public function createFromDocBlock(string $docBlock) : \Symplify\CodingStandard\ValueObject\DocBlockLines
    {
        ...
        // Remove the suffix '*/'
        $docBlock = \ECSPrefix20210517\Nette\Utils\Strings::replace($docBlock, self::END_OF_DOC_BLOCK_REGEX);
        ...
        $docBlockLines = \array_map(function (string $line) : string {
            $noWhitespace = \ECSPrefix20210517\Nette\Utils\Strings::trim($line, \ECSPrefix20210517\Nette\Utils\Strings::TRIM_CHARACTERS);
            // Remove asterisks on the left side, plus additional whitespace
            return \ltrim($noWhitespace, \ECSPrefix20210517\Nette\Utils\Strings::TRIM_CHARACTERS . '*');
        }, $docBlockLines);
    ...
    private function createFromLines(array $docBlockLines) : \Symplify\CodingStandard\ValueObject\DocBlockLines
    {
        ...
        foreach ($docBlockLines as $docBlockLine) {
            if (\ECSPrefix20210517\Nette\Utils\Strings::startsWith($docBlockLine, '@') || \ECSPrefix20210517\Nette\Utils\Strings::startsWith($docBlockLine, '{@')) {
