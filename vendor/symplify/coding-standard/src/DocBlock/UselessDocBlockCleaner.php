...
final class UselessDocBlockCleaner
{
    ...
    public function clearDocTokenContent(array $tokens, $position, $docContent)
    {
        ...
        foreach (self::CLEANING_REGEXES as $cleaningRegex) {
            $docContent = \ECSPrefix20210510\Nette\Utils\Strings::replace($docContent, $cleaningRegex, '');
        }
    ...
    private function cleanClassMethodCommentMimicMethodName(array $reversedTokens, $index, $docContent)
    {
        $index = (int) $index;
        $docContent = (string) $docContent;
        $matchMethodClass = \ECSPrefix20210510\Nette\Utils\Strings::match($docContent, self::COMMENT_METHOD_CLASS_REGEX);
        if ($matchMethodClass) {
            return $docContent;
        }
        ...
        $matchAnyMethodClass = \ECSPrefix20210510\Nette\Utils\Strings::match($docContent, self::COMMENT_ANY_METHOD_CLASS_REGEX);
        ...
        return \ECSPrefix20210510\Nette\Utils\Strings::replace($docContent, self::COMMENT_ANY_METHOD_CLASS_REGEX, '');
    }
    /**
     * @param int $index
     * @return bool
    ...
    private function removeSpaces($content)
    {
        ...
        return \ECSPrefix20210510\Nette\Utils\Strings::replace($content, self::SPACE_STAR_SLASH_REGEX, '');
    }
}
