...
final class UselessDocBlockCleaner
{
    ...
    public function clearDocTokenContent(array $tokens, int $position, string $docContent) : string
    {
        foreach (self::CLEANING_REGEXES as $cleaningRegex) {
            $docContent = \ECSPrefix20211024\Nette\Utils\Strings::replace($docContent, $cleaningRegex, '');
        }
    ...
    {
        $matchMethodClass = \ECSPrefix20211024\Nette\Utils\Strings::match($docContent, self::COMMENT_METHOD_CLASS_REGEX);
        if ($matchMethodClass) {
            return $docContent;
        }
        ...
        $matchAnyMethodClass = \ECSPrefix20211024\Nette\Utils\Strings::match($docContent, self::COMMENT_ANY_METHOD_CLASS_REGEX);
        ...
        return \ECSPrefix20211024\Nette\Utils\Strings::replace($docContent, self::COMMENT_ANY_METHOD_CLASS_REGEX, '');
    }
    ...
    {
        return \ECSPrefix20211024\Nette\Utils\Strings::replace($content, self::SPACE_STAR_SLASH_REGEX, '');
    }
}
