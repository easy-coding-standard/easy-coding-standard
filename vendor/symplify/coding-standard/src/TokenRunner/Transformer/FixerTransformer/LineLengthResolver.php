...
final class LineLengthResolver
{
    ...
    private function isNewLineOrOpenTag(\PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : bool
    {
        ...
        if (\ECSPrefix20210602\Nette\Utils\Strings::startsWith($currentToken->getContent(), \ECSPrefix20210602\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar())) {
            return \true;
        }
    ...
    private function getLengthFromEndOfArgumentToLineBreak(\Symplify\CodingStandard\TokenRunner\ValueObject\BlockInfo $blockInfo, \PhpCsFixer\Tokenizer\Tokens $tokens) : int
    {
        ...
        while (!\ECSPrefix20210602\Nette\Utils\Strings::startsWith($currentToken->getContent(), \ECSPrefix20210602\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar())) {
            $length += \strlen($currentToken->getContent());
            ++$end;
            if (!isset($tokens[$end])) {
                break;
