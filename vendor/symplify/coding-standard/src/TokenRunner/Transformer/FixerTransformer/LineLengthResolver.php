...
final class LineLengthResolver
{
    ...
    private function isNewLineOrOpenTag(\PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : bool
    {
        ...
        if (\strncmp($currentToken->getContent(), \ECSPrefix20210803\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar(), \strlen(\ECSPrefix20210803\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar())) === 0) {
            return \true;
        }
    ...
    private function getLengthFromEndOfArgumentToLineBreak(\Symplify\CodingStandard\TokenRunner\ValueObject\BlockInfo $blockInfo, \PhpCsFixer\Tokenizer\Tokens $tokens) : int
    {
        ...
        while (\strncmp($currentToken->getContent(), \ECSPrefix20210803\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar(), \strlen(\ECSPrefix20210803\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar())) !== 0) {
            $length += \strlen($currentToken->getContent());
            ++$end;
            if (!isset($tokens[$end])) {
                break;
