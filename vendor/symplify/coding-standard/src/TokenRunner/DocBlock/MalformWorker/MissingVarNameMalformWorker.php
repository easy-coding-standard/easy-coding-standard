...
final class MissingVarNameMalformWorker implements \Symplify\CodingStandard\TokenRunner\Contract\DocBlock\MalformWorkerInterface
{
    ...
    public function work($docContent, \PhpCsFixer\Tokenizer\Tokens $tokens, $position)
    {
        ...
        if (!\ECSPrefix20210514\Nette\Utils\Strings::match($docContent, self::VAR_WITHOUT_NAME_REGEX)) {
            return $docContent;
        }
        ...
        return \ECSPrefix20210514\Nette\Utils\Strings::replace($docContent, self::VAR_WITHOUT_NAME_REGEX, function (array $match) use($nextVariableToken) : string {
