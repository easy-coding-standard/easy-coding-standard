...
final class SuperfluousVarNameMalformWorker implements \Symplify\CodingStandard\TokenRunner\Contract\DocBlock\MalformWorkerInterface
{
    ...
    public function work($docContent, $tokens, $position) : string
    {
        ...
        foreach ($lines as $line) {
            $match = \ECSPrefix20211107\Nette\Utils\Strings::match($line->getContent(), self::VAR_VARIABLE_NAME_REGEX);
            if ($match === null) {
                continue;
            }
            $newLineContent = \ECSPrefix20211107\Nette\Utils\Strings::replace($line->getContent(), self::VAR_VARIABLE_NAME_REGEX, function (array $match) : string {
                $replacement = $match['tag'];
                if ($match['type'] !== []) {
                    $replacement .= $match['type'];
                }
                if (\ECSPrefix20211107\Nette\Utils\Strings::match($match['propertyName'], self::THIS_VARIABLE_REGEX)) {
