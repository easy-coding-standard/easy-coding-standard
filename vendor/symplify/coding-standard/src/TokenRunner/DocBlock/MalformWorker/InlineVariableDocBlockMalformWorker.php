...
final class InlineVariableDocBlockMalformWorker implements \Symplify\CodingStandard\TokenRunner\Contract\DocBlock\MalformWorkerInterface
{
    ...
    public function work(string $docContent, \PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : string
    {
        ...
        if (\substr_count($docContent, \ECSPrefix20220418\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar()) > 2) {
            return $docContent;
        }
        // asterisk start
        $docContent = \ECSPrefix20220418\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_START_REGEX, '/**$1');
        // inline
        $docContent = \ECSPrefix20220418\Nette\Utils\Strings::replace($docContent, self::SPACE_REGEX, ' ');
        // remove asterisk leftover
        return \ECSPrefix20220418\Nette\Utils\Strings::replace($docContent, self::ASTERISK_LEFTOVERS_REGEX, '$1');
    }
    /**
     * @param Tokens<Token> $tokens
     */
