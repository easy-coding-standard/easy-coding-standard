...
use ECSPrefix20220107\Symplify\PackageBuilder\Configuration\StaticEolConfiguration;
final class MissingParamNameMalformWorker implements \Symplify\CodingStandard\TokenRunner\Contract\DocBlock\MalformWorkerInterface
{
    ...
    private function filterOutExistingParamNames(string $docContent, array $functionArgumentNames) : array
    {
        foreach ($functionArgumentNames as $key => $functionArgumentName) {
            ...
            if (\ECSPrefix20220107\Nette\Utils\Strings::match($docContent, $pattern)) {
    ...
    private function shouldSkipLine(\PhpCsFixer\DocBlock\Line $line) : bool
    {
        ...
        if (\ECSPrefix20220107\Nette\Utils\Strings::match($line->getContent(), self::PARAM_WITH_NAME_REGEX)) {
            return \true;
        }
        $match = \ECSPrefix20220107\Nette\Utils\Strings::match($line->getContent(), self::PARAM_WITHOUT_NAME_REGEX);
        return $match === null;
    }
    private function createNewLineContent(string $newArgumentName, \PhpCsFixer\DocBlock\Line $line) : string
    {
        ...
        if (\ECSPrefix20220107\Nette\Utils\Strings::match($line->getContent(), $missingDollarSignPattern)) {
            return \ECSPrefix20220107\Nette\Utils\Strings::replace($line->getContent(), $missingDollarSignPattern, '$1$$3');
        }
        $replacement = '@param $1 ' . $newArgumentName . '$2' . \ECSPrefix20220107\Symplify\PackageBuilder\Configuration\StaticEolConfiguration::getEolChar();
        return \ECSPrefix20220107\Nette\Utils\Strings::replace($line->getContent(), self::PARAM_WITHOUT_NAME_REGEX, $replacement);
    }
}
