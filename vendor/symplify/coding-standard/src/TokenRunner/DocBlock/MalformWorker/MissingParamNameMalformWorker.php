...
final class MissingParamNameMalformWorker implements \Symplify\CodingStandard\TokenRunner\Contract\DocBlock\MalformWorkerInterface
{
    ...
    private function filterOutExistingParamNames($docContent, array $functionArgumentNames)
    {
        ...
        foreach ($functionArgumentNames as $key => $functionArgumentName) {
            ...
            if (\ECSPrefix20210514\Nette\Utils\Strings::match($docContent, $pattern)) {
    ...
    {
        if (!\ECSPrefix20210514\Nette\Utils\Strings::contains($line->getContent(), self::PARAM_ANNOTATOIN_START_REGEX)) {
            return \true;
        }
        // already has a param name
        if (\ECSPrefix20210514\Nette\Utils\Strings::match($line->getContent(), self::PARAM_WITH_NAME_REGEX)) {
            return \true;
        }
        $match = \ECSPrefix20210514\Nette\Utils\Strings::match($line->getContent(), self::PARAM_WITHOUT_NAME_REGEX);
        return $match === null;
    }
    /**
