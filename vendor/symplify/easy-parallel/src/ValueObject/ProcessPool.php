...
final class ProcessPool
{
     ...
     */
    private $tcpServer;
    public function __construct(\ECSPrefix20220607\React\Socket\TcpServer $tcpServer)
    {
        $this->tcpServer = $tcpServer;
    }
    public function getProcess(string $identifier) : \ECSPrefix20220607\Symplify\EasyParallel\ValueObject\ParallelProcess
    {
        if (!\array_key_exists($identifier, $this->processes)) {
            throw new \ECSPrefix20220607\Symplify\EasyParallel\Exception\ParallelShouldNotHappenException(\sprintf('Process "%s" not found.', $identifier));
        }
        return $this->processes[$identifier];
    }
    public function attachProcess(string $identifier, \ECSPrefix20220607\Symplify\EasyParallel\ValueObject\ParallelProcess $parallelProcess) : void
    {
