...
namespace ECSPrefix20220607\Symplify\PackageBuilder\Testing;
...
use ECSPrefix20220607\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use ECSPrefix20220607\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \ECSPrefix20220607\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService(string $type) : object
    {
        if (self::$container === null) {
            throw new \ECSPrefix20220607\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \ECSPrefix20220607\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \ECSPrefix20220607\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \ECSPrefix20220607\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \ECSPrefix20220607\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \ECSPrefix20220607\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
    ...
    private function ensureIsConfigAwareKernel($kernel) : void
    {
        ...
        if ($kernel instanceof \ECSPrefix20220607\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \ECSPrefix20220607\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \ECSPrefix20220607\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\ECSPrefix20220607\Symfony\Component\HttpKernel\KernelInterface $kernel) : \ECSPrefix20220607\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \ECSPrefix20220607\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \ECSPrefix20220607\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\ECSPrefix20220607\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\ECSPrefix20220607\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\ECSPrefix20220607\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
