...
namespace ECSPrefix20211002\Symplify\PackageBuilder\Testing;
...
use ECSPrefix20211002\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \ECSPrefix20211002\PHPUnit\Framework\TestCase
{
    /**
     ...
     */
    protected function bootKernelWithConfigs($kernelClass, $configs) : \ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface
    {
     ...
     * @param string[]|SmartFileInfo[] $configs
     */
    protected function bootKernelWithConfigsAndStaticCache($kernelClass, $configs) : \ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface
    {
        // unwrap file infos to real paths
        $configFilePaths = $this->resolveConfigFilePaths($configs);
        $configsHash = $this->resolveConfigsHash($configFilePaths);
    ...
    protected function getService($type)
    {
        if (self::$container === null) {
            throw new \ECSPrefix20211002\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, crewate container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \ECSPrefix20211002\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    protected function bootKernel($kernelClass) : void
    {
        ...
        if (!$kernel instanceof \ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \ECSPrefix20211002\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths($configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \ECSPrefix20211002\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
        return $configFilePaths;
    }
    private function ensureIsConfigAwareKernel(\ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface $kernel) : void
    {
        if ($kernel instanceof \ECSPrefix20211002\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \ECSPrefix20211002\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \ECSPrefix20211002\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface $kernel) : \ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \ECSPrefix20211002\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \ECSPrefix20211002\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\ECSPrefix20211002\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\ECSPrefix20211002\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\ECSPrefix20211002\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
     ...
     */
    private function createBootedKernelFromConfigs(string $kernelClass, string $configsHash, array $configFilePaths) : \ECSPrefix20211002\Symfony\Component\HttpKernel\KernelInterface
    {
