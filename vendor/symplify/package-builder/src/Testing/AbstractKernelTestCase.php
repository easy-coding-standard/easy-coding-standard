...
namespace ConfigTransformer20210601\Symplify\PackageBuilder\Testing;
...
use ConfigTransformer20210601\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \ConfigTransformer20210601\PHPUnit\Framework\TestCase
{
    /**
     * @var KernelInterface
     */
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface
    {
     ...
     * @param string[]|SmartFileInfo[] $configs
     */
    protected function bootKernelWithConfigsAndStaticCache(string $kernelClass, array $configs) : \ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface
    {
        // unwrap file infos to real paths
        $configFilePaths = $this->resolveConfigFilePaths($configs);
        $configsHash = $this->resolveConfigsHash($configFilePaths);
    ...
    protected function getService(string $type)
    {
        if (self::$container === null) {
            throw new \ConfigTransformer20210601\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, crewate container with booKernel(KernelClass::class)');
        }
    ...
    protected function bootKernel(string $kernelClass)
    {
        ...
        if (!$kernel instanceof \ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown()
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \ConfigTransformer20210601\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \ConfigTransformer20210601\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
     ...
     * @return void
     */
    private function ensureIsConfigAwareKernel(\ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface $kernel)
    {
        if ($kernel instanceof \ConfigTransformer20210601\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \ConfigTransformer20210601\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \ConfigTransformer20210601\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface $kernel) : \ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \ConfigTransformer20210601\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \ConfigTransformer20210601\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\ConfigTransformer20210601\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\ConfigTransformer20210601\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\ConfigTransformer20210601\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
     ...
     */
    private function createBootedKernelFromConfigs(string $kernelClass, string $configsHash, array $configFilePaths) : \ConfigTransformer20210601\Symfony\Component\HttpKernel\KernelInterface
    {
