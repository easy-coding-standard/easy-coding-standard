...
namespace ECSPrefix20210727\Symplify\SmartFileSystem;

use ECSPrefix20210727\Nette\Utils\Strings;
use ECSPrefix20210727\Symfony\Component\Finder\SplFileInfo;
use ECSPrefix20210727\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use ECSPrefix20210727\Symplify\EasyTesting\StaticFixtureSplitter;
use ECSPrefix20210727\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
use ECSPrefix20210727\Symplify\SmartFileSystem\Exception\FileNotFoundException;
/**
 * @see \Symplify\SmartFileSystem\Tests\SmartFileInfo\SmartFileInfoTest
 */
final class SmartFileInfo extends \ECSPrefix20210727\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \ECSPrefix20210727\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \ECSPrefix20210727\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
    ...
    {
        return \ECSPrefix20210727\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory($directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \ECSPrefix20210727\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\ECSPrefix20210727\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\ECSPrefix20210727\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
