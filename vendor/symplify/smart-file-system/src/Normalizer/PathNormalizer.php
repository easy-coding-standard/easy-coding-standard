...
namespace ECSPrefix20210611\Symplify\SmartFileSystem\Normalizer;
...
final class PathNormalizer
{
    ...
    {
        $matches = \ECSPrefix20210611\Nette\Utils\Strings::match($originalPath, self::SCHEME_PATH_REGEX);
        if ($matches !== null) {
        ...
        $path = \ECSPrefix20210611\Nette\Utils\Strings::replace($path, self::TWO_AND_MORE_SLASHES_REGEX, '/');
    ...
    private function normalizePathParts(array $pathParts, string $scheme) : array
    {
        ...
        foreach ($pathParts as $pathPart) {
            ...
            if (!\ECSPrefix20210611\Nette\Utils\Strings::endsWith($removedPart, '.phar')) {
                continue;
            }
            $scheme = self::SCHEME_UNDEFINED;
        }
