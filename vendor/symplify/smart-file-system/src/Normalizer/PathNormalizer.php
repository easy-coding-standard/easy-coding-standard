...
final class PathNormalizer
{
    ...
    public function normalizePath($originalPath, $directorySeparator = \DIRECTORY_SEPARATOR)
    {
        ...
        $matches = \ECSPrefix20210513\Nette\Utils\Strings::match($originalPath, self::SCHEME_PATH_REGEX);
        if ($matches !== null) {
        ...
        $path = \ECSPrefix20210513\Nette\Utils\Strings::replace($path, self::TWO_AND_MORE_SLASHES_REGEX, '/');
    ...
    private function normalizePathParts(array $pathParts, $scheme)
    {
        ...
        foreach ($pathParts as $pathPart) {
            ...
            if (!\ECSPrefix20210513\Nette\Utils\Strings::endsWith($removedPart, '.phar')) {
                continue;
            }
            $scheme = self::SCHEME_UNDEFINED;
        }
