...
final class StaticFixtureSplitter
{
    ...
    {
        $splitLineCount = \count(\ECSPrefix20210512\Nette\Utils\Strings::matchAll($smartFileInfo->getContents(), \Symplify\EasyTesting\ValueObject\SplitLine::SPLIT_LINE_REGEX));
        // if more or less, it could be a test cases for monorepo line in it
        if ($splitLineCount === 1) {
            // input → expected
            list($input, $expected) = \ECSPrefix20210512\Nette\Utils\Strings::split($smartFileInfo->getContents(), \Symplify\EasyTesting\ValueObject\SplitLine::SPLIT_LINE_REGEX);
    ...
    {
        $prefix = (string) $prefix;
        $hash = \ECSPrefix20210512\Nette\Utils\Strings::substring(\md5($smartFileInfo->getRealPath()), -20);
