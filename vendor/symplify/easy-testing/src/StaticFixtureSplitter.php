...
namespace ECSPrefix202306\Symplify\EasyTesting;

use ECSPrefix202306\Nette\Utils\Strings;
use ECSPrefix202306\Symplify\EasyTesting\ValueObject\InputAndExpected;
use ECSPrefix202306\Symplify\EasyTesting\ValueObject\InputFileInfoAndExpected;
use ECSPrefix202306\Symplify\EasyTesting\ValueObject\InputFileInfoAndExpectedFileInfo;
...
final class StaticFixtureSplitter
{
    ...
    {
        if (!\is_numeric(\trim((string) $expected))) {
            return $expected;
        }
        // value re-type
        if (\strlen((string) (int) $expected) === \strlen(\trim((string) $expected))) {
            return (int) $expected;
        }
        if (\strlen((string) (float) $expected) === \strlen(\trim((string) $expected))) {
