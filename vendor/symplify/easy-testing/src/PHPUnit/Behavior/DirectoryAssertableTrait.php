...
namespace ECSPrefix20210523\Symplify\EasyTesting\PHPUnit\Behavior;

use ECSPrefix20210523\Symfony\Component\Finder\Finder;
use ECSPrefix20210523\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \ECSPrefix20210523\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \ECSPrefix20210523\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \ECSPrefix20210523\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
