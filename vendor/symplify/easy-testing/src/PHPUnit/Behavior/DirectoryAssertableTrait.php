...
namespace ECSPrefix20220512\Symplify\EasyTesting\PHPUnit\Behavior;

use ECSPrefix20220512\Symfony\Component\Finder\Finder;
use ECSPrefix20220512\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \ECSPrefix20220512\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \ECSPrefix20220512\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \ECSPrefix20220512\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
     ...
     */
    private function resolveFileInfoByRelativeFilePath(array $fileInfos, string $directory, string $desiredRelativeFilePath) : ?\ECSPrefix20220512\Symplify\SmartFileSystem\SmartFileInfo
    {
