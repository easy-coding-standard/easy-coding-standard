...
final class StaticFixtureFinder
{
    ...
    private static function findFilesInDirectory($directory, $suffix)
    {
        $directory = (string) $directory;
        $suffix = (string) $suffix;
        $finder = \ECSPrefix20210513\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    private static function ensureNoOtherFileName($directory, $suffix)
    {
        $directory = (string) $directory;
        $suffix = (string) $suffix;
        $iterator = \ECSPrefix20210513\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \ECSPrefix20210513\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
