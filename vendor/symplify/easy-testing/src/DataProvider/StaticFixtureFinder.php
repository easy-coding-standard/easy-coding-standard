...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos)
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \ECSPrefix20210517\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\ECSPrefix20210517\Symplify\SmartFileSystem\Exception\FileNotFoundException $fileNotFoundException) {
            }
        }
    }
    /**
    ...
    private static function yieldFileInfosWithRelativePathname(array $fileInfos)
    {
        foreach ($fileInfos as $fileInfo) {
            ...
            } catch (\ECSPrefix20210517\Symplify\SmartFileSystem\Exception\FileNotFoundException $e) {
            }
        }
    }
    /**
    ...
    private static function findFilesInDirectory($directory, $suffix)
    {
        $directory = (string) $directory;
        $suffix = (string) $suffix;
        $finder = \ECSPrefix20210517\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    private static function ensureNoOtherFileName($directory, $suffix)
    {
        $directory = (string) $directory;
        $suffix = (string) $suffix;
        $iterator = \ECSPrefix20210517\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \ECSPrefix20210517\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \ECSPrefix20210517\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
