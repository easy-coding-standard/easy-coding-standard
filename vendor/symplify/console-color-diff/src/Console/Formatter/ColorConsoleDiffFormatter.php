...
namespace ECSPrefix20211123\Symplify\ConsoleColorDiff\Console\Formatter;
...
final class ColorConsoleDiffFormatter
{
    ...
    {
        $escapedDiff = \ECSPrefix20211123\Symfony\Component\Console\Formatter\OutputFormatter::escape(\rtrim($diff));
        $escapedDiffLines = \ECSPrefix20211123\Nette\Utils\Strings::split($escapedDiff, self::NEWLINES_REGEX);
    ...
    {
        return \ECSPrefix20211123\Nette\Utils\Strings::replace($string, self::PLUS_START_REGEX, '<fg=green>$1</fg=green>');
    }
    private function makeMinusLinesRed(string $string) : string
    {
        return \ECSPrefix20211123\Nette\Utils\Strings::replace($string, self::MINUT_START_REGEX, '<fg=red>$1</fg=red>');
    }
    private function makeAtNoteCyan(string $string) : string
    {
        return \ECSPrefix20211123\Nette\Utils\Strings::replace($string, self::AT_START_REGEX, '<fg=cyan>$1</fg=cyan>');
    }
}
