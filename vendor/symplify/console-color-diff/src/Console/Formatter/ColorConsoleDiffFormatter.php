...
final class ColorConsoleDiffFormatter
{
    ...
    private function formatWithTemplate($diff, $template)
    {
        ...
        $escapedDiff = \ECSPrefix20210512\Symfony\Component\Console\Formatter\OutputFormatter::escape(\rtrim($diff));
        $escapedDiffLines = \ECSPrefix20210512\Nette\Utils\Strings::split($escapedDiff, self::NEWLINES_REGEX);
    ...
    private function makePlusLinesGreen($string)
    {
        ...
        return \ECSPrefix20210512\Nette\Utils\Strings::replace($string, self::PLUS_START_REGEX, '<fg=green>$1</fg=green>');
    }
    /**
     * @param string $string
     * @return string
    ...
    {
        $string = (string) $string;
        return \ECSPrefix20210512\Nette\Utils\Strings::replace($string, self::MINUT_START_REGEX, '<fg=red>$1</fg=red>');
    }
    /**
     * @param string $string
     * @return string
    ...
    {
        $string = (string) $string;
        return \ECSPrefix20210512\Nette\Utils\Strings::replace($string, self::AT_START_REGEX, '<fg=cyan>$1</fg=cyan>');
    }
}
