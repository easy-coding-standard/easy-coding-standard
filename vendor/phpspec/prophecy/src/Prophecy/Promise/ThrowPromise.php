 ...
 */
class ThrowPromise implements \ECSPrefix20210803\Prophecy\Promise\PromiseInterface
{
    private $exception;
    /**
    ...
    public function __construct($exception)
    {
        if (\is_string($exception)) {
            if (!\class_exists($exception) && !\interface_exists($exception) || !$this->isAValidThrowable($exception)) {
                throw new \ECSPrefix20210803\Prophecy\Exception\InvalidArgumentException(\sprintf('Exception / Throwable class or instance expected as argument to ThrowPromise, but got %s.', $exception));
            }
        } elseif (!$exception instanceof \Exception && !$exception instanceof \Throwable) {
            throw new \ECSPrefix20210803\Prophecy\Exception\InvalidArgumentException(\sprintf('Exception / Throwable class or instance expected as argument to ThrowPromise, but got %s.', \is_object($exception) ? \get_class($exception) : \gettype($exception)));
        }
