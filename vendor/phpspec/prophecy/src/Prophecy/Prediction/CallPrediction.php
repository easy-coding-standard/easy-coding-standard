...
use ECSPrefix20210803\Prophecy\Argument\Token\AnyValuesToken;
use ECSPrefix20210803\Prophecy\Util\StringUtil;
use ECSPrefix20210803\Prophecy\Exception\Prediction\NoCallsException;
/**
 * Call prediction.
 *
 * @author Konstantin Kudryashov <ever.zet@gmail.com>
 */
class CallPrediction implements \ECSPrefix20210803\Prophecy\Prediction\PredictionInterface
{
    private $util;
    /**
    ...
    public function check(array $calls, \ECSPrefix20210803\Prophecy\Prophecy\ObjectProphecy $object, \ECSPrefix20210803\Prophecy\Prophecy\MethodProphecy $method)
    {
        ...
        $methodCalls = $object->findProphecyMethodCalls($method->getMethodName(), new \ECSPrefix20210803\Prophecy\Argument\ArgumentsWildcard(array(new \ECSPrefix20210803\Prophecy\Argument\Token\AnyValuesToken())));
        if (\count($methodCalls)) {
            throw new \ECSPrefix20210803\Prophecy\Exception\Prediction\NoCallsException(\sprintf("No calls have been made that match:\n" . "  %s->%s(%s)\n" . "but expected at least one.\n" . "Recorded `%s(...)` calls:\n%s", \get_class($object->reveal()), $method->getMethodName(), $method->getArgumentsWildcard(), $method->getMethodName(), $this->util->stringifyCalls($methodCalls)), $method);
        }
