...
final class FileHashComputerTest extends \Symplify\PackageBuilder\Testing\AbstractKernelTestCase
{
    ...
    public function testInvalidateCacheOnConfigurationChange() : void
    {
        ...
        $this->smartFileSystem->copy(__DIR__ . '/Source/first_config.php', self::INCLUDED_CONFIG_FILE, \true);
        $fileOneHash = $this->fileHashComputer->computeConfig(__DIR__ . '/Fixture/config-including-another-one.php');
        // B. create on another one with no fixer
        $this->smartFileSystem->copy(__DIR__ . '/Source/empty_config.php', self::INCLUDED_CONFIG_FILE, \true);
        $fileTwoHash = $this->fileHashComputer->computeConfig(__DIR__ . '/Fixture/config-including-another-one.php');
