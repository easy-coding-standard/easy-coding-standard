...
final class SnippetFormatter
{
    ...
    public function format(\ECSPrefix20210524\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, string $snippetRegex, string $kind) : string
    {
        $this->currentParentFileInfoProvider->setParentFileInfo($fileInfo);
        return \ECSPrefix20210524\Nette\Utils\Strings::replace($fileInfo->getContents(), $snippetRegex, function ($match) use($kind) : string {
            if (\ECSPrefix20210524\Nette\Utils\Strings::contains($match[self::CONTENT], '-----')) {
                // do nothing
    ...
    private function fixContent(string $content, string $kind) : string
    {
        ...
        if (!\ECSPrefix20210524\Nette\Utils\Strings::startsWith($this->isPhp73OrAbove ? \trim($content) : $content, '<?php')) {
        ...
        $temporaryFileInfo = new \ECSPrefix20210524\Symplify\SmartFileSystem\SmartFileInfo($temporaryFilePath);
        try {
        ...
        return \ECSPrefix20210524\Nette\Utils\Strings::replace($fileContent, self::OPENING_TAG_HERENOWDOC_REGEX, '$1');
    }
    /**
    ...
    {
        $content = \ECSPrefix20210524\Nette\Utils\Strings::replace($content, self::DECLARE_REGEX, '');
    ...
    {
        return \ECSPrefix20210524\Nette\Utils\Strings::replace($fileContent, self::OPENING_TAG_REGEX, '$1');
    }
}
