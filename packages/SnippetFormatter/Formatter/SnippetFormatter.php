...
final class SnippetFormatter
{
    ...
    public function format(\ECSPrefix20210816\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, string $snippetRegex, string $kind, \Symplify\EasyCodingStandard\ValueObject\Configuration $configuration) : string
    {
        $this->currentParentFileInfoProvider->setParentFileInfo($fileInfo);
        return \ECSPrefix20210816\Nette\Utils\Strings::replace($fileInfo->getContents(), $snippetRegex, function ($match) use($kind, $configuration) : string {
    ...
    private function fixContent(string $content, string $kind, \Symplify\EasyCodingStandard\ValueObject\Configuration $configuration) : string
    {
        ...
        $temporaryFileInfo = new \ECSPrefix20210816\Symplify\SmartFileSystem\SmartFileInfo($temporaryFilePath);
        try {
        ...
        return \ECSPrefix20210816\Nette\Utils\Strings::replace($changedFileContent, self::OPENING_TAG_HERENOWDOC_REGEX, '$1');
    }
    /**
    ...
    {
        $content = \ECSPrefix20210816\Nette\Utils\Strings::replace($content, self::DECLARE_REGEX, '');
    ...
    {
        return \ECSPrefix20210816\Nette\Utils\Strings::replace($fileContent, self::OPENING_TAG_REGEX, '$1');
    }
