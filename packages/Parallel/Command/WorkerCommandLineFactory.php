...
final class WorkerCommandLineFactory
{
     ...
     */
    public function create(string $mainScript, $projectConfigFile, \ECSPrefix20210627\Symfony\Component\Console\Input\InputInterface $input) : string
    {
        $args = \array_merge([\PHP_BINARY, $mainScript], \array_slice($_SERVER['argv'], 1));
        $processCommandArray = [];
        foreach ($args as $arg) {
            if ($arg === \ECSPrefix20210627\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Symplify\EasyCodingStandard\Console\Command\CheckCommand::class)) {
                break;
            }
            $processCommandArray[] = \escapeshellarg($arg);
        }
        $processCommandArray[] = \ECSPrefix20210627\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Symplify\EasyCodingStandard\Console\Command\WorkerCommand::class);
