...
final class WorkerRunner
{
    ...
    public function __construct(\Symplify\EasyCodingStandard\Application\SingleFileProcessor $singleFileProcessor, \ECSPrefix20220531\Symplify\PackageBuilder\Yaml\ParametersMerger $parametersMerger)
    {
        $this->singleFileProcessor = $singleFileProcessor;
        $this->parametersMerger = $parametersMerger;
    }
    public function run(\ECSPrefix20220531\Clue\React\NDJson\Encoder $encoder, \ECSPrefix20220531\Clue\React\NDJson\Decoder $decoder, \Symplify\EasyCodingStandard\ValueObject\Configuration $configuration) : void
    {
        ...
        $handleErrorCallback = static function (\Throwable $throwable) use($encoder) : void {
            ...
            $encoder->write([\ECSPrefix20220531\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \ECSPrefix20220531\Symplify\EasyParallel\Enum\Action::RESULT, \ECSPrefix20220531\Symplify\EasyParallel\Enum\Content::RESULT => [\Symplify\EasyCodingStandard\Parallel\ValueObject\Bridge::SYSTEM_ERRORS => [$systemErrors], \Symplify\EasyCodingStandard\Parallel\ValueObject\Bridge::FILES_COUNT => 0, \Symplify\EasyCodingStandard\Parallel\ValueObject\Bridge::SYSTEM_ERRORS_COUNT => 1]]);
            $encoder->end();
        };
        $encoder->on(\ECSPrefix20220531\Symplify\EasyParallel\Enum\ReactEvent::ERROR, $handleErrorCallback);
        // 2. collect diffs + errors from file processor
        $decoder->on(\ECSPrefix20220531\Symplify\EasyParallel\Enum\ReactEvent::DATA, function (array $json) use($encoder, $configuration) : void {
            $action = $json[\ECSPrefix20220531\Symplify\EasyParallel\Enum\ReactCommand::ACTION];
            if ($action !== \ECSPrefix20220531\Symplify\EasyParallel\Enum\Action::MAIN) {
                return;
            }
            $systemErrorsCount = 0;
            /** @var string[] $filePaths */
            $filePaths = $json[\ECSPrefix20220531\Symplify\EasyParallel\Enum\Content::FILES] ?? [];
            $errorAndFileDiffs = [];
            $systemErrors = [];
            foreach ($filePaths as $filePath) {
                try {
                    $smartFileInfo = new \ECSPrefix20220531\Symplify\SmartFileSystem\SmartFileInfo($filePath);
        ...
        $decoder->on(\ECSPrefix20220531\Symplify\EasyParallel\Enum\ReactEvent::ERROR, $handleErrorCallback);
    }
}
