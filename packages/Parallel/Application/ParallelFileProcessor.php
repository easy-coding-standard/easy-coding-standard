...
use ECSPrefix20211007\React\Socket\TcpServer;
...
final class ParallelFileProcessor
{
    ...
    public function __construct(\ECSPrefix20211007\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider, \Symplify\EasyCodingStandard\Parallel\Command\WorkerCommandLineFactory $workerCommandLineFactory)
    {
     ...
     * @return mixed[]
     */
    public function check(\Symplify\EasyCodingStandard\Parallel\ValueObject\Schedule $schedule, string $mainScript, \Closure $postFileCallback, ?string $projectConfigFile, \ECSPrefix20211007\Symfony\Component\Console\Input\InputInterface $input) : array
    {
        ...
        $tcpServer = new \ECSPrefix20211007\React\Socket\TcpServer('127.0.0.1:0', $streamSelectLoop);
        $this->processPool = new \Symplify\EasyCodingStandard\Parallel\ValueObject\ProcessPool($tcpServer);
        $tcpServer->on(\Symplify\EasyCodingStandard\Parallel\ValueObject\ReactEvent::CONNECTION, function (\ECSPrefix20211007\React\Socket\ConnectionInterface $connection) use(&$jobs) : void {
            $inDecoder = new \ECSPrefix20211007\Clue\React\NDJson\Decoder($connection, \true, 512, 0, 4 * 1024 * 1024);
            $outEncoder = new \ECSPrefix20211007\Clue\React\NDJson\Encoder($connection);
        ...
        for ($i = 0; $i < $numberOfProcesses; ++$i) {
            ...
            $processIdentifier = \ECSPrefix20211007\Nette\Utils\Random::generate();
            ...
            $parallelProcess->start(
                ...
                function ($exitCode, string $stdErr) use(&$systemErrors, $processIdentifier) : void {
                    ...
                    if ($exitCode === \ECSPrefix20211007\Symfony\Component\Console\Command\Command::SUCCESS) {
                        return;
                    }
                    if ($exitCode === null) {
                        return;
