...
{
    public function createFromFromInput(\ECSPrefix20210618\Symfony\Component\Console\Input\InputInterface $input) : \ECSPrefix20210618\Symfony\Component\DependencyInjection\ContainerInterface
    {
        $environment = 'prod' . \random_int(1, 100000);
        $easyCodingStandardKernel = new \Symplify\EasyCodingStandard\HttpKernel\EasyCodingStandardKernel($environment, \ECSPrefix20210618\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
        ...
        if ($input->hasParameterOption(['--config', '-c'])) {
            ...
            if (\is_string($commandLineConfigFile) && \file_exists($commandLineConfigFile)) {
                $inputConfigFileInfos[] = new \ECSPrefix20210618\Symplify\SmartFileSystem\SmartFileInfo($commandLineConfigFile);
            }
        } elseif (\file_exists($rootECSConfig)) {
            $inputConfigFileInfos[] = new \ECSPrefix20210618\Symplify\SmartFileSystem\SmartFileInfo($rootECSConfig);
        }
