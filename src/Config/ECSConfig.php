...
use ECSPrefix20220529\Webmozart\Assert\InvalidArgumentException;
/**
 * @api
 */
final class ECSConfig extends \Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator
{
    ...
    {
        \ECSPrefix20220529\Webmozart\Assert\Assert::allString($paths);
    ...
    {
        \ECSPrefix20220529\Webmozart\Assert\Assert::allString($sets);
        \ECSPrefix20220529\Webmozart\Assert\Assert::allFileExists($sets);
        foreach ($sets as $set) {
            $this->import($set);
        }
    }
    ...
    public function ruleWithConfiguration(string $checkerClass, array $configuration) : void
    {
        ...
        if (\is_a($checkerClass, \PhpCsFixer\Fixer\FixerInterface::class, \true)) {
            \ECSPrefix20220529\Webmozart\Assert\Assert::isAnyOf($checkerClass, [\PhpCsFixer\Fixer\ConfigurableFixerInterface::class, \ECSPrefix20220529\Symplify\RuleDocGenerator\Contract\ConfigurableRuleInterface::class]);
        ...
        if (\is_a($checkerClass, \PHP_CodeSniffer\Sniffs\Sniff::class, \true)) {
            foreach ($configuration as $propertyName => $value) {
                \ECSPrefix20220529\Webmozart\Assert\Assert::propertyExists($checkerClass, $propertyName);
    ...
    {
        \ECSPrefix20220529\Webmozart\Assert\Assert::classExists($checkerClass);
        \ECSPrefix20220529\Webmozart\Assert\Assert::isAnyOf($checkerClass, [\PHP_CodeSniffer\Sniffs\Sniff::class, \PhpCsFixer\Fixer\FixerInterface::class]);
    }
    /**
    ...
    private function ensureCheckerClassesAreUnique(array $checkerClasses) : void
    {
        ...
        throw new \ECSPrefix20220529\Webmozart\Assert\InvalidArgumentException($errorMessage);
    }
}
