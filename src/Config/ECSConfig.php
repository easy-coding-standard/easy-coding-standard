...
use ECSPrefix20220520\Webmozart\Assert\InvalidArgumentException;
/**
 * @api
 */
final class ECSConfig extends \Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator
{
    ...
    {
        \ECSPrefix20220520\Webmozart\Assert\Assert::allString($paths);
    ...
    {
        \ECSPrefix20220520\Webmozart\Assert\Assert::allString($sets);
        \ECSPrefix20220520\Webmozart\Assert\Assert::allFileExists($sets);
        foreach ($sets as $set) {
            $this->import($set);
        }
    }
    ...
    public function ruleWithConfiguration(string $checkerClass, array $configuration) : void
    {
        ...
        if (\is_a($checkerClass, \PhpCsFixer\Fixer\FixerInterface::class, \true)) {
            \ECSPrefix20220520\Webmozart\Assert\Assert::isAnyOf($checkerClass, [\PhpCsFixer\Fixer\ConfigurableFixerInterface::class, \ECSPrefix20220520\Symplify\RuleDocGenerator\Contract\ConfigurableRuleInterface::class]);
        ...
        if (\is_a($checkerClass, \PHP_CodeSniffer\Sniffs\Sniff::class, \true)) {
            foreach ($configuration as $propertyName => $value) {
                \ECSPrefix20220520\Webmozart\Assert\Assert::propertyExists($checkerClass, $propertyName);
    ...
    {
        \ECSPrefix20220520\Webmozart\Assert\Assert::classExists($checkerClass);
        \ECSPrefix20220520\Webmozart\Assert\Assert::isAnyOf($checkerClass, [\PHP_CodeSniffer\Sniffs\Sniff::class, \PhpCsFixer\Fixer\FixerInterface::class]);
    }
    /**
    ...
    private function ensureCheckerClassesAreUnique(array $checkerClasses) : void
    {
        ...
        throw new \ECSPrefix20220520\Webmozart\Assert\InvalidArgumentException($errorMessage);
    }
}
