...
final class ConstantReflectionSetFactory
{
    ...
    public function createSetsFromClass($setClassName)
    {
        ...
        foreach ($constants as $name => $setPath) {
            ...
            $setName = \ECSPrefix20210516\Nette\Utils\Strings::replace($setName, self::REMOVE_DASH_BEFORE_NUMBER_REGEX, '$1$2');
            $sets[] = new \ECSPrefix20210516\Symplify\SetConfigResolver\ValueObject\Set($setName, new \ECSPrefix20210516\Symplify\SmartFileSystem\SmartFileInfo($setPath));
        }
        return $sets;
    }
    /**
    ...
    private function constantToDashes($string)
    {
        ...
        return \ECSPrefix20210516\Nette\Utils\Strings::replace($string, self::UNDERSCORE_REGEX, '-');
    }
}
