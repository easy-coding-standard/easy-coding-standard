...
final class SourceFinder
{
    ...
    public function __construct(\ECSPrefix20210705\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \ECSPrefix20210705\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider, \Symplify\EasyCodingStandard\Git\GitDiffProvider $gitDiffProvider)
    {
    ...
    public function find(array $source, bool $doesMatchGitDiff = \false) : array
    {
        ...
        foreach ($source as $singleSource) {
            if (\is_file($singleSource)) {
                $fileInfos[] = new \ECSPrefix20210705\Symplify\SmartFileSystem\SmartFileInfo($singleSource);
            } else {
    ...
    private function processDirectory(string $directory) : array
    {
        ...
        $finder = \ECSPrefix20210705\Symfony\Component\Finder\Finder::create()->files()->name($normalizedFileExtensions)->in($directory)->exclude('vendor')->size('> 0')->sortByName();
