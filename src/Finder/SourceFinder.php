...
final class SourceFinder
{
    ...
    public function __construct(\ECSPrefix20211202\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \ECSPrefix20211202\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider)
    {
    ...
    public function find(array $source) : array
    {
        ...
        foreach ($source as $singleSource) {
            if (\is_file($singleSource)) {
                $fileInfos[] = new \ECSPrefix20211202\Symplify\SmartFileSystem\SmartFileInfo($singleSource);
            } else {
    ...
    private function processDirectory(string $directory) : array
    {
        ...
        $finder = \ECSPrefix20211202\Symfony\Component\Finder\Finder::create()->files()->name($normalizedFileExtensions)->in($directory)->exclude('vendor')->size('> 0')->sortByName();
