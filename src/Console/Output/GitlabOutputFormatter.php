...
use ECSPrefix202406\SebastianBergmann\Diff\Chunk;
use ECSPrefix202406\SebastianBergmann\Diff\Line;
use ECSPrefix202406\SebastianBergmann\Diff\Parser as DiffParser;
...
final class GitlabOutputFormatter implements OutputFormatterInterface
{
    ...
    {
        $reportedQualityIssues = !$configuration->isFixer() && $configuration->shouldShowDiffs() ? merge($this->generateIssuesForErrors($errorAndDiffResult->getErrors()), $this->generateIssuesForFixes($errorAndDiffResult->getFileDiffs())) : $this->generateIssuesForErrors($errorAndDiffResult->getErrors());
    ...
    {
        return merge(...map(function (FileDiff $fileDiff) : array {
            return map(function (Chunk $chunk) use($fileDiff) : array {
                return $this->generateIssueForChunk($fileDiff, $chunk);
    ...
    private function generateIssueForChunk(FileDiff $fileDiff, Chunk $chunk) : array
    {
        ...
        }, $chunk->lines()))), 'severity' => 'minor', 'categories' => ['Style'], 'remediation_points' => 50000, 'location' => ['path' => $fileDiff->getRelativeFilePath(), 'lines' => ['begin' => $lineStart, 'end' => $lineEnd]]];
    }
    /**
