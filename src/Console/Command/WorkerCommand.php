...
use ECSPrefix20220509\React\EventLoop\StreamSelectLoop;
use ECSPrefix20220509\React\Socket\ConnectionInterface;
use ECSPrefix20220509\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute(\ECSPrefix20220509\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20220509\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20220509\React\EventLoop\StreamSelectLoop();
        $parallelIdentifier = $configuration->getParallelIdentifier();
        $tcpConnector = new \ECSPrefix20220509\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\ECSPrefix20220509\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \ECSPrefix20220509\Clue\React\NDJson\Decoder($connection, \true, 512, 0);
            $outEncoder = new \ECSPrefix20220509\Clue\React\NDJson\Encoder($connection, 0);
            // handshake?
            $outEncoder->write([\ECSPrefix20220509\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \ECSPrefix20220509\Symplify\EasyParallel\Enum\Action::HELLO, \ECSPrefix20220509\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
