...
use ECSPrefix20220131\React\EventLoop\StreamSelectLoop;
use ECSPrefix20220131\React\Socket\ConnectionInterface;
use ECSPrefix20220131\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute(\ECSPrefix20220131\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20220131\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20220131\React\EventLoop\StreamSelectLoop();
        $parallelIdentifier = $configuration->getParallelIdentifier();
        $tcpConnector = new \ECSPrefix20220131\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\ECSPrefix20220131\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \ECSPrefix20220131\Clue\React\NDJson\Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            $outEncoder = new \ECSPrefix20220131\Clue\React\NDJson\Encoder($connection, \JSON_INVALID_UTF8_IGNORE);
            // handshake?
            $outEncoder->write([\ECSPrefix20220131\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \ECSPrefix20220131\Symplify\EasyParallel\Enum\Action::HELLO, \ECSPrefix20220131\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
