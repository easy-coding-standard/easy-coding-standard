...
use ECSPrefix20220219\React\EventLoop\StreamSelectLoop;
use ECSPrefix20220219\React\Socket\ConnectionInterface;
use ECSPrefix20220219\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute(\ECSPrefix20220219\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20220219\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $tcpConnector = new \ECSPrefix20220219\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\ECSPrefix20220219\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \ECSPrefix20220219\Clue\React\NDJson\Decoder($connection, \true, 512, 0);
            $outEncoder = new \ECSPrefix20220219\Clue\React\NDJson\Encoder($connection, 0);
            // handshake?
            $outEncoder->write([\ECSPrefix20220219\Symplify\EasyParallel\Enum\ReactCommand::ACTION => \ECSPrefix20220219\Symplify\EasyParallel\Enum\Action::HELLO, \ECSPrefix20220219\Symplify\EasyParallel\Enum\ReactCommand::IDENTIFIER => $parallelIdentifier]);
