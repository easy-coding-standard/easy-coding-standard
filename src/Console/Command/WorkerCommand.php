...
use ECSPrefix20210705\Clue\React\NDJson\Decoder;
use ECSPrefix20210705\Clue\React\NDJson\Encoder;
use ECSPrefix20210705\React\EventLoop\StreamSelectLoop;
use ECSPrefix20210705\React\Stream\ReadableResourceStream;
use ECSPrefix20210705\React\Stream\WritableResourceStream;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute(\ECSPrefix20210705\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20210705\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20210705\React\EventLoop\StreamSelectLoop();
        $stdOutEncoder = new \ECSPrefix20210705\Clue\React\NDJson\Encoder(new \ECSPrefix20210705\React\Stream\WritableResourceStream(\STDOUT, $streamSelectLoop));
        ...
        $decoder = new \ECSPrefix20210705\Clue\React\NDJson\Decoder(new \ECSPrefix20210705\React\Stream\ReadableResourceStream(\STDIN, $streamSelectLoop), \true);
        $decoder->on(\Symplify\EasyCodingStandard\Parallel\ValueObject\ReactEvent::DATA, function (array $json) use($stdOutEncoder, $configuration) {
            ...
            if ($action === \Symplify\EasyCodingStandard\Parallel\ValueObject\Action::CHECK) {
                ...
                foreach ($filePaths as $filePath) {
                    try {
                        $smartFileInfo = new \ECSPrefix20210705\Symplify\SmartFileSystem\SmartFileInfo($filePath);
        ...
        return \ECSPrefix20210705\Symplify\PackageBuilder\Console\ShellCode::SUCCESS;
    }
}
