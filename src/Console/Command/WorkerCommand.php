...
use ECSPrefix20211027\Clue\React\NDJson\Decoder;
use ECSPrefix20211027\Clue\React\NDJson\Encoder;
use ECSPrefix20211027\React\EventLoop\StreamSelectLoop;
use ECSPrefix20211027\React\Socket\ConnectionInterface;
use ECSPrefix20211027\React\Socket\TcpConnector;
...
use ECSPrefix20211027\Symplify\PackageBuilder\Yaml\ParametersMerger;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    public function __construct(\Symplify\EasyCodingStandard\Application\SingleFileProcessor $singleFileProcessor, \ECSPrefix20211027\Symplify\PackageBuilder\Yaml\ParametersMerger $parametersMerger)
    {
    ...
    protected function execute($input, $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20211027\React\EventLoop\StreamSelectLoop();
        $parallelIdentifier = $configuration->getParallelIdentifier();
        $tcpConnector = new \ECSPrefix20211027\React\Socket\TcpConnector($streamSelectLoop);
        $tcpConnector->connect(\sprintf('127.0.0.1:%d', $configuration->getParallelPort()))->then(function (\ECSPrefix20211027\React\Socket\ConnectionInterface $connection) use($output, $parallelIdentifier, $configuration) : void {
            $inDecoder = new \ECSPrefix20211027\Clue\React\NDJson\Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            $outEncoder = new \ECSPrefix20211027\Clue\React\NDJson\Encoder($connection, \JSON_INVALID_UTF8_IGNORE);
            // handshake?
    ...
    private function runWorker(\ECSPrefix20211027\Clue\React\NDJson\Encoder $encoder, \ECSPrefix20211027\Clue\React\NDJson\Decoder $decoder, \Symplify\EasyCodingStandard\ValueObject\Configuration $configuration) : void
    {
        ...
        $decoder->on(\Symplify\EasyCodingStandard\Parallel\ValueObject\ReactEvent::DATA, function (array $json) use($encoder, $configuration) : void {
            ...
            foreach ($filePaths as $filePath) {
                try {
                    $smartFileInfo = new \ECSPrefix20211027\Symplify\SmartFileSystem\SmartFileInfo($filePath);
