...
use ECSPrefix20211112\Clue\React\NDJson\Decoder;
use ECSPrefix20211112\Clue\React\NDJson\Encoder;
use ECSPrefix20211112\React\EventLoop\StreamSelectLoop;
use ECSPrefix20211112\React\Socket\ConnectionInterface;
use ECSPrefix20211112\React\Socket\TcpConnector;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute($input, $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20211112\React\EventLoop\StreamSelectLoop();
        $parallelIdentifier = $configuration->getParallelIdentifier();
        $tcpConnector = new \ECSPrefix20211112\React\Socket\TcpConnector($streamSelectLoop);
        $promise = $tcpConnector->connect('127.0.0.1:' . $configuration->getParallelPort());
        $promise->then(function (\ECSPrefix20211112\React\Socket\ConnectionInterface $connection) use($parallelIdentifier, $configuration) : void {
            $inDecoder = new \ECSPrefix20211112\Clue\React\NDJson\Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
            $outEncoder = new \ECSPrefix20211112\Clue\React\NDJson\Encoder($connection, \JSON_INVALID_UTF8_IGNORE);
            // handshake?
