...
use ECSPrefix20210722\Clue\React\NDJson\Decoder;
use ECSPrefix20210722\Clue\React\NDJson\Encoder;
use ECSPrefix20210722\React\EventLoop\StreamSelectLoop;
...
final class WorkerCommand extends \Symplify\EasyCodingStandard\Console\Command\AbstractCheckCommand
{
    ...
    protected function execute($input, $output) : int
    {
        ...
        $streamSelectLoop = new \ECSPrefix20210722\React\EventLoop\StreamSelectLoop();
        $stdOutEncoder = new \ECSPrefix20210722\Clue\React\NDJson\Encoder(new \ECSPrefix20210722\React\Stream\WritableResourceStream(\STDOUT, $streamSelectLoop));
        ...
        $decoder = new \ECSPrefix20210722\Clue\React\NDJson\Decoder(new \ECSPrefix20210722\React\Stream\ReadableResourceStream(\STDIN, $streamSelectLoop), \true);
        $decoder->on(\Symplify\EasyCodingStandard\Parallel\ValueObject\ReactEvent::DATA, function (array $json) use($stdOutEncoder, $configuration) {
            ...
            if ($action === \Symplify\EasyCodingStandard\Parallel\ValueObject\Action::CHECK) {
                ...
                foreach ($filePaths as $filePath) {
                    try {
                        $smartFileInfo = new \ECSPrefix20210722\Symplify\SmartFileSystem\SmartFileInfo($filePath);
