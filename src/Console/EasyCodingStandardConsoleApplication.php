...
use ECSPrefix20210516\Composer\XdebugHandler\XdebugHandler;
...
final class EasyCodingStandardConsoleApplication extends \ECSPrefix20210516\Symfony\Component\Console\Application
{
    /**
    ...
    public function __construct(array $commands)
    {
        ...
        $commandNaming = new \ECSPrefix20210516\Symplify\PackageBuilder\Console\Command\CommandNaming();
        foreach ($commands as $command) {
            $commandName = $commandNaming->resolveFromCommand($command);
            $command->setName($commandName);
            $this->add($command);
        }
        $this->setDefaultCommand(\ECSPrefix20210516\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Symplify\EasyCodingStandard\Console\Command\CheckCommand::class));
    }
    /**
     * @return int
     */
    public function doRun(\ECSPrefix20210516\Symfony\Component\Console\Input\InputInterface $input, \ECSPrefix20210516\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        if (!$isXdebugAllowed && !\defined('PHPUNIT_COMPOSER_INSTALL')) {
            $xdebugHandler = new \ECSPrefix20210516\Composer\XdebugHandler\XdebugHandler('ecs');
            $xdebugHandler->check();
            unset($xdebugHandler);
        }
     ...
     * @return bool
     */
    private function shouldPrintMetaInformation(\ECSPrefix20210516\Symfony\Component\Console\Input\InputInterface $input)
    {
     ...
     * @return void
     */
    private function addExtraOptions(\ECSPrefix20210516\Symfony\Component\Console\Input\InputDefinition $inputDefinition)
    {
        $inputDefinition->addOption(new \ECSPrefix20210516\Symfony\Component\Console\Input\InputOption(\Symplify\EasyCodingStandard\ValueObject\Option::XDEBUG, null, \ECSPrefix20210516\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Allow running xdebug'));
        $inputDefinition->addOption(new \ECSPrefix20210516\Symfony\Component\Console\Input\InputOption(\Symplify\EasyCodingStandard\ValueObject\Option::DEBUG, null, \ECSPrefix20210516\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Run in debug mode (alias for "-vvv")'));
    }
    /**
     * @return string
     */
    private function resolveEasyCodingStandardVersion()
    {
        ...
        $installedRawData = \ECSPrefix20210516\Composer\InstalledVersions::getRawData();
        ...
        if ($ecsPackageData['version'] === 'dev-main') {
            if ($ecsPackageData['reference'] !== null) {
                return 'dev-main@' . \ECSPrefix20210516\Nette\Utils\Strings::substring($ecsPackageData['reference'], 0, 7);
            }
