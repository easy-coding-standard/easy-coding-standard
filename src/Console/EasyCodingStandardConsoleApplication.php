...
use ECSPrefix20210804\Composer\XdebugHandler\XdebugHandler;
...
final class EasyCodingStandardConsoleApplication extends \ECSPrefix20210804\Symfony\Component\Console\Application
{
    /**
    ...
    public function __construct(array $commands)
    {
        ...
        $commandNaming = new \ECSPrefix20210804\Symplify\PackageBuilder\Console\Command\CommandNaming();
        foreach ($commands as $command) {
            $commandName = $commandNaming->resolveFromCommand($command);
            $command->setName($commandName);
            $this->add($command);
        }
        $this->setDefaultCommand(\ECSPrefix20210804\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Symplify\EasyCodingStandard\Console\Command\CheckCommand::class));
    }
    /**
    ...
    public function doRun($input, $output) : int
    {
        ...
        if (!$isXdebugAllowed && !\defined('PHPUNIT_COMPOSER_INSTALL')) {
            $xdebugHandler = new \ECSPrefix20210804\Composer\XdebugHandler\XdebugHandler('ecs');
            $xdebugHandler->check();
            unset($xdebugHandler);
        }
    ...
    protected function getDefaultInputDefinition() : \ECSPrefix20210804\Symfony\Component\Console\Input\InputDefinition
    {
        $inputDefinition = parent::getDefaultInputDefinition();
        $this->addExtraOptions($inputDefinition);
        return $inputDefinition;
    }
    private function shouldPrintMetaInformation(\ECSPrefix20210804\Symfony\Component\Console\Input\InputInterface $input) : bool
    {
